...
function stripWidgets(text) {
  if (!text) return text;
  const isPUA = (c) => { const x = c.codePointAt(0); return x >= 0xE000 && x <= 0xF8FF; };
  let out = "";
  for (let i = 0; i < text.length; ) {
    if (isPUA(text[i])) {
      ...
      if (text[j] === "{") { // balanced json payload
        let depth = 0, k = j;
        for (; k < text.length; k++) {
          if (text[k] === "{") depth++;
          else if (text[k] === "}") { depth--; if (depth === 0) { k++; break; } }
        }
        if (depth === 0) { i = k; continue; } // 균형 잡힘 → 페이로드까지 제거
  ...
  for (let i = 0; i < lines.length; ) {
    if (lines[i].trim() === "```") {
      ...
      if (j < lines.length) {
        ...
        if (inner === "" || inner === CLAUDE_PLACEHOLDER) { i = j + 1; continue; }
        for (let k = i; k <= j; k++) out.push(lines[k]);
        i = j + 1;
        continue;
      }
    }
    if (lines[i].trim() === CLAUDE_PLACEHOLDER) { i++; continue; }
    push(lines[i]);
    i++;
  }
  return out.join("\n");
...
function _claudeText(msg) {
  const raw = (msg.text && msg.text.trim()) ? msg.text.trim()
...
function normalizeClaude(raw, externalId) {
  ...
  for (const m of raw.chat_messages || []) {
    ...
    messages.push({ role, idx: messages.length, text, createdAt: m.created_at || null, blocks, _raw: m });
  }
  return {
    source: "CLAUDE",
    externalId,
...
function scanGeminiImages(node) {
  ...
  const walk = (n) => {
    ...
    for (let i = 0; i < n.length; i++) {
      const v = n[i];
      if (typeof v === "string" && v.indexOf("https:") === 0 && v.indexOf("googleusercontent.com/gg/") >= 0) {
        if (!seen.has(v)) {
          seen.add(v);
          const prev = n[i - 1];
  ...
  if (turns) {
    ...
    for (let i = turns.length - 1; i >= 0; i--) {
      ...
      if (typeof userText === "string" && userText.trim())
        messages.push({ role: "USER", idx: messages.length, text: userText.trim(), createdAt: iso, _raw: getPath(t, [2]) });
      const modelText = getPath(t, [3, 0, 0, 1, 0]);
      if (typeof modelText === "string" && modelText.trim())
        messages.push({ role: "AI", idx: messages.length, text: modelText.trim(), createdAt: iso, _raw: getPath(t, [3]) });
    }
  }
  return {
    source: "GEMINI", externalId,
...
function ensureNonEmpty(conv) {
  ...
  return titleFallback({ source: conv.source, externalId: conv.externalId, title: conv.title, createdAt: conv.createdAt });
}
...
const Normalize = { stripWidgets, normalizeChatGPT, normalizeClaude, parseGeminiList, parseGeminiListPage, titleFallback, ensureNonEmpty, _iso, parseBatchEnvelope, normalizeGemini, parseWizTokens, scanGeminiImages };
