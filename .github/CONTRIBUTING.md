...
- If the latest RC tag exists but is missing its GitHub Release or has an incomplete draft, the workflow resumes that tag before cutting the next RC. This keeps interrupted builds from leaving orphaned tags behind.
- If the latest RC draft has a complete asset set but was not published, the workflow leaves it private and cuts the next RC. This prevents a failed publish gate, including E2E, from wedging the RC series.
