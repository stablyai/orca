...
public final class HardwareKeyboardCommandRegistry {
  ...
  private func addHandlerMethod(to controllerClass: AnyClass) {
    ...
    installCompositionGuard(on: controllerClass)
  }

  private func installCompositionGuard(on controllerClass: AnyClass) {
    let selector = #selector(UIResponder.canPerformAction(_:withSender:))
    guard let method = class_getInstanceMethod(controllerClass, selector) else { return }
    typealias Original = @convention(c) (AnyObject, Selector, Selector, AnyObject?) -> Bool
    let original = unsafeBitCast(method_getImplementation(method), to: Original.self)
    typealias GuardBlock = @convention(block) (AnyObject, Selector, AnyObject?) -> Bool
    let block: GuardBlock = { receiver, action, sender in
      if action == hardwareKeyboardSelector,
         let controller = receiver as? UIViewController,
         Self.hasMarkedText(in: controller.view) {
        return false
      }
      return original(receiver, selector, action, sender)
    }
    class_replaceMethod(controllerClass, selector, imp_implementationWithBlock(block), method_getTypeEncoding(method))
  }

  private static func hasMarkedText(in view: UIView) -> Bool {
    if view.isFirstResponder, let input = view as? UITextInput, input.markedTextRange != nil {
      return true
    }
...
public final class ExpoHardwareKeyboardNavigationModule: Module {
  ...
  public func definition() -> ModuleDefinition {
    ...
    Events("onHardwareKeyboardCommand", "onHardwareKeyboardConnectionChanged")

    OnStartObserving("onHardwareKeyboardConnectionChanged") {
      self.connectionObservers = [Notification.Name.GCKeyboardDidConnect, Notification.Name.GCKeyboardDidDisconnect].map { name in
        NotificationCenter.default.addObserver(forName: name, object: nil, queue: .main) { [weak self] _ in
          self?.sendEvent("onHardwareKeyboardConnectionChanged", ["connected": GCKeyboard.coalesced != nil])
        }
      }
    }

    OnStopObserving("onHardwareKeyboardConnectionChanged") {
      self.connectionObservers.forEach(NotificationCenter.default.removeObserver)
      self.connectionObservers.removeAll()
    }
