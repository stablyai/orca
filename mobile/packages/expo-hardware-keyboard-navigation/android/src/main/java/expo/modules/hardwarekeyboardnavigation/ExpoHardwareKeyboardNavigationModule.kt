...
class ExpoHardwareKeyboardNavigationModule : Module() {
  ...
  private val deviceListener = object : InputManager.InputDeviceListener {
    override fun onInputDeviceAdded(deviceId: Int) = publishConnection()
    override fun onInputDeviceRemoved(deviceId: Int) = publishConnection()
    override fun onInputDeviceChanged(deviceId: Int) = publishConnection()
  }
  ...
  override fun definition() = ModuleDefinition {
    ...
    OnStartObserving("onHardwareKeyboardConnectionChanged") {
      inputManager = appContext.reactContext?.getSystemService(Context.INPUT_SERVICE) as? InputManager
  ...
  private fun publishConnection() {
    ...
    val connected = manager.inputDeviceIds.any { id ->
      val device = manager.getInputDevice(id)
      device != null && !device.isVirtual && device.keyboardType == InputDevice.KEYBOARD_TYPE_ALPHABETIC
    }
    sendEvent("onHardwareKeyboardConnectionChanged", mapOf("connected" to connected))
  }
}
