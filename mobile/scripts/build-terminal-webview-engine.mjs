...
const nerdFontPath = path.join(mobileRoot, 'assets', 'fonts', 'SymbolsNerdFontMono-Regular.woff2')
...
async function main() {
  const [engineJs, rawEngineCss, nerdFont, ...versions] = await Promise.all([
    buildEngineJs(),
  ...
  const nerdFontCss = [
    ...
    `src:url('data:font/woff2;base64,${nerdFont.toString('base64')}') format('woff2');`,
    'font-weight:400;',
    'font-style:normal;',
    'font-display:swap;',
  ...
  const source = [
    ...
    `export const TERMINAL_NERD_FONT_CSS = ${JSON.stringify(htmlText(nerdFontCss, 'style'))}`,
    ''
  ].join('\n')
