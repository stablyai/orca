...
import {
  ...
  trustedMCodeHooksWithSetupApproval,
...
type MCodeYamlTrustPrompt = WorkspaceCreateArgs & {
  repoId: string
  repoName: string
  scriptContent: string
  contentHash: string
...
type ProjectRepoNotInMCodePrompt = {
  owner: string
  repo: string
  url: string | null
}
...
export default function MobileTasksScreen() {
  ...
  const [mcodeYamlTrustPrompt, setMCodeYamlTrustPrompt] = useState<MCodeYamlTrustPrompt | null>(null)
  ...
  const [projectRepoNotInMCode, setProjectRepoNotInMCode] =
    useState<ProjectRepoNotInMCodePrompt | null>(null)
  ...
  const persistSetupHookTrust = useCallback(
    async (repoId: string, contentHash: string, alwaysTrust: boolean): Promise<void> => {
      ...
      const next = trustedMCodeHooksWithSetupApproval({
        trust: trustedMCodeHooks,
        repoId,
        contentHash,
        alwaysTrust
      })
      const response = await client.sendRequest('ui.set', { trustedMCodeHooks: next })
      if (!isSuccess(response)) {
        throw new Error(response.error.message)
      }
      setTrustedMCodeHooks(next)
    },
    [client, trustedMCodeHooks]
  )
  ...
  useEffect(() => {
    ...
    const hydrateTaskState = async (): Promise<void> => {
      ...
      if (!status.capabilities?.includes(MOBILE_TASKS_CAPABILITY)) {
        ...
        setMergeMethodProjectRow(null)
        resetWorkspaceCreateState()
        setError('Update MCode desktop to use Tasks on mobile.')
        setTaskStateHydrated(false)
        return
      }
      ...
      const uiState = isSuccess(uiResponse)
        ...
        : null
      setTrustedMCodeHooks(uiState?.trustedMCodeHooks ?? {})
    ...
    async (project: GitHubProjectRef, options: { viewNumber?: number } = {}): Promise<void> => {
      ...
      try {
        ...
        if (explicitView && explicitView.layout !== 'TABLE_LAYOUT') {
          throw new Error("MCode doesn't support this GitHub Project layout yet.")
        }
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          // Why: desktop prompts before running repo-owned mcode.yaml hooks. Mobile
          ...
          setMCodeYamlTrustPrompt({
            ...
            previouslyApproved: wasSetupHookPreviouslyApproved(trustedMCodeHooks, targetRepo.id)
          })
          return
        }
  ...
  const createWorkspaceFromProjectRow = useCallback(
    async (row: GitHubProjectRow): Promise<void> => {
      ...
      if (!kind || !row.content.number || !row.content.url) {
        setError('Add the project item repository to MCode before creating a workspace.')
        return
      }
      if (!repo) {
        const slug = splitRepositorySlug(row.content.repository)
        setProjectRepoNotInMCode({
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {!tasksSupported ? (
        tasksUnsupported ? (
          <View style={styles.centered}>
            <Text style={styles.emptyText}>Update MCode desktop</Text>
      ...
      <PickerModal
        ...
        onSelect={(viewId) => {
          ...
          if (view && view.layout !== 'TABLE_LAYOUT') {
            setGithubProjectError("MCode doesn't support this GitHub Project layout yet.")
            return
          }
      ...
      <BottomDrawer
        visible={taskUiReady && mcodeYamlTrustPrompt != null}
        onClose={() => setMCodeYamlTrustPrompt(null)}
        zIndex={TASK_SECONDARY_DRAWER_Z_INDEX + 1}
      >
        {mcodeYamlTrustPrompt ? (
          <View>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>
                {mcodeYamlTrustPrompt.previouslyApproved
                  ? `${mcodeYamlTrustPrompt.repoName}'s setup script changed`
                  : `Run setup from ${mcodeYamlTrustPrompt.repoName}?`}
              </Text>
              <Text style={styles.sheetSubtitle}>
                This repository's mcode.yaml runs on your machine before the workspace starts. Only
            ...
            <View style={styles.setupPromptBox}>
              <View style={styles.detailSectionHeader}>
                <Text style={styles.detailSectionTitle}>
                  {mcodeYamlTrustPrompt.previouslyApproved ? 'New setup script' : 'Setup script'}
                </Text>
              </View>
              <Text style={styles.setupPromptCommand}>{mcodeYamlTrustPrompt.scriptContent}</Text>
            </View>

            <View style={styles.actionGroup}>
              <Pressable
                style={styles.actionRow}
                disabled={creatingKey === mcodeYamlTrustPrompt.item.key}
                onPress={() =>
                  void (async () => {
                    try {
                      await persistSetupHookTrust(
                        mcodeYamlTrustPrompt.repoId,
                        mcodeYamlTrustPrompt.contentHash,
                        false
                      )
                      setMCodeYamlTrustPrompt(null)
                      await createWorkspace(
                        mcodeYamlTrustPrompt.item,
                        mcodeYamlTrustPrompt.repoIdOverride,
                        'run',
                        mcodeYamlTrustPrompt.agentOverride,
                        mcodeYamlTrustPrompt.workspaceNameOverride,
                        mcodeYamlTrustPrompt.noteOverride,
                        mcodeYamlTrustPrompt.baseBranchOverride,
                        mcodeYamlTrustPrompt.branchNameOverride,
                        mcodeYamlTrustPrompt.sparseCheckoutOverride,
                        mcodeYamlTrustPrompt.contentHash
                      )
                    } catch (err) {
              ...
              <Pressable
                ...
                disabled={creatingKey === mcodeYamlTrustPrompt.item.key}
                onPress={() => {
                  const prompt = mcodeYamlTrustPrompt
                  setMCodeYamlTrustPrompt(null)
                  void createWorkspace(
                    prompt.item,
      ...
      <BottomDrawer
        visible={taskUiReady && projectRepoNotInMCode != null}
        onClose={() => {
          setProjectRepoNotInMCode(null)
        }}
      >
        {projectRepoNotInMCode ? (
          <View>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>Repository not in MCode</Text>
              <Text style={styles.sheetSubtitle}>
                {projectRepoNotInMCode.owner}/{projectRepoNotInMCode.repo} is not added to MCode. Add
                this repository from the desktop app, then refresh mobile Tasks.
              </Text>
            </View>

            <View style={styles.actionGroup}>
              {projectRepoNotInMCode.url ? (
                <Pressable
                  style={styles.actionRow}
                  onPress={() => {
                    if (projectRepoNotInMCode.url) {
                      void Linking.openURL(projectRepoNotInMCode.url)
                    }
                  }}
                >
              ...
              {projectRepoNotInMCode.url ? <View style={styles.actionSeparator} /> : null}
              <Pressable
                style={styles.actionRow}
                onPress={() =>
                  void copyTextToClipboard(
                    `project-repo:${projectRepoNotInMCode.owner}/${projectRepoNotInMCode.repo}`,
                    `${projectRepoNotInMCode.owner}/${projectRepoNotInMCode.repo}`
                  )
                }
              >
            ...
            <View style={styles.actionGroup}>
              ...
              projectRowItem.content.state !== 'MERGED' ? (
                <>
                  ...
                  {!projectRowHostedRepo ? (
                    <Text style={styles.emptyInlineText}>
                      Merge requires this repository in MCode.
                    </Text>
                  ) : null}
                </>
              ) : null}
