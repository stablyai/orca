...
// multi-window transition — including activity recreation, because the
// holder is seeded from the Application context once per process.
...
// create/resume/configurationChange, emit didUpdateDimensions, and request
// a decorView relayout; `density` is added to configChanges so pane-density
// switches flow through configurationChange without activity recreation —
// the WebView (XTerm) CSS-density divergence that recreation cannot fix is
// instead healed JS-side by unit-converting RN-passed heights into the
// WebView's own CSS px (see terminal fit-measure rnPixelRatio contract).
