...
- On the RN side `PixelRatio.get()` is read inside a `useMemo` keyed only on
  `[layout, browserViewMode]` (`src/browser/use-mobile-browser-stream.ts`), so
  the browser screencast request keeps the old DPR after a density change.
...
- `use-mobile-browser-stream.ts`: include the
  `Dimensions.addEventListener('change')`-driven `usePixelRatio()` hook
  (`src/platform/use-pixel-ratio.ts`) in the screencast request `useMemo` deps.
...
| 13  | `src/platform/use-pixel-ratio.ts` (new), `src/browser/use-mobile-browser-stream.ts`                                                                                                                                                                     | DPR-reactive screencast request                                                                                                                                                                                                                                        | extend `browser-screencast-request.test.ts` with a DPR change                                                                                                                                                                                                                                                                                                     |
