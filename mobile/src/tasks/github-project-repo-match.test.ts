...
const repos = [
  { id: 'repo-1', path: '/Users/me/mcode', displayName: 'mcode' },
...
describe('GitHub project repo matching', () => {
  it('normalizes owner/repo slugs case-insensitively', () => {
    expect(normalizeGitHubRepositorySlug(' StablyAI/MCode ')).toBe('mcode-ide/mcode')
    expect(normalizeGitHubRepositorySlug('mcode')).toBeNull()
    expect(normalizeGitHubRepositorySlug('mcode-ide/mcode/extra')).toBeNull()
  })

  it('matches project rows by resolved repo slug before path/display heuristics', () => {
    expect(
      findRepoForGitHubProjectRepository('mcode-ide/mcode', repos, {
        'repo-1': {
          path: '/Users/me/mcode',
          repository: { owner: 'stablyai', repo: 'mcode' }
        }
      })
    ).toBe(repos[0])
  })
  ...
  it('falls back to exact display/path slug matching when slug resolution is unavailable', () => {
    expect(
      findRepoForGitHubProjectRepository('mcode-ide/mcode', [
        { id: 'repo-1', path: '/Users/me/mcode-ide/mcode', displayName: 'mcode' }
      ])
    ).toEqual({ id: 'repo-1', path: '/Users/me/mcode-ide/mcode', displayName: 'mcode' })
  })

  it('normalizes Windows paths before path slug fallback matching', () => {
    expect(
      findRepoForGitHubProjectRepository('mcode-ide/mcode', [
        { id: 'repo-1', path: 'C:\\Users\\me\\stablyai\\mcode', displayName: 'mcode' }
      ])
    ).toEqual({ id: 'repo-1', path: 'C:\\Users\\me\\stablyai\\mcode', displayName: 'mcode' })
  })
  ...
  it('does not path-match a repo whose resolved slug points somewhere else', () => {
    expect(
      findRepoForGitHubProjectRepository(
        ...
        {
          'repo-1': {
            path: '/Users/me/mcode-ide/mcode',
            repository: { owner: 'fork', repo: 'mcode' }
          }
        }
      )
    ).toBeNull()
  ...
  it('filters project rows to rows backed by open repositories', () => {
    const rows = [
      { id: 'row-1', content: { repository: 'mcode-ide/mcode' } },
    ...
    expect(
      findRepoForGitHubProjectRepository(
        ...
        repos,
        {
          'repo-1': {
            path: '/Users/me/mcode',
            repository: { owner: 'stablyai', repo: 'mcode', host: 'github.com' }
          },
          'repo-2': {
            path: '/Users/me/other',
            repository: {
  ...
  it('leaves a failed slug lookup matchable by the path fallback', () => {
    expect(
      findRepoForGitHubProjectRepository(
        'mcode-ide/mcode',
        [{ id: 'repo-1', path: '/Users/me/mcode-ide/mcode', displayName: 'mcode' }],
        { 'repo-1': { path: '/Users/me/mcode-ide/mcode', repository: null, failed: true } }
      )
    ).toEqual({ id: 'repo-1', path: '/Users/me/mcode-ide/mcode', displayName: 'mcode' })
  })
})
...
describe('dropFailedGitHubRepoSlugEntries', () => {
  it('drops only the entries a retry could still resolve', () => {
    expect(
      dropFailedGitHubRepoSlugEntries({
        'repo-1': { path: '/a', repository: { owner: 'stablyai', repo: 'mcode' } },
