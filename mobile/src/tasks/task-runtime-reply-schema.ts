 ...
 * — `status.capabilities?.includes(…)` in use-mobile-tasks-runtime-hydration.tsx:208 and in
 * src/shared/file-mutation-ownership.ts:10, `result.capabilities ?? []` in
 * worktree-create-capability.ts:55. So the requirement is the container: main read `.capabilities`
 ...
 *
 * `worktreeCreateIdempotency` is `unknown` on purpose. worktree-create-capability.ts:59-72 does its
 ...
 * the container is the requirement and every member under it stays optional: :277 spells
 * `uiState?.trustedOrcaHooks ?? {}` and :278 `uiState?.taskResumeState ?? {}`.
 *
 ...
 * screen's whole persisted view state, re-read field by field with its own defaults at :279 and
 ...
 * (use-mobile-tasks-runtime-hydration.tsx:305, mobile-home-host-requests.ts:107) and the
 ...
 *
 * `connected` is compared to `true` at use-mobile-tasks-runtime-hydration.tsx:304,
 * mobile-home-host-requests.ts:108 and use-new-workspace-runtime-context.ts:96, so absence and
 ...
 * (use-mobile-tasks-client-settings-actions.tsx:201, setup-hook-trust.ts:49); `settings.update` is
