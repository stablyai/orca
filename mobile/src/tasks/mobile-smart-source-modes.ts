...
export type SmartModeIcon =
  ...
  | { type: 'provider'; provider: 'github' | 'gitlab' | 'linear' | 'plane' }
...
// Smart · GitHub · Linear · GitLab · Plane · Branch · Name.
export const SMART_MODE_OPTIONS: readonly SmartModeOption[] = [
  ...
  { id: 'plane', label: 'Plane', icon: { type: 'provider', provider: 'plane' } },
...
export function resolveAvailableSmartModes(input: SmartModeAvailabilityInput): SmartNameMode[] {
  ...
  return SMART_MODE_OPTIONS.filter((option) => {
    switch (option.id) {
      ...
      case 'plane':
        return input.tasksSupported && input.planeAvailable
      case 'branches':
