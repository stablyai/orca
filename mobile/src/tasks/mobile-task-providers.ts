export type TaskProvider = 'github' | 'gitlab' | 'linear' | 'clickup'

const MOBILE_TASK_PROVIDERS: readonly TaskProvider[] = ['github', 'gitlab', 'linear', 'clickup']
...
export function filterAvailableTaskProviders(
  ...
  const available = visibleProviders.filter((provider) => {
    ...
    return availability.clickUpConnected
  })
