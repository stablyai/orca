...
// Same-named repos on different hosts (a local "mcode" and a runtime "mcode")
...
describe('every worktree stays visible regardless of repo.list contents', () => {
  it('shows worktrees whose repoId is missing from repo.list', () => {
    ...
    const worktrees = [
      worktree('mcode-local', 'a'),
      worktree('mcode-local', 'b'),
      worktree('mcode-runtime', 'c')
    ]
    const repoIdsByName = new Map([['mcode-runtime', 'mcode-runtime']])
  ...
  it('shows every worktree when the same repo name maps to two host-specific ids', () => {
    // The exact trigger: a local "mcode" and a runtime "mcode". repo.list returns
    // both under the same displayName; a name-keyed map keeps only the last id.
    const worktrees = [
      worktree('mcode-local', 'local-1', 'mcode'),
      worktree('mcode-local', 'local-2', 'mcode'),
      worktree('mcode-runtime', 'runtime-1', 'mcode')
    ]
    // Name-keyed map collapses "mcode" to a single id, as the host screen builds it.
    const repoIdsByName = new Map([['mcode', 'mcode-runtime']])
