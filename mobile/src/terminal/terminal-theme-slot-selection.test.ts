...
describe('selectMobileTerminalTheme', () => {
  ...
  it('passes a mode-mismatched host theme through by identity on a null slot', () => {
    // Pins pure passthrough until Appearance is revealed: light app + dark host
    // must still return the host palette, not a desktop default.
    expect(selectMobileTerminalTheme(FOLLOW_DESKTOP, 'light', hostTheme)).toBe(hostTheme)
  })
})
