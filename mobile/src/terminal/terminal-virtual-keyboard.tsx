...
// Why: keyboard mode renders inside a fixed-height frame, so the exported height must track geometry.
export const KEYBOARD_HEIGHT = ROW_COUNT * KEY_HEIGHT + (ROW_COUNT - 1) * KEY_GAP + spacing.xs * 2
...
export function TerminalVirtualKeyboard({ canSend, onSendBytes }: Props): ReactElement {
  ...
  function startRepeat(key: string): void {
    ...
    repeatTimer.current = setTimeout(() => {
      repeatInterval.current = setInterval(() => onSendBytes(built.bytes), REPEAT_INTERVAL)
    }, REPEAT_START_DELAY)
  }
...
function Key({
  ...
  return (
    <Pressable
      ...
      accessibilityState={{ disabled, ...(def.modifier ? { selected: active === true } : {}) }}
      disabled={disabled}
