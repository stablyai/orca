...
// Why: Metro cannot bundle runtime imports from repo-root src/shared, so mobile
// mirrors the desktop summary contract locally.
...
function buildCommitFailurePromptFileLines(
  entries: Pick<MobileGitStatusEntry, 'path' | 'status' | 'area'>[]
): string[] {
  if (entries.length === 0) {
    return ['- No staged files were reported by Source Control. Start with git status.']
  }

  return entries.map((entry) => `- ${JSON.stringify(entry.path)} (${entry.status}, ${entry.area})`)
}
