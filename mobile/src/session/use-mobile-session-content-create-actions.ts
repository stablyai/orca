...
export function useMobileSessionContentCreateActions(
  ...
  async function handleCreateBrowser(rawUrl = 'about:blank'): Promise<boolean> {
    if (!sessionOperations || creatingBrowser) {
      return false
    }
    ...
    try {
      ...
      const created = await sessionOperations.tab.createBrowser(worktreeId, url)
  ...
  async function handleBrowserNavigationCommand(
    ...
    if (!sessionOperations || !tab.browserPageId) {
    ...
    try {
      ...
      if (method === 'browser.back') {
        await sessionOperations.browser.back(target)
      } else if (method === 'browser.forward') {
        await sessionOperations.browser.forward(target)
      } else {
        await sessionOperations.browser.reload(target)
      }
