...
export function useMobileKeypadHeight(): {
  ...
  function measureContentHeight(contentHeight: number): void {
    ...
    // Why: the grid can shrink (shortcuts removed); clamp so the dock doesn't keep reserving rows that no longer exist.
    setHeight((current) => Math.min(current, snapped(contentHeight)))
  }
