...
describe('useMobileNativeChatMessageSend', () => {
  ...
  const mount = (
    ...
    options?: {
      handleRef?: { current: string | null }
      recoverInputLease?: (handle: string, sessionTabId: string | null) => Promise<boolean>
    }
  ): void => {
    function Probe(): null {
      api = useMobileNativeChatMessageSend({
        ...
        enabled: true,
        handleRef: options?.handleRef ?? { current: 'term' },
        activeSessionTabIdRef: { current: 'tab' },
        deviceTokenRef: { current: 'device' },
        recoverInputLease: options?.recoverInputLease ?? (async () => false),
  ...
  it('recovers the lease after a rejected clear and retries the whole attempt', async () => {
    const handleRef = { current: 'old' as string | null }
    const recoverInputLease = vi.fn(async () => {
      handleRef.current = 'new'
      return true
    })
    clearInputWrite.mockResolvedValueOnce(false).mockResolvedValueOnce(true)
    mount(() => ({ text: DRAFT, createdAt: 1 }), { handleRef, recoverInputLease })

    await act(async () => {
      await api!.send('hello')
    })

    expect(recoverInputLease).toHaveBeenCalledWith('old', 'tab')
    expect(clearInputWrite.mock.calls.map(([args]) => args.terminal)).toEqual(['old', 'new'])
    expect(sendWithOutcome).toHaveBeenCalledTimes(1)
    expect(sendWithOutcome.mock.calls[0]![0]).toMatchObject({ terminal: 'new' })
  })

  it('re-clears a recovered handle before retrying a rejected body', async () => {
    const handleRef = { current: 'old' as string | null }
    const recoverInputLease = vi.fn(async () => {
      handleRef.current = 'new'
      return true
    ...
    expect(clearInputWrite.mock.calls.map(([args]) => args.terminal)).toEqual(['old', 'new'])
    expect(sendWithOutcome.mock.calls.map(([args]) => args.terminal)).toEqual(['old', 'new'])
  })
