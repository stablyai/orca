...
// Why: mirror the desktop placeholder marks (icons.tsx) — no bundled Cursor
// brand mark or MuseSpark branding yet, so a monogram / spark keeps the row
// recognizable without inventing a trademark.
export function CursorIcon({ size = 16 }: { size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 16 16">
      <Rect width={16} height={16} rx={3} fill={colors.textPrimary} opacity={0.14} />
      <SvgText
        x={8}
        y={11.5}
        textAnchor="middle"
...
export function MuseSparkIcon({ size = 16 }: { size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 16 16">
      <Rect width={16} height={16} rx={3} fill={colors.textPrimary} opacity={0.14} />
      <Path
