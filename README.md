...
Optional local base refresh fast-forwards the base branch when safe. If incoming files would overwrite untracked or ignored files in its checkout, Orca skips the refresh and continues creating the new worktree. Orca also skips checked-out base branches whose names contain `=`, because it cannot safely override their merge preferences on all supported Git versions. These safeguards apply to local and SSH worktrees. If refresh fails, the warning includes local-file collisions among the possible causes and advises preserving files before a manual update.
