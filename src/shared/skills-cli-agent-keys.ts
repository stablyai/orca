...
/**
 * The community `skills` CLI's own `--agent` key for each agent MCode detects.
 *
 ...
 * rather than guessed. MCode ids and skills keys agree less often than they look
 * (`claude` is `claude-code`, `rovo` is `rovodev`, `aug` is `augment`), and some
 * near-matches are different products — MCode's `aider` CLI is not the CLI's
 * `aider-desk`, and MCode's `openclaude` is its `openclaw` in name only, so it
 * follows MCode's own rule that OpenClaude reads Claude-owned roots.
 */
export const SKILLS_CLI_AGENT_KEY_BY_TUI_AGENT = {
  claude: 'claude-code',
  'claude-agent-teams': 'claude-code',
  // Why: MCode states OpenClaude reads Claude-owned roots (native-chat-agent-profiles).
...
/** Map detected MCode agents onto `skills --agent` keys, plus the universal target. */
