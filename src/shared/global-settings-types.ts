...
export type GlobalSettings = {
  ...
  workspaceDirHistory?: MCodeWorkspaceLayout[]
  ...
  /** When enabled, MCode renames a workspace's auto-generated creature branch to
  ...
  /** macOS: toggles the additive menu-bar entry (MCode survives last-window close); doesn't change Dock behavior. */
  ...
  /** Whether to show the MCode app name in the titlebar. */
  ...
  /** Only toggles the sidebar shortcut; MCode Mobile stays reachable from Settings. */
  ...
  /** MCode-first keeps app shortcuts from TUIs; terminal-first is opt-in to let shell/TUI bindings win. */
  ...
  /** Legacy keyboard-shortcut overrides; new writes go to ~/.mcode/keybindings.json, migrated once when present. */
  ...
  /** Why: durable main-owned pref so MCode can prepare shared ~/.codex before the renderer hydrates. */
   ...
   *  History-only: does not change which account/config/hooks MCode uses. */
  ...
  /** When true, MCode requests local awake assertions while hook-reported agents are working. */
...
export type MCodeWorkspaceLayout = {
  path: string
  nestWorkspaces: boolean
}
