...
describe('resolveNativeChatTranscriptAgent', () => {
  ...
  it('passes codex, grok, omp and muse through and rejects everything else', () => {
    ...
    expect(resolveNativeChatTranscriptAgent('muse')).toBe('muse')
...
describe('isNativeChatSupportedAgent', () => {
  it('recognizes the parseable agents and rejects unknown / nullish input', () => {
    ...
    expect(isNativeChatSupportedAgent('muse')).toBe(true)
...
describe('nativeChatRequiresLocalTranscript', () => {
  it('covers the agents whose hook discloses no transcript path', () => {
    // Claude/Codex report `transcript_path`; Grok, omp and muse report only an
    ...
    expect(nativeChatRequiresLocalTranscript('muse')).toBe(true)
...
describe('shouldStepNativeChatAskAnswer', () => {
  ...
  it('does not step other or unknown agents', () => {
    ...
    expect(shouldStepNativeChatAskAnswer('muse')).toBe(false)
