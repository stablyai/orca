...
// the relay sends to MCode. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between MCode's main process and the remote relay.
//
// Invariants both ends rely on:
// - The relay normalizes; MCode routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. MCode's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is MCode's
//   local handle on an `ssh2` connection, not a wire identity. MCode stamps the
...
//   POST body so MCode's warn-once protocol diagnostics still fire. The relay
...
// with the same value so MCode can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → MCode. */
export type AgentHookRelayEnvelope = {
  ...
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know MCode's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so MCode can
  ...
  /** True when the relay is replaying its cache after MCode reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets MCode's warn-once
   ...
   *  MCode's `ingestRemote` validates it again at the SSH trust boundary. */
...
/**
 * Re-attaches shed fields from MCode's cached payload for this pane, so a transport-level
...
/** JSON-RPC request method MCode issues after `--connect` reattach to ask the
 *  relay to replay its per-paneKey last-payload cache. Pull, not push: a relay
 *  that pushed on `setWrite` can emit before MCode has wired its `agent.hook`
...
/** JSON-RPC request method MCode issues at session-ready to ship the
...
export type AgentHookInstallManagedHooksParams = {
  /** SHA-256 fingerprint of the server key negotiated by MCode's SSH transport. */
  hostKeyFingerprint?: string
  /** Positively detected and enabled agents allowed to mutate remote config. */
  agents: readonly AgentHookTarget[]
}

/** Feature-flag env var. Read once at process start by MCode and the relay.
 *  Remote agent hooks ship as the default SSH behavior; set "0" to opt out. */
export const MCODE_FEATURE_REMOTE_AGENT_HOOKS_ENV = 'MCODE_FEATURE_REMOTE_AGENT_HOOKS' as const

export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[MCODE_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
