...
import { agentTurnEndedUncleanly } from './agent-main-agent-verdict'
 ...
 *   - a `done` that neither stopped nor failed (its `stateStartedAt` — unmoved by same-state tool/prompt pings); or
...
export function agentEntryCompletionAt(entry: AgentCompletionSource): number | null {
  if (entry.state !== 'done' || agentTurnEndedUncleanly(entry)) {
    return null
  }
