 ...
 */
export type PiAgentKind = 'pi' | 'omp' | 'prime-agent' | 'kimchi'
...
export const PRIMARY_AGENT_DIR_ENV_BY_KIND: Readonly<Record<PiAgentKind, string>> = {
  ...
  kimchi: 'KIMCHI_CODING_AGENT_DIR'
}
...
export const SOURCE_AGENT_DIR_ENV_BY_KIND: Readonly<Record<PiAgentKind, string>> = {
  ...
  kimchi: 'ORCA_KIMCHI_SOURCE_AGENT_DIR'
}
...
const KIMCHI_LAUNCH_BINARY = getLaunchBinary(TUI_AGENT_CONFIG.kimchi.launchCmd)
...
export function detectExplicitPiAgentKindFromCommand(
  ...
  if (binary === KIMCHI_LAUNCH_BINARY) {
    return 'kimchi'
  }
