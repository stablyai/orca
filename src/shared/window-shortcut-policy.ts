...
export function resolveWindowShortcutAction(
  ...
  if (actionMatches('workspace.openInLastApp', input, platform, keybindings, options)) {
    return { type: 'openWorkspaceInLastApp' }
  }
...
export function getWindowShortcutActionId(action: WindowShortcutAction): KeybindingActionId | null {
  switch (action.type) {
    ...
    case 'openWorkspaceInLastApp':
      return 'workspace.openInLastApp'
    case 'openTasks':
      return 'view.tasks'
