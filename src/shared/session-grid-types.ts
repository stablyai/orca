...
/** Wheel ownership over a terminal; Shift temporarily inverts the destination. */
export type SessionGridWheelTarget = 'focus' | 'terminal' | 'grid'

/** Keep the shipped wire enum: older hosts cannot store a new Focus value. */
export type PersistedSessionGridWheelTarget = 'auto' | 'terminal' | 'grid'
...
export function serializeSessionGridWheelTarget(
  value: SessionGridWheelTarget
): PersistedSessionGridWheelTarget {
  return value === 'focus' ? 'auto' : value
}
