...
export function normalizeTerminalFontWeight(fontWeight) {
    ...
    return Math.min(TERMINAL_FONT_WEIGHT_MAX, Math.max(TERMINAL_FONT_WEIGHT_MIN, Math.round(numericFontWeight)));
}
export function resolveTerminalFontWeights(fontWeight) {
    const normalizedFontWeight = normalizeTerminalFontWeight(fontWeight);
    return {
        fontWeight: normalizedFontWeight,
        fontWeightBold: Math.min(TERMINAL_FONT_WEIGHT_MAX, Math.max(DEFAULT_TERMINAL_FONT_WEIGHT_BOLD, normalizedFontWeight + 200))
    };
}
