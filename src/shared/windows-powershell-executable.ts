...
export type WindowsPowerShellResolutionDiagnostic = {
  family: 'pwsh.exe' | 'powershell.exe'
  resolvedPath: string | null
  candidateCount: number
  rejectedAliasCandidates: string[]
  searchedPath: boolean
  reason: 'resolved' | 'not_win32' | 'alias_only' | 'not_found'
}
...
export function getWindowsPowerShellResolutionDiagnostic(
  family: 'pwsh.exe' | 'powershell.exe',
  options: WindowsPowerShellResolveOptions = {}
): WindowsPowerShellResolutionDiagnostic {
  const platform = options.platform ?? process.platform
  const env = options.env ?? process.env
  if (platform !== 'win32') {
    return {
  ...
  const candidates =
    family === 'pwsh.exe' ? getPwshCandidatePaths(env) : [getWindowsPowerShellPath(env)]
  const rejectedAliasCandidates = candidates.filter(isWindowsAppExecutionAliasPath)
  const resolvedPath = resolveWindowsPowerShellExecutablePath(family, options)
  return {
    family,
    resolvedPath,
    candidateCount: candidates.length,
    rejectedAliasCandidates,
    searchedPath: family === 'pwsh.exe',
    reason: resolvedPath
      ? 'resolved'
      : rejectedAliasCandidates.length > 0
        ? 'alias_only'
        : 'not_found'
  }
}
