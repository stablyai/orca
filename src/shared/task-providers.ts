export type TaskProvider = 'github' | 'gitlab' | 'linear' | 'jira' | 'businessmap'
...
function isTaskProviderAvailable(
  ...
  // Why: Jira and Businessmap connect from the Tasks surface itself, so hiding
  // them when disconnected would remove the entry point for first-time setup.
  if (provider === 'jira' || provider === 'businessmap') {
    return true
  }
