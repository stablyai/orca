export type TaskProvider = 'github' | 'gitlab' | 'linear' | 'jira' | 'odoo'
...
function isTaskProviderAvailable(
  ...
  // Why: Jira and Odoo can be connected from the Tasks surface itself, so
  // hiding them when disconnected would remove the entry point for setup.
  if (provider === 'jira' || provider === 'odoo') {
    return true
  }
