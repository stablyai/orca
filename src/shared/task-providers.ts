export type TaskProvider = 'github' | 'gitlab' | 'linear' | 'jira' | 'shortcut'
...
function isTaskProviderAvailable(
  ...
  // Why: Jira and Shortcut can be connected from the Tasks surface itself, so
  // hiding them when disconnected would remove the entry point for first-time setup.
  if (provider === 'jira' || provider === 'shortcut') {
    return true
  }
