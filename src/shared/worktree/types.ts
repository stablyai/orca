...
export type WorkspaceLinkedItem = {
  /** `'plugin'` covers every contributed task source. One member, not one per
   *  installed plugin: the union must not grow with what the user installs. */
  provider: 'github' | 'gitlab' | 'linear' | 'jira' | 'plugin'
  type: 'issue' | 'pr' | 'mr'
  number: number
  title: string
  url: string
  ...
  /** Installed plugin that owns a `'plugin'` item. */
  pluginKey?: string
  /** Contributed source within that plugin. */
  sourceId?: string
  repoId?: string
}
