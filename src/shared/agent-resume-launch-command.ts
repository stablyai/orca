import {
  findAgentExecutableIndex,
  isAgentExecutableToken,
  isSpliceSafeStartupCommand
} from './agent-command-terminator'
...
export function buildClaudeResumeLaunchCommand(
  ...
  const claudeIndex = findAgentExecutableIndex(tokens, shell, (token) =>
    isAgentExecutableToken(token, ['claude'])
  )
  if (claudeIndex === -1) {
    return appended
  }
  if (!isSpliceSafeStartupCommand(baseCommand, tokens, spans, shell)) {
    return appended
  }
