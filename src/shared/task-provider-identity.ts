...
export type HamTeamBoardTaskProviderIdentity = {
...
export function isStoredTaskProviderIdentity(provider: TaskProvider, identity: unknown): boolean {
  ...
  switch (provider) {
    ...
    case 'hamteamboard':
      return ['serverUrl', 'projectId', 'projectKey'].every((key) =>
        isNullableOptionalString(raw[key])
      )
  }
}
...
const TASK_PROVIDER_IDENTITY_FIELDS: Record<TaskProvider, readonly string[]> = {
  ...
  hamteamboard: ['serverUrl', 'projectId', 'projectKey']
}
...
export function taskProviderIdentityCachePart(
  ...
  switch (identity.provider) {
    ...
    case 'hamteamboard':
      return [identity.serverUrl, identity.projectId ?? identity.projectKey]
        .filter(Boolean)
        .join('/')
  }
}
