...
import type {
  MCodeDefaultTabTemplate,
  MCodeHooks,
  MCodeVmRecipe,
  MCodeVmRecipeDiagnostic
} from './mcode-yaml-hook-types'
import {
  isMCodeYamlFieldWithinLimit,
  isMCodeYamlTextWithinLimit,
...
function asTrimmedString(value: unknown): string | undefined {
  if (typeof value !== 'string' || !isMCodeYamlFieldWithinLimit(value)) {
    return undefined
  }
...
export const MCODE_VM_RECIPE_ID_PATTERN = /^[a-z0-9][a-z0-9._-]{0,63}$/
export const MCODE_VM_RECIPE_ID_RULE =
...
function normalizeDefaultTabs(value: unknown): MCodeDefaultTabTemplate[] {
  if (!Array.isArray(value) || value.length > MAX_MCODE_YAML_COLLECTION_ENTRIES) {
    return []
  }
  ...
  return value
    ...
    .filter((entry): entry is MCodeDefaultTabTemplate => entry !== null)
}

type VmRecipeParseResult = {
  recipes: MCodeVmRecipe[]
  diagnostics: MCodeVmRecipeDiagnostic[]
}

function normalizeVmRecipes(value: unknown): VmRecipeParseResult {
  const diagnostics: MCodeVmRecipeDiagnostic[] = []
  if (!Array.isArray(value)) {
    return { recipes: [], diagnostics }
  }
  if (value.length > MAX_MCODE_YAML_COLLECTION_ENTRIES) {
    return {
      recipes: [],
      diagnostics: [
        {
          index: MAX_MCODE_YAML_COLLECTION_ENTRIES,
          message: `At most ${MAX_MCODE_YAML_COLLECTION_ENTRIES} environment recipes are supported.`
        }
      ]
    }
  }
  ...
  const recipes = value
    .map((entry, index) => {
      ...
      if (!MCODE_VM_RECIPE_ID_PATTERN.test(id)) {
        diagnostics.push({
          index,
          field: 'id',
          message: `Invalid recipe id "${id}". ${MCODE_VM_RECIPE_ID_RULE}`
        })
        return null
      }
      if (seenIds.has(id)) {
      ...
      if (checkoutMode && checkoutMode !== 'mcode-worktree' && checkoutMode !== 'provisioned-root') {
        diagnostics.push({
          index,
          field: 'checkoutMode',
          message: `Recipe "${id}" checkoutMode must be "mcode-worktree" or "provisioned-root".`
        })
        return null
      }
    ...
    .filter((entry): entry is MCodeVmRecipe => entry !== null)
  return { recipes, diagnostics }
}

/**
 * Parse the supported project defaults from `mcode.yaml`.
 */
export function parseMCodeYaml(content: string): MCodeHooks | null {
  if (!isMCodeYamlTextWithinLimit(content)) {
    return null
  }
  ...
  try {
    ...
    root = document.toJS({ maxAliasCount: MAX_MCODE_YAML_ALIAS_COUNT })
  } catch {
    return null
  }
