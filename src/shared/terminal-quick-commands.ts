...
// Why: project commands use a reserved namespace so their label-derived ids do
// not collide with ids generated by the personal-command editor.
...
export function isProjectTerminalQuickCommand(command: TerminalQuickCommand): boolean {
  return command.origin === 'orca-yaml'
}
...
export function getProjectTerminalQuickCommands(
  ...
  // Why: settings normalization strips provenance; add it only after project
  // projection so a personal id cannot masquerade as executable orca.yaml data.
  return normalizeTerminalQuickCommands(candidates)
    .filter(isTerminalQuickCommandComplete)
    .map((command) => ({ ...command, origin: 'orca-yaml' as const }))
}
...
export function terminalQuickCommandListsMatch(
  ...
  return previous.every((command, index) => {
    ...
    if (
      ...
      command.origin !== candidate.origin ||
