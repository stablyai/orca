...
describe('agent process recognition', () => {
  ...
  it('does not recognize Trae headless one-shot commands as interactive agents', () => {
    ...
    // Why: past `--` nothing is a flag, so this is the interactive pane MCode itself launches.
  ...
  it('does not recognize Prime Agent non-interactive --mode runs as interactive agents', () => {
    ...
    // Why: `text` is the interactive TUI mode MCode hosts.
  ...
  it('recognizes only the agent subcommand of the generic MCode CLI', () => {
    expect(recognizeAgentProcessFromCommandLine('mcode claude-teams')).toEqual({
      agent: 'claude-agent-teams',
      processName: 'mcode'
    })
    expect(recognizeAgentProcessFromCommandLine('mcode status')).toBeNull()
    expect(recognizeAgentProcessFromCommandLine('mcode-dev terminal list')).toBeNull()
    expect(recognizeAgentProcessFromCommandLine('node /usr/local/bin/mcode claude-teams')).toEqual({
      agent: 'claude-agent-teams',
      processName: 'mcode'
    })
    expect(recognizeAgentProcessFromCommandLine('node /usr/local/bin/mcode status')).toBeNull()
  })
  ...
  it('does not classify prompt text as a wrapped agent command', () => {
    expect(
      recognizeAgentProcessFromCommandLine(
        'node /tmp/not-an-agent.js "compare opencode vs mcode in Gemini CLI"'
      )
    ).toBeNull()
