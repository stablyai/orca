...
/** A queue only travels once, so the rules that bound it are checked on whichever scope carries it. */
function refineSubmission(
  submission: {
    action: z.infer<typeof MobileWebProviderReviewSubmissionActionSchema>
    summary: string
...
export const MobileWebProviderReviewSubmissionPayloadSchema = z
  .object({ ...MobileWebProviderReviewPageScope, ...submissionFields })
  .strict()
  .superRefine(refineSubmission)

export const MobileWebProviderReviewSubmissionHostParamsSchema = z
  .object({ ...MobileWebProviderReviewHostScope, ...submissionFields })
  .strict()
  .superRefine(refineSubmission)
