...
export { createHookListenerState } from './agent-hook-listener/listener-state'
...
function isCursorCompatibilityHookPayload(
  ...
  return (
    ...
    typeof hookPayload.cursor_version === 'string' &&
    hookPayload.cursor_version.trim().length > 0
  )
}
...
function shouldSuppressDuplicateCursorHookDelivery(
  ...
  const delivery: CursorHookDelivery = { source, receivedAt }
  state.cursorHookDeliveryByFingerprint.set(cacheKey, delivery)
  return false
}
...
export function normalizeHookPayload(
  ...
  return {
    ...
    teammateName:
      payloadSource === 'claude' && eventName === 'TeammateIdle'
