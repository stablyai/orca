...
export function createAgentStatusAuthorityId(role: string): string {
  return `${role}:${createRandomUuid()}`
}

// Why: the LAN web client runs on plain HTTP, where browsers hide
// crypto.randomUUID (secure-context-only). Fall back like browser-uuid.
