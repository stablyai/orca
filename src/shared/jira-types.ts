...
// 'cloud-scoped' = Atlassian Cloud with a scoped API token (email + token,
// Basic auth). Atlassian only honours those on the api.atlassian.com gateway,
// so REST calls go to `apiBaseUrl` while `siteUrl` keeps serving browse links.
