...
export type BrowserNetworkTunnelClientOptions = {
  tunnelGeneration: number
  sendBinary: (bytes: Uint8Array<ArrayBufferLike>) => boolean
  outboundMemory?: { claimApplicationBytes: (bytes: number) => (() => void) | null }
  maxStreamIds?: number
  onClosed?: (error: Error) => void
}

export class BrowserNetworkTunnelClient<Socket extends BrowserNetworkTunnelClientSocket> {
  ...
  constructor(
    options: BrowserNetworkTunnelClientOptions,
    private readonly createSocket: (callbacks: BrowserNetworkTunnelClientSocketCallbacks) => Socket
  ) {
  ...
  open(target: BrowserNetworkTunnelOpen): Promise<Socket> {
    if (this.closed) {
    ...
    const { stream, opening } = createBrowserNetworkTunnelClientStream(id, this.createSocket, {
