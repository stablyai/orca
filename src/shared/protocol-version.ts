...
// Why: hosts without this strip worktree.set's colorTag (zod drops unknown keys) and still
// report success, so the client would paint a grouping strip the next refresh silently erases.
export const WORKSPACE_COLOR_TAG_RUNTIME_CAPABILITY = 'workspace.color-tag.v1' as const
