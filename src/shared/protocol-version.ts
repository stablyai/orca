...
// Why: older hosts strip worker-start's spec/taskTitle (zod drops unknown keys) and then
// reject the call as missing --task, so the CLI must fail clearly unless advertised.
export const ORCHESTRATION_WORKER_START_SPEC_RUNTIME_CAPABILITY =
  'orchestration.worker-start-spec.v1' as const
