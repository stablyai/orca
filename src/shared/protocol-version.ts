...
// Why: adding kimchi to RESUMABLE_TUI_AGENTS grows terminal.ensureAgentSession's enum, and an
// older host answers the unknown member with invalid_argument — a code the launch fallback does
// not retry on — so clients must probe before taking the host-authority path.
export const AGENT_SESSION_KIMCHI_RESUME_RUNTIME_CAPABILITY =
  'agent-session.kimchi-resume.v1' as const
