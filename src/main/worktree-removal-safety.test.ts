...
describe('canCleanupUnregisteredMCodeLeftoverDirectory', () => {
  const repo = { path: '/repos/main' }
  const ownedMeta = { mcodeCreatedAt: 1, mcodeCreationSource: 'runtime' as const }
  const baseArgs = {
    meta: ownedMeta,
    worktreePath: '/workspaces/mcode-owned',
    runtimeWorktreePath: '/workspaces/mcode-owned',
    repo,
  ...
  it('rejects unregistered existing targets that are files or symlinks', async () => {
    ...
    await expect(
      canCleanupUnregisteredMCodeLeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/mcode-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
    await expect(
      canCleanupUnregisteredMCodeLeftoverDirectory({
        ...baseArgs,
        statPath: async (path) => {
          if (path === '/workspaces/mcode-owned') {
            return { type: 'symlink' }
          }
          throw missingPath(path)
        },
  ...
  it('rejects unregistered leftover directories with a .git marker', async () => {
    ...
    await expect(
      canCleanupUnregisteredMCodeLeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/mcode-owned/.git'], ['/workspaces/mcode-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
  ...
  it('rejects no-marker cleanup when only the MCode path shape matches', async () => {
    const isGitRepository = vi.fn().mockResolvedValue(false)
  ...
  it('rejects unregistered leftover directories that still answer git status', async () => {
    ...
    expect(isGitRepository).toHaveBeenCalledWith('/workspaces/mcode-owned')
  })
    ...
    await expect(
      canCleanupUnregisteredMCodeLeftoverDirectory({
        ...
        registeredWorktrees: [
          ...
          makeGitWorktree('/workspaces/mcode-owned/child')
        ],
        statPath: makeStatPath([], ['/workspaces/mcode-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: /workspaces/mcode-owned/child'
    )
  })
  ...
  it('uses runtime paths for filesystem proof and original paths for nested worktree checks', async () => {
    const statPath = vi.fn(async (path: string) => {
      if (path === '/mnt/c/workspaces/mcode-owned') {
        return { type: 'directory' }
      }
      throw missingPath(path)
    })
    ...
    await expect(
      canCleanupUnregisteredMCodeLeftoverDirectory({
        ...baseArgs,
        worktreePath: 'C:\\workspaces\\mcode-owned',
        runtimeWorktreePath: '/mnt/c/workspaces/mcode-owned',
        repo: { path: 'C:\\repos\\main' },
        runtimeRepoPath: '/mnt/c/repos/main',
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\mcode-owned-sibling')
        ],
        statPath,
        isGitRepository
      })
    ...
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/mcode-owned')
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/mcode-owned/.git')
    expect(statPath).not.toHaveBeenCalledWith('C:\\workspaces\\mcode-owned')
    expect(isGitRepository).toHaveBeenCalledWith('/mnt/c/workspaces/mcode-owned')
  })
  ...
  it('rejects translated-runtime cleanup when original path contains a registered child', async () => {
    await expect(
      canCleanupUnregisteredMCodeLeftoverDirectory({
        ...
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\mcode-owned\\child')
        ],
        statPath: makeStatPath([], ['/mnt/c/workspaces/mcode-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: C:\\workspaces\\mcode-owned\\child'
    )
  })
})
