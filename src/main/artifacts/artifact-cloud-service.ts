...
import { prepareArtifactCloudUse } from '../mcode-profiles/profile-artifact-cloud-cleanup'
...
import type { ActiveMCodeProfileState } from '../mcode-profiles/profile-index-store'
...
async function deleteArtifactRequest(
  ...
  try {
    ...
    if (
      !(error instanceof MCodeCloudRequestError) ||
...
function authContext(
  ...
  return {
    ...
    scope,
    assertCurrent: () => {
      const current = ensureActiveMCodeProfile(userDataPath)
      const cloudCurrent =
        !expectedCloud ||
      ...
      if (
        ...
        throw new Error(
          'The signed-in MCode account changed while the artifact request was running.'
        )
      }
    }
  }
...
function storedSessionAuthContext(
  ...
  if (!active.profile.cloud) {
    throw new Error('The active MCode profile is not linked to a cloud account.')
  }
  return authContext(
    active,
    {
...
export class ArtifactCloudService {
  ...
  async update(request: ArtifactWriteRequest): Promise<ArtifactCloudOperation<ArtifactListItem>> {
    ...
    return this.withAuth(request, (token, apiUrl, auth) =>
      this.publisher.runForSource(request.sourceKey, auth, async () => {
        ...
        if (!record) {
          throw new Error('This file has not been shared from the active MCode profile.')
        }
  ...
  private async withAuth<T>(
    ...
    const active = ensureActiveMCodeProfile(this.userDataPath)
