...
const PRODUCTION_ARTIFACTS_API_URL = 'https://share.mcode.dev'
...
export function resolveArtifactCloudApiUrl(
  ...
  const candidate = override?.trim() || env.MCODE_ARTIFACTS_API_URL?.trim()
  const url = new URL(candidate || PRODUCTION_ARTIFACTS_API_URL)
  const loopback = ['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname)
  const firstParty = url.hostname === 'mcode.dev' || url.hostname.endsWith('.mcode.dev')
  ...
  if (!firstParty && !loopback) {
    throw new Error('Artifact API URLs must use an mcode.dev or loopback host.')
  }
