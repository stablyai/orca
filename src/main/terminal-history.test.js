...
const { existsSyncMock, mkdirSyncMock, writeFileSyncMock, readFileSyncMock, rmSyncMock, readdirSyncMock, statSyncMock, getPathMock } = vi.hoisted(() => ({
...
const { parseWslPathMock, toLinuxPathMock } = vi.hoisted(() => ({
    parseWslPathMock: vi.fn((_path) => null),
    toLinuxPathMock: vi.fn((p) => p)
}));
vi.mock('./wsl', () => ({
    parseWslPath: parseWslPathMock,
    toLinuxPath: toLinuxPathMock
}));
import { resolveShellKind, hashWorktreeId, ensureHistoryDir, injectHistoryEnv, updateHistFileForFallback, deleteWorktreeHistoryDir, runHistoryGc } from './terminal-history';
describe('terminal-history', () => {
    ...
    describe('ensureHistoryDir', () => {
        it('creates directory with mode 0o700', () => {
            ...
            expect(mkdirSyncMock).toHaveBeenCalledWith('/fake/userData/terminal-history/abcdef0123456789', { recursive: true, mode: 0o700 });
        });
    ...
    describe('injectHistoryEnv', () => {
        ...
        it('preserves caller-provided HISTFILE (check-before-set)', () => {
            const env = { HISTFILE: '/my/custom/histfile' };
            const result = injectHistoryEnv(env, 'repo-1::/path/wt', '/bin/zsh', '/path/wt');
    ...
    describe('runHistoryGc', () => {
        ...
        it('skips recently-created directories to avoid TOCTOU race', () => {
            ...
            readFileSyncMock.mockReturnValue(JSON.stringify({ worktreeId: 'unknown-wt', createdAt: new Date().toISOString() }));
    ...
    describe('WSL path conversion', () => {
        it('converts HISTFILE to Linux path for WSL cwd', () => {
            ...
            try {
                ...
                toLinuxPathMock.mockReturnValue('/mnt/c/Users/user/AppData/Roaming/Orca/terminal-history-wsl/Ubuntu/abc123/bash_history');
                mkdirSyncMock.mockReturnValue(undefined);
                existsSyncMock.mockReturnValue(true);
                const env = {};
                const result = injectHistoryEnv(env, 'repo-1::/wsl/path', '/bin/bash', '\\\\wsl.localhost\\Ubuntu\\home\\user\\project');
        ...
        it('stores WSL history under terminal-history-wsl/<distro>/', () => {
            ...
            try {
                ...
                toLinuxPathMock.mockImplementation((p) => p);
                mkdirSyncMock.mockReturnValue(undefined);
                existsSyncMock.mockReturnValue(true);
                const env = {};
                injectHistoryEnv(env, 'repo-1::/wsl/path', '/bin/bash', '\\\\wsl.localhost\\Ubuntu\\home\\user');
                expect(mkdirSyncMock).toHaveBeenCalledWith(expect.stringContaining('terminal-history-wsl/Ubuntu'), expect.any(Object));
            }
            finally {
                Object.defineProperty(process, 'platform', { configurable: true, value: originalPlatform });
            }
