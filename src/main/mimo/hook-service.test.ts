...
describe('MimoCodeHookService buildPtyEnv', () => {
  ...
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'mcode-mimocode-userdata-'))
    ...
    mimocodeHome = mkdtempSync(join(tmpdir(), 'mcode-mimocode-home-'))
    ...
    writeFileSync(join(configDir, 'plugins', 'mcode-mimocode-status.js'), 'USER PLUGIN')
  })
  ...
  it('mirrors user config into shared overlay and installs MCode status plugin', () => {
    ...
    const mcodePlugin = join(overlayHome, 'config', 'plugins', 'mcode-mimocode-status.js')
    expect(existsSync(mcodePlugin)).toBe(true)
    expect(readFileSync(mcodePlugin, 'utf8')).toContain('/hook/mimo-code')

    expect(
      readFileSync(join(mimocodeHome, 'config', 'plugins', 'mcode-mimocode-status.js'), 'utf8')
    ).toBe('USER PLUGIN')
  })
  ...
  it('reuses the overlay home on a second buildPtyEnv call', () => {
    ...
    expect(
      readFileSync(join(overlayHome, 'config', 'plugins', 'mcode-mimocode-status.js'), 'utf8')
