/* eslint-disable max-lines -- Why: the collector's platform-specific
   enumeration paths plus the history
...
import { queryWindowsProcessRows } from '../providers/windows-foreground-process-rows'
...
/** Indexed view of a single host process-table sweep. */
type ProcIndex = {
...
async function enumerateWindows(): Promise<ProcRow[]> {
  ...
  return rows.map(({ pid, ppid, cpu, memory }) => ({ pid, ppid, cpu, memory }))
}
