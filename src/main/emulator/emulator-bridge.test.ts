...
import { RuntimeEmulatorCommands } from '../runtime/mcode-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous MCode-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous MCode-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills MCode-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
