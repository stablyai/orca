 ...
 * MCode descendant also reads `own`. It bounds the blast radius of a bad
 ...
 * usually belongs to an unrelated process whose chain never passes through MCode,
 ...
 * `own`. That is not remote during teardown, when MCode is itself the process
...
export function classifyWindowsTreeKillTarget(
  ...
  // `taskkill /T /F` on ourselves would take MCode and every pane down with it.
