...
function syncSplit(runtime: MCodeRuntimeService, snapshot = makeSplitSnapshot()): void {
...
describe('MCodeRuntimeService terminal surface retirement', () => {
  ...
  it('rejects one stale shared-PTY surface without removing its live sibling', () => {
    ...
    const runtime = new MCodeRuntimeService(runtimeStore({ getWorkspaceSession: () => session }))
    runtime.attachWindow(1)
