...
async function createRuntime(rows: AgentStatusIpcPayload[] = []): Promise<MCodeRuntimeService> {
...
function publishRendererWorkingPane(runtime: MCodeRuntimeService, paneTitle: string): void {
...
function publishRendererReleasedPane(runtime: MCodeRuntimeService, title: string): void {
  runtime.attachWindow(1)
  runtime.syncWindowGraph(1, {
    tabs: [
      {
