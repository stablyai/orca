...
export class OrcaRuntimeRpcServer {
    ...
    server = null;
    transport = null;
    constructor({ runtime, userDataPath, pid = process.pid, platform = process.platform }) {
    ...
    async start() {
        ...
        const transport = createRuntimeTransportMetadata(this.userDataPath, this.pid, this.platform, this.runtime.getRuntimeId());
    ...
    handleConnection(socket) {
        ...
        socket.on('data', (chunk) => {
            ...
            if (Buffer.byteLength(buffer, 'utf8') > MAX_RUNTIME_RPC_MESSAGE_BYTES) {
                socket.write(`${JSON.stringify(this.errorResponse('unknown', 'request_too_large', 'RPC request exceeds the maximum size'))}\n`);
                socket.end();
                return;
            }
    ...
    async handleMessage(rawMessage) {
        ...
        if (request.method === 'terminal.list') {
            try {
                ...
                const result = await this.runtime.listTerminals(typeof worktreeSelector === 'string' ? worktreeSelector : undefined, typeof params?.limit === 'number' && Number.isFinite(params.limit)
                    ? params.limit
                    : undefined);
                return {
                    id: request.id,
        ...
        if (request.method === 'terminal.show') {
            try {
                const terminalHandle = request.params && typeof request.params === 'object' && request.params !== null
                    ? (request.params.terminal ?? null)
                    : null;
        ...
        if (request.method === 'terminal.wait') {
            try {
                ...
                if (params?.for !== 'exit') {
                    return this.errorResponse(request.id, 'not_supported_in_v1', 'Only terminal wait --for exit is supported in focused v1');
                }
        ...
        if (request.method === 'worktree.ps') {
            try {
                const limit = request.params && typeof request.params === 'object' && request.params !== null
                    ? (request.params.limit ?? null)
                    : null;
                const result = await this.runtime.getWorktreePs(typeof limit === 'number' && Number.isFinite(limit) ? limit : undefined);
                return {
                    id: request.id,
                    ok: true,
                    result,
        ...
        if (request.method === 'repo.add') {
            try {
                const pathValue = request.params && typeof request.params === 'object' && request.params !== null
                    ? (request.params.path ?? null)
                    : null;
        ...
        if (request.method === 'repo.show') {
            try {
                const selector = request.params && typeof request.params === 'object' && request.params !== null
                    ? (request.params.repo ?? null)
                    : null;
        ...
        if (request.method === 'repo.searchRefs') {
            try {
                ...
                const result = await this.runtime.searchRepoRefs(selector, query, typeof params?.limit === 'number' ? params.limit : undefined);
                return {
                    id: request.id,
                    ok: true,
                    result,
        ...
        if (request.method === 'worktree.list') {
            try {
                ...
                const result = await this.runtime.listManagedWorktrees(typeof repoSelector === 'string' ? repoSelector : undefined, typeof params?.limit === 'number' && Number.isFinite(params.limit)
                    ? params.limit
                    : undefined);
                return {
                    id: request.id,
        ...
        if (request.method === 'worktree.create') {
            try {
                ...
                const result = await this.runtime.createManagedWorktree({
                    ...
                    linkedIssue: typeof params?.linkedIssue === 'number'
                        ? params.linkedIssue
...
export function createRuntimeTransportMetadata(userDataPath, pid, platform, runtimeId = 'runtime') {
