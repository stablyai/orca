...
describe('orchestration new-worktree workers', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(runtime, 'createDeferredAgentTerminal').mockResolvedValue({
      handle: 'term_worker',
      surface: 'background'
    } as never)
  ...
  it('starts an interactive ZCode worktree before injecting the Dispatch preamble', async () => {
    ...
    expect(runtime.createDeferredAgentTerminal).toHaveBeenCalledWith(
      'id:repo::created',
      expect.objectContaining({
        agent: 'zcode',
        bareShell: true,
        title: expect.stringContaining('worker-')
      })
    )
    ...
    expect(runtime.sendTerminalAgentPrompt).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ acceptQueued: true })
    )
