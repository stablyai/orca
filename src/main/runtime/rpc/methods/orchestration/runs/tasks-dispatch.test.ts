...
describe('orchestration RPC methods', () => {
  ...
  describe('orchestration.dispatchShow', () => {
    ...
    it('--preamble regenerates the collaboration protocol of the real dispatch', async () => {
      ...
      // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: the RPC harness returns unknown; this assertion names the dispatchShow preamble field exercised by this focused collaboration protocol test.
      const result = (await call('orchestration.dispatchShow', {
        task: task.id,
        preamble: true,
        from: 'term_coord'
