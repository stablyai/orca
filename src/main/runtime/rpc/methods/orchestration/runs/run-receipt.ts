import type { RunCompletion, RunRow } from '../../../../orchestration/types'
...
export function exposeRun(run: RunRow, completion?: RunCompletion): RunReceipt {
  const exposed: Partial<RunRow> = { ...run }
  for (const column of INTERNAL_RUN_COLUMNS) {
    delete exposed[column]
  }
  return { ...(exposed as RunReceipt), ...(completion ? { completion } : {}) }
}
