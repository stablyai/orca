...
describe('orchestration.collaborationPublish', () => {
  ...
  async function call(params: Record<string, unknown>, ctx: Partial<RpcContext> = {}) {
    const parsed = method.params!.parse(params)
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: the method reads runtime plus mutation/capability envelope fields, all supplied by this focused fixture.
  ...
  it('publishes to derived subscribers and returns the receipt', async () => {
    ...
    const result = await call(publishParams(), mutationCtx())
  ...
  it('returns an empty receipt and notifies nothing for a topic with zero subscribers', async () => {
    ...
    const result = await call(publishParams({ topic: 'unsubscribed-topic' }), mutationCtx())
