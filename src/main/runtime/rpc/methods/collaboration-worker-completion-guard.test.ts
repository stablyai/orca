...
function rpcContext(runtime: OrcaRuntimeService, extra: Partial<RpcContext> = {}): RpcContext {
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: these focused RPC calls exercise only the supplied runtime and explicit orchestration envelope fields.
  return { runtime, ...extra } as RpcContext
}
...
describe('collaboration worker completion guard', () => {
  ...
  async function sendWorkerDone(
    ...
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: orchestration.send returns this lifecycle receipt on the worker_done path exercised by this focused guard test.
    return (await method.handler(params, rpcContext(fixture.runtime))) as {
