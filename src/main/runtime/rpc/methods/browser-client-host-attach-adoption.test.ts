...
describe('browser.clientHost.attach adoption', () => {
  ...
  it('does not recover a page created after the attach inventory was captured', async () => {
    ...
    // The handler only settles once recovery has returned, so this is the barrier the assertions
    // need: draining microtasks would let a regression slip through as a not-yet-issued command.
    rig.cleanups.get(`browser-client-host:${HOST_CLIENT_ID}`)?.()
    await rig.dispatch
