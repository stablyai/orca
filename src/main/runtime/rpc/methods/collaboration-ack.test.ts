...
describe('orchestration.collaborationAck', () => {
  ...
  async function call(params: Record<string, unknown>, ctx: Partial<RpcContext> = {}) {
    ...
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: the method reads runtime plus optional orchestration envelope fields, all supplied by this focused fixture.
  ...
  it('marks unread ids consumed and returns duplicate false', async () => {
    ...
    const result = await call(ackParams())
  ...
  it('acks multiple ids and echoes them back', async () => {
    ...
    const result = await call(ackParams({ messageIds: ['m0', 'm1'] }))
