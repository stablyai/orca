...
function missionSettingsFixture(): ReturnType<OrcaRuntimeService['getClientSettings']> {
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: mission.plan reads only the settings fields supplied here; the focused runtime intentionally has no persisted settings store.
  return {
...
describe('mission.plan', () => {
  ...
  function setup(): void {
    ...
    vi.spyOn(runtime, 'getClientSettings').mockReturnValue(missionSettingsFixture())
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: mission planning reads only the managed workspace id/path from this focused workspace fixture.
  ...
  async function call(params: Record<string, unknown>) {
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: mission.plan reads only runtime from RpcContext in this focused suite.
  ...
  it('plans with the selected agent and returns the parsed orchestration plan', async () => {
    ...
    expect(generateTextFromPrompt).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        agentId: 'pi',
        model: getCommitMessageAgentSpec('pi')?.defaultModelId
      }),
