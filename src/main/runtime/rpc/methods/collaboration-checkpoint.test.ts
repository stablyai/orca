...
describe('orchestration.collaborationCheckpoint', () => {
  ...
  async function call(
    ...
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: this focused RPC fixture supplies the runtime and optional envelope fields the handler reads.
  ...
  it('derives the current task from the authenticated terminal dispatch', async () => {
    ...
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: the value is returned by the collaborationCheckpoint handler under test; the assertion only names fields from that receipt.
