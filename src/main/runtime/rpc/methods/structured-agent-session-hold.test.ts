...
// was talking to lived until the desktop quit.
...
// hold. Subscribing is the only thing they do to say a chat is open, and the host no longer resumes
// every record at launch on their behalf.
describe('a paired client that predates holds', () => {
  /** Leaves the session where a restart leaves one: readable, with no provider child. */
  async function releaseAndRestoreForReading(): Promise<void> {
  ...
  async function subscribe(capabilities: string[]): Promise<void> {
    await dispatcher.dispatchStreaming(
      {
  ...
  it('keeps its session alive for as long as it is subscribed', async () => {
    await subscribe([STRUCTURED_AGENT_SESSION_RUNTIME_CAPABILITY])
  ...
  it('releases it when that subscription connection goes away', async () => {
    await subscribe([STRUCTURED_AGENT_SESSION_RUNTIME_CAPABILITY])
  ...
  it('gets its provider child back by subscribing to a session nothing is running', async () => {
    await releaseAndRestoreForReading()

    await subscribe([STRUCTURED_AGENT_SESSION_RUNTIME_CAPABILITY])

    await vi.waitFor(() => expect(store.getRecord(SESSION)?.lease.claimStatus).toBe('live'))
  })

  it('does not let a current client stream alone resume a released session', async () => {
    await releaseAndRestoreForReading()
