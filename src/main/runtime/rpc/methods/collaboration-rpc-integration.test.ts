...
function rpcContext(runtime: OrcaRuntimeService, extra: Partial<RpcContext> = {}): RpcContext {
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: these integration calls exercise only the supplied runtime and explicit orchestration envelope fields.
...
describe('local collaboration RPC integration', () => {
  ...
  it('routes five tasks through fan-in, relay, and fan-out', async () => {
    ...
    const publish = async (
      ...
      return await publishMethod.handler(
        params,
        rpcContext(runtime, {
    ...
    const checkpoint = async (
      ...
      return await checkpointMethod.handler(params, rpcContext(runtime))
    }

    const ack = async (from: string, messageIds: string[]): Promise<void> => {
      const params = ackMethod.params!.parse({ from, messageIds })
      await ackMethod.handler(params, rpcContext(runtime))
    }
  ...
  it('wakes a task-scoped checkpoint from publish and then acknowledges the context', async () => {
    ...
    const checkpointPromise = checkpointMethod.handler(checkpointParams, rpcContext(runtime))
    ...
    const ack = await ackMethod.handler(ackParams, rpcContext(runtime))
