...
describe('orchestration.collaborationConfigure', () => {
  ...
  async function call(params: Record<string, unknown>, ctx: Partial<RpcContext> = {}) {
    ...
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: the method reads runtime plus optional orchestration scope fields, all supplied by this focused fixture.
  ...
  it('registers the topology for the coordinator bound run and reports step count', async () => {
    ...
    const result = await call(configureParams())
