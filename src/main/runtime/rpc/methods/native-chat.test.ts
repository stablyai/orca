...
describe('nativeChat.readSession clientKind truncation gating', () => {
  it('requests a root-contained, bounded tail before applying the client window', async () => {
    cachedResult.value = { messages: [] }
    watcher.readArgs = null
    ...
    expect(watcher.readArgs).toEqual({
      agent: 'codex',
      sessionId: 's',
      transcriptPath: '/outside/rollout.jsonl.zst',
  ...
  it('applies the same path guard and initial window to paired-client subscriptions', async () => {
    watcher.args = null
    ...
    expect(activeWatcherArgs()).toMatchObject({
      agent: 'codex',
      sessionId: 's',
      transcriptPath: '/outside/rollout.jsonl.zst',
      requireTranscriptPathInAgentRoots: true,
