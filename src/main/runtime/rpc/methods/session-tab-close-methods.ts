...
export const SESSION_TAB_CLOSE_METHODS: RpcAnyMethod[] = [
  defineMethod({
    ...
    handler: async (params, context) => {
      // Why: unauthenticated in-process callers must not gain destructive authority
      // without an explicit user reason (main's missing-intent gate).
      ...
      // recordRuntimeCloseAttribution already opens the effect span named after the method.
