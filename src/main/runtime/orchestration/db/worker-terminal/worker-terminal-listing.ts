...
export function markWorkerTerminalUserOwned(
  ...
  try {
      ...
      .prepare(
        `SELECT id, owner_dispatch_id, pane_key, process_incarnation FROM worker_terminal_resources
          WHERE pane_key = ? AND ownership_state = 'owned'
            ...
            AND NOT EXISTS (
              SELECT 1 FROM remote_dispatch_attachments a
               WHERE a.dispatch_id = owner_dispatch_id AND a.state = 'stopping'
            )`
      )
      .all(paneKey) as {
      id: string
    ...
    for (const candidate of candidates) {
      if (processIncarnation !== null && candidate.process_incarnation !== processIncarnation) {
        continue
      }
