...
export type PreambleParams = {
  ...
  workerHandle: string
  devMode?: boolean
  // Why: packaged WSL panes install the scoped launcher as `mcode-ide`;
  // other execution hosts keep their existing bare `mcode` bridge.
  ...
  // shells have no agent prompt for MCode to reuse.
...
// Why: the dispatch preamble teaches agents about MCode's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use mcode-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong MCode instance (Section 6.4).
  const cli = params.devMode ? 'mcode-dev' : (params.cliCommand ?? 'mcode')
  ...
  const header = `You are working inside MCode, a multi-agent IDE. You are a dispatched worker.
...
prompt for MCode to reuse; if the coordinator has more for you it will
