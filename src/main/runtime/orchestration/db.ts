...
export class OrchestrationDb {
  ...
  private migrate(): void {
    ...
    try {
      ...
      // Why: bounded-worker builds also shipped user_version 26 before upstream assigned
      // v26 to receipt capacity. Reapply the idempotent migration for every pre-v29
      // database so both v26 lineages converge safely.
    ...
    try {
      ...
      for (const settledReport of settledReports) {
        ...
        if (
          ...
          !report ||
          !['worker_done', 'runtime_failure'].includes(report.kind) ||
          (settledReport.outcome !== undefined &&
            parseFederatedWorkerReportOutcome(report.payload) !== settledReport.outcome)
        ) {
          throw new OrchestrationError(
            'request_mismatch',
            `Federation acknowledgment for ${params.dispatchId} does not match its queued terminal report.`
          )
        }
      }
      const attachment = this.getRemoteDispatchAttachment(params.dispatchId)
      if (
          ...
          .prepare(
            `SELECT sequence FROM federation_relay_items
             WHERE dispatch_id = ? AND direction = 'to_home'
               AND kind IN ('worker_done', 'runtime_failure')
        ...
        if (acknowledgedReports.some((report) => !settledSequences.has(report.sequence))) {
          throw new OrchestrationError(
            'request_mismatch',
            `Federation acknowledgment for ${params.dispatchId} omits a terminal report settlement.`
          )
        }
      }
      const terminalOutcomes = new Set(
        settledReports.flatMap((report) => {
          const queued = this.getFederationRelayItem(
            params.dispatchId,
            params.direction,
            report.sequence
          )
          return queued?.kind === 'worker_done' && report.outcome ? [report.outcome] : []
        })
      )
