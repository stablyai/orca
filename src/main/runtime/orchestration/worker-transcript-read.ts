...
export async function readWorkerTranscript(
  ...
  // Why: the rowid-cursored reader owns the whole path (see
  // worker-transcript-read-opencode). A remote provider or a WSL guest session
  // can never serve the local SQLite worker (it holds the desktop's disks, not
  // the execution host's or guest's), so those workers fall back to the
  // terminal instead of failing the read. Local callers pass the deps-injected
  // reader owned by worker-transcript-read-opencode.
