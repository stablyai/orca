...
export function ensureWedgedWorkerMonitor(runtime: OrcaRuntimeService): void {
  ...
  timer.unref?.()
  monitorByRuntime.set(runtime, { detector, timer })
}
...
export function runWedgedWorkerScan(runtime: OrcaRuntimeService): WedgedWorkerScanSummary | null {
  ...
  try {
    ...
    // Why swallow: this is an advisory signal; a scan failure must never take down
    // the runtime timer chain or surface as a worker-facing error.
    console.warn('[orchestration] wedged-worker scan failed', { error })
    stopWedgedWorkerMonitor(runtime)
    return null
  }
}
