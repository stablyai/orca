...
describe('buildDispatchPreamble', () => {
  ...
  it('tells prompt-returning workers to idle without post-done polling', () => {
    ...
    expect(section).toContain('ROLE BOUNDARY (ORCH-R15)')
    expect(section).toContain('Ordinary owner/user chat is NOT a coordinator')
    expect(section).toContain('orchestration role-lease --role coordinator')
