...
import { type AccountsSnapshot, OrcaRuntimeService } from './orca-runtime'
...
describe('OrcaRuntimeRpcServer', () => {
  ...
  describe('long-poll transport (§3.1)', () => {
    ...
    it('arms keepalive for accounts.list without consuming a long-poll slot, leaving terminal.wait admission unaffected', async () => {
      ...
      // Why: holds refreshAccountsForMobile pending so both accounts.list
      // calls stay in flight while we probe the admission counter.
      ...
      try {
        ...
        // Give the keepalive interval at least one tick before resolving.
        await sleep(100)
