...
const { MOCK_GIT_WORKTREES, addWorktreeMock, computeWorktreePathMock, ensurePathWithinWorkspaceMock, invalidateAuthorizedRootsCacheMock } = vi.hoisted(() => ({
    MOCK_GIT_WORKTREES: [
        {
            path: '/tmp/worktree-a',
            head: 'abc',
...
const store = {
    ...
    getWorktreeMeta: (worktreeId) => store.getAllWorktreeMeta()[worktreeId],
    setWorktreeMeta: (_worktreeId, meta) => ({
...
computeWorktreePathMock.mockImplementation((sanitizedName, repoPath, settings) => {
...
ensurePathWithinWorkspaceMock.mockImplementation((targetPath) => targetPath);
describe('OrcaRuntimeService', () => {
    ...
    it('fails terminal stop closed while the renderer graph is reloading', async () => {
        ...
        runtime.markRendererReloading(1);
        await expect(runtime.stopTerminalsForWorktree('id:repo-1::/tmp/worktree-a')).rejects.toThrow('runtime_unavailable');
    ...
    it('fails terminal listing closed if the graph reloads during selector resolution', async () => {
        ...
        vi.mocked(listWorktrees).mockImplementationOnce(() => new Promise((resolve) => {
    ...
    it('returns a setup launch payload for CLI-created worktrees when orca.yaml defines setup', async () => {
        ...
        expect(createSetupRunnerScript).toHaveBeenCalledWith(expect.objectContaining({ id: 'repo-1', path: '/tmp/repo' }), '/tmp/workspaces/runtime-hook-test', 'pnpm worktree:setup');
        expect(runHook).not.toHaveBeenCalled();
        expect(addWorktree).toHaveBeenCalledWith('/tmp/repo', '/tmp/workspaces/runtime-hook-test', 'runtime-hook-test', 'origin/main', false);
    ...
    it('preserves create-time metadata on later runtime listings when Windows path formatting differs', async () => {
        const metaById = {};
        const runtimeStore = {
            getRepo: (id) => runtimeStore.getRepos().find((repo) => repo.id === id),
            getRepos: () => [
                {
                    id: 'repo-1',
                    path: 'C:\\repo',
            ...
            getWorktreeMeta: (worktreeId) => metaById[worktreeId],
