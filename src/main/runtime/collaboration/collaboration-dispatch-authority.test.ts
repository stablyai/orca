...
describe('requireLocalCollaborationDispatchAuthority', () => {
  ...
  function setup(handle = 'term_worker', paneKey: string | null = WORKER_PANE_KEY): string {
    ...
    const run = db.createRun({
      objective: 'collaboration authority',
      coordinatorHandle: 'term_coord',
      coordinatorPaneKey: 'tab_coord:leaf_coord'
    })
    const task = db.createTask({ spec: 'collaboration worker', runId: run.id })
    return task.id
  }
  ...
  it('throws dispatch_capability_invalid with the DB reason when the capability is wrong', () => {
    ...
    try {
      ...
      expect(error.code).toBe('dispatch_capability_invalid')
      expect(error.message).toContain('The Dispatch capability is invalid.')
    }
  })
