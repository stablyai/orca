...
describe('createCollaborationTopology', () => {
  ...
  it('copies arrays and objects instead of retaining caller references', () => {
    ...
    const acceptedTypes = ['status']
    const admission: AdmissionPolicy = { acceptedTypes, minPriority: 'normal' }
    const topology = createCollaborationTopology([
    ...
    publishesTo.push('t9')
    subscribesTo.push('t9')
    acceptedTypes.push('t9')
...
describe('requiredPublishTopicsForTask', () => {
  ...
  it('returns a fresh array each call', () => {
    ...
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: requiredPublishTopicsForTask intentionally returns a readonly copy; this mutation proves the copy cannot alias persisted topology state.
