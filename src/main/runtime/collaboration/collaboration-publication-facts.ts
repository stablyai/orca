...
function publishedTopics(db: OrchestrationDb, runId: string, producerTaskId: string): Set<string> {
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: the SELECT projects exactly one nullable payload column from the local messages schema.
  const rows = db.db
    .prepare(
