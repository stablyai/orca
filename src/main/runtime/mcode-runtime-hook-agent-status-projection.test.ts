// #11761: on a headless `mcode serve` host the HTTP agent hook is the only carrier of
...
async function projectAgentStatus(
  rows: AgentStatusIpcPayload[],
  preparePane?: (runtime: MCodeRuntimeService) => void
...
function observePaneTitle(runtime: MCodeRuntimeService, title: string): void {
  runtime.onPtyData(PTY_ID, `\x1b]0;${title}\x07`, Date.now())
}

function lastOscTitleEpochMs(runtime: MCodeRuntimeService): number {
  const pty = (
