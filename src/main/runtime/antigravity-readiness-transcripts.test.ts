 ...
 *
 * Five detector attempts were tuned against a five-line screen someone typed from memory. Nothing
 * here asserts what Antigravity prints: the transcripts do. Six are recorded from a live `agy`;
 * the rest name themselves as skipped until someone can reach them.
 *
 * Antigravity is intentionally unsupported for positive automated readiness: the captured ready
 * screen is evidence for the detector's shape, not a complete startup/account/mode contract.
 *
...
type TranscriptCase = {
  ...
  capture: string
  what: string
  /** Whether this launch path has a supported positive readiness contract. */
  expectReady: boolean
}
...
/**
 * A `tui-idle` wait resolves with an explicit readiness facet. Unsupported or unobserved
 * Antigravity evidence is never promoted to `satisfied` and cannot unlock a prompt write.
 */
async function readinessVerdict(
  ...
  try {
    ...
    return {
      ...
      readinessState: result.readiness?.state ?? null
    }
  } catch (error) {
    return {
      ready: false,
...
describe('scaffold self-check', () => {
  ...
  it('keeps an Antigravity-shaped ready screen unsupported through the harness', async () => {
    const verdict = await readinessVerdict(
      [
