...
function parseRecord(json: string): Record<string, unknown> {
  ...
  if (!isRecord(value)) {
    throw new Error('Expected runtime reply JSON object')
  }
  return value
}
  ...
  it('applies the same long-poll classification at the WebSocket boundary', async () => {
    ...
    const planner = defineMethod({
      ...
      handler: async (_params, { signal }) =>
        await new Promise<{ mission: string }>((resolve) => {
          signal?.addEventListener('abort', () => resolve({ mission: 'cancelled generation' }), {
            once: true
          })
        })
    })
    ...
    try {
      const first = server['handleWebSocketMessage'](
        ...
        (response) => replies.push(parseRecord(response)),
        () => {},
        undefined,
