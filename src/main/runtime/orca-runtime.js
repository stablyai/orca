...
import { getGitUsername, getDefaultBaseRef, getBranchConflictKind, isGitRepo, getRepoName, searchBaseRefs } from '../git/repo';
...
import { computeBranchName, computeWorktreePath, ensurePathWithinWorkspace, formatWorktreeRemovalError, isOrphanedWorktreeError, mergeWorktree, sanitizeWorktreeName, shouldSetDisplayName, areWorktreePathsEqual } from '../ipc/worktree-logic';
...
export class OrcaRuntimeService {
    ...
    leaves = new Map();
    handles = new Map();
    handleByLeafKey = new Map();
    waitersByHandle = new Map();
    ptyController = null;
    notifier = null;
    resolvedWorktreeCache = null;
    agentDetector = null;
    constructor(store = null, stats) {
        this.store = store;
        if (stats) {
    ...
    syncWindowGraph(windowId, graph) {
        if (this.authoritativeWindowId === null) {
            this.authoritativeWindowId = windowId;
        }
        ...
        for (const leaf of graph.leaves) {
            ...
            const ptyGeneration = existing && existing.ptyId !== leaf.ptyId
    ...
    async listTerminals(worktreeSelector, limit = DEFAULT_TERMINAL_LIST_LIMIT) {
    ...
    async waitForTerminal(handle, options) {
        const { leaf } = this.getLiveLeafForHandle(handle);
    ...
    async getWorktreePs(limit = DEFAULT_WORKTREE_PS_LIMIT) {
        if (!Number.isInteger(limit) || limit <= 0) {
            throw new Error('invalid_limit');
        }
        ...
        for (const worktree of resolvedWorktrees) {
            const meta = this.store?.getWorktreeMeta?.(worktree.id) ?? this.store?.getAllWorktreeMeta()[worktree.id];
    ...
    async showRepo(repoSelector) {
        return await this.resolveRepoSelector(repoSelector);
    }
    async setRepoBaseRef(repoSelector, baseRef) {
        if (!this.store) {
            throw new Error('runtime_unavailable');
        }
    ...
    async searchRepoRefs(repoSelector, query, limit = DEFAULT_REPO_SEARCH_REFS_LIMIT) {
        if (!Number.isInteger(limit) || limit <= 0) {
            throw new Error('invalid_limit');
        }
        const repo = await this.resolveRepoSelector(repoSelector);
    ...
    async listManagedWorktrees(repoSelector, limit = DEFAULT_WORKTREE_LIST_LIMIT) {
        if (!Number.isInteger(limit) || limit <= 0) {
            throw new Error('invalid_limit');
        }
    ...
    async showManagedWorktree(worktreeSelector) {
    ...
    async createManagedWorktree(args) {
        ...
        if (branchConflictKind) {
            throw new Error(`Branch "${branchName}" already exists ${branchConflictKind === 'local' ? 'locally' : 'on a remote'}.`);
        }
        let existingPR = null;
        try {
        ...
        await addWorktree(repo.path, worktreePath, branchName, baseBranch, settings.refreshLocalBaseRefOnWorktreeCreate);
    ...
    async updateManagedWorktreeMeta(worktreeSelector, updates) {
        if (!this.store) {
            throw new Error('runtime_unavailable');
        }
    ...
    async removeManagedWorktree(worktreeSelector, force = false) {
        if (!this.store) {
            throw new Error('runtime_unavailable');
        }
        const worktree = await this.resolveWorktreeSelector(worktreeSelector);
    ...
    async stopTerminalsForWorktree(worktreeSelector) {
    ...
    markRendererReloading(windowId) {
        if (windowId !== this.authoritativeWindowId) {
            return;
        }
    ...
    assertStableReadyGraph(expectedGraphEpoch) {
        if (this.graphStatus !== 'ready' || this.rendererGraphEpoch !== expectedGraphEpoch) {
            throw new Error('runtime_unavailable');
        }
    }
    async resolveWorktreeSelector(selector) {
        ...
        if (selector.startsWith('id:')) {
            ...
            candidates = worktrees.filter((worktree) => branchSelectorMatches(worktree.branch, branchSelector));
        }
        else if (selector.startsWith('issue:')) {
            candidates = worktrees.filter((worktree) => worktree.linkedIssue !== null && String(worktree.linkedIssue) === selector.slice(6));
        }
        else {
            candidates = worktrees.filter((worktree) => worktree.id === selector ||
    ...
    async resolveRepoSelector(selector) {
        if (!this.store) {
            throw new Error('repo_not_found');
        }
        ...
        if (selector.startsWith('id:')) {
            ...
            candidates = repos.filter((repo) => repo.id === selector || repo.path === selector || repo.displayName === selector);
        }
        if (candidates.length === 1) {
            return candidates[0];
        }
    ...
    buildTerminalSummary(leaf, worktreesById) {
    ...
    rejectWaitersForHandle(handle, code) {
        const waiters = this.waitersByHandle.get(handle);
        if (!waiters || waiters.size === 0) {
            return;
        }
...
function buildPreview(lines, partialLine) {
...
function appendToTailBuffer(previousLines, previousPartialLine, chunk) {
...
function buildTailLines(lines, partialLine) {
    return partialLine.length > 0 ? [...lines, partialLine] : lines;
}
function getTerminalState(leaf) {
    if (leaf.connected) {
        return 'running';
    }
...
function buildTerminalWaitResult(handle, leaf) {
    return {
        handle,
        condition: 'exit',
        satisfied: true,
...
function branchSelectorMatches(branch, selector) {
...
function compareWorktreePs(left, right) {
