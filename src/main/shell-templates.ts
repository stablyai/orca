...
/** Basename of the file every MCode-generated zsh wrapper dir is stamped with. */
export const ZSH_WRAPPER_DIR_MARKER_FILE = '.mcode-shell-wrapper'

export const ZSH_WRAPPER_DIR_MARKER_CONTENT = `# MCode-generated zsh startup wrapper directory.
# Its presence is how MCode recognises its own wrapper dir instead of treating it
 ...
 *
 * Why destroy it here: `_mcode_shell_features` is a plain (non-exported) shell
 ...
 * MCode's feature selection.
 */
export const ZSH_FEATURE_CHANNEL_BLOCK = `builtin typeset -ga _mcode_shell_features
_mcode_shell_features=(\${(s:,:)\${${SHELL_STARTUP_FEATURE_ENV}:-}})
...
export const BASH_FEATURE_CHANNEL_BLOCK = `_mcode_shell_features=",\${${SHELL_STARTUP_FEATURE_ENV}:-},"
builtin unset ${SHELL_STARTUP_FEATURE_ENV}
__mcode_has_feature() { [[ "$_mcode_shell_features" == *",$1,"* ]]; }`

// Why one line usable by both languages: __mcode_has_feature is defined with the
// same name and semantics in the zsh and bash channel blocks above.
export const SHELL_STARTUP_IDENTITY_MARKER_BLOCK = `__mcode_has_feature identity && printf "\\033]777;mcode-shell-start:%s\\007" "$$"`
 ...
 * instead of one inside MCode's wrapper dir, so #11044 cannot happen rather than
 * having to be repaired afterwards.
 *
 * MCODE_ORIG_ZDOTDIR is consumed: it has done its job, and leaving it exported
 ...
 * environment, so a stale MCODE_ORIG_ZDOTDIR written by an older build can arrive
 * on its own. Handing that back would point ZDOTDIR at an MCode wrapper dir — the
 ...
 * stamped marker file, or MCode's own path shape for wrappers older builds wrote.
 */
export const ZSH_ZDOTDIR_HANDBACK_BLOCK = `__mcode_usable_zdotdir() {
  [[ -n "\${1:-}" ]] || return 1
 ...
 * check-before-set, and it runs before any wrapper file MCode controls — so the
 * injected value is already gone, and because ZDOTDIR still points at MCode's
 ...
 *
 * `builtin unset MCODE_HISTFILE` is the root-cause fix for #11146: the variable
 ...
 */
export const BASH_HISTFILE_RESTORE_BLOCK = `if [[ -n "\${MCODE_HISTFILE:-}" ]]; then
  HISTFILE="$MCODE_HISTFILE"
  builtin unset MCODE_HISTFILE
elif [[ "\${HISTFILE:-}" == "$ZDOTDIR/.zsh_history" ]]; then
  # Why also when MCode injected nothing: /etc/zshrc derived this from MCode's
  ...
  HISTFILE="\${MCODE_ORIG_ZDOTDIR:-$HOME}/.zsh_history"
fi`
...
export function getFishShellReadyInitCommand(escapedMarker: string): string {
  return `function __mcode_shell_ready_marker --on-event fish_prompt
  builtin printf "${escapedMarker}"
  functions -e __mcode_shell_ready_marker
end`
}
