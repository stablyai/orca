...
export function parseClaudeUsageRecord(line) {
    let parsed;
    try {
...
export async function parseClaudeUsageFile(filePath) {
    const turns = [];
    const lines = createInterface({
        input: createReadStream(filePath, { encoding: 'utf-8' }),
        crlfDelay: Infinity
...
export async function buildWorktreeLookup(worktrees) {
...
export async function attributeClaudeUsageTurns(turns, worktreeLookup) {
    const attributed = [];
...
export function aggregateClaudeUsage(turns) {
    ...
    for (const turn of turns) {
        ...
        const location = session.locationBreakdown.find((entry) => entry.locationKey === turn.projectKey) ?? null;
        if (location) {
            location.turnCount++;
    ...
    return {
        sessions: [...sessionsById.values()].sort((left, right) => right.lastTimestamp.localeCompare(left.lastTimestamp)),
        dailyAggregates: [...dailyByKey.values()].sort((left, right) => left.day === right.day
            ? left.projectLabel.localeCompare(right.projectLabel)
            : left.day.localeCompare(right.day))
    };
}
export async function scanClaudeUsageFiles(worktrees) {
...
export function getClaudeProjectsDirectory() {
