...
function makeHostStatus(
  launcherPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\MCode\\resources\\bin\\mcode.exe'
) {
  return {
    platform: 'win32',
    commandName: 'mcode',
    commandPath: launcherPath,
    pathDirectory: 'C:\\Users\\me\\AppData\\Local\\Programs\\MCode\\resources\\bin',
    pathConfigured: true,
    launcherPath,
...
set -euo pipefail
# MCode managed WSL CLI launcher
# MCODE_WIN_LAUNCHER_B64=QzpcUHJvZ3JhbSBGaWxlc1xPcmNhXHJlc291cmNlc1xiaW5cb3JjYS5jbWQ=
MCODE_WIN_LAUNCHER='C:\\Program Files\\MCode\\resources\\bin\\mcode.cmd'
MCODE_BRIDGE_PS1='/home/alice/.local/share/mcode/mcode-wsl-bridge.ps1'
...
exec "$MCODE_POWERSHELL" -NoProfile -ExecutionPolicy Bypass -File "$MCODE_BRIDGE_PS1_WIN" "$MCODE_WIN_LAUNCHER" "$@"
`
...
function createWslRunner(
  ...
  const commandPath = '/home/alice/.local/bin/mcode-ide'
  const legacyCommandPath = '/home/alice/.local/bin/mcode'
  const bridgePath = '/home/alice/.local/share/mcode/mcode-wsl-bridge.ps1'
  ...
  const runner = vi.fn(async (_distro: string, command: string) => {
    ...
    if (command.includes('cat > "$command_tmp"')) {
      ...
      if (
        ...
        !files.get(bridgePath)?.includes('# MCode managed WSL CLI PowerShell bridge')
      ) {
        throw new Error('__MCODE_CONFLICT__')
      }
      const launcher =
        command.match(/cat > "\$command_tmp" <<'MCODE_WSL_CLI'\n([\s\S]*)\nMCODE_WSL_CLI/)?.[1] ?? ''
      const bridge =
        command.match(
      ...
      if (files.get(legacyCommandPath)?.includes('# MCode managed WSL CLI launcher')) {
    ...
    if (command.includes('rm -f')) {
      ...
      if (
        ...
        files.get(legacyCommandPath)?.includes('# MCode managed WSL CLI launcher')
      ) {
        files.delete(legacyCommandPath)
      }
      return ''
    ...
    if (command.includes('cat ')) {
      if (command.includes(commandPath)) {
        return files.get(commandPath) ?? '__MCODE_MISSING__'
      }
      if (command.includes(bridgePath)) {
        return files.get(bridgePath) ?? '__MCODE_MISSING__'
      }
      if (command.includes(legacyCommandPath)) {
        return files.get(legacyCommandPath) ?? '__MCODE_MISSING__'
      }
    }
...
describe('WslCliInstaller', () => {
  ...
  it('installs a WSL launcher that forwards to the Windows MCode launcher', async () => {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'not_installed',
      commandPath: '/home/alice/.local/bin/mcode-ide'
    })
    ...
    expect(wsl.getFile()).toBe(
      _internals.buildWslLauncher(
        'C:\\Users\\me\\AppData\\Local\\Programs\\MCode\\resources\\bin\\mcode.exe',
        '/home/alice/.local/share/mcode/mcode-wsl-bridge.ps1'
      )
    )
    ...
    expect(installCommand).toContain("legacy_command_path='/home/alice/.local/bin/mcode'")
  ...
  it('derives the shared WSL bridge path for current and legacy command names', () => {
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/mcode-ide')).toBe(
      '/home/alice/.local/share/mcode/mcode-wsl-bridge.ps1'
    )
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/mcode')).toBe(
      '/home/alice/.local/share/mcode/mcode-wsl-bridge.ps1'
    )
  })
  ...
  it('reports installed WSL launchers whose bin directory is missing from PATH', async () => {
    ...
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\MCode\\mcode.cmd') },
      wslRunner: wsl.runner
    })
  ...
  it('accepts current managed WSL scripts with an extra heredoc trailing newline', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      wslRunner: async (distro, command) => {
        if (command.includes('cat /home/alice/.local/share/mcode/mcode-wsl-bridge.ps1')) {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\MCode\\mcode.cmd'
    })
  })
  ...
  it('generates a launcher that forwards arguments through a PowerShell file bridge', () => {
    const launcher = _internals.buildWslLauncher(
      'C:\\Program Files\\MCode\\resources\\bin\\mcode.exe',
      '/home/alice/.local/share/mcode/mcode-wsl-bridge.ps1'
    )
    ...
    expect(launcher).toContain(
      'MCode WSL CLI requires Windows interop and could not find powershell.exe.'
    )
    expect(launcher).toContain('"$MCODE_POWERSHELL" -NoProfile -ExecutionPolicy Bypass -File')
    expect(launcher).toContain('MCODE_WSL_CWD=$(pwd -P 2>/dev/null) || {')
    expect(launcher).toContain('MCODE_WSL_CWD=/')
    expect(launcher).toContain('cd /')
    expect(launcher).toContain('MCODE_WSL_CWD_WIN=$(wslpath -w "$MCODE_WSL_CWD")')
    expect(launcher.indexOf('MCODE_WSL_CWD=$(pwd -P')).toBeLessThan(
      launcher.indexOf('MCODE_BRIDGE_PS1_WIN=$(wslpath')
    )
    expect(launcher).toContain('"$MCODE_WIN_LAUNCHER" -WslCwd "$MCODE_WSL_CWD_WIN" "$@"')
    ...
    expect(bridge).toContain('[string]$MCodeLauncher = $args[0]')
    ...
    expect(bridge).toContain('$env:MCODE_CLI_CWD = $WslCwd')
    expect(bridge).toContain('Push-Location -LiteralPath (Split-Path -Parent $MCodeLauncher)')
    ...
    expect(bridge).not.toContain('& $MCodeLauncher @ForwardArgs')
    expect(bridge).toContain('Remove-Item Env:MCODE_CLI_CWD -ErrorAction SilentlyContinue')
  ...
  it('marks stale managed launchers that point at the old app bin instead of packaged resources', async () => {
    const oldLauncher = _internals.buildWslLauncher(
      'C:\\Users\\me\\AppData\\Local\\Programs\\MCode\\bin\\mcode.cmd',
      '/home/alice/.local/share/mcode/mcode-wsl-bridge.ps1'
    )
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'stale',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\MCode\\bin\\mcode.cmd',
      launcherPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\MCode\\resources\\bin\\mcode.exe'
    })

    await expect(installer.install()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\MCode\\resources\\bin\\mcode.exe'
    })
  })

  it('repairs the frozen pre-rc4 registration so orchestration send/reply reach native rc4', async () => {
    const nativeLauncher = 'C:\\Program Files\\MCode\\resources\\bin\\mcode.exe'
    ...
    const simulateRc4Launch = (args: string[]): number => {
      ...
      return target?.toLowerCase().endsWith('mcode.cmd') &&
  ...
  it('repairs a managed launcher whose bridge is missing, but preserves a conflicting bridge', async () => {
    const nativeLauncher = 'C:\\MCode\\resources\\bin\\mcode.exe'
    ...
    const staleBridge = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER, true, {
      initialBridge: '# MCode managed WSL CLI PowerShell bridge\nWrite-Output "stale"\n'
    })
  ...
  it('moves a legacy-only managed registration to mcode-ide without touching unmanaged names', async () => {
    const nativeLauncher = 'C:\\MCode\\resources\\bin\\mcode.exe'
  ...
  it('keeps the pre-rc4 files on a transactional replacement failure', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      distro: 'Ubuntu',
      hostInstaller: {
        getStatus: async () => makeHostStatus('C:\\Program Files\\MCode\\resources\\bin\\mcode.exe')
      },
      wslRunner: wsl.runner
    })
    ...
    // would leave a window where a concurrent shell finds no mcode-ide at all.
    expect(installCommand).not.toContain('command_backup')
    expect(installCommand).not.toContain(`mv -f '/home/alice/.local/bin/mcode-ide'`)
  })

  it.skipIf(process.platform === 'win32')(
    'rolls both files back when the command replacement fails after the bridge move',
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'mcode-wsl-cli-rollback-'))
      const home = join(root, 'home with spaces')
      const commandPath = join(home, '.local', 'bin', 'mcode-ide')
      const bridgePath = join(home, '.local', 'share', 'mcode', 'mcode-wsl-bridge.ps1')
      const bridge = _internals.buildWslBridgeScript()
      await mkdir(join(home, '.local', 'bin'), { recursive: true })
      await mkdir(join(home, '.local', 'share', 'mcode'), { recursive: true })
  ...
  it('is idempotent after repairing an old managed registration', async () => {
    const nativeLauncher = 'D:\\Custom MCode\\resources\\bin\\mcode.exe'
    const wsl = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER)
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
    ...
    expect(wsl.getFile()).toContain("MCODE_WIN_LAUNCHER='D:\\Custom MCode\\resources\\bin\\mcode.exe'")
  })
  ...
  it('refuses to remove an old managed launcher when the bridge path is user-owned', async () => {
    ...
    await expect(installer.remove()).rejects.toThrow('__MCODE_CONFLICT__')
  })
})
