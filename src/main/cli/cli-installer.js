...
export class CliInstaller {
    ...
    async getStatus() {
        ...
        const baseStatus = spec.installMethod === 'symlink'
    ...
    async installSymlink(status) {
        try {
            ...
            await this.privilegedRunner(`mkdir -p ${quoteShell(dirname(status.commandPath))} && ` +
                `ln -sfn ${quoteShell(status.launcherPath)} ${quoteShell(status.commandPath)}`);
        }
    }
    async removeSymlink(commandPath) {
        try {
            ...
            await this.privilegedRunner(`if [ -L ${quoteShell(commandPath)} ]; then rm ${quoteShell(commandPath)}; fi`);
        }
    }
    async installWindowsWrapper(commandPath, launcherPath) {
        await writeFile(commandPath, buildWindowsForwarder(launcherPath), 'utf8');
    }
    async inspectSymlink(commandPath, launcherPath) {
        try {
            ...
            return this.buildStatus({
                ...
                detail: resolvedCurrentTarget === resolvedLauncher
    ...
    async inspectWindowsWrapper(commandPath, launcherPath) {
        try {
            const stats = await lstat(commandPath);
            if (!stats.isFile()) {
                return this.buildStatus({
            ...
            return this.buildStatus({
                ...
                detail: currentContent === expectedContent
                    ? `Registered at ${commandPath}.`
                    : `${commandPath} points to a different launcher.`
            });
        }
    ...
    async isPathConfigured(pathDirectory) {
        const pathValue = this.platform === 'win32' ? await this.userPathReader() : (this.processPathEnv ?? '');
        return splitPathEntries(this.platform, pathValue).some((entry) => samePathEntry(this.platform, entry, pathDirectory));
    }
    withPathInfo(status, pathDirectory, pathConfigured) {
        if (status.state !== 'installed') {
            return {
                ...status,
                pathDirectory,
    ...
    async ensureWindowsPathEntry(pathDirectory) {
    ...
    async removeWindowsPathEntry(pathDirectory) {
        if (this.platform !== 'win32') {
            return;
        }
        const current = await this.userPathReader();
        const nextEntries = splitPathEntries('win32', current).filter((entry) => !samePathEntry('win32', entry, pathDirectory));
        await this.userPathWriter(nextEntries.join(';'));
    }
}
async function ensureDevLauncher(args) {
    if (!isAbsoluteForPlatform(args.platform, args.execPath) ||
        !isAbsolute(args.cliEntryPath) ||
        !existsSync(args.cliEntryPath)) {
        return null;
    }
    const launcherPath = join(args.userDataPath, ...DEV_LAUNCHER_DIR, args.platform === 'win32' ? 'orca.cmd' : 'orca');
    ...
    const content = args.platform === 'win32'
...
function buildUnixDevLauncher(execPathValue, cliEntryPath) {
...
function buildWindowsDevLauncher(execPathValue, cliEntryPath) {
    return `@echo off
setlocal
...
function buildWindowsForwarder(launcherPath) {
    return `@echo off
setlocal
...
function splitPathEntries(platform, value) {
    if (!value) {
        return [];
    }
    return value
...
function samePathEntry(platform, left, right) {
...
async function writeWindowsUserPath(value) {
...
export function getBundledLauncherPath(platform, resourcesPath) {
