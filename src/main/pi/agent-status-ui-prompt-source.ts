...
export function getPiAgentStatusUiPromptHandlerSourceLines(kind: PiAgentKind): string[] {
  ...
  return [
    ...
    '    // session-switching modal invalidates the runner, so local turn state is the floor,',
    '    // not a fallback: with no turn in flight, no later event is coming to correct a',
    '    // working verdict, so only consult ctx when this process believes work is running.',
