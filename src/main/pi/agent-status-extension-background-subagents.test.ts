...
describe('Pi background subagent status', () => {
  ...
  it('preserves active child state across a resumed session', async () => {
    vi.useFakeTimers()
    const harness = createAgentStatusExtensionHarness({ kind: 'pi' })
    ...
    await harness.callHook('agent_settled')
    harness.reload()
    await harness.callHook('session_start', { reason: 'resume' })
    harness.emitEvent('subagent:async-complete', { runId: 'run-1' })
    await vi.advanceTimersByTimeAsync(0)
  ...
  it('cancels a deferred completion when the session shuts down', async () => {
    vi.useFakeTimers()
    const harness = createAgentStatusExtensionHarness({ kind: 'pi' })
