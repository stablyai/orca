import { cpSync, existsSync, linkSync, mkdirSync, readdirSync, rmSync, statSync, symlinkSync, writeFileSync } from 'fs';
...
function getDefaultPiAgentDir() {
    return join(homedir(), PI_AGENT_DIR_NAME, PI_AGENT_SUBDIR);
}
function mirrorEntry(sourcePath, targetPath) {
...
export class PiTitlebarExtensionService {
    ...
    mirrorAgentDir(sourceAgentDir, overlayDir) {
        ...
        for (const entry of readdirSync(sourceAgentDir, { withFileTypes: true })) {
            ...
            if (entry.name === 'extensions' && entry.isDirectory()) {
                ...
                for (const extensionEntry of readdirSync(sourcePath, { withFileTypes: true })) {
                    mirrorEntry(join(sourcePath, extensionEntry.name), join(overlayExtensionsDir, extensionEntry.name));
                }
                continue;
            }
    ...
    buildPtyEnv(ptyId, existingAgentDir) {
