...
export function getPiAgentStatusHandlerSourceLines(kind: PiAgentKind): string[] {
  ...
  return [
    ...
    ...captureSessionMetadata,
    '    if (isOmpRuntime()) {',
    '      // Why: OMP has no agent_settled and its ctx.isIdle can stay false through',
    '      // completion (the idle-recheck below would stick at "working"); willContinue',
    '      // is its terminal signal.',
    '      clearPendingAgentEndCheck()',
    '      if (!event || event.willContinue !== true) postAgentEndOnce()',
    '      return',
    '    }',
