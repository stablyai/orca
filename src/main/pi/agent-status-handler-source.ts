...
export function getPiAgentStatusHandlerSourceLines(kind: PiAgentKind): string[] {
  ...
  return [
    ...
    "// Why: a restarted agent can inherit a dead owner's PID, which would suppress",
    '// its status reporting forever. Treat a dead owner as claimable; EPERM means the',
    '// process exists under another user, so it still counts as alive.',
