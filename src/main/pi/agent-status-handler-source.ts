...
export function getPiAgentStatusHandlerSourceLines(kind: PiAgentKind): string[] {
  ...
  return [
    ...
    ...captureSessionMetadata,
    '    if (isOmpRuntime()) {',
    '      // Why: OMP has no agent_settled; willContinue is its terminal signal.',
    '      // ctx.isIdle can stay false through OMP completion, so the idle-recheck',
    '      // fallback below would leave the pane stuck at "working".',
