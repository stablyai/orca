...
describe('getPiAgentStatusExtensionSource', () => {
  ...
  it.each(['pi', 'omp', 'prime-agent'] as const)(
    'reclaims %s reporting when the inherited owner PID is dead',
    (kind) => {
      // Why: a restart can hand the new process its dead predecessor's PID. Before
      // the liveness probe that suppressed the pane's status forever.
      const ownerKey =
        kind === 'prime-agent' ? 'ORCA_PRIME_AGENT_STATUS_OWNED' : 'ORCA_PI_STATUS_OWNED'
      const restarted = createHarness({
        kind,
        pid: SELF_PID,
        env: { [ownerKey]: String(SELF_PID + 99) },
        livePids: [SELF_PID]
      })

      expect(Object.keys(restarted.handlers).length).toBeGreaterThan(0)
      expect(restarted.processEnv[ownerKey]).toBe(String(SELF_PID))
    }
  )

  it('posts again after reclaiming the pane from a dead owner', async () => {
    const restarted = createHarness({
      kind: 'pi',
      pid: SELF_PID,
      env: { ORCA_PI_STATUS_OWNED: String(SELF_PID + 99) },
      livePids: [SELF_PID]
    })

    await restarted.callHook('agent_end')

    expect(restarted.fetchMock).toHaveBeenCalledTimes(1)
  })

  it('keeps suppressing when the foreign owner runs as another user (EPERM)', () => {
    // Why: EPERM means the process exists but is not ours to signal, so the lead is
    // still alive and this process must stay quiet.
    const child = createHarness({
      kind: 'pi',
      pid: SELF_PID,
      env: { ORCA_PI_STATUS_OWNED: String(SELF_PID + 7) },
      killImpl: () => {
    ...
    expect(child.handlers).toEqual({})
    expect(child.processEnv.ORCA_PI_STATUS_OWNED).toBe(String(SELF_PID + 7))
    expect(child.fetchMock).not.toHaveBeenCalled()
  })

  it('reclaims when the inherited owner PID is not a usable pid', () => {
    const harness = createHarness({
      kind: 'pi',
      pid: SELF_PID,
    ...
    expect(Object.keys(harness.handlers).length).toBeGreaterThan(0)
