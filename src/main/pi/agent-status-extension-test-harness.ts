...
export function createHarness(args: {
  ...
  // Why: the owner guard probes liveness with process.kill(pid, 0). Declaring the
  // live set keeps that deterministic instead of depending on the runner's PIDs.
  livePids?: number[]
  ...
  const processMock = {
    ...
    kill:
      args.killImpl ??
      ((pid: number) => {
        if ((args.livePids ?? [SELF_PID]).includes(pid)) {
          return
        }
