...
export function getPiTitlebarExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  return [
    ...
    '    if (markerPainted) {',
    "      if (!paintTitle(ctx, () => getMarkedTitle(pi, '!'))) {",
    '        clearAnimation() // 80ms residual; markerTimer stays',
    '        return false',
    '      }',
    '      frameIndex++',
    '      return true',
    ...
    '    timer = setInterval(() => { if (!renderFrame(ctx)) clearAnimation() }, FRAME_INTERVAL_MS)',
    '  }',
    '',
