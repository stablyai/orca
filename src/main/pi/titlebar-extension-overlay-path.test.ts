...
const userDataDir = mkdtempSync(join(tmpdir(), 'mcode-pi-overlay-path-userdata-'))
...
describe('PiTitlebarExtensionService legacy overlay paths', () => {
  ...
  it('does not redirect path-shaped PTY ids into active Pi homes', () => {
    const piHome = mkdtempSync(join(tmpdir(), 'mcode-pi-overlay-path-home-'))
    ...
    try {
      ...
      expect(env.MCODE_PI_SOURCE_AGENT_DIR).toBe(piHome)
