...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  return [
    ...
    '// Why: Orca can restart or rotate its hook endpoint while OMP is settling.',
    '// Retry only terminal completion: losing it strands the pane in working state.',
    'const OMP_AGENT_END_RETRY_DELAYS_MS = [250, 1000, 4000] as const',
    'type PendingPost = {',
    ...
    'let runGeneration = 0',
    'let pendingAgentEndRetry: ReturnType<typeof setTimeout> | null = null',
    ...
    '',
    'function beginAgentRun(): void {',
    '  runGeneration += 1',
    '  if (pendingAgentEndRetry !== null) clearTimeout(pendingAgentEndRetry)',
    '  pendingAgentEndRetry = null',
    '}',
    '',
    ...
    '}',
    '',
    'function scheduleOmpAgentEndRetry(failed: PendingPost): void {',
    "  if (failed.hookEventName !== 'agent_end' || !failed.ompRuntime) return",
    '  if (failed.runGeneration !== runGeneration) return',
    '  const delay = OMP_AGENT_END_RETRY_DELAYS_MS[failed.retryAttempt]',
    '  if (delay === undefined) return',
    '  if (pendingAgentEndRetry !== null) clearTimeout(pendingAgentEndRetry)',
    '  pendingAgentEndRetry = setTimeout(() => {',
    '    pendingAgentEndRetry = null',
    '    if (failed.runGeneration !== runGeneration || pendingPost) return',
    '    pendingPost = { ...failed, retryAttempt: failed.retryAttempt + 1 }',
    '    drainPosts()',
    '  }, delay)',
    '}',
    '',
    ...
    '    .then((delivered) => {',
    '      if (!delivered) scheduleOmpAgentEndRetry(next)',
    '    })',
    '    .catch(() => scheduleOmpAgentEndRetry(next))',
    '    .finally(() => {',
    '      activePost = false',
    '      drainPosts()',
    '    })',
    ...
    '  } catch {',
    '    // Why: status delivery falls through to the WSL bridge or retry.',
    '  } finally {',
    '    if (timeout) clearTimeout(timeout)',
    '  }',
    '  if (delivered) return true',
    '  if (!isWslRuntime()) return false',
    '  return postViaWindowsCurl(body, ompRuntime)',
    '}',
    '',
