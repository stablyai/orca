...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  return [
    ...
    '}',
    '',
    'function resolveSessionPath(ctx: unknown): string {',
    '  const manager = (ctx as { sessionManager?: { getSessionFile?: () => unknown } } | null)',
    '    ?.sessionManager',
    '  try {',
    ...
    '      ...(sessionPath ? { session_path: sessionPath } : {}),',
    '      ...extra,',
    '    },',
    '  })',
    '  try {',
    ...
    "    await post('before_agent_start', { prompt: event.prompt ?? '' }, ctx)",
    '  })',
    '',
    "  pi.on('agent_start', async (_event, ctx) => {",
    "    await post('agent_start', {}, ctx)",
    '  })',
    '',
    ...
    '    if (!text) return',
    "    await post('message_end', { role: 'assistant', text }, ctx)",
    '  })',
    '',
    "  pi.on('agent_end', async (_event, ctx) => {",
    "    await post('agent_end', {}, ctx)",
    '  })',
    '',
    "  pi.on('session_shutdown', async (_event, ctx) => {",
    "    await post('session_shutdown', {}, ctx)",
    '  })',
    '}',
    ''
  ].join('\n')
