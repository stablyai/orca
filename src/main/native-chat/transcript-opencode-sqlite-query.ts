...
/** A v2 (OpenCode 2) DB has session_v2 instead of session — treat it as
 *  "session not found" rather than a thrown no-such-table error, mirroring
 *  session-scanner-opencode2-sqlite-list.ts's guarded reads. */
...
/** node:sqlite hands back unknown rows; this carries the module's one unchecked
 *  boundary — T names exactly the columns the caller's SELECT projects. */
function rowsOf<T>(statement: SqliteStatement, ...params: BindValue[]): T[] {
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: node:sqlite rows are unknown; T names the projected columns.
...
export function readOpenCodeTranscriptPage(args: {
  ...
  try {
    ...
    // Why: a 0/negative budget has no meaningful page semantics (callers clamp positive).
    ...
    // Why: `limit` counts renderable MESSAGES, not raw rows — a window dense in
    // non-renderable rows (step-start only) must not come back under-filled (or
    // empty) while older history remains, so keep batching older raw rows until
    // the message budget fills or rows run out.
