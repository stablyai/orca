...
const MANAGED_HOME = '/tmp/mcode-user-data/codex-runtime-home/home'
...
vi.mock('../codex/codex-home-paths', () => ({
  getMCodeManagedCodexHomePath: vi.fn(() => MANAGED_HOME),
  resolveMCodeManagedCodexHomePath: vi.fn(() => MANAGED_HOME)
}))
...
import { getMCodeManagedCodexHomePath } from '../codex/codex-home-paths'
...
beforeEach(() => {
  ...
  vi.mocked(getMCodeManagedCodexHomePath).mockClear()
  scanned.dirs = []
})
...
describe('codex sessions roots', () => {
  it('scans the managed home without materializing it', async () => {
    ...
    expect(vi.mocked(getMCodeManagedCodexHomePath)).not.toHaveBeenCalled()
  })
})
