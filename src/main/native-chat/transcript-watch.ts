...
export async function subscribeNativeChatTranscript(
  ...
  // Why: the signal-poll subscription replaces the line-decoder watch below.
  // filePath can never resolve a DB session, so only the session id gate
  // applies: a blank id must bail, not resolve-poll an unresolvable target forever.
