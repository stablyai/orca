...
import { resolveMCodeManagedCodexHomePath } from '../codex/codex-home-paths'
import {
...
// Why: MCode launches Codex with MCODE_CODEX_HOME pointing at its own managed
// runtime home, so MCode-started Codex rollout files land under
...
// fall back to CODEX_HOME/~/.codex so a non-MCode Codex transcript still resolves.
...
// Why: resolveMCodeManagedCodexHomePath avoids the mkdirSync performed by the
// getter; creating the runtime home belongs to launch, not this resolve poll.
function codexSessionsDirs(): string[] {
  const candidates = [
    join(resolveMCodeManagedCodexHomePath(), 'sessions'),
...
export type ResolveSessionFileOptions = {
   ...
   *  scans). Defaults to the mcode-managed home then CODEX_HOME/~/.codex. */
