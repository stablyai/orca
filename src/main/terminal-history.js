...
export function resolveShellKind(shellPath) {
...
function historyFilename(shell) {
    switch (shell) {
        case 'zsh':
            return 'zsh_history';
        case 'bash':
...
function getHistoryRootWsl(distro) {
...
export function ensureHistoryDir(worktreeHash, wslDistro) {
    try {
        ...
        console.warn(`[pty:history] Failed to create history directory: ${err instanceof Error ? err.message : String(err)}`);
        return null;
    }
}
/** Write meta.json alongside history files for debuggability. */
function writeMetaFile(dir, worktreeId) {
    try {
...
export function injectHistoryEnv(spawnEnv, worktreeId, shellPath, cwd) {
    const shell = resolveShellKind(shellPath);
    const result = { shell, histFile: null };
...
export function updateHistFileForFallback(spawnEnv, fallbackShellPath) {
    if (!spawnEnv.HISTFILE) {
        return;
    }
...
export function logHistoryInjection(worktreeId, result) {
    const truncatedId = worktreeId.length > 60 ? `${worktreeId.slice(0, 60)}...` : worktreeId;
    console.log(`[pty:history] worktreeId=${truncatedId} shell=${result.shell} histFile=${result.histFile ?? 'none'}`);
}
// ─── Cleanup ───────────────────────────────────────────────────────
/** Delete the history directory for a removed worktree. Non-fatal. */
export function deleteWorktreeHistoryDir(worktreeId) {
    const worktreeHash = hashWorktreeId(worktreeId);
    ...
    try {
        ...
        console.warn(`[pty:history] Failed to delete history dir: ${err instanceof Error ? err.message : String(err)}`);
    }
...
function gcScanRoot(root, liveWorktreeIds) {
...
export function runHistoryGc(liveWorktreeIds) {
    try {
        ...
        console.log(`[pty:history:gc] totalDirs=${totalDirs} orphaned=${orphaned} pruned=${pruned} totalSizeKB=${totalSizeKB}`);
    }
    catch (err) {
...
export function scheduleHistoryGc(getLiveWorktreeIds) {
    ...
    setTimeout(async () => {
        try {
            ...
            console.warn(`[pty:history:gc] Failed to enumerate live worktrees for GC: ${err instanceof Error ? err.message : String(err)}`);
        }
    }, 10_000);
}
