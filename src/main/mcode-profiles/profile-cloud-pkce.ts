...
import {
  MCODE_CLOUD_CALLBACK_RESPONSE_HEADERS,
  MCODE_CLOUD_CALLBACK_SUCCESS_PAGE
} from './profile-cloud-callback-page'

export type MCodeCloudAuthorizationCode = {
  code: string
  codeVerifier: string
  nonce: string
  redirectUri: string
...
export function beginMCodeCloudPkceFlow(
  config: MCodeCloudAuthConfig,
  localProfileId: string
): Promise<MCodeCloudAuthorizationCode> {
  ...
  return new Promise((resolve, reject) => {
    ...
    function writeInvalidCallback(response: ServerResponse): void {
      ...
      response.end('Invalid MCode sign-in response.')
    }
    ...
    const server = createServer((request, response) => {
      try {
        ...
        if (url.searchParams.has('error')) {
          response.writeHead(400)
          response.end('MCode sign-in was cancelled.')
          rejectFlow(new Error('mcode_cloud_auth_denied'))
          return
        }
        if (!code) {
          writeInvalidCallback(response)
        ...
        response.writeHead(200, MCODE_CLOUD_CALLBACK_RESPONSE_HEADERS)
        response.end(MCODE_CLOUD_CALLBACK_SUCCESS_PAGE)
        resolveFlow(code)
      } catch (error) {
        rejectFlow(error instanceof Error ? error : new Error('mcode_cloud_auth_callback_failed'))
      }
    })

    const timeout = setTimeout(() => {
      rejectFlow(new Error('mcode_cloud_auth_timeout'))
    }, AUTH_TIMEOUT_MS)
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        rejectFlow(new Error('mcode_cloud_auth_loopback_unavailable'))
        return
      }
      ...
      void shell.openExternal(authorizeUrl.toString()).catch((error) => {
        rejectFlow(
          error instanceof Error ? error : new Error('mcode_cloud_auth_browser_open_failed')
        )
      })
    })
  })
