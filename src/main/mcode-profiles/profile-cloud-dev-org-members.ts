...
function devSelf(): MCodeOrgMember {
  return {
  ...
  return {
    members: [
      devSelf(),
      {
        ...
        email: 'teammate@mcode.local',
        displayName: 'Dev Teammate',
        role: 'admin'
      },
      // Why: userId null exercises the "hasn't signed in to MCode yet" disabled row.
      { userId: null, email: 'invited-member@mcode.local', displayName: undefined, role: 'member' }
    ],
    pendingInvites: [{ email: 'pending@mcode.local', role: 'member', createdAt: Date.now() }]
  }
}
...
export function listDevMCodeCloudOrgMembers(orgId: string): MCodeOrgMembersRoster {
...
export function inviteDevMCodeCloudOrgMember(
...
export function revokeDevMCodeCloudOrgInvite(
...
export function changeDevMCodeCloudOrgMemberRole(
...
export function removeDevMCodeCloudOrgMember(
