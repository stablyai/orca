...
export type MCodeCloudAuthConfig = {
  apiBaseUrl: string
...
const PRODUCTION_API_BASE_URL = 'https://login.mcode.dev'
const PRODUCTION_CLIENT_ID = 'mcode-desktop'
const PRODUCTION_RELAY_DIRECTOR_URL = 'https://relay.mcode.dev'
...
function isPackagedMCodeBuild(): boolean {
  try {
...
export function getMCodeCloudAuthConfig(
  env: NodeJS.ProcessEnv = process.env,
  packaged: boolean = isPackagedMCodeBuild()
): { configured: true; config: MCodeCloudAuthConfig } | { configured: false; setupMessage: string } {
  ...
  const configuredApiBaseUrl = env.MCODE_CLOUD_API_URL?.trim()
  ...
  const apiBaseUrl = configuredApiBaseUrl
    ...
    : packaged
      ...
      : null
  const clientId = env.MCODE_CLOUD_CLIENT_ID?.trim() || (packaged ? PRODUCTION_CLIENT_ID : undefined)
  ...
  const authBaseUrl = cleanEndpointUrl(env.MCODE_CLOUD_AUTH_URL) ?? apiBaseUrl
  return {
    configured: true,
    config: {
      apiBaseUrl,
      authorizeEndpoint:
        cleanEndpointUrl(env.MCODE_CLOUD_AUTHORIZE_URL) ??
        endpoint(authBaseUrl, '/v1/desktop/auth/authorize'),
      sessionEndpoint:
        cleanEndpointUrl(env.MCODE_CLOUD_SESSION_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/session'),
      refreshEndpoint:
        cleanEndpointUrl(env.MCODE_CLOUD_REFRESH_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/refresh'),
      capabilitiesEndpoint:
        cleanEndpointUrl(env.MCODE_CLOUD_CAPABILITIES_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/capabilities'),
      profileEndpoint:
        cleanEndpointUrl(env.MCODE_CLOUD_PROFILE_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/profile'),
      orgEndpoint:
        cleanEndpointUrl(env.MCODE_CLOUD_ORG_URL) ?? endpoint(apiBaseUrl, '/v1/desktop/auth/org'),
      logoutEndpoint:
        cleanEndpointUrl(env.MCODE_CLOUD_LOGOUT_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/logout'),
      relayTokenEndpoint:
        cleanEndpointUrl(env.MCODE_CLOUD_RELAY_TOKEN_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/relay-token'),
      relayDirectorUrl:
        cleanOrigin(env.MCODE_RELAY_URL, allowLoopbackHttp) ?? PRODUCTION_RELAY_DIRECTOR_URL,
      clientId,
      scope: env.MCODE_CLOUD_AUTH_SCOPE?.trim() || DEFAULT_SCOPE
    }
  }
}

export function allowsPlaintextMCodeCloudSession(
  env: NodeJS.ProcessEnv = process.env,
  packaged: boolean = isPackagedMCodeBuild()
): boolean {
  return (
    env.MCODE_CLOUD_ALLOW_PLAINTEXT_SESSION === '1' && env.NODE_ENV !== 'production' && !packaged
  )
}

export function isMCodeCloudDevAuthEnabled(
  env: NodeJS.ProcessEnv = process.env,
  packaged: boolean = isPackagedMCodeBuild()
): boolean {
  return env.MCODE_CLOUD_DEV_AUTH === '1' && env.NODE_ENV !== 'production' && !packaged
}
