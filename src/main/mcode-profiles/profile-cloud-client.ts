...
export class MCodeCloudRequestError extends Error {
...
function assertString(value: unknown, field: string): string {
  if (typeof value !== 'string') {
    throw new Error(`invalid_mcode_cloud_${field}`)
  }
...
function normalizeCapabilities(value: unknown): MCodeCloudCapabilities {
...
function normalizeOrganizations(value: unknown): MCodeCloudOrgSummary[] | undefined {
...
function normalizeCloudSummary(value: unknown): MCodeProfileCloudSummary {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_mcode_cloud_profile')
  }
  const record = value as Record<string, unknown>
  return {
...
function normalizeSessionResponse(value: unknown): MCodeCloudSessionExchangeResponse {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_mcode_cloud_session')
  }
  const record = value as Record<string, unknown>
  return {
...
async function postJson<T>(url: string, body: unknown, accessToken?: string): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new MCodeCloudRequestError(response.status)
  }
  return (await response.json()) as T
}

export async function exchangeMCodeCloudAuthCode(
  config: MCodeCloudAuthConfig,
  args: ExchangeCodeArgs
): Promise<MCodeCloudSessionExchangeResponse> {
...
export async function refreshMCodeCloudCapabilities(
  config: MCodeCloudAuthConfig,
...
export async function refreshMCodeCloudSession(
  config: MCodeCloudAuthConfig,
  session: MCodeCloudSession
): Promise<MCodeCloudSessionExchangeResponse> {
  return normalizeSessionResponse(
...
export async function createMCodeCloudProfile(
  config: MCodeCloudAuthConfig,
  session: MCodeCloudSession,
...
export async function selectMCodeCloudOrg(
  config: MCodeCloudAuthConfig,
  session: MCodeCloudSession,
  orgId: string
...
export async function revokeMCodeCloudSession(
  config: MCodeCloudAuthConfig,
  session: MCodeCloudSession
): Promise<void> {
