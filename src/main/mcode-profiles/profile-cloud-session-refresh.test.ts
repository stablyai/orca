...
vi.mock('./profile-cloud-session-store', () => ({
  readMCodeCloudSession: readMock,
  saveMCodeCloudSessionIfCurrent: saveIfCurrentMock,
  clearMCodeCloudSession: clearMock
}))
...
vi.mock('./profile-cloud-client', async (importOriginal) => {
  ...
  return { ...original, refreshMCodeCloudSession: refreshMock }
})

vi.mock('./profile-cloud-index', () => ({ linkMCodeProfileToCloud: linkMock }))

import { readFreshMCodeCloudSession } from './profile-cloud-session-refresh'

const config = {} as MCodeCloudAuthConfig
const active = {
  profile: {
    id: 'profile-1',
    cloud: {
...
describe('profile cloud session refresh', () => {
  ...
  it('does not publish a refresh whose persistent mutation snapshot became stale', async () => {
    ...
    const refreshing = readFreshMCodeCloudSession(config, active, '/data')
  ...
  it('single-flights concurrent rotating refresh-token use per profile and store', async () => {
    ...
    const first = readFreshMCodeCloudSession(config, active, '/data')
    const second = readFreshMCodeCloudSession(config, active, '/data')
