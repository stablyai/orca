...
async function startedFlow(): Promise<{
  authUrl: URL
  flow: ReturnType<typeof beginMCodeCloudPkceFlow>
  nonce: string
  redirectUri: string
  state: string
}> {
  const flow = beginMCodeCloudPkceFlow(config, 'local-default')
...
describe('MCode cloud PKCE flow', () => {
  beforeEach(() => {
  ...
  it('keeps the loopback listener alive after an invalid callback', async () => {
    ...
    expect(validResponse.body).toContain('<h1>Signed in to MCode</h1>')
  ...
  it('rejects a provider error that matches the flow state', async () => {
    ...
    await expect(observedFlow).resolves.toMatchObject({ message: 'mcode_cloud_auth_denied' })
  })
