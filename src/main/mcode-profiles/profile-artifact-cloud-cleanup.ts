...
import type { MCodeProfileCloudSummary, MCodeProfileSummary } from '../../shared/mcode-profiles'
...
function cleanupMarkerPath(profileId: string, userDataPath: string): string {
  return join(getMCodeProfileDirectory(profileId, userDataPath), 'artifact-cloud-cleanup.json')
}

export function artifactCloudIdentity(cloud: MCodeProfileCloudSummary | undefined): string {
  return cloud
...
export function prepareArtifactCloudCleanup(
  profileId: string,
  userDataPath: string,
  targetCloud: MCodeProfileCloudSummary | undefined
): void {
...
export function completeArtifactCloudCleanupIfCommitted(
  ...
  bestEffortFsyncDirectorySync(getMCodeProfileDirectory(profileId, userDataPath))
}
...
function assertArtifactCloudCleanupReady(
  ...
  if (
    ...
    throw new Error('The MCode profile transition must be retried before publishing artifacts.')
  }
}

export function prepareArtifactCloudUse(
  profile: Pick<MCodeProfileSummary, 'id' | 'cloud'>,
  userDataPath: string
): void {
