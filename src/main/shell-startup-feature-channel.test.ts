 ...
 *    status both times.)
 * 2. Behaviour parity — a pane wrapped only because MCode injected a worktree
...
describe('shell startup feature selection', () => {
  it('ignores an inherited MCODE_SHELL_FEATURES in the spawn env', () => {
    ...
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { MCODE_SHELL_FEATURES: 'overlay,markers,ready,identity,history' },
        ...PLAIN_PANE
      })
    ).toEqual([])
  })
  ...
  it('cannot be turned off for a pane that needs a feature', () => {
    ...
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { MCODE_SHELL_FEATURES: '', MCODE_HISTFILE: '/tmp/wt/zsh_history' },
        ...PLAIN_PANE
      })
    ).toEqual(['history'])
  })
  ...
  it('gives a history-only pane no command markers', () => {
    ...
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { MCODE_HISTFILE: '/tmp/wt/zsh_history' },
        ...PLAIN_PANE
      })
  ...
  it('keeps markers on a pane that already had them', () => {
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { MCODE_CODEX_HOME: '/tmp/codex' },
        ...PLAIN_PANE
      })
    ).toContain('markers')
  })
  ...
  it('does not wrap bash for history alone', () => {
    ...
    // replace its login startup-file chain with MCode's approximation.
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/bash',
        env: { MCODE_HISTFILE: '/tmp/wt/bash_history' },
        ...PLAIN_PANE
      })
    ).toEqual([])
  })
...
describePosix('zsh launch config', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'mcode-feature-channel-'))
    setTestUserDataPath(userDataPath)
    previousFeatures = process.env.MCODE_SHELL_FEATURES
  })
  ...
  it('publishes exactly the selected features, whatever process.env holds', async () => {
    process.env.MCODE_SHELL_FEATURES = 'overlay,markers,ready,identity'
  ...
  it('does not treat another terminal’s hijacked ZDOTDIR as the user config dir', async () => {
    // Why: MCode can be launched from a terminal that already owns ZDOTDIR. Only
    // a dir MCode can prove is its own, or one holding no zsh startup file at
    // all, may be rejected — never a vendor guessed at by name.
    const home = mkdtempSync(join(tmpdir(), 'mcode-stacked-zdotdir-'))
    ...
    try {
      ...
      expect(getShellLaunchConfig('/bin/zsh', ['history']).env.MCODE_ORIG_ZDOTDIR).toBeUndefined()

      // Stamped as MCode-owned: rejected by positive identification, even though
      // the path shape is not one of MCode's.
      ...
      expect(
        stamped.getShellLaunchConfig('/bin/zsh', ['history']).env.MCODE_ORIG_ZDOTDIR
      ).toBeUndefined()
      ...
      expect(real.getShellLaunchConfig('/bin/zsh', ['history']).env.MCODE_ORIG_ZDOTDIR).toBe(
        foreignWrapper
      )
    } finally {
...
describePosix('epilogue under hostile user shell options', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'mcode-feature-channel-options-'))
    setTestUserDataPath(userDataPath)
    home = mkdtempSync(join(tmpdir(), 'mcode-feature-options-home-'))
  })
  ...
  it.each(['no_unset', 'ksh_arrays'])(
    ...
    async (option) => {
      ...
      const spawnEnv: Record<string, string> = {
        ...
        MCODE_OPENCODE_CONFIG_DIR: opencodeDir
      }
      ...
      expect(values.LINEINIT).toBe('user:__mcode_prompt_mark')
      expect(values.PRECMD).toContain('__mcode_osc133_precmd')
...
function makeUserHome(): string {
  const home = mkdtempSync(join(tmpdir(), 'mcode-feature-home-'))
...
describePosix('history-only pane in a real zsh', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'mcode-feature-channel-live-'))
    setTestUserDataPath(userDataPath)
    home = makeUserHome()
  })
  ...
  function withoutInheritedZdotdir(env: Record<string, string>): Record<string, string> {
    const { MCODE_ORIG_ZDOTDIR: _inherited, ...rest } = env
    return rest
  }
  ...
  async function launchHistoryOnly(): Promise<{
    ...
    return {
      ...
      env: {
        PATH: '/usr/bin:/bin',
        ...spawnEnv,
        // Why MCODE_ORIG_ZDOTDIR is stripped: the launch config computes it from
  ...
  itWithZsh('unsets the feature channel for the shell and for a grandchild', async () => {
    ...
    const output = execFileSync(
      ...
      [
        ...
        '-i',
        '-c',
        `print -r -- "SELF=[\${MCODE_SHELL_FEATURES:-}]"; zsh -c 'printenv MCODE_SHELL_FEATURES; print -r -- CHILD_CLEAN'`
      ],
  ...
  itWithZsh(
    ...
    async () => {
      ...
      // started with -c would report a pane MCode had not finished setting up.
      const { env } = await launchHistoryOnly()
      const capture = [
        'PRECMD="${precmd_functions[*]}"; PREEXEC="${preexec_functions[*]}"',
        'LINEINIT="${widgets[zle-line-init]:-none}"'
      ]
      const report = ['PRECMD', 'PREEXEC', 'LINEINIT', 'ZDOTDIR', 'MCODE_SHELL_FEATURES']
      ...
      // place: a history-only pane leaves no MCode name in the user's hook arrays.
      ...
      // installs its own zle-line-init widget has one either way, and what MCode
      // owes is that it looks the same wrapped as unwrapped.
      expect(wrapped.values.LINEINIT).toBe(unwrapped.values.LINEINIT)
      expect(wrapped.values.PRECMD).not.toContain('mcode')
      ...
      expect(wrapped.values.MCODE_SHELL_FEATURES).toBe('UNSET')
    }
  )
})
