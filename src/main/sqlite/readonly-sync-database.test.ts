...
async function createDatabase(): Promise<{ directory: string; path: string }> {
...
describe('openReadonlySyncDatabase', () => {
  it('reads after a concurrent exclusive writer releases within the busy timeout', async () => {
    ...
    const worker = await holdExclusive(directory, path, 200)
    try {
      worker.postMessage('reader-started')
      const db = openReadonlySyncDatabase(path, { timeoutMs: 2_000 })
  ...
  it('fails when the writer outlives the busy timeout', async () => {
    const { directory, path } = await createDatabase()
    const worker = await holdExclusive(directory, path, 2_000)
    try {
      worker.postMessage('reader-started')
      expect(() => openReadonlySyncDatabase(path, { timeoutMs: 50 })).toThrow(/database is locked/i)
    } finally {
      await worker.terminate()
    }
  })

  it('keeps the handle query-only', async () => {
    const { path } = await createDatabase()
    const db = openReadonlySyncDatabase(path)
    expect(db.pragma('query_only', { simple: true })).toBe(1)
    expect(() => db.exec('INSERT INTO items VALUES (2)')).toThrow()
    db.close()
  })

  it('closes the handle when setup fails', async () => {
    const { path } = await createDatabase()
    ...
    expect(() => openReadonlySyncDatabase(path)).toThrow(setupError)
    expect(closeSpy).toHaveBeenCalledOnce()
  })

  it('preserves the setup error when close also fails', async () => {
    const { path } = await createDatabase()
    const setupError = new Error('query_only setup failed')
