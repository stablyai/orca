...
// must use the user's own ~/.codex. If MCode itself was launched from a nested
// MCode terminal it can inherit an MCode-owned CODEX_HOME override; strip only
// that (CODEX_HOME matching the private MCODE_CODEX_HOME marker), preserving a
// user-set CODEX_HOME.
function cloneProcessEnvWithoutMCodeCodexHomeOverride(): Record<string, string> {
...
function prepareShellConfigDirEnv(agentId: string): { ok: true; env?: NodeJS.ProcessEnv } | null {
  ...
  // Why: each kind owns a distinct MCODE_*_SOURCE_* shadow so a headless commit
  ...
  const sourceVar =
    agentId === 'opencode'
      ? 'MCODE_OPENCODE_SOURCE_CONFIG_DIR'
      : agentId === 'pi'
        ? 'MCODE_PI_SOURCE_AGENT_DIR'
        : agentId === 'omp'
          ? 'MCODE_OMP_SOURCE_AGENT_DIR'
          : undefined
  ...
  // Why: GUI-launched MCode may not inherit shell startup exports, but these
  // vars point the headless CLI at the user's auth/config root. Nested MCode
  // launches inherit PTY overlays, so prefer MCODE_*_SOURCE_* when present.
  ...
  try {
    if (agentId === 'codex' && resolvers.prepareForCodexLaunch) {
      ...
      if (target?.runtime === 'wsl') {
        ...
        // Why: the fallback must still strip MCode-owned overrides, or a
        // system-default WSL run inherits the managed CODEX_HOME.
        return {
          ok: true,
          env: codexHomeForTarget
            ? { ...cloneProcessEnvWithoutMCodeCodexHomeOverride(), CODEX_HOME: codexHomeForTarget }
            : cloneProcessEnvWithoutMCodeCodexHomeOverride()
        }
      }
