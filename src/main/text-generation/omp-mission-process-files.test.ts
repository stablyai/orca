...
function installGuard(source: string, api: GuardApi) {
  ...
  const module: { exports: { default?: (api: GuardApi) => void } } = { exports: {} }
...
describe('withOmpMissionProcessFiles', () => {
  ...
  it('fails loading when the trusted OMP extension API is unsupported', async () => {
    await withOmpMissionProcessFiles(async ({ extensionPath }) => {
      ...
      // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: this negative test intentionally passes an invalid OMP extension API to verify the generated guard rejects it at runtime.
      expect(() => installGuard(extension, unsupportedApi as unknown as GuardApi)).toThrow(
        'Orca Mission Planner guard requires the OMP extension API.'
      )
    })
  })
