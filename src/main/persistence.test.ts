...
describe('Store', () => {
  ...
  it('returns default settings when no data file exists', async () => {
    ...
    expect(settings.visibleTaskProviders).toEqual(['github', 'gitlab', 'linear', 'clickup', 'jira'])
  ...
  it('normalizes malformed visible task providers on load', async () => {
    ...
    const store = await createStore()
    expect(store.getSettings().visibleTaskProviders).toEqual(['gitlab', 'clickup', 'jira'])
  })
