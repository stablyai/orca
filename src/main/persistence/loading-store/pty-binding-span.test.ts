...
function finishFastLane(): void {
  const span = startPtyBindingSpan({ hostKind: 'local', savePending: false, generationGap: 0 })
...
describe('persistence.pty-binding span', () => {
  it('records the entry counters, eligibility, and outcome', () => {
    const span = startPtyBindingSpan({ hostKind: 'ssh', savePending: true, generationGap: 2 })
  ...
  it('caps fast-lane spans per window and marks the last one recorded', () => {
    ...
    expect(records.at(-1)?.attributes['binding.sampled']).toBe(true)
    expect(records.at(-2)?.attributes['binding.sampled']).toBeUndefined()
