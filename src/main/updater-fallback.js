export function statusesEqual(left, right) {
    switch (left.state) {
        case 'idle':
        ...
        case 'downloading':
            return (right.state === 'downloading' &&
                left.version === right.version &&
                left.activeNudgeId === right.activeNudgeId &&
                left.percent === right.percent);
        case 'downloaded':
            return (right.state === 'downloaded' &&
                left.version === right.version &&
                left.activeNudgeId === right.activeNudgeId &&
                left.releaseUrl === right.releaseUrl);
        case 'error':
            return (right.state === 'error' &&
                left.message === right.message &&
                left.userInitiated === right.userInitiated &&
                left.activeNudgeId === right.activeNudgeId);
    }
}
export function isGitHubReleaseTransitionFailure(normalizedMessage) {
    return (normalizedMessage.includes('unable to find latest version on github') ||
...
export function isBenignCheckFailure(message) {
    ...
    return (isGitHubReleaseTransitionFailure(normalizedMessage) ||
        normalizedMessage.includes('no published versions on github'));
}
