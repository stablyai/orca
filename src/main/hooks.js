 ...
 */
export function parseOrcaYaml(content) {
    const hooks = { scripts: {} };
 ...
 */
export function loadHooks(repoPath) {
 ...
 */
export function hasHooksFile(repoPath) {
 ...
 */
export function hasUnrecognizedOrcaYamlKeys(repoPath) {
    try {
...
export function getIssueCommandFilePath(repoPath) {
    return join(repoPath, ORCA_DIR, ISSUE_COMMAND_FILENAME);
}
export function getSharedIssueCommand(repoPath) {
 ...
 */
export function readIssueCommand(repoPath) {
 ...
 */
export function writeIssueCommand(repoPath, content) {
    const filePath = getIssueCommandFilePath(repoPath);
 ...
 */
function ensureOrcaDirIgnored(repoPath) {
...
export function getEffectiveHooks(repo) {
...
export function getEffectiveSetupRunPolicy(repo) {
    return repo.hookSettings?.setupRunPolicy ?? getDefaultRepoHookSettings().setupRunPolicy;
}
export function shouldRunSetupForCreate(repo, decision = 'inherit') {
    if (decision === 'run') {
        return true;
    }
    if (decision === 'skip') {
...
export function getSetupCommandSource(repo) {
...
function getSetupEnvVars(repo, worktreePath) {
    return {
...
function getGitPath(cwd, relativePath) {
    return gitExecFileSync(['rev-parse', '--git-path', relativePath], {
        cwd
    }).trim();
}
function buildWindowsRunnerScript(script) {
...
export function createSetupRunnerScript(repo, worktreePath, script) {
    return createWorktreeRunnerScript(repo, worktreePath, script, 'setup-runner');
}
export function createIssueCommandRunnerScript(repo, worktreePath, command) {
...
function createWorktreeRunnerScript(repo, worktreePath, script, runnerBaseName) {
 ...
 */
export function runHook(hookName, cwd, repo) {
    ...
    if (wslInfo) {
        ...
        return new Promise((resolve) => {
            execFile('wsl.exe', ['-d', wslInfo.distro, '--', 'bash', '-c', bashCmd], {
