 ...
 */
export async function getStatus(worktreePath) {
    const entries = [];
...
async function parseUnmergedEntry(worktreePath, line) {
...
async function getConflictCompatibilityStatus(worktreePath, filePath, conflictKind) {
...
export async function detectConflictOperation(worktreePath) {
 ...
 */
export async function getDiff(worktreePath, filePath, staged) {
    let originalContent = '';
    let modifiedContent = '';
...
export async function getBranchCompare(worktreePath, baseRef) {
    const summary = {
        baseRef,
        baseOid: null,
        compareRef: 'HEAD',
...
export async function getBranchDiff(worktreePath, args) {
    try {
        ...
        return buildDiffResult(leftBlob.content, rightBlob.content, leftBlob.isBinary, rightBlob.isBinary, args.filePath);
    }
    catch {
        return {
            kind: 'text',
...
async function loadBranchChanges(worktreePath, mergeBase, headOid) {
    const { stdout } = await gitExecFileAsync(['diff', '--name-status', '-M', '-C', mergeBase, headOid], { cwd: worktreePath, maxBuffer: MAX_GIT_SHOW_BYTES });
    const entries = [];
...
function parseBranchChangeLine(line) {
    const parts = line.split('\t');
...
async function resolveCompareRef(worktreePath) {
    try {
...
async function resolveRefOid(worktreePath, ref) {
    const { stdout } = await gitExecFileAsync(['rev-parse', '--verify', ref], {
        cwd: worktreePath
    });
    return stdout.trim();
}
async function resolveMergeBase(worktreePath, baseOid, headOid) {
    const { stdout } = await gitExecFileAsync(['merge-base', baseOid, headOid], {
        cwd: worktreePath
    });
    return stdout.trim();
}
async function countAheadCommits(worktreePath, baseOid, headOid) {
    const { stdout } = await gitExecFileAsync(['rev-list', '--count', `${baseOid}..${headOid}`], {
        cwd: worktreePath
    });
    return Number.parseInt(stdout.trim(), 10) || 0;
}
async function readUnstagedLeftBlob(worktreePath, filePath) {
...
async function readGitBlobAtIndexPath(worktreePath, filePath) {
    try {
...
async function readGitBlobAtOidPath(worktreePath, oid, filePath) {
    try {
...
const PREVIEWABLE_BINARY_MIME_TYPES = {
    '.png': 'image/png',
    '.jpg': 'image/jpeg',
    '.jpeg': 'image/jpeg',
    '.gif': 'image/gif',
 ...
 * Stage a file.
 */
export async function stageFile(worktreePath, filePath) {
    await gitExecFileAsync(['add', '--', filePath], { cwd: worktreePath });
}
/**
 * Unstage a file.
 */
export async function unstageFile(worktreePath, filePath) {
    await gitExecFileAsync(['restore', '--staged', '--', filePath], { cwd: worktreePath });
}
/**
 * Discard working tree changes for a file.
 */
export async function discardChanges(worktreePath, filePath) {
...
export function isWithinWorktree(pathApi, resolvedWorktree, resolvedTarget) {
 ...
 */
export async function bulkStageFiles(worktreePath, filePaths) {
 ...
 */
export async function bulkUnstageFiles(worktreePath, filePaths) {
    if (filePaths.length === 0) {
        return;
    }
    const CHUNK_SIZE = 100;
