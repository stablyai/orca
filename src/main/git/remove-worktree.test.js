...
vi.mock('./runner', () => ({
    ...
    translateWslOutputPaths: (output) => output
}));
import { removeWorktree } from './worktree';
function mockGitCommands(results) {
    const callCounts = new Map();
    gitExecFileAsyncMock.mockImplementation((args) => {
...
function expectGitCallOrder(calls, beforeCall, afterCall) {
...
`
        ...
        expect(warnSpy).toHaveBeenCalledWith('[git] Failed to delete local branch "feature/test" after removing worktree', expect.any(Error));
        warnSpy.mockRestore();
    });
});
