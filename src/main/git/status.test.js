...
const { gitExecFileAsyncMock, gitExecFileAsyncBufferMock, readFileMock, rmMock, existsSyncMock } = vi.hoisted(() => ({
...
import { bulkStageFiles, bulkUnstageFiles, detectConflictOperation, discardChanges, getBranchCompare, getDiff, getStatus, isWithinWorktree } from './status';
describe('discardChanges', () => {
    ...
    it('restores tracked files from HEAD', async () => {
        ...
        expect(gitExecFileAsyncMock).toHaveBeenNthCalledWith(1, ['ls-files', '--error-unmatch', '--', 'src/file.ts'], {
            cwd: '/repo'
        });
        expect(gitExecFileAsyncMock).toHaveBeenNthCalledWith(2, ['restore', '--worktree', '--source=HEAD', '--', 'src/file.ts'], {
            cwd: '/repo'
        });
    ...
    it('rejects paths that traverse outside the worktree', async () => {
        await expect(discardChanges('/repo', '../../etc/passwd')).rejects.toThrow('resolves outside the worktree');
...
describe('bulk git helpers', () => {
    ...
    it('chunks bulk stage requests to avoid oversized argv payloads', async () => {
        ...
        expect(gitExecFileAsyncMock).toHaveBeenNthCalledWith(1, ['add', '--', ...filePaths.slice(0, 100)], {
            cwd: '/repo'
        });
        expect(gitExecFileAsyncMock).toHaveBeenNthCalledWith(3, ['add', '--', ...filePaths.slice(200)], {
            cwd: '/repo'
        });
    });
    it('chunks bulk unstage requests to avoid oversized argv payloads', async () => {
        ...
        expect(gitExecFileAsyncMock).toHaveBeenNthCalledWith(2, ['restore', '--staged', '--', ...filePaths.slice(100)], {
            cwd: '/repo'
        });
    });
});
...
describe('getStatus', () => {
    ...
    it('parses unmerged porcelain v2 entries into unresolved conflict rows', async () => {
        ...
        existsSyncMock.mockImplementation((target) => target.endsWith('MERGE_HEAD'));
        gitExecFileAsyncMock.mockResolvedValueOnce({
            stdout: 'u UU N... 100644 100644 100644 100644 aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb cccccccccccccccccccccccccccccccccccccccc src/app.ts\n'
        });
    ...
    it('maps deleted conflicts to deleted when the working tree file is absent', async () => {
        ...
        gitExecFileAsyncMock.mockResolvedValueOnce({
            stdout: 'u UD N... 100644 100644 000000 100644 aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb cccccccccccccccccccccccccccccccccccccccc src/deleted.ts\n'
        });
        const result = await getStatus('/repo');
    ...
    it('falls back to modified when the filesystem existence check throws', async () => {
        ...
        gitExecFileAsyncMock.mockResolvedValueOnce({
            stdout: 'u AU N... 100644 100644 100644 100644 aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb cccccccccccccccccccccccccccccccccccccccc src/new.ts\n'
        });
        const result = await getStatus('/repo');
