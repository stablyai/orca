...
function getGitConfigValue(path, key) {
    try {
 ...
 */
export function getGitUsername(path) {
    return normalizeUsername(getGitConfigValue(path, 'github.user') ||
        getGitConfigValue(path, 'user.username') ||
        getGhLogin() ||
        getGitConfigValue(path, 'user.email').split('@')[0] ||
        getGitConfigValue(path, 'user.name'));
}
function hasGitRef(path, ref) {
    try {
 ...
 */
export function getDefaultBaseRef(path) {
    try {
...
export async function getBaseRefDefault(path) {
    return getDefaultBaseRefAsync(path);
}
async function getDefaultBaseRefAsync(path) {
    try {
        const { stdout } = await gitExecFileAsync(['symbolic-ref', '--quiet', 'refs/remotes/origin/HEAD'], { cwd: path });
...
export async function searchBaseRefs(path, query, limit = 25) {
...
function normalizeRefSearchQuery(query) {
    return query.trim().replace(/[*?[\]\\]/g, '');
}
async function hasGitRefAsync(path, ref) {
    try {
...
export async function getBranchConflictKind(path, branchName) {
    if (await hasGitRefAsync(path, `refs/heads/${branchName}`)) {
        return 'local';
    }
    try {
        const { stdout } = await gitExecFileAsync(['for-each-ref', '--format=%(refname)', 'refs/remotes'], { cwd: path });
 ...
 */
export function getRemoteFileUrl(repoPath, relativePath, line) {
