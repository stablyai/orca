...
const BASE_APP_NAME = 'MCode'
const BASE_APP_USER_MODEL_ID = 'com.mcode.desktop'
...
export type DevInstanceIdentity = AppIdentity & {
  ...
  // creating a new one per branch and re-prompting. Distinct from prod's 'MCode'.
  appName: string
}
 ...
 * CFBundleName, which downstream forks ship differently ("MCode ALab Edition").
...
export function getDevInstanceIdentity(
  ...
  const repoRoot = cleanEnvValue(env.MCODE_DEV_REPO_ROOT)
  const branch = cleanEnvValue(env.MCODE_DEV_BRANCH)
  const worktreeName =
    cleanEnvValue(env.MCODE_DEV_WORKTREE_NAME) ??
    cleanEnvValue(path.basename(repoRoot ?? process.cwd()))
  const devLabel = cleanEnvValue(env.MCODE_DEV_INSTANCE_LABEL) ?? formatLabel(branch, worktreeName)
  const dockTitle =
    cleanEnvValue(env.MCODE_DEV_DOCK_TITLE) ?? `${BASE_APP_NAME}: ${branch ?? devLabel ?? 'dev'}`

  return {
    name: dockTitle,
    // Why: one stable Keychain key ('MCode Dev Safe Storage') for all dev
