...
describe('run-electron-vite-dev', () => {
  ...
  it('forwards dev instance identity to electron-vite', async () => {
    ...
    expect(envSnapshot.dockTitle).toBe('MCode: feature/billing-shell')
  ...
  it.skipIf(process.platform === 'win32')(
    'prepares userData mcode and mcode-dev wrappers for dev terminals',
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'mcode-dev-wrapper-'))
      ...
      const trackedPids = trackPidFile(pidFile)
      const devWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'mcode-dev'), 'utf8')
      const publicAliasWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'mcode'), 'utf8')
      expect(publicAliasWrapper).toBe(devWrapper)
      expect(publicAliasWrapper).toContain('MCODE_USER_DATA_PATH')
    ...
    const wrapper = spawn(
      ...
      [wrapperPath, '--stable-name', '--remote-debugging-port=9445'],
      {
        ...
        env: devWrapperTestEnv({
          ...
          MCODE_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          MCODE_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          MCODE_DEV_BRANCH: 'feature/stable-name',
          MCODE_DEV_WORKTREE_NAME: 'stable-ui'
        }),
        stdio: 'ignore'
      }
    )
  ...
  it.skipIf(process.platform !== 'darwin')(
    ...
    async () => {
      ...
      const baseEnv = devWrapperTestEnv({
        ...
        MCODE_SKIP_DEV_CLI_PREPARE: '1',
        MCODE_SKIP_DEV_WEB_PREPARE: '1',
        MCODE_DEV_BRANCH: 'feature/rebuild-electron-app',
        MCODE_DEV_WORKTREE_NAME: 'electron-app-rebuild'
      })
    ...
    async () => {
      ...
      const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9448'], {
        ...
        env: devWrapperTestEnv({
          ...
          MCODE_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          MCODE_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          MCODE_DEV_BRANCH: 'feature/framework-symlinks',
          MCODE_DEV_WORKTREE_NAME: 'symlink-ui'
        }),
        stdio: 'ignore'
      })
