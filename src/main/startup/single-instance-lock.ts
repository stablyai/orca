...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another MCode instance is already running for this userData profile; exiting this launch after requesting the existing window. If no MCode process is running, this may be an Electron/macOS single-instance lock failure.'
export const SINGLE_INSTANCE_LOCK_BYPASS_ENV = 'MCODE_BYPASS_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_E2E_ENFORCE_ENV = 'MCODE_E2E_ENFORCE_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] MCODE_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another MCode instance running for the same profile.'
...
// Why: a duplicate `mcode serve` is a supervisor artifact, not a user asking for a window; fail open when argv is unavailable.
...
/**
 * Why: MCode writes two canonical discovery files into `<userData>/`:
 * `mcode-runtime.json` (RPC endpoint + authToken for the bundled CLI) and
 ...
 * instance quits, metadata points at a dead pid and `mcode status` reports
 ...
 * way dev (`mcode-dev` userData) and packaged (`mcode` userData) runs lock in
