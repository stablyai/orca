...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('MCODE_OPENCODE_CONFIG_DIR')
    expect(script).toContain('MCODE_MIMOCODE_HOME')
    expect(script).not.toContain('MCODE_PI_CODING_AGENT_DIR')
    expect(script).not.toContain('MCODE_OMP_CODING_AGENT_DIR')
    expect(script).toContain('MCODE_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:MCODE_OMP_STATUS_EXTENSION')
    expect(script).toContain('MCODE_CODEX_HOME')
    expect(script).toContain('MCODE_CODEX_LAUNCH_PREFLIGHT')
    expect(script).toContain('function Global:codex')
    expect(script).not.toContain('$Global:__MCodeCodexExecutable')
    ...
    const codexHomeRestore = script.indexOf('if ($env:MCODE_CODEX_HOME)')
