export function getStatusPluginOwnershipSource(): string[] {
  return [
    ...
    '}',
    '',
    'function rememberRootTurnError(sessionID, factoryID, error) {',
    '  const name = String(error?.name ?? "").slice(0, 64);',
    '  rootTurnErrorNameBySessionID.delete(sessionID);',
    '  if (!name) return;',
    '  if (rootTurnErrorNameBySessionID.size >= 128) {',
    '    const first = rootTurnErrorNameBySessionID.keys().next().value;',
    '    if (first !== undefined) rootTurnErrorNameBySessionID.delete(first);',
    '  }',
    '  rootTurnErrorNameBySessionID.set(sessionID, { name, factoryID });',
    '}',
    '',
    "// Why: the aggregate folds child work into the pane; the root session's own",
    '// state travels beside it so a root ending is never inferred from the fold.',
    'function rootOwnState(sessionID) {',
    '  for (const attention of pendingAttentionByKey.values()) {',
    '    if (attention.sourceSessionID === sessionID) return "waiting";',
    '  }',
    '  if (busyRootOwnerBySessionID.has(sessionID)) return "working";',
    '  for (const provisional of provisionalBusyByKey.values()) {',
    '    if (provisional.sessionID === sessionID) return "working";',
    '  }',
    '  return "done";',
    '}',
    '',
    'function rootTurnFields(sessionID) {',
    '  const rootState = rootOwnState(sessionID);',
    '  const errorName =',
    '    rootState === "done" ? rootTurnErrorNameBySessionID.get(sessionID)?.name : undefined;',
    '  return errorName',
    '    ? { root_state: rootState, root_turn_error_name: errorName }',
