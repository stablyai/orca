export function getStatusPluginMessagePreviewSource(): string[] {
  return [
    ...
    "  // Why: OpenCode records the root's prompt before its Busy; a new turn retires the last error.",
    '  if (properties.role === "user") rootTurnErrorNameBySessionID.delete(properties.sessionID);',
    '  const delivery = post("MessagePart", { ...properties, ...rootTurnFields(properties.sessionID) });',
