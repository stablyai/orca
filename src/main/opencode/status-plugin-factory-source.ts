export function getStatusPluginFactorySource(options: { emitSessionStart: boolean }): string[] {
  return [
    ...
    '};',
    '',
    '// Why: OpenCode V1 resolves plugins through the module default export\'s',
    '// `server()` factory, while OpenCode V2 requires `id` plus `setup()` (or',
    '// `effect()`) and ignores `server()`. Export all three so one file runs on',
    '// both: V1 calls server(), V2 calls setup(). Keep the named export so the',
    '// factory-based loader still finds the same instance.',
    'function v1SessionIDFromArgs(args) {',
    '  const first = args && args[0];',
    '  return first?.sessionID || first?.path?.id || first?.id || null;',
    '}',
    '',
    'function makeV1ClientFacade(ctx) {',
    '  const session = ctx?.session;',
    '  return {',
    '    session: {',
    '      get: async (...args) => {',
    '        const sessionID = v1SessionIDFromArgs(args);',
    '        if (!session?.get || !sessionID) throw new Error("session lookup unavailable");',
    '        const result = await session.get({ sessionID });',
    '        const data = result?.data ?? result;',
    '        if (!data || data.id !== sessionID) throw new Error("session not found");',
    '        return { data };',
    '      },',
    '      list: async () => ({ data: [] }),',
    '    },',
    ...
    '}',
    '',
    'function v2EventProperties(raw) {',
    '  return raw?.properties ?? raw?.data?.form ?? raw?.data ?? {};',
    '}',
    '',
    'function normalizeV2Event(raw) {',
    '  if (!raw || typeof raw.type !== "string") return [];',
    '  if (raw.type === "form.created") {',
    '    const form = raw?.data?.form ?? v2EventProperties(raw);',
    '    const sessionID = form?.sessionID ?? raw?.properties?.sessionID ?? null;',
    '    const id = form?.id ?? raw?.properties?.id ?? sessionID ?? "";',
    '    if (!sessionID) return [];',
    '    return [{',
    '      ...raw,',
    '      type: "question.asked",',
    '      properties: { ...form, sessionID, id },',
    '    }];',
    '  }',
    '  if (raw.type === "form.replied") {',
    '    const data = raw?.data ?? {};',
    '    const sessionID = data?.sessionID ?? raw?.properties?.sessionID ?? null;',
    '    const id = data?.id ?? data?.requestID ?? raw?.properties?.id ?? "";',
    '    if (!sessionID) return [];',
    '    return [{',
    '      ...raw,',
    '      type: "question.replied",',
    '      properties: { ...(raw.properties ?? {}), sessionID, requestID: id, id },',
    '    }];',
    '  }',
    '  if (raw.type === "form.cancelled") {',
    '    const data = raw?.data ?? {};',
    '    const sessionID = data?.sessionID ?? raw?.properties?.sessionID ?? null;',
    '    const id = data?.id ?? data?.requestID ?? raw?.properties?.id ?? "";',
    '    if (!sessionID) return [];',
    ...
    '    }];',
    '  }',
    '  if (raw.properties !== undefined || raw.data === undefined) return [raw];',
    '  return [{ ...raw, properties: raw.data }];',
    '}',
    '',
    'async function OrcaOpenCodeStatusSetup(ctx) {',
    '  const instance = await OrcaOpenCodeStatusPlugin({',
    '    ...(ctx || {}),',
    '    client: makeV1ClientFacade(ctx),',
    '    options: ctx?.options,',
    '  });',
    ...
    '        for (const event of normalizeV2Event(rawEvent)) {',
    '          try {',
    ...
    '      }',
    '    } catch {',
    '      // Abort during unload ends the subscription; never throw into OpenCode.',
    '    }',
    '  })();',
    '  return async () => {',
    '    controller.abort();',
    ...
    '      await pump;',
    '    } catch {',
    '      // Unload must not reject into OpenCode.',
    '    }',
