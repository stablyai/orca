export function getStatusPluginFactorySource(options: { emitSessionStart: boolean }): string[] {
  return [
    ...
    '};',
    '',
    '// Why: OpenCode resolves plugins through the module default export as well as',
    '// the named factory export. The pre-v2 loader requires the default to expose',
    '// `server()`, while the v2 loader requires `setup()` (or `effect`) — each',
    '// rejects a default carrying only the other key. Emit both keys pointing at',
    '// the same implementation so either loader resolves without a second copy.',
    '// `OrcaOpenCodeStatusSetup` is emitted by the v2-setup source part.',
    'export default {',
