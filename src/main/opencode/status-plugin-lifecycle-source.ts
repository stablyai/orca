export function getStatusPluginLifecycleSource(): string[] {
  return [
    ...
    '      // Why wider than source-only: a descendant blocker is displayed on the',
    '      // root it rolled up to, and the root id can drop out of the bounded',
    '      // ancestry cache, so a failed re-lookup would otherwise strand it.',
    '      const attentionRootSessionID = clearAttentionForTurnEnd(sessionID, factoryID);',
