...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    '}',
    '',
    'function resolveTmuxPaneRef() {',
    '  // Why: every pane inside one tmux client inherits the same ORCA_PANE_KEY,',
    '  // so status from sibling panes collides. tmux stamps TMUX_PANE per pane',
    '  // ("%3"); the TMUX socket keeps ids from nested servers apart.',
    '  const pane = process.env.TMUX_PANE;',
    '  if (!pane) return "";',
    '  const socket = String(process.env.TMUX || "").split(",")[0];',
    ...
    '    tmuxPaneRef: resolveTmuxPaneRef(),',
    '    env: coords.env,',
