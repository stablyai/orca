...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    'async function createOrcaOpenCodeStatusPlugin(_ctx) {',
    ...
    '};',
    '',
    '// opencode V1: named-export factory (legacy loader contract).',
    'export const OrcaOpenCodeStatusPlugin = createOrcaOpenCodeStatusPlugin;',
    '',
    "// opencode V2: module default export is the define'd plugin object (define()",
    '// is identity). `server` also keeps the V1 readV1Plugin path on the same',
    '// default export; `id` + `setup` satisfy the V2 schema so the module loads.',
    'export default {',
    '  id: "orca-opencode-status",',
    '  server: createOrcaOpenCodeStatusPlugin,',
    '  setup: async (ctx) => {',
    '    const subscribe = ctx?.event?.subscribe',
    '    if (typeof subscribe !== "function") {',
    '      // V2 does not expose session events on PluginContext yet.',
    '      console.debug("[orca-opencode-status] opencode 2.x does not expose session events on the plugin context; status reporting disabled")',
    '      return',
    '    }',
    '    const hooks = await createOrcaOpenCodeStatusPlugin(ctx)',
    '    if (!hooks?.event) return',
    '    const types = ["session.status","session.idle","session.error","permission.asked","permission.replied","question.asked","question.replied","question.rejected","message.updated","message.part.updated","message.part.delta"]',
    '    for (const type of types) {',
    '      try {',
    '        const stream = subscribe(type)',
    '        const onEvent = (event) => { void hooks.event({ event }).catch(() => {}) }',
    '        if (stream?.onValue) stream.onValue(onEvent)',
    '        else if (stream?.pipe) stream.pipe({ onValue: onEvent })',
    '        else if (stream && typeof stream[Symbol.asyncIterator] === "function") {',
    '          void (async () => { for await (const event of stream) onEvent(event) })().catch(() => {})',
    '        }',
    '      } catch (error) {',
    '        console.debug("[orca-opencode-status] failed to subscribe to", type, error)',
    '      }',
    '    }',
    '  }',
    '};',
