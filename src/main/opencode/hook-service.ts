...
function getOpenCodePluginSource(): string {
  ...
  return [
    ...
    '      return out;',
    '    } catch (statErr) {',
    '      // Why: a stat failure (file yanked mid-read, permission race) should',
    '      // fall through to the outer catch so we still warn-once and return',
    '      // null — invalidate the cache so a transient failure does not lock',
    '      // in a stale parse for the remaining process lifetime.',
    '      cachedEndpointKey = "";',
