...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  const lifecycleProvider = hookPathname === '/hook/mimo-code' ? 'mimo' : 'opencode'
  ...
  return [
    'import { createHash as orcaCreateHash } from "node:crypto";',
    '',
    ...
    '}',
    '',
    `const ORCA_CHILD_PROVIDER = ${JSON.stringify(lifecycleProvider)};`,
    'const HOOK_POST_TIMEOUT_MS = 1000;',
    'const MAX_ID_COMPONENT_CHARS = 1024;',
    '// Why: MiMo hashes two length-prefixed identity components. Derive the',
    "// raw cap from the encoder's exact maximum so valid boundary ids survive.",
    'const MAX_ID_LENGTH_PREFIX_CHARS = String(MAX_ID_COMPONENT_CHARS).length;',
    'const MAX_RAW_IDENTITY_CHARS =',
    '  MAX_ID_COMPONENT_CHARS * 2 + MAX_ID_LENGTH_PREFIX_CHARS * 2 + 2;',
    'const MAX_SESSION_METADATA = 256;',
    'const MAX_ACTOR_METADATA = 256;',
    '// Why: the listener persists at most 32 live child rows. Keeping the same',
    '// ceiling prevents the plugin from deduping children the listener dropped.',
    'const MAX_ACTIVE_CHILDREN = 32;',
    '',
    'function isBoundedIdentityPart(value) {',
    '  return typeof value === "string" && value.length > 0 && value.length <= MAX_ID_COMPONENT_CHARS;',
    '}',
    '',
    'function encodeMimoActorIdentity(sessionID, actorID) {',
    '  if (!isBoundedIdentityPart(sessionID) || !isBoundedIdentityPart(actorID)) return null;',
    '  // Why: length prefixes keep the tuple unambiguous even when either',
    '  // provider id contains delimiters, control characters, or another id.',
    '  return sessionID.length + ":" + sessionID + actorID.length + ":" + actorID;',
    '}',
    '',
    ...
    '}',
    '',
    'function deriveAgentId(rawIdentity) {',
    '  if (typeof rawIdentity !== "string" || rawIdentity.length === 0 || rawIdentity.length > MAX_RAW_IDENTITY_CHARS) return null;',
    '  const digest = orcaCreateHash("sha256")',
    '    .update(ORCA_CHILD_PROVIDER + "\\0" + rawIdentity)',
    '    .digest("hex")',
    '    .slice(0, 32);',
    '  return ORCA_CHILD_PROVIDER + "-" + digest;',
    '}',
    '',
    'function makeLifecyclePayload(rawIdentity, metadata) {',
    '  const agentId = deriveAgentId(rawIdentity);',
    '  if (!agentId) return null;',
    '  const payload = { agent_id: agentId };',
    '  if (metadata?.agentType) payload.agent_type = metadata.agentType;',
    '  if (metadata?.description) payload.description = metadata.description;',
    '  return payload;',
    '}',
    '',
    'let lastStatus = { value: "idle", postable: false, announcedGeneration: null };',
    'const sessionMetadataById = new Map();',
    '// Why: event callbacks overlap. Sharing lineage lookup keeps later idle',
    '// behind earlier busy, while tombstones prevent delete-time resurrection.',
    'const sessionMetadataLookupById = new Map();',
    'const deletedSessionIds = new Map();',
    'const activeOpenCodeChildren = new Map();',
    'const actorMetadataByKey = new Map();',
    'const actorStatusBeforeRegistration = new Map();',
    'const activeMimoActors = new Map();',
    'let lifecyclePostChain = Promise.resolve();',
    'let statusPostChain = Promise.resolve();',
    '',
    'function serializeLifecyclePost(deliver) {',
    '  // Why: plugin event callbacks can overlap. Serialize only lifecycle',
    '  // delivery so a slow Start cannot arrive after Stop and resurrect a row.',
    '  const queued = lifecyclePostChain.then(deliver, deliver);',
    '  lifecyclePostChain = queued.then(() => {}, () => {});',
    '  return queued.catch(() => {});',
    '}',
    '',
    'function postLifecycle(hookEventName, payload) {',
    '  return serializeLifecyclePost(() => post(hookEventName, payload));',
    '}',
    '',
    'function ensureLifecycleStart(active) {',
    '  const target = resolveHookTarget();',
    '  if (!target || active.announcedGeneration === target.generation) return Promise.resolve();',
    '  if (active.pendingGeneration === target.generation && active.pendingStart) {',
    '    return active.pendingStart;',
    '  }',
    '  const pending = serializeLifecyclePost(async () => {',
    '    const delivery = await post("SubagentStart", active.payload, target);',
    '    if (delivery.delivered) active.announcedGeneration = delivery.generation;',
    '  });',
    '  active.pendingGeneration = target.generation;',
    '  active.pendingStart = pending.finally(() => {',
    '    if (active.pendingGeneration === target.generation) {',
    '      active.pendingGeneration = null;',
    '      active.pendingStart = null;',
    '    }',
    '  });',
    '  return active.pendingStart;',
    '}',
    ...
    '}',
    '',
    '// Why: OpenCode background tasks run in child sessions whose root can go',
    '// idle first. Cache stable parent metadata so child lifecycle feeds the',
    '// roster without allowing child messages or statuses to overwrite root UI.',
    'function toSessionMetadata(info) {',
    '  if (!info || typeof info !== "object" || !isBoundedIdentityPart(info.id)) return null;',
    '  const hasParent = info.parentID !== undefined && info.parentID !== null;',
    '  if (hasParent && !isBoundedIdentityPart(info.parentID)) return null;',
    '  return {',
    '    id: info.id,',
    '    isChild: hasParent,',
    '    agentType: boundedOptionalString(info.agent, 120),',
    '    description: boundedOptionalString(info.title, 300),',
    '  };',
    '}',
    '',
    'function rememberSessionMetadata(info) {',
    '  const metadata = toSessionMetadata(info);',
    '  if (!metadata) return null;',
    '  setBounded(sessionMetadataById, metadata.id, metadata, MAX_SESSION_METADATA);',
    '  return metadata;',
    '}',
    '',
    'async function resolveSessionMetadata(client, sessionID) {',
    '  if (!isBoundedIdentityPart(sessionID)) return null;',
    '  if (deletedSessionIds.has(sessionID)) return null;',
    '  const cached = sessionMetadataById.get(sessionID);',
    '  if (cached) return cached;',
    '  const inFlight = sessionMetadataLookupById.get(sessionID);',
    '  if (inFlight) return await inFlight;',
    '  if (!client?.session?.list) return null;',
    '  if (sessionMetadataLookupById.size >= MAX_SESSION_METADATA) return null;',
    '  const lookup = (async () => {',
    '    try {',
    ...
    '      return rememberSessionMetadata(session);',
    '    } catch {',
    '      // Why: unknown lineage must suppress the event without inventing a',
    '      // child gate. A later authoritative create/update/status can recover.',
    '      return null;',
    '    }',
    '  })();',
    '  sessionMetadataLookupById.set(sessionID, lookup);',
    '  try {',
    '    return await lookup;',
    '  } finally {',
    '    if (sessionMetadataLookupById.get(sessionID) === lookup) {',
    '      sessionMetadataLookupById.delete(sessionID);',
    '    }',
    '  }',
    '}',
    '',
    'async function startOpenCodeChild(metadata) {',
    '  if (ORCA_CHILD_PROVIDER !== "opencode" || !metadata?.isChild) return;',
    '  const payload = makeLifecyclePayload(metadata.id, metadata);',
    '  if (!payload) return;',
    '  let active = activeOpenCodeChildren.get(metadata.id);',
    '  if (!active) {',
    '    if (activeOpenCodeChildren.size >= MAX_ACTIVE_CHILDREN) return;',
    '    active = { payload, announcedGeneration: null };',
    '    activeOpenCodeChildren.set(metadata.id, active);',
    '  } else {',
    '    active.payload = payload;',
    '  }',
    '  await ensureLifecycleStart(active);',
    '}',
    '',
    'async function stopOpenCodeChild(sessionID) {',
    '  if (ORCA_CHILD_PROVIDER !== "opencode") return;',
    '  const active = activeOpenCodeChildren.get(sessionID);',
    '  if (!active) return;',
    '  activeOpenCodeChildren.delete(sessionID);',
    '  await postLifecycle("SubagentStop", active.payload);',
    '}',
    '',
    'function toMimoActorMetadata(properties) {',
    '  if (ORCA_CHILD_PROVIDER !== "mimo" || !properties || typeof properties !== "object") return null;',
    '  const key = encodeMimoActorIdentity(properties.sessionID, properties.actorID);',
    '  if (!key) return null;',
    '  if (deletedSessionIds.has(properties.sessionID)) return null;',
    '  if (properties.mode !== "main" && properties.mode !== "peer" && properties.mode !== "subagent") return null;',
    '  if (properties.parentActorID !== undefined && !isBoundedIdentityPart(properties.parentActorID)) return null;',
    '  return {',
    '    key,',
    '    sessionID: properties.sessionID,',
    '    mode: properties.mode,',
    '    agentType: boundedOptionalString(properties.agent, 120),',
    '    description: boundedOptionalString(properties.description, 300),',
    '  };',
    '}',
    '',
    'function mimoActorKey(properties) {',
    ...
    '}',
    '',
    'async function startMimoActor(metadata) {',
    '  if (ORCA_CHILD_PROVIDER !== "mimo" || !metadata || metadata.mode === "main") return;',
    '  const payload = makeLifecyclePayload(metadata.key, metadata);',
    '  if (!payload) return;',
    '  let active = activeMimoActors.get(metadata.key);',
    '  if (!active) {',
    '    if (activeMimoActors.size >= MAX_ACTIVE_CHILDREN) return;',
    '    active = { sessionID: metadata.sessionID, payload, announcedGeneration: null };',
    '    activeMimoActors.set(metadata.key, active);',
    '  } else {',
    ...
    '}',
    '',
    'async function stopMimoActor(key) {',
    '  if (ORCA_CHILD_PROVIDER !== "mimo") return;',
    '  const active = activeMimoActors.get(key);',
    '  if (!active) return;',
    '  activeMimoActors.delete(key);',
    '  await postLifecycle("SubagentStop", active.payload);',
    '}',
    '',
    'async function rememberMimoActor(properties) {',
    '  const metadata = toMimoActorMetadata(properties);',
    '  if (!metadata) return;',
    '  setBounded(actorMetadataByKey, metadata.key, metadata, MAX_ACTOR_METADATA);',
    '  const pending = actorStatusBeforeRegistration.get(metadata.key);',
    '  actorStatusBeforeRegistration.delete(metadata.key);',
    '  if (metadata.mode === "main") {',
    '    await stopMimoActor(metadata.key);',
    '    return;',
    '  }',
    '  if (!pending) {',
    '    if (activeMimoActors.has(metadata.key)) await startMimoActor(metadata);',
    '    return;',
    '  }',
    '  if (pending.status === "idle" || pending.lastOutcome) {',
    '    await stopMimoActor(metadata.key);',
    '    return;',
    '  }',
    '  await startMimoActor(metadata);',
    '}',
    '',
    'async function handleMimoActorStatus(properties) {',
    '  if (ORCA_CHILD_PROVIDER !== "mimo") return;',
    '  const key = mimoActorKey(properties);',
    '  if (!key) return;',
    '  const status = properties.status;',
    '  if (status !== "pending" && status !== "running" && status !== "idle") return;',
    '  const lastOutcome = properties.lastOutcome;',
    '  if (lastOutcome !== undefined && lastOutcome !== "success" && lastOutcome !== "failure" && lastOutcome !== "cancelled") return;',
    '  const terminal = status === "idle" || lastOutcome !== undefined;',
    '  const metadata = actorMetadataByKey.get(key);',
    '  if (!metadata) {',
    '    if (activeMimoActors.has(key)) {',
    '      if (terminal) await stopMimoActor(key);',
    '      return;',
    '    }',
    '    setBounded(actorStatusBeforeRegistration, key, { sessionID: properties.sessionID, status, lastOutcome }, MAX_ACTOR_METADATA);',
    '    return;',
    '  }',
    '  if (metadata.mode === "main") return;',
    '  if (terminal) {',
    '    await stopMimoActor(key);',
    '    return;',
    '  }',
    '  await startMimoActor(metadata);',
    '}',
    '',
    'async function stopMimoActorsForSession(sessionID) {',
    '  if (ORCA_CHILD_PROVIDER !== "mimo" || !isBoundedIdentityPart(sessionID)) return;',
    '  const stopPayloads = [];',
    '  for (const [key, active] of activeMimoActors) {',
    '    if (active.sessionID !== sessionID) continue;',
    '    activeMimoActors.delete(key);',
    '    stopPayloads.push(active.payload);',
    '  }',
    '  for (const [key, metadata] of actorMetadataByKey) {',
    '    if (metadata.sessionID === sessionID) actorMetadataByKey.delete(key);',
    '  }',
    '  for (const [key, pending] of actorStatusBeforeRegistration) {',
    '    if (pending.sessionID === sessionID) actorStatusBeforeRegistration.delete(key);',
    '  }',
    '  // Why: all actor state is removed before the first awaited POST, so an',
    '  // overlapping callback observes one atomic teardown boundary.',
    '  for (const payload of stopPayloads) await postLifecycle("SubagentStop", payload);',
    '}',
    '',
    ...
    '  if (!coords.port || !coords.token || !paneKey) return null;',
    `  const url = \`http://127.0.0.1:\${coords.port}${hookPathname}\`;`,
    '  // Why: the token changes on every Orca/relay restart. Keying successful',
    '  // announcements to port+token lets a surviving plugin re-announce live',
    '  // children and root state without exposing the token outside this process.',
    '  const generation = orcaCreateHash("sha256")',
    '    .update(String(coords.port) + "\\0" + coords.token)',
    '    .digest("hex")',
    '    .slice(0, 32);',
    '  return { coords, paneKey, url, generation };',
    '}',
    '',
    'async function post(hookEventName, extraProperties, providedTarget) {',
    '  const target = providedTarget || resolveHookTarget();',
    '  if (!target) return { delivered: false, generation: null };',
    '  const { coords, paneKey, url, generation } = target;',
    ...
    '      timeoutPromise,',
    '    ]);',
    '    return { delivered: response?.ok === true, generation };',
    '  } catch {',
    ...
    '    return { delivered: false, generation };',
    '  } finally {',
    ...
    '  if (lastStatus.value !== next) {',
    '    lastStatus = { value: next, postable: true, announcedGeneration: null };',
    '  } else if (!lastStatus.postable) {',
    '    // Why: preserve the original initial-idle suppression before a real',
    '    // transition, while allowing failed or restarted same-state retries.',
    '    return;',
    '  }',
    '  const status = lastStatus;',
    '  const hookEventName = next === "busy" ? "SessionBusy" : "SessionIdle";',
    '  const target = resolveHookTarget();',
    '  if (!target || status.announcedGeneration === target.generation) return;',
    '  if (status.pendingGeneration === target.generation && status.pendingPost) {',
    '    await status.pendingPost;',
    '    return;',
    '  }',
    '  const deliver = async () => {',
    '    const delivery = await post(hookEventName, extraProperties, target);',
    '    if (delivery.delivered) status.announcedGeneration = delivery.generation;',
    '  };',
    '  const queued = statusPostChain.then(deliver, deliver);',
    '  statusPostChain = queued.then(() => {}, () => {});',
    '  status.pendingGeneration = target.generation;',
    '  status.pendingPost = queued.catch(() => {}).finally(() => {',
    '    if (status.pendingGeneration === target.generation) {',
    '      status.pendingGeneration = null;',
    '      status.pendingPost = null;',
    '    }',
    '  });',
    '  await status.pendingPost;',
    '}',
    ...
    '',
    '    // Why: cache the message role BEFORE the async lineage lookup.',
    '    // OpenCode fires message.updated (user) and message.part.updated (text)',
    '    // back-to-back; if we awaited resolveSessionMetadata first, the part.updated',
    ...
    '      await rememberMimoActor(event.properties);',
    '      return;',
    '    }',
    '',
    '    if (event.type === "actor.status") {',
    '      await handleMimoActorStatus(event.properties);',
    '      return;',
    '    }',
    '',
    '    if (event.type === "session.created" || event.type === "session.updated") {',
    '      const info = event.properties?.info;',
    '      if (!deletedSessionIds.has(info?.id)) rememberSessionMetadata(info);',
    '      return;',
    '    }',
    '',
    '    if (event.type === "session.deleted") {',
    '      const info = event.properties?.info;',
    '      const deletedID = info?.id;',
    '      const cached = isBoundedIdentityPart(deletedID) ? sessionMetadataById.get(deletedID) : null;',
    '      const metadata = cached || toSessionMetadata(info);',
    '      if (isBoundedIdentityPart(deletedID)) {',
    '        sessionMetadataById.delete(deletedID);',
    '        setBounded(deletedSessionIds, deletedID, true, MAX_SESSION_METADATA);',
    '      }',
    '      if (metadata?.isChild || activeOpenCodeChildren.has(deletedID)) {',
    '        await stopOpenCodeChild(deletedID);',
    '      }',
    '      await stopMimoActorsForSession(deletedID);',
    '      return;',
    '    }',
    '',
    '    const sessionID = event.properties?.sessionID;',
    '    if (sessionID) {',
    '      const metadata = await resolveSessionMetadata(client, sessionID);',
    '      const knownOpenCodeChild = activeOpenCodeChildren.has(sessionID);',
    '      // Why: unresolved lineage remains isolated and emits no lifecycle.',
    '      // A cache/list failure must not make an unknown session look root.',
    '      if (!metadata && !knownOpenCodeChild) return;',
    '      if (metadata?.isChild || knownOpenCodeChild) {',
    '        if (ORCA_CHILD_PROVIDER === "opencode") {',
    '          const statusType = getStatusType(event);',
    '          if (event.type === "session.status" && (statusType === "busy" || statusType === "retry")) {',
    '            if (metadata?.isChild) await startOpenCodeChild(metadata);',
    '          } else if ((event.type === "session.status" && statusType === "idle") || event.type === "session.idle") {',
    '            await stopOpenCodeChild(sessionID);',
    '          }',
    '        }',
    '        return;',
    '      }',
    ...
    '',
    '    if (event.type === "message.updated") {',
    '      // Why: role is already cached above the lineage await so the',
