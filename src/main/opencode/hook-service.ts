...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    'const activeChildByKey = new Map();',
    ...
    '}',
    '',
    'function childActivityKey(factoryID, sessionID) {',
    '  return JSON.stringify([factoryID, sessionID || ""]);',
    '}',
    '',
    'function rememberActiveChild(rootSessionID, sessionID, factoryID) {',
    '  const key = childActivityKey(factoryID, sessionID);',
    '  activeChildByKey.delete(key);',
    '  activeChildByKey.set(key, { sessionID: rootSessionID, factoryID });',
    '}',
    '',
    'function clearActiveChild(sessionID, factoryID) {',
    '  return activeChildByKey.delete(childActivityKey(factoryID, sessionID));',
    '}',
    '',
    ...
    '  }',
    '  for (const child of activeChildByKey.values()) {',
    '    latest = child;',
    '  }',
    ...
    '    let clearedActiveChild = false;',
    '    if (isIdleEvent) {',
    '      attentionRootSessionID = clearAttentionForSession(sessionID, factoryID);',
    '      clearedActiveChild = clearActiveChild(sessionID, factoryID);',
    '    }',
    '    if (statusType === "busy" || statusType === "retry") {',
    '      clearAttentionForSession(sessionID, factoryID);',
    '      rememberActiveChild(rootSessionID, sessionID, factoryID);',
    '      await publishAggregateStatus(factoryID, rootSessionID);',
    '      return;',
    '    }',
    '    if (resolvedProvisionalBusy || attentionRootSessionID || clearedActiveChild) {',
    ...
    '      if (!part || part.type !== "text" || !part.text || part.synthetic) return;',
    ...
    '      }',
    '      for (const [key, child] of activeChildByKey) {',
    '        if (child.factoryID === factoryID) activeChildByKey.delete(key);',
    '      }',
    '      for (const [key, attention] of pendingAttentionByKey) {',
