...
type CreateMainWindowOptions = {
  ...
  /** Returns true when MCode should reload after renderer loss; update-relaunch/quit tear down children intentionally, so don't fight shutdown. */
...
export function createMainWindow(
  ...
  const mainWindow = new BrowserWindow({
    ...
    title: opts?.title ?? 'MCode',
    show: false,
    // Why: macOS swallows the app-activating click by default, so clicking back into MCode needed a second click (Windows/Linux already deliver it).
  ...
  mainWindow.webContents.on('will-attach-webview', (event, webPreferences, params) => {
    ...
    Object.assign(webPreferences, MCODE_BROWSER_GUEST_WEB_PREFERENCES)
  ...
  const dispatchResolvedWindowShortcutAction = (
    ...
    const capturedTerminalActionId =
      ...
      focusedShortcutContext.terminalShortcutPolicy === 'mcode-first' &&
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    ...
    if (isMacAppPasteInput(input)) {
      // Why: chat/terminal panes hold focus without native editable controls, so route Cmd+V through MCode's paste ownership.
      event.preventDefault()
      ...
      try {
        new Notification({
          ...
          body: translateMain(
            ...
            'MCode is still running in the system tray'
          )
        }).show()
      } catch {
