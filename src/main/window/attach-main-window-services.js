...
import { checkForUpdatesFromMenu, downloadUpdate, getUpdateStatus, quitAndInstall, setupAutoUpdater, dismissNudge } from '../updater';
import { scheduleHistoryGc } from '../terminal-history';
import { listRepoWorktrees } from '../repo-worktrees';
export function attachMainWindowServices(mainWindow, store, runtime, getSelectedCodexHomePath) {
...
function registerRuntimeWindowLifecycle(mainWindow, runtime) {
    ...
    runtime.setNotifier({
        ...
        activateWorktree: (repoId, worktreeId, setup) => {
            if (!mainWindow.isDestroyed()) {
...
function registerFileDropRelay(mainWindow) {
    ipcMain.removeAllListeners('terminal:file-dropped-from-preload');
    ipcMain.on('terminal:file-dropped-from-preload', (_event, args) => {
        if (mainWindow.isDestroyed()) {
            return;
        }
...
export function registerUpdaterHandlers(_store) {
