...
describe('createMainWindow', () => {
    ...
    it('enables renderer sandboxing and opens external links safely', () => {
        ...
        windowHandlers['will-navigate']({ preventDefault }, 'https://example.com/docs');
        ...
        windowHandlers['will-navigate']({ preventDefault: localhostPreventDefault }, 'localhost:3000');
        ...
        windowHandlers['will-navigate']({ preventDefault: fileNavigationPreventDefault }, 'file:///etc/passwd');
        ...
        windowHandlers['will-attach-webview'](allowBlankEvent, allowBlankPrefs, { src: 'data:text/html,' });
        expect(allowBlankEvent.preventDefault).not.toHaveBeenCalled();
        const denyInlineHtmlEvent = { preventDefault: vi.fn() };
        windowHandlers['will-attach-webview'](denyInlineHtmlEvent, { partition: 'persist:orca-browser' }, { src: 'data:text/html,<script>alert(1)</script>' });
        expect(denyInlineHtmlEvent.preventDefault).toHaveBeenCalledTimes(1);
        const guest = { marker: 'guest' };
        windowHandlers['did-attach-webview']({}, guest);
    ...
    it('supports all minus key variants for terminal zoom out', () => {
        ...
        for (const input of [
            ...
            const preventDefault = vi.fn();
            beforeInputEvent({ preventDefault }, input);
            expect(preventDefault).toHaveBeenCalledTimes(1);
        }
    ...
    it('routes Electron zoom command events to terminal zoom', () => {
        ...
        const preventDefault = vi.fn();
        onZoomChanged({ preventDefault }, 'out');
        onZoomChanged({ preventDefault }, 'in');
    ...
    it('toggles devtools on F12 in development', () => {
        ...
        createMainWindow(null);
        const preventDefault = vi.fn();
        windowHandlers['before-input-event']({ preventDefault }, { type: 'keyDown', code: 'F12', key: 'F12', meta: false, control: false, alt: false });
        expect(preventDefault).toHaveBeenCalledTimes(1);
    ...
    it('clears the quit latch when the renderer prevents unload', () => {
        ...
        const preventDefault = vi.fn();
        windowHandlers.close({ preventDefault });
        expect(preventDefault).toHaveBeenCalledTimes(1);
