...
function writeLegacyCopyMarker(relativePath: string, sourcePath: string, targetPath: string): void {
  ...
  const markerPath = join(getRuntimeCodexHomePath(), '.mcode-session-copies', `${relativePath}.json`)
...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'mcode-codex-session-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'mcode-codex-session-user-data-'))
