...
beforeEach(() => {
  previousDisableTrustRpc = process.env.MCODE_DISABLE_CODEX_TRUST_RPC
...
afterEach(() => {
  ...
  if (previousDisableTrustRpc === undefined) {
    ...
    process.env.MCODE_DISABLE_CODEX_TRUST_RPC = previousDisableTrustRpc
  }
...
// separated (the shape the real 0.144.x binary writes). MCode's
...
describe('CodexHookService app-server trust grant lane', () => {
  it('treats Codex hashes as authoritative and records the verified grant', () => {
    ...
    const selfComputed = computeTrustedHash({
      ...
      groupIndex: 0,
      handlerIndex: 0,
      command: wrapPosixHookCommand(join(tmpHome, '.mcode', 'agent-hooks', 'codex-hook.sh')),
      timeoutSec: 10
    })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      const material = getCodexManagedHookInstallMaterial()
      const userHook = { type: 'command' as const, command: 'after-mcode.sh' }
      writeFileSync(
        targetPath,
        `${JSON.stringify(
          {
