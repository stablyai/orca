...
// stamped `complete` on first app-server startup), so rollouts that MCode's
...
// Codex's DB-driven surfaces. MCode never writes Codex's sqlite schema itself.
...
function buildNativeHealInvocation(
  ...
  return {
    command: spawnCmd,
    args: spawnArgs,
    // Why: pin the real home explicitly — nested MCode launches can inherit a
