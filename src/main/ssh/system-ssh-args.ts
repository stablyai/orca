...
export type SystemSshBuildArgsOptions = {
  ...
  suppressMCodeControlMaster?: boolean
  gssapiOnly?: boolean
}
...
export function buildSshArgs(target: SshTarget, options?: SystemSshBuildArgsOptions): string[] {
  ...
  if (options?.gssapiOnly) {
    ...
    // credential prompt; failure belongs to MCode's existing ssh2 prompt path.
  ...
  const controlPath = getMCodeControlSocketPath(target, options)
...
export function getMCodeControlSocketPath(
  target: SshTarget,
  options?: SystemSshBuildArgsOptions
): string | null {
  if (shouldDisableMCodeControlMaster(target, options)) {
    return null
  }
...
export function getSystemSshBuildArgsFromOperationOptions(
  ...
  if (options?.suppressMCodeControlMaster === true) {
    buildArgsOptions.suppressMCodeControlMaster = true
  }
...
function shouldDisableMCodeControlMaster(
  target: SshTarget,
  options?: SystemSshBuildArgsOptions
): boolean {
  // Why: unresolved ssh_config aliases could otherwise share one MCode socket
  ...
  return (
    options?.disableControlMaster === true ||
    options?.suppressMCodeControlMaster === true ||
    target.systemSshConnectionReuse === false ||
...
function hasUserConfiguredControlMaster(
  ...
  // created, but they do not create the setup-burst master MCode needs.
  return (
