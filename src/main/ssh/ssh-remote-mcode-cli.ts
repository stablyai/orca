...
import {
  ...
  type RemoteMCodeCliRequest,
  type RemoteMCodeCliResult
...
export type { RemoteMCodeCliRequest, RemoteMCodeCliResult } from './ssh-remote-cli-host-passthrough'
...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'mcode serve starts a foreground headless MCode server and cannot run through the SSH relay bridge. Run it directly on the machine that should host MCode.',
  'claude-teams':
    'mcode claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the MCode host machine.',
  'agent-teams-tmux':
    'mcode agent-teams-tmux is a tmux pane shim for the MCode host machine and cannot run through the SSH relay bridge.',
  'account add':
    'mcode account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the MCode host machine.'
}

export async function runRemoteMCodeCli(
  ...
  if (command === 'orchestration check' || command === 'orchestration ask') {
    ...
    return await runLegacyRemoteMCodeCli(
      runtime,
      request,
      parsed,
      json,
  ...
  try {
    return await runHostMCodeCliPassthrough(request, passthroughOptions)
  } catch (err) {
  ...
  return await runLegacyRemoteMCodeCli(runtime, request, parsed, json, passthroughFailure)
}

async function runLegacyRemoteMCodeCli(
  runtime: MCodeRuntimeService,
  request: RemoteMCodeCliRequest,
  ...
  switch (command) {
    case 'status': {
      ...
      const cliStatus: CliStatusResult = {
        ...
        // Why: this answers for the MCode host the caller reached over SSH, not for the caller's
    ...
    case 'orchestration send': {
      ...
      return await call(
        ...
        'orchestration.send',
        {
          ...
          senderPaneKey: env.MCODE_PANE_KEY || undefined
        },
        {
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH MCode CLI command: ${command} (full MCode CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
