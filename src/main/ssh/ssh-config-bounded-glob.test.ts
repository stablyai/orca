...
describe('bounded SSH config include globbing', () => {
  ...
  it('treats ** as a single segment, the way the ssh client does', () => {
    // Why: ssh resolves Include patterns with glob(3), where ** is just two wildcards in one
    // segment and never recurses. The previous globSync call did recurse, so `conf.d/**/*.conf`
    // silently pulled in nested files ssh itself would not read.
    const root = makeTempRoot()
    mkdirSync(join(root, 'conf.d', 'nested'), { recursive: true })
    writeFileSync(join(root, 'conf.d', 'top.conf'), '')
    writeFileSync(join(root, 'conf.d', 'nested', 'inner.conf'), '')

    const result = resolveBoundedSshConfigGlob(join(root, 'conf.d', '**', '*.conf'), pathApi, 256)

    expect(result.matches.map((filePath) => basename(filePath))).toEqual(['inner.conf'])
  })

  it('descends through a symlinked directory, the way the ssh client does', () => {
    const root = makeTempRoot()
    mkdirSync(join(root, 'real'), { recursive: true })
    writeFileSync(join(root, 'real', 'linked.conf'), '')
    try {
      symlinkSync(join(root, 'real'), join(root, 'link'), 'dir')
    } catch {
      return // Windows without developer mode cannot create the symlink.
    }

    const result = resolveBoundedSshConfigGlob(join(root, '*', '*.conf'), pathApi, 256)

    expect(result.matches.map((filePath) => basename(filePath)).sort()).toEqual([
      'linked.conf',
      'linked.conf'
    ])
  })
