...
function createMockConn(forwardOutErr) {
    const mockChannel = {
...
describe('SshPortForwardManager', () => {
    ...
    it('adds a port forward and returns entry', async () => {
        ...
        const entry = await manager.addForward('conn-1', conn, 3000, 'localhost', 8080);
    ...
    it('throws when SSH client is not connected', async () => {
        ...
        await expect(manager.addForward('conn-1', conn, 3000, 'localhost', 8080)).rejects.toThrow('SSH connection is not established');
    });
    it('lists forwards filtered by connectionId', async () => {
        const conn = createMockConn();
        await manager.addForward('conn-1', conn, 3000, 'localhost', 8080);
        await manager.addForward('conn-2', conn, 3001, 'localhost', 8081);
        await manager.addForward('conn-1', conn, 3002, 'localhost', 8082);
    ...
    it('removes all forwards for a connection', async () => {
        const conn = createMockConn();
        await manager.addForward('conn-1', conn, 3000, 'localhost', 8080);
        await manager.addForward('conn-1', conn, 3001, 'localhost', 8081);
        await manager.addForward('conn-2', conn, 3002, 'localhost', 8082);
    ...
    it('stores label in the entry', async () => {
        const conn = createMockConn();
        const entry = await manager.addForward('conn-1', conn, 3000, 'localhost', 8080, 'Web Server');
