...
type ActionabilityStack = {
  actionability: CdpElementActionability
  bridgeState: CdpBridgeState
  lifecycle: CdpDebuggerLifecycle
  tabState: Map<string, CdpTabState>
}

function createStack(): ActionabilityStack {
  const tabState = new Map<string, CdpTabState>()
  const registeredTabs = new Map([['tab-1', 11]])
  const bridgeState = new CdpBridgeState({
    getActiveWebContentsId: () => 11,
  ...
  const lifecycle = new CdpDebuggerLifecycle(bridgeState)
  return {
    actionability: new CdpElementActionability(bridgeState, lifecycle),
    bridgeState,
    lifecycle,
    tabState
  }
...
function createIframeHarness(options?: {
  ...
  harnessState.iframeSessions = new Map(
    options?.nested
      ? [
          ['target-1', 'session-1'],
          ['target-2', 'session-2']
  ...
  harnessState.iframeParentSessions = new Map(
    options?.nested
      ? [
          ['session-1', null],
          ['session-2', 'session-1']
  ...
  return {
    actionability: owned.actionability,
    guest: { id: 11, debugger: { sendCommand } },
    sendCommand
  }
}
...
describe('CDP element actionability', () => {
  ...
  it('uses a non-zero border quad when the content quad is empty', async () => {
    ...
    await expect(createStack().actionability.getElementCenter(sender, 7, '@e1')).resolves.toEqual({
      cx: 22,
      cy: 22
    })
  })
  ...
  it('rejects a collinear layout quad with positive x and y extents', async () => {
    ...
    await expect(
      createStack().actionability.getElementCenter(sender, 7, '@e1')
  ...
  it.each([
    ...
    await expect(
      createStack().actionability.assertElementInteractable(
        createInteractabilitySender(options),
        7,
        '@e1',
        { requireEnabled: true }
      )
