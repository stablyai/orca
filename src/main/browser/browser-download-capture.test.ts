...
it.each(['page', 'child'])('retires an unclaimed capture when %s closes', async (closedPage) => {
  vi.useFakeTimers()
  const path = join(root, 'closed.txt')
  const pending = captures.begin('page', path)
  captures.inherit('page', 'child', 'https://example.com/attachment')
  captures.cancelPage('unrelated')
  expect(captures.hasPending('page')).toBe(true)
  captures.cancelPage(closedPage)
  await expect(pending.result).rejects.toMatchObject({ code: 'browser_download_canceled' })
  expect(captures.hasPending('page')).toBe(false)
  expect(captures.hasPending('child')).toBe(false)
  expect(captures.takeNavigation('child')).toBeUndefined()
  expect(captures.claim('child', vi.fn())).toBeUndefined()
  expect(vi.getTimerCount()).toBe(0)
  expect(() => captures.begin('replacement', path)).not.toThrow()
})

it('keeps a claimed child alive when its opener closes, then cancels only once on owner close', async () => {
  vi.useFakeTimers()
  const path = join(root, 'child.txt')
  const pending = captures.begin('opener', path)
  captures.inherit('opener', 'child', 'https://example.com/attachment')
  const cancelItem = vi.fn()
  const capture = captures.claim('child', cancelItem)!
  captures.cancelPage('opener')
  captures.cancelPage('unrelated')
  expect(cancelItem).not.toHaveBeenCalled()
  expect(vi.getTimerCount()).toBe(1)
  captures.cancelPage('child')
  captures.cancelPage('child')
  pending.cancel(new Error('late click failure'))
  captures.cancelAll()
  capture.finish({ downloadId: 'late', status: 'completed', savePath: path, error: null })
  await vi.advanceTimersByTimeAsync(BROWSER_DOWNLOAD_TIMEOUT_MS)
  await expect(pending.result).rejects.toMatchObject({ code: 'browser_download_canceled' })
  expect(cancelItem).toHaveBeenCalledOnce()
  expect(vi.getTimerCount()).toBe(0)
...
it('preserves completion that wins the page-close race', async () => {
  vi.useFakeTimers()
  const path = join(root, 'completed.txt')
  const pending = captures.begin('page', path)
  const cancelItem = vi.fn()
  const capture = captures.claim('page', cancelItem)!
  writeFileSync(path, 'done')
  capture.finish({ downloadId: 'done', status: 'completed', savePath: path, error: null })
  captures.cancelPage('page')
  pending.cancel(new Error('late click failure'))
  captures.cancelAll()
  await expect(pending.result).resolves.toMatchObject({ path, bytes: 4 })
  expect(cancelItem).not.toHaveBeenCalled()
  expect(vi.getTimerCount()).toBe(0)
})
