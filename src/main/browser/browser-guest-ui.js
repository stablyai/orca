...
import { isWindowShortcutModifierChord, resolveWindowShortcutAction } from '../../shared/window-shortcut-policy';
export function setupGuestContextMenu(args) {
    ...
    const handler = (_event, params) => {
        ...
        const linkUrl = rawLinkUrl.length > 0
    ...
    const contextMenuHandler = (_event, params) => {
        handler(_event, params);
        removeDismissListener();
        dismissHandler = (_evt, mouse) => {
...
export function resolveRendererWebContents(rendererWebContentsIdByTabId, browserTabId) {
