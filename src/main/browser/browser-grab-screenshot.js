 ...
 */
export async function captureSelectionScreenshot(rect, guest) {
    try {
        ...
        const safeN = (n, fallback = 0) => typeof n === 'number' && Number.isFinite(n) ? n : fallback;
        const safeRect = {
            x: safeN(rect.x),
            y: safeN(rect.y),
        ...
        await guest
            .executeJavaScript(`(function(){ var g = window.__orcaGrab; if (g && g.host) g.host.style.display = 'none'; })()`)
            .catch(() => { });
        let image;
        try {
            ...
            await guest
                .executeJavaScript(`(function(){ var g = window.__orcaGrab; if (g && g.host) g.host.style.display = ''; })()`)
                .catch(() => { });
        }
        if (image.isEmpty()) {
            return null;
        ...
        const viewportCSSWidth = await guest.executeJavaScript('window.innerWidth');
