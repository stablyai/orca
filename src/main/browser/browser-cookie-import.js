...
import { appendFileSync, copyFileSync, existsSync, mkdtempSync, readFileSync, readdirSync, rmSync, unlinkSync } from 'node:fs';
...
function discoverProfiles(browserRoot) {
    try {
...
// has no cookies DB.
export function selectBrowserProfile(browser, profileDirectory) {
...
// to supply it.
function deriveUrl(domain, secure) {
...
function validateCookieEntry(raw) {
    ...
    const expirationDate = typeof raw.expirationDate === 'number' && raw.expirationDate > 0
        ? raw.expirationDate
        : undefined;
    return {
        url,
...
async function importValidatedCookies(cookies, totalInput, targetPartition) {
    diag(`importValidatedCookies: ${cookies.length} validated of ${totalInput} total, partition="${targetPartition}"`);
    ...
    const stripNonPrintable = (s) => s.replace(/[^\x20-\x7E]/g, '');
    for (const cookie of cookies) {
        try {
            ...
            if (skipped <= 5) {
                ...
                diag(`  cookie.set FAILED: domain=${cookie.domain} name=${cookie.name} valLen=${val.length} badChar=${badInfo} err=${err}`);
            }
        }
    }
    diag(`importValidatedCookies result: imported=${importedCount} skipped=${skipped} domains=${domainSet.size}`);
    const summary = {
...
export async function importCookiesFromFile(filePath, targetPartition) {
    let rawContent;
    try {
    ...
    for (const entry of parsed) {
        ...
        const cookie = validateCookieEntry(entry);
        if (cookie) {
...
function getUserAgentForBrowser(family) {
    ...
    function readBrowserVersion(appPath, plistKey = 'CFBundleShortVersionString') {
        try {
            return (execFileSync('defaults', ['read', `${appPath}/Contents/Info`, plistKey], {
                encoding: 'utf-8',
                timeout: 5_000
            }).trim() || null);
        }
...
function chromiumTimestampToUnix(chromiumTs) {
    ...
    try {
        const ts = typeof chromiumTs === 'bigint'
            ? chromiumTs
...
function getEncryptionKey(keychainService, keychainAccount, browser) {
    if (process.platform === 'darwin') {
...
function getMacEncryptionKey(keychainService, keychainAccount) {
    try {
        const raw = execFileSync('security', ['find-generic-password', '-s', keychainService, '-a', keychainAccount, '-w'], { encoding: 'utf-8', timeout: 30_000 }).trim();
        return {
...
function getLinuxEncryptionKey(keychainService, keychainAccount) {
    ...
    try {
        ...
        keyringPassword = execFileSync('secret-tool', ['lookup', 'service', keychainService, 'account', keychainAccount], { encoding: 'utf-8', timeout: 5_000 }).trim();
    }
    catch {
...
function getWindowsEncryptionKey(browser) {
    const browserDef = CHROMIUM_BROWSERS.find((b) => b.family === browser.family);
    if (!browserDef) {
        return null;
    }
    ...
    try {
        ...
        const result = execFileSync('powershell', ['-NoProfile', '-NonInteractive', '-Command', script], { encoding: 'utf-8', timeout: 10_000, input: dpapiData }).trim();
...
function stripHmac(buf) {
    return hasHmacPrefix(buf) ? buf.subarray(CHROMIUM_COOKIE_HMAC_LEN) : buf;
}
function decryptCookieValueRaw(encryptedBuffer, keyResult) {
    ...
    const keysToTry = version === 'v10' && keyResult.fallbackKey
...
function decryptAes256Gcm(payload, key) {
...
// ---------------------------------------------------------------------------
function decodeSafariBinaryCookies(buffer) {
    if (buffer.length < 8) {
        return [];
    }
...
function decodeSafariPage(page) {
    if (page.length < 16) {
        return [];
    }
...
function decodeSafariCookie(buf) {
    if (buf.length < 48) {
        return null;
    }
...
// Firefox import
// ---------------------------------------------------------------------------
async function importCookiesFromFirefox(browser, targetPartition) {
...
// Safari import
// ---------------------------------------------------------------------------
async function importCookiesFromSafari(browser, targetPartition) {
    ...
    try {
        ...
        const isPermError = err instanceof Error && 'code' in err && err.code === 'EPERM';
        if (isPermError) {
            return {
                ok: false,
                reason: 'macOS denied access to Safari cookies. Grant Full Disk Access to Orca in System Settings → Privacy & Security → Full Disk Access.'
            };
        }
...
// Import dispatcher
// ---------------------------------------------------------------------------
export async function importCookiesFromBrowser(browser, targetPartition) {
    ...
    try {
        ...
        const targetCols = stagingDb.prepare('PRAGMA table_info(cookies)').all().map((r) => r.name);
        ...
        const insertStmt = stagingDb.prepare(`INSERT OR REPLACE INTO cookies (${colList}) VALUES (${placeholders})`);
        ...
        for (const sourceRow of sourceRows) {
            ...
            const expiresUtc = chromiumTimestampToUnix(sourceRow.expires_utc);
        ...
        diag(`  cleared existing session cookies before loading ${decryptedCookies.length} imported cookies`);
