...
const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  var g = window.__mcodeGrab;
  if (g && g.host) g.host.style.display = 'none';
  document.querySelectorAll('[data-mcode-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-mcode-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-mcode-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-mcode-previous-display') || '';
    el.removeAttribute('data-mcode-previous-display');
  });
})()`
