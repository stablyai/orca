...
const { webContentsFromIdMock, guestOnMock, guestOffMock, guestSetBackgroundThrottlingMock, guestSetWindowOpenHandlerMock, guestExecuteJavaScriptMock, guestIsDestroyedMock, guestGetZoomFactorMock, guestCapturePageMock, menuBuildFromTemplateMock, rendererSendMock, rendererIsDestroyedMock } = vi.hoisted(() => ({
...
describe('browserManager grab operations', () => {
    ...
    describe('grab shortcut forwarding', () => {
        it('forwards cmd/ctrl+c from the guest when the page is not using copy', async () => {
            const handler = guestOnMock.mock.calls.find(([eventName]) => eventName === 'before-input-event')?.[1];
            expect(handler).toBeTypeOf('function');
            guestExecuteJavaScriptMock.mockResolvedValueOnce(true);
            const preventDefault = vi.fn();
            handler?.({ preventDefault }, { type: 'keyDown', meta: true, control: true, shift: false, alt: false, key: 'c' });
    ...
    describe('guest app shortcut forwarding', () => {
        it('forwards Cmd/Ctrl+Shift+B to the renderer and prevents the guest default', () => {
            ...
            const preventDefault = vi.fn();
            forwardingHandler?.({ preventDefault }, {
                type: 'keyDown',
                meta: true,
                control: true,
                shift: true,
    ...
    describe('awaitGrabSelection', () => {
        ...
        it('replacement op skips teardown injection to preserve overlay', async () => {
            ...
            const teardownCalls = newCalls.filter(([script]) => script.includes('if (!grab) return true;'));
        ...
        it('ignores a late guest selection after the op was already cancelled', async () => {
            let resolveGuestSelection;
            guestExecuteJavaScriptMock.mockImplementation(() => new Promise((resolve) => {
    ...
    describe('navigation auto-cancel', () => {
        it('cancels grab when guest navigates in main frame', async () => {
            ...
            const navHandler = guestOnMock.mock.calls.find(([event]) => event === 'did-start-navigation')?.[1];
    ...
    describe('destruction auto-cancel', () => {
        it('cancels grab when guest is destroyed', async () => {
            ...
            const destroyedHandler = guestOnMock.mock.calls.find(([event]) => event === 'destroyed')?.[1];
    ...
    describe('captureSelectionScreenshot', () => {
        it('captures, crops, and converts screenshot dimensions back to CSS pixels', async () => {
            ...
            guestExecuteJavaScriptMock.mockImplementation(async (script) => script === 'window.innerWidth' ? 1000 : undefined);
            const screenshot = await browserManager.captureSelectionScreenshot('tab-1', { x: 10, y: 20, width: 100, height: 50 }, guest);
            ...
            expect(guestExecuteJavaScriptMock).toHaveBeenNthCalledWith(1, `(function(){ var g = window.__orcaGrab; if (g && g.host) g.host.style.display = 'none'; })()`);
            expect(guestExecuteJavaScriptMock).toHaveBeenNthCalledWith(2, `(function(){ var g = window.__orcaGrab; if (g && g.host) g.host.style.display = ''; })()`);
        ...
        it('omits screenshots that exceed the byte budget', async () => {
            ...
            guestExecuteJavaScriptMock.mockImplementation(async (script) => script === 'window.innerWidth' ? 1000 : undefined);
            const screenshot = await browserManager.captureSelectionScreenshot('tab-1', { x: 0, y: 0, width: 100, height: 50 }, guest);
