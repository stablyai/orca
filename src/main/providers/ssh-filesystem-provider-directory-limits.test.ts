...
describe('SshFilesystemProvider directory limits', () => {
  ...
  it('validates bounded relay results on the mobile path too', async () => {
    // Why: the mobile branch asserts against its own tighter ceiling, so a relay that ignores the
    // forwarded limits has to be caught here rather than trusted because the request looked right.
    const request = vi.fn().mockResolvedValue(
      Array.from({ length: MOBILE_FILE_DIRECTORY_MAX_ENTRIES + 1 }, (_unused, index) => ({
        name: `entry-${index}`,
        isDirectory: false,
        isSymlink: false
      }))
    ...
    await expect(
      provider.readDir('/home/user/project', {
