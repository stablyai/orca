...
describe('LocalPtyProvider', () => {
    ...
    describe('spawn', () => {
        ...
        it('throws when cwd does not exist', async () => {
            existsSyncMock.mockImplementation((p) => p !== '/nonexistent');
            await expect(provider.spawn({ cols: 80, rows: 24, cwd: '/nonexistent' })).rejects.toThrow('does not exist');
        });
        ...
        it('invokes buildSpawnEnv callback to customize environment', async () => {
            const buildSpawnEnv = vi.fn((_id, env) => {
