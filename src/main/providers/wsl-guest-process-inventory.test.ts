...
describe('WSL guest process inventory', () => {
  ...
  it.each([
    [
      'distro_mismatch',
      { distro: 'Debian', bootId, shellPid: 100, shellStartTime: 12345, tty: '/dev/pts/0' }
    ],
    [
      'anchor_missing',
      { distro: 'Ubuntu', bootId, shellPid: 999, shellStartTime: 12345, tty: '/dev/pts/0' }
    ],
    [
      'tty_mismatch',
      { distro: 'Ubuntu', bootId, shellPid: 100, shellStartTime: 12345, tty: '/dev/pts/1' }
    ]
  ] as const)('rejects the %s identity fence', (reason, anchor) => {
    const inventory = parseWslGuestProcessInventoryPayload(
      payload('row 100 90 90 100 100 pts/0 Ss+ 12345 bash'),
      'Ubuntu'
    )
    expect(resolveWslGuestForegroundProcess(inventory, anchor)).toEqual({
      status: 'unverifiable',
      reason
    })
  })

  it('rejects a shell with no foreground process group', () => {
    const inventory = parseWslGuestProcessInventoryPayload(
      payload('row 100 90 90 100 -1 pts/0 Ss+ 12345 bash'),
      'Ubuntu'
    )
    expect(
      resolveWslGuestForegroundProcess(inventory, {
    ...
    ).toEqual({ status: 'unverifiable', reason: 'foreground_group_missing' })
  })

  it('rejects an ambiguous foreground group containing multiple agents', () => {
    const inventory = parseWslGuestProcessInventoryPayload(
      payload(
        [
          'row 100 90 90 100 120 pts/0 Ss+ 12345 bash',
          'row 120 100 100 120 120 pts/0 Sl+ 54321 codex',
          'row 121 100 100 120 120 pts/0 Sl+ 54322 claude'
        ].join('\n'),
        3
      ),
      'Ubuntu'
    ...
    ).toEqual({ status: 'unverifiable', reason: 'ambiguous_foreground_group' })
  })
