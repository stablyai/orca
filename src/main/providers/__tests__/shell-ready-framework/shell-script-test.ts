...
export async function shellScriptTest(
  ...
  try {
    ...
    const env: Record<string, string> = {
      ...
      MCODE_SHELL_FEATURES: (config.env.MCODE_SHELL_FEATURES ?? '')
        .split(',')
      ...
      HOME: testHome,
      MCODE_ORIG_ZDOTDIR: testHome,
      MCODE_ZSHENV_SOURCE_DIR: testHome
    }
...
const TEMP_PATH_PATTERN =
  /\/(?:var\/folders|tmp)\/[^\s]+?\/(?:shell-test|mcode|shell-ready)-[a-z]+-[a-z0-9-]+/g
