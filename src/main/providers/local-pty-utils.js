 ...
 */
export function getShellValidationError(shellPath) {
    if (!existsSync(shellPath)) {
        return (`Shell "${shellPath}" does not exist. ` +
...
export function ensureNodePtySpawnHelperExecutable() {
    ...
    try {
        ...
        console.warn(`[pty] Failed to ensure node-pty spawn-helper is executable: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 ...
 */
export function spawnShellWithFallback(params) {
    const { shellPath, shellArgs, cols, rows, cwd, env, ptySpawn, getShellReadyConfig, onBeforeFallbackSpawn } = params;
    ...
    if (process.platform !== 'win32') {
        ...
        for (const fallback of fallbackShells) {
            ...
            try {
                ...
                console.warn(`[pty] Primary shell "${shellPath}" failed (${primaryError ?? 'unknown error'}), fell back to "${fallback}"`);
    ...
    throw new Error(`Failed to spawn shell "${shellPath}": ${primaryError ?? 'unknown error'} (${diag}). ` +
