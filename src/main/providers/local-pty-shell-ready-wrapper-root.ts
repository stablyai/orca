...
function getShellReadyWrapperBaseDir(): string {
  // Why: bundled into the daemon fork (no electron), so read MCODE_USER_DATA_PATH rather than electron's userData; main and the fork both set it to the same path.
  // Why a truthiness test rather than `??`: a set-but-empty MCODE_USER_DATA_PATH
  // would leave a relative base dir, so wrapper trees would be written under
  // whatever the process cwd happens to be.
  const userDataPath = process.env.MCODE_USER_DATA_PATH
  ...
  // reaches it -- MCODE_USER_DATA_PATH is seeded before anything spawns and the
  // daemon fork inherits it -- so this stays a documented trust boundary rather
  // than an ownership check on the spawn path.
  return userDataPath ? join(userDataPath, 'shell-wrappers') : join(tmpdir(), 'mcode-shell-wrappers')
}
...
// self-invalidates if MCODE_USER_DATA_PATH is ever re-pointed mid-process.
