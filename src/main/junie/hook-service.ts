...
import {
  applyJunieManagedHooks,
  getJunieConfigPath,
  getJunieManagedCommand,
  getJunieManagedScriptFileName,
  getJunieManagedScriptPath,
  getJuniePosixManagedScriptFileName,
  getJunieRemoteConfigPath,
  getJunieRemoteManagedCommand,
  JUNIE_EVENTS,
  removeJunieManagedHooks
} from './hook-settings'
import {
  parseJunieHooksConfigText,
  readJunieHooksConfig,
  readJunieHooksSource,
  serializeJunieHooksConfig
} from './hook-config-json'

// Why: the script must never write to stdout — Junie injects non-empty non-JSON hook
// stdout into the agent context as additionalContext; empty stdout is a clean no-op.
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      ...buildWindowsHookEnvironmentGuardLines(),
      buildWindowsAgentHookPostCommand('junie'),
      'exit /b 0',
      ...buildWindowsHookStdinDrainEpilogue(),
      ''
    ].join('\r\n')
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${ORCA_AGENT_HOOK_PORT}/hook/junie" \\',
...
export class JunieHookService {
  async refreshManagedScripts(): Promise<void> {
    await refreshManagedScriptIfPresent(getJunieManagedScriptPath(), getManagedScript())
  }

  getStatus(): AgentHookInstallStatus {
    const configPath = getJunieConfigPath()
    const scriptPath = getJunieManagedScriptPath()
    const config = readJunieHooksConfig(configPath)
    if (!config) {
      return {
        agent: 'junie',
        state: 'error',
        ...
        detail: 'Could not parse Junie config.json'
      }
    }
  ...
  install(): AgentHookInstallStatus {
    const configPath = getJunieConfigPath()
    const scriptPath = getJunieManagedScriptPath()
    const source = readJunieHooksSource(configPath)
    if (!source) {
      return {
        agent: 'junie',
        state: 'error',
    ...
    const command = getJunieManagedCommand(scriptPath)
    const nextConfig = applyJunieManagedHooks(
      source.config,
      command,
      getJunieManagedScriptFileName()
    )
    // Why: write script before config so a mid-install failure never leaves config.json referencing a missing script.
    writeManagedScript(scriptPath, getManagedScript())
    writeHooksJson(configPath, nextConfig, {
      serialized: serializeJunieHooksConfig(source.text, nextConfig)
    })
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    try {
      ...
      const config =
        body === null ? {} : parseJunieHooksConfigText(body, 'remote Junie config.json')
      if (!config) {
        return {
          agent: 'junie',
          state: 'error',
          ...
          managedHooksPresent: false,
          detail: 'Could not parse remote Junie config.json'
        }
      }

      // Why: the POSIX wrapper body is path-independent, so reuse the same helper.
      const command = getJunieRemoteManagedCommand(remoteScriptPath)
      const nextConfig = applyJunieManagedHooks(config, command, remoteScriptFileName)
      ...
      await writeHooksJsonRemote(sftp, remoteConfigPath, nextConfig, {
        serialized: serializeJunieHooksConfig(body, nextConfig)
      })
  ...
  remove(): AgentHookInstallStatus {
    ...
    const { config: nextConfig, changed } = removeJunieManagedHooks(
      source.config,
      getJunieManagedScriptFileName()
    )
    if (changed) {
...
export const junieHookService = new JunieHookService()
