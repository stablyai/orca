...
const MCODE_CLAUDE_SERVICE = 'MCode Claude Code Managed Credentials'
...
export async function readManagedClaudeKeychainCredentials(
  ...
  return readKeychainPassword(MCODE_CLAUDE_SERVICE, accountId)
}
...
export async function writeManagedClaudeKeychainCredentials(
  ...
  await writeKeychainPassword(MCODE_CLAUDE_SERVICE, accountId, contents)
}

export async function deleteManagedClaudeKeychainCredentials(accountId: string): Promise<void> {
  await deleteKeychainPassword(MCODE_CLAUDE_SERVICE, accountId)
}
