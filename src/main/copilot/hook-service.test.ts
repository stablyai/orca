...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mcode-copilot-hooks-'))
...
function readConfig(): Record<string, unknown> {
  return JSON.parse(readFileSync(join(copilotHome, 'hooks', 'mcode.json'), 'utf-8'))
}
...
describe('CopilotHookService', () => {
  it('installs a user-level Copilot hook file under COPILOT_HOME', () => {
    ...
    expect(status.configPath).toBe(join(copilotHome, 'hooks', 'mcode.json'))
    ...
    if (process.platform === 'win32') {
      ...
      expect(decoded).toContain("$env:MCODE_COPILOT_HOOK_EVENT = 'UserPromptSubmit'")
    } else {
      expect(firstPromptHook.bash).toContain('if [ -f ')
      expect(firstPromptHook.bash).toContain('] && [ -x ')
      expect(firstPromptHook.bash).toContain('.mcode/agent-hooks/copilot-hook.sh')
      expect(firstPromptHook.bash).toContain("MCODE_COPILOT_HOOK_EVENT='UserPromptSubmit'")
    }
    expect(existsSync(join(tmpDir, '.mcode', 'agent-hooks', 'copilot-hook.sh'))).toBe(
  ...
  it('preserves user-authored hooks and sweeps stale managed entries', () => {
    const configPath = join(copilotHome, 'hooks', 'mcode.json')
  ...
  it('remove deletes only MCode-managed Copilot hooks', () => {
    const service = new CopilotHookService()
    service.install()
    const configPath = join(copilotHome, 'hooks', 'mcode.json')
    const config = readConfig()
  ...
  it('remove does not create an mcode.json file when nothing is installed', () => {
    const status = new CopilotHookService().remove()

    expect(status.state).toBe('not_installed')
    expect(existsSync(join(copilotHome, 'hooks', 'mcode.json'))).toBe(false)
  })
  ...
  it('returns an error status for malformed JSON', () => {
    mkdirSync(join(copilotHome, 'hooks'), { recursive: true })
    writeFileSync(join(copilotHome, 'hooks', 'mcode.json'), '{not json')
    ...
    expect(status).toEqual({
      agent: 'copilot',
      state: 'error',
      configPath: join(copilotHome, 'hooks', 'mcode.json'),
      managedHooksPresent: false,
      detail: 'Could not parse Copilot hooks/mcode.json'
    })
  })
})
