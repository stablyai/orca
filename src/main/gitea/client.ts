...
function getAuthConfig(): GiteaAuthConfig {
  const apiBaseUrl = envValue('MCODE_GITEA_API_BASE_URL')
  return {
    apiBaseUrl: apiBaseUrl ? normalizeGiteaApiBaseUrl(apiBaseUrl) : null,
    token: envValue('MCODE_GITEA_TOKEN')
  }
}
...
/** Invalidate the shared /pulls scan after MCode itself creates a PR so the
