...
describe('CodexUsageStore', () => {
  ...
  it('reports no data for MCode scope when only non-MCode Codex usage exists', async () => {
  ...
  it('uses only MCode-scoped models when projecting mixed-scope sessions', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const breakdown = await store.getBreakdown('mcode', '30d', 'model')
    const recentSessions = await store.getRecentSessions('mcode', '30d', 10)
