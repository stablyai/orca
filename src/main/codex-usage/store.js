...
function estimateCostUsd(model, inputTokens, cachedInputTokens, outputTokens) {
    ...
    return ((nonCachedInputTokens * pricing.input +
...
function getWorktreeFingerprint(worktreesByRepo) {
    const rows = [...worktreesByRepo.entries()]
        .flatMap(([repoId, worktrees]) => worktrees.map((worktree) => JSON.stringify({
        repoId,
...
export class CodexUsageStore {
    ...
    async runScan() {
        ...
        this.scanPromise = (async () => {
            try {
                ...
                const result = await scanCodexUsageFiles(createWorktreeRefs(repos, worktreesByRepo), this.state.worktreeFingerprint === worktreeFingerprint ? this.state.processedFiles : []);
    ...
    async getSummary(scope, range) {
        ...
        for (const row of filteredDaily) {
            ...
            byModel.set(row.model ?? 'Unknown model', (byModel.get(row.model ?? 'Unknown model') ?? 0) + row.totalTokens);
            byProject.set(row.projectLabel, (byProject.get(row.projectLabel) ?? 0) + row.totalTokens);
            const cost = estimateCostUsd(row.model, row.inputTokens, row.cachedInputTokens, row.outputTokens);
            if (cost !== null) {
    ...
    async getBreakdown(scope, range, kind) {
        ...
        for (const row of rows.values()) {
            row.estimatedCostUsd = estimateCostUsd(kind === 'model' ? row.key : null, row.inputTokens, row.cachedInputTokens, row.outputTokens);
        }
    ...
    async getRecentSessions(scope, range, limit = 12) {
        ...
        return this.getFilteredSessions(scope, range)
            ...
            return {
                ...
                durationMinutes,
                projectLabel: scopedLocations.length > 1
                    ? 'Multiple locations'
    ...
    getScopedSessionModels(session, scope) {
    ...
    getScopedSessionPrimaryModel(session, scope) {
    ...
    async getCurrentWorktreeFingerprint() {
        const repos = this.store.getRepos();
        const worktreesByRepo = await this.loadWorktreesByRepo(repos);
