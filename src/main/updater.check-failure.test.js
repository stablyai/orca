...
const { appMock, browserWindowMock, nativeUpdaterMock, autoUpdaterMock, isMock, killAllPtyMock } = vi.hoisted(() => {
    const appEventHandlers = new Map();
    const eventHandlers = new Map();
    const appOn = vi.fn((event, handler) => {
...
describe('updater check failure handling', () => {
    ...
    it('treats GitHub release transition errors as idle for user-initiated checks', async () => {
        ...
        await vi.waitFor(() => {
            ...
            expect(statuses).not.toContainEqual(expect.objectContaining({ state: 'not-available', userInitiated: true }));
        });
    });
    it('treats missing latest-mac.yml during user-initiated checks as idle', async () => {
        autoUpdaterMock.checkForUpdates.mockResolvedValueOnce(undefined).mockImplementationOnce(() => {
            autoUpdaterMock.emit('checking-for-update');
            queueMicrotask(() => {
                autoUpdaterMock.emit('error', new Error('Cannot find channel "latest-mac.yml" update info: HttpError: 404'));
            });
            return Promise.reject(new Error('Cannot find channel "latest-mac.yml" update info: HttpError: 404'));
        });
        const sendMock = vi.fn();
        const mainWindow = { webContents: { send: sendMock } };
        const { setupAutoUpdater, checkForUpdatesFromMenu } = await import('./updater');
