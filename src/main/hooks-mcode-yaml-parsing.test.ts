...
import { parseMCodeYaml } from './hooks'
...
describe('parseMCodeYaml', () => {
  it('parses YAML with setup script only', () => {
    const yaml = `scripts:\n  setup: |\n    echo "setting up"\n    npm install\n`
    const result = parseMCodeYaml(yaml)
  ...
  it('returns null when there is no scripts block', () => {
    const yaml = `other:\n  key: value\n`
    expect(parseMCodeYaml(yaml)).toBeNull()
  })
  ...
  it('returns null for empty string', () => {
    expect(parseMCodeYaml('')).toBeNull()
  })
  ...
  it('parses default terminal tabs from mcode.yaml', () => {
    const yaml = [
      'defaultTabs:',
      '  - title: Claude',
      '    color: "#f97316"',
    ...
    expect(parseMCodeYaml(yaml)).toEqual({
      scripts: {},
      defaultTabs: [
  ...
  it('parses environmentRecipes from mcode.yaml', () => {
    const yaml = [
      'environmentRecipes:',
      '  - id: cloud-sandbox',
      '    name: Cloud Sandbox',
      ...
      '    create: ./scripts/mcode-vm/start-cloud-sandbox.sh',
      '    suspend: ./scripts/mcode-vm/suspend-cloud-sandbox.sh',
      '    resume: ./scripts/mcode-vm/resume-cloud-sandbox.sh',
      '    destroy: ./scripts/mcode-vm/destroy-cloud-sandbox.sh'
    ].join('\n')
      ...
      environmentRecipeDiagnostics: [
        {
          ...
          field: 'checkoutMode',
          message:
            'Recipe "cloud-sandbox" checkoutMode must be "mcode-worktree" or "provisioned-root".'
        }
      ]
    })
  })
  ...
  it('parses legacy environmentRecipes command and cleanup aliases', () => {
    const yaml = [
      ...
      '  - id: manual-sandbox',
      '    name: Manual Sandbox',
      '    command: ./scripts/mcode-vm/start-manual-sandbox.sh',
      '    cleanup: none'
    ].join('\n')
      ...
      environmentRecipes: [
        {
          id: 'manual-sandbox',
          name: 'Manual Sandbox',
          create: './scripts/mcode-vm/start-manual-sandbox.sh',
  ...
  it('drops invalid and duplicate environmentRecipes', () => {
    const yaml = [
      ...
      '    create: ./scripts/mcode-vm/start-duplicate.sh',
      '  - id: missing-create',
      '    name: Missing Create',
      '  - name: Missing Id',
      '    create: ./scripts/mcode-vm/start-missing-id.sh',
      '  - id: "Cloud Sandbox"',
      '    name: Unsafe Id',
      '    create: ./scripts/mcode-vm/start-unsafe-id.sh',
      '  - 42'
    ].join('\n')
  ...
  it('parses worktree.sharedDirectories from mcode.yaml', () => {
    const result = parseMCodeYaml(
  ...
  it('returns null when sharedDirectories is the only key and holds nothing usable', () => {
    expect(parseMCodeYaml('worktree:\n  sharedDirectories: []\n')).toBeNull()
    expect(parseMCodeYaml('worktree:\n  sharedDirectories: node_modules\n')).toBeNull()
  })

  it('keeps sharedDirectories alongside other mcode.yaml keys', () => {
    const result = parseMCodeYaml(
      [
        'scripts:',
        '  setup: pnpm install',
...
describe('hasUnrecognizedMCodeYamlKeys', () => {
  it('returns true when the file contains only keys this version does not handle', async () => {
    ...
    const { hasUnrecognizedMCodeYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedMCodeYamlKeys('/test/repo')).toBe(true)
  })
  ...
  it('returns false when the file contains only recognised keys', async () => {
    ...
    const { hasUnrecognizedMCodeYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedMCodeYamlKeys('/test/repo')).toBe(false)
  })
