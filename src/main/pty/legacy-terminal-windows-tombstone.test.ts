...
describe('legacy terminal Windows tombstone text', () => {
  ...
  const makeUserDataDir = (): string => {
    const userData = mkdtempSync(join(tmpdir(), 'mcode-legacy-shim-win-'))
  ...
  it('rejects stale Windows real-command paths inside the wrapper directory', () => {
    ...
    expect(cmd).toContain('if defined mcode_real if not exist "%mcode_real%" set "mcode_real="')
    expectOrdered(cmd, 'if not exist "%mcode_real%"', ':mcode_try_candidate')
  ...
  it('resolves Windows fallbacks against PATH only, never the current directory', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      ...
      expect(cmd).toContain('for %%P in ("%mcode_clean_path:;=" "%") do (')
      expect(cmd).toContain(`if exist "%mcode_candidate_dir%\\${command}.exe"`)
      ...
      expect(cmd).toContain('set "mcode_sep=\\"')
      ...
      // Why: a captured MCODE_REAL_* may be relative, and both the existence test and the
      // invocation resolve it against the cwd.
      expect(cmd).toContain('if defined mcode_real call :mcode_check_rooted')
      expect(cmd).toContain('if defined mcode_real if not defined mcode_rooted set "mcode_real="')
      ...
      // Why the exact block, not an ordering pin: the first ':mcode_append_path' in the file is
      // the top-level CALL, and the first 'if not defined mcode_rooted' belongs to a different
      // subroutine, so an ordering pin stayed green with this subroutine's guard deleted.
      expect(cmd).toContain(':mcode_append_path\r\nif not defined mcode_entry exit /b\r\n')
      expect(cmd).toContain(
      ...
      expect(cmd).not.toContain('call :mcode_check_rooted "')
      expect(cmd).not.toContain('call :mcode_try_candidate "')
      expect(cmd).not.toContain('call :mcode_append_path "')
      expect(cmd).not.toContain('%~1')
      expect(cmd).toContain('set "mcode_probe=%mcode_entry%"\r\ncall :mcode_check_rooted\r\n')
      // Why: mcode_sep is compared against before the legacy dir is normalized; defining it later
      // made that strip silently no-op and left the legacy dir on PATH.
      expectOrdered(cmd, 'set "mcode_sep=', 'if "%mcode_legacy_norm:~-1%."')
      // Why: %mcode_sep% expands before the line is parsed, so `=="%mcode_sep%"` becomes `=="\"` —
      // the literal trap the variable exists to avoid. Both sides must carry the trailing dot.
      expect(cmd).not.toContain('=="%mcode_sep%"')
      ...
      expectOrdered(
        cmd,
        'call :mcode_check_rooted\r\nif not defined mcode_rooted exit /b',
        'for %%G in ("%mcode_entry%") do set "mcode_candidate_dir='
      )
      expect(cmd).toContain('if not defined mcode_rooted exit /b')
      expect(cmd).toContain('if "%mcode_candidate_dir:~-1%."=="%mcode_sep%."')
      expect(cmd).not.toContain(':\\#=#%')
      ...
      expect(cmd).toContain('if /I "%mcode_candidate_dir%\\"=="%mcode_wrapper_dir%" exit /b')
      ...
      expect(cmd).toContain('if "%mcode_probe:~1,2%"==":\\" set "mcode_rooted=1"')
      expect(cmd).toContain('if "%mcode_probe:~0,2%"=="\\\\" set "mcode_rooted=1"')
      ...
      expect(cmd).toContain('if not defined mcode_entry exit /b')
      ...
      expect(cmd).toContain('if "%mcode_legacy_norm:~-1%."=="%mcode_sep%."')
      ...
      expect(cmd).toContain('if defined mcode_legacy_wrapper_dir call :mcode_normalize_legacy_dir')
      expect(cmd).not.toContain('if defined mcode_legacy_wrapper_dir for ')
  ...
  it('guards both cmd PATH walks so an empty variable cannot break parsing', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      const cmd = readFileSync(join(win32Dir, `${command}.cmd`), 'utf8')
      expect(cmd).toContain('if not defined PATH goto :mcode_path_walked')
      expect(cmd).toContain('if not defined mcode_clean_path goto :mcode_candidates_walked')
      for (const [guard, loop] of [
        ['if not defined PATH goto :mcode_path_walked', 'for %%P in ("%PATH:;='],
        [
          'if not defined mcode_clean_path goto :mcode_candidates_walked',
          'for %%P in ("%mcode_clean_path:;='
        ]
      ] as const) {
