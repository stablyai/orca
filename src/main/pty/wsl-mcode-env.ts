...
function worktreeSetupWslenvEntries(env: Record<string, string | undefined>): string[] {
  return [
    ...
    ...['MCODE_ROOT_PATH', 'MCODE_WORKTREE_PATH', 'CONDUCTOR_ROOT_PATH', 'GHOSTX_ROOT_PATH'].map(
      (name) => `${name}/${env[name]?.startsWith('/') ? 'u' : 'p'}`
    ),
    // Why: a display name, never a path — never path-translate it.
    'MCODE_WORKSPACE_NAME/u'
  ]
}
...
export function addMCodeWslInteropEnv(env: Record<string, string>): void {
  ...
  env.MCODE_SHELL_READY_ROOT = getShellReadyWrapperRoot()
  ...
  const endpointFlag = env.MCODE_AGENT_HOOK_ENDPOINT?.startsWith('/') ? 'u' : 'p'
  ...
  const opencodeOverlayEntries = (['OPENCODE_CONFIG_DIR', 'MCODE_OPENCODE_CONFIG_DIR'] as const)
  ...
  const passthroughEntries = [
    ...
    'MCODE_USER_DATA_PATH/p',
    // Why /p: the guest reads the content-addressed wrapper tree through /mnt/c,
    // and it cannot derive the hash segment from MCODE_USER_DATA_PATH alone.
    'MCODE_SHELL_READY_ROOT/p',
    'MCODE_CLI_COMMAND/u',
    'MCODE_PANE_KEY/u',
    'MCODE_TAB_ID/u',
    'MCODE_WORKTREE_ID/u',
    'MCODE_AGENT_LAUNCH_TOKEN/u',
    `${SETUP_AGENT_SEQUENCE_STARTUP_COMMAND_ENV}/u`,
    `${SETUP_AGENT_SEQUENCE_STARTUP_SCRIPT_ENV}/u`,
    'MCODE_ORCHESTRATION_COMPATIBILITY_HOST_KIND/u',
    'MCODE_ORCHESTRATION_COMPATIBILITY_HOST_ID/u',
    'MCODE_ORCHESTRATION_COMPATIBILITY_HOST_INCARNATION/u',
    'MCODE_AGENT_HOOK_PORT/u',
    'MCODE_AGENT_HOOK_TOKEN/u',
    'MCODE_AGENT_HOOK_ENV/u',
    'MCODE_AGENT_HOOK_VERSION/u',
    `MCODE_AGENT_HOOK_ENDPOINT/${endpointFlag}`,
    ...opencodeOverlayEntries,
    'MCODE_WSL_HOOK_RELAY_VERSION/u',
    'MCODE_WSL_HOOK_INSTANCE/u',
    'MCODE_OMP_SOURCE_AGENT_DIR/p',
    'MCODE_OMP_STATUS_EXTENSION/p',
