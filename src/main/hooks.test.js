...
describe('readIssueCommand', () => {
    it('prefers the local override over the shared orca.yaml command', async () => {
        const fs = await import('fs');
        vi.mocked(fs.existsSync).mockImplementation((path) => path === '/test/repo/.orca/issue-command' || path === '/test/repo/orca.yaml');
...
describe('writeIssueCommand', () => {
    it('writes only the local override file and keeps .orca ignored locally', async () => {
        const fs = await import('fs');
        vi.mocked(fs.existsSync).mockImplementation((path) => path === '/test/repo/.gitignore' || path === '/test/repo/.orca');
        vi.mocked(fs.readFileSync).mockImplementation((path) => {
        ...
        expect(vi.mocked(fs.writeFileSync)).toHaveBeenCalledWith('/test/repo/.gitignore', 'node_modules/\n.orca\n', 'utf-8');
        expect(vi.mocked(fs.writeFileSync)).toHaveBeenCalledWith('/test/repo/.orca/issue-command', 'local command\n', 'utf-8');
    });
...
describe('runHook', () => {
    ...
    it('uses the Windows command shell when running hooks', async () => {
        ...
        try {
            ...
            expect(execMock).toHaveBeenCalledWith('echo hello', expect.objectContaining({
                cwd: 'C:\\repo\\worktree',
    ...
    it('runs WSL hooks through wsl.exe and translates env paths to Linux', async () => {
        ...
        try {
            ...
            expect(result).toEqual({ success: true, output: '' });
            expect(execFileMock).toHaveBeenCalledWith('wsl.exe', ['-d', 'Ubuntu', '--', 'bash', '-c', "cd '/home/jin/feature' && echo hello"], expect.any(Object), expect.any(Function));
...
describe('shouldRunSetupForCreate', () => {
    ...
    it('requires an explicit decision when the repo policy is ask', async () => {
        ...
        expect(() => shouldRunSetupForCreate(makeRepo('ask'))).toThrow('Setup decision required for this repository');
    });
