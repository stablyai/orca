...
function mockPtyProcess(pid = 12345) {
    ...
    return {
        ...
        _simulateData: (data) => onDataListeners.forEach((cb) => cb(data)),
        _simulateExit: (code) => onExitListeners.forEach((cb) => cb({ exitCode: code }))
    };
}
describe('createPtySubprocess', () => {
    it('spawns node-pty with correct options', () => {
        ...
        expect(spawnMock).toHaveBeenCalledWith('/bin/bash', expect.any(Array), expect.objectContaining({
            cols: 80,
            rows: 24,
            cwd: '/home/user',
            name: 'xterm-256color'
    ...
    it('combines HOMEDRIVE and HOMEPATH for Windows default cwd', () => {
        ...
        expect(spawnMock).toHaveBeenCalledWith(expect.any(String), expect.any(Array), expect.objectContaining({ cwd: 'D:\\Users\\orca' }));
    });
});
