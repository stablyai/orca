...
import {
  CODEX_DEFAULT_HOME_AFTER_PROFILE_DAEMON_PROTOCOL_VERSION,
...
describe('DaemonPtyRouter', () => {
  ...
  it('preserves older session owners and routes new sessions to the current daemon', async () => {
    ...
    const legacyV36 = createAdapter(
      ...
      ['v36-session'],
      undefined,
      CODEX_DEFAULT_HOME_AFTER_PROFILE_DAEMON_PROTOCOL_VERSION - 1
    )
    const router = new DaemonPtyRouter({
      current,
    ...
    await router.spawn({ sessionId: 'v36-session', cols: 80, rows: 24 })
    ...
    expect(legacyV36.spawn).toHaveBeenCalledWith({ sessionId: 'v36-session', cols: 80, rows: 24 })
