...
describe('DaemonServer', () => {
    ...
    describe('RPC routing', () => {
        ...
        it('handles listSessions', async () => {
            ...
            const result = await c.request('listSessions', undefined);
        ...
        it('handles getCwd', async () => {
            ...
            const result = await c.request('getCwd', {
                sessionId: 'test-session'
            });
        ...
        it('returns error for unknown session operations', async () => {
            await startServer();
            const c = await connectClient();
            await expect(c.request('write', { sessionId: 'nonexistent', data: 'hi' })).rejects.toThrow('Session not found');
        });
    });
