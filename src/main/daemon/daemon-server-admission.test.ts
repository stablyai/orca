...
import { NOTIFY_PREFIX, PROTOCOL_VERSION, type DaemonRequest } from './types'
...
describe('DaemonServer admission', () => {
  ...
  // Why: the limit table is unit-tested directly, so these assert the server actually consults it
  // on the live socket — without them, unwiring the admission call leaves the suite green.
  it.each([
    {
      name: 'an oversized createOrAttach cwd',
      request: {
        id: 'req-bad-cwd',
        type: 'createOrAttach',
        payload: { sessionId: 'session-1', cwd: 'x'.repeat(DAEMON_PTY_CWD_MAX_BYTES + 1) }
      },
      error: /payload\.cwd exceeds/
    },
    {
      name: 'PTY dimensions that would force a pathological allocation',
      request: {
        id: 'req-bad-size',
        type: 'createOrAttach',
        payload: { sessionId: 'session-1', cols: 1_000_000, rows: 1_000_000 }
      },
      error: /cols|rows/
    },
    {
      name: 'a resize with a non-string sessionId',
      request: {
        id: 'req-bad-resize',
        type: 'resize',
        payload: { sessionId: 42, cols: 80, rows: 24 }
      },
      error: /payload\.sessionId must be a string/
    }
  ])('answers an error and spawns nothing for $name', async ({ request, error }) => {
    ...
    const { socket } = await hello('control', 'client-malformed')
    const responsePromise = readJsonLine(socket)
    ...
    expect(String(response.error)).toMatch(error)
    expect(spawnSubprocess).not.toHaveBeenCalled()
  })

  it('drops a malformed notification without answering or spawning', async () => {
    const spawnSubprocess = vi.fn(() => {
      throw new Error('Test unexpectedly spawned a subprocess')
    })
    server = new DaemonServer({ socketPath, tokenPath, spawnSubprocess })
    ...
    const { socket } = await hello('control', 'client-notify')
    const responsePromise = readJsonLine(socket)
    socket.write(
      encodeNdjson({
        id: `${NOTIFY_PREFIX}1`,
    ...
    // Why: the control stream is ordered, so a well-formed request behind the notify is a
    // deterministic barrier — an error answered for the notify would arrive first.
    socket.write(encodeNdjson({ id: 'req-after-notify', type: 'listSessions' }))

    const response = await responsePromise
    expect(response.id).toBe('req-after-notify')
    expect(response.ok).toBe(true)
    expect(spawnSubprocess).not.toHaveBeenCalled()
  })
})
