...
describe('DaemonPtyAdapter (IPtyProvider)', () => {
    ...
    describe('killed-session tombstones', () => {
        it('prevents spawn after shutdown for same sessionId', async () => {
            ...
            await expect(adapter.spawn({ cols: 80, rows: 24, sessionId })).rejects.toThrow('was explicitly killed');
        });
        ...
        it('evicts oldest tombstone when exceeding limit', async () => {
            ...
            for (const id of ids) {
                await expect(adapter.spawn({ cols: 80, rows: 24, sessionId: id })).rejects.toThrow('was explicitly killed');
            }
            ...
            await expect(adapter.spawn({ cols: 80, rows: 24, sessionId: ids[0] })).rejects.toThrow('was explicitly killed');
        });
    });
    ...
    describe('history integration', () => {
        ...
        it('writes scrollback to disk on data events', async () => {
            ...
            await new Promise((r) => setTimeout(r, 50));
            const scrollback = readFileSync(join(historyDir, getHistorySessionDirName(id), 'scrollback.bin'), 'utf-8');
        ...
        it('writes meta.json with endedAt on exit', async () => {
            ...
            await new Promise((r) => setTimeout(r, 50));
            const meta = JSON.parse(readFileSync(join(historyDir, getHistorySessionDirName(id), 'meta.json'), 'utf-8'));
        ...
        it('records post-cold-restore data to disk for future restores', async () => {
            ...
            const seeded = readFileSync(join(historyDir, getHistorySessionDirName(sessionId), 'scrollback.bin'), 'utf-8');
            ...
            const scrollback = readFileSync(join(historyDir, getHistorySessionDirName(sessionId), 'scrollback.bin'), 'utf-8');
        ...
        it('stores history under an encoded directory key for Windows-safe session ids', async () => {
            ...
            expect(existsSync(join(historyDir, getHistorySessionDirName(sessionId), 'meta.json'))).toBe(true);
        });
    });
});
