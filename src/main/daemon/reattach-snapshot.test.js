...
function buildReattachPayload(snapshot) {
...
async function simulateReattachToFreshTerminal(reattachPayload, sigwinchRepaintData, cols, rows) {
...
describe('reattach snapshot flow', () => {
    ...
    describe('normal-screen TUI (Claude Code style)', () => {
        ...
        it('SIGWINCH repaint after snapshot produces clean state', async () => {
            ...
            const { content } = await simulateReattachToFreshTerminal(payload.fullPayload, repaintData, 80, 10);
