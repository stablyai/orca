...
describe('DaemonClient outbound admission', () => {
  ...
  it('drops a notification and closes before growing a saturated control buffer', () => {
    ...
    // Why: the caller keys write-failure recovery off this boolean, so a rejected notify must
    // report undelivered rather than look like a successful send.
    expect(client.notify('write', { sessionId: 'session-a', data: 'hello' })).toBe(false)

    expect(socket.write).not.toHaveBeenCalled()
    expect(socket.destroy).toHaveBeenCalledOnce()
  })

  it('reports a delivered notification that fits the control buffer', () => {
    const { client, socket } = createConnectedClient()
    socket.writableLength = 0

    expect(client.notify('write', { sessionId: 'session-a', data: 'hello' })).toBe(true)
