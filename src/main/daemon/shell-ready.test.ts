...
const SHELL_READY_MARKER_OUTPUT = '\x1b]777;mcode-shell-ready\x07'
...
async function runInteractiveZshLogin(args: {
  ...
  const proc = pty.spawn('zsh', ['-o', 'noglobalrcs', '-l'], {
    ...
    env: {
      ...
      MCODE_ORIG_ZDOTDIR: args.tempHome,
      MCODE_ZSHENV_SOURCE_DIR: args.tempHome,
...
describePosix('daemon shell-ready launch config', () => {
      ...
      try {
        ...
        const sentinelPoll = setInterval(() => {
          if (commandWritten && !erasureProbeWritten && existsSync(sentinel)) {
            ...
            proc.write(
              `functions -q __mcode_shell_ready_marker; and touch ${stillRegistered}; or touch ${erased}\n`
            )
            return
          }
  ...
  it('sets no MCODE_ORIG_ZDOTDIR when the inherited ZDOTDIR points at a wrapper dir', async () => {
    // Why: an MCode-PTY parent has ZDOTDIR=.../shell-ready/zsh; propagating it makes the wrapper source itself (recursion loop).
  ...
  it('sets no MCODE_ORIG_ZDOTDIR when the inherited one points at a wrapper dir', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    const previousOrigZdotdir = process.env.MCODE_ORIG_ZDOTDIR
    const previousHome = process.env.HOME
    delete process.env.ZDOTDIR
  ...
  itWithZsh(
    ...
    async () => {
      const { getShellReadyLaunchConfig } = await importFreshShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      const tempHome = mkdtempSync(join(tmpdir(), 'mcode-zsh-vi-mode-'))
      writeFileSync(
        join(tempHome, '.zshrc'),
        [
  ...
  itWithZsh(
    ...
    async () => {
      const { getShellReadyLaunchConfig } = await importFreshShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      const tempHome = mkdtempSync(join(tmpdir(), 'mcode-zsh-azhw-'))
      const userHookOutput = 'MCODE-TEST-USER-HOOK'
      writeFileSync(
        join(tempHome, '.zshrc'),
        [
          `__mcode_test_line_init_hook() { printf "${userHookOutput}" }`,
          'autoload -Uz add-zle-hook-widget',
          'zle -N __mcode_test_line_init_hook',
          'add-zle-hook-widget line-init __mcode_test_line_init_hook',
          ''
        ].join('\n')
      )
      try {
  ...
  // Why: a re-source (nested MCode, manual) must stay idempotent — keep chaining the user's original zle-line-init.
  itWithZsh(
    'keeps chaining the prior zle-line-init widget when the marker block is sourced twice',
    async () => {
      const zdotdir = mkdtempSync(join(tmpdir(), 'mcode-zsh-resource-'))
      const userHookOutput = 'MCODE-TEST-PRIOR-WIDGET'
      const block = getZshShellReadyMarkerRegistrationBlock('\\033]777;mcode-shell-ready\\007')
      writeFileSync(
        join(zdotdir, '.zshrc'),
        [
          // A user widget that mimics oh-my-zsh vi-mode owning zle-line-init.
          `__mcode_test_prior_widget() { printf "${userHookOutput}" }`,
          'zle -N zle-line-init __mcode_test_prior_widget',
          block,
