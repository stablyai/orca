...
export function getDaemonBashShellReadyRcfileContent(): string {
  return `# MCode daemon bash shell-ready wrapper
...
__mcode_osc133_precmd() {
  ...
  # displaced by one of MCode's own hooks.
  [[ -n "$__mcode_ready_marker" ]] && printf "${SHELL_READY_MARKER}"
  return "$exit_code"
}
    ...
    if [[ -n "\${__mcode_in_prompt_dispatch:-}" ]]; then
      ...
      if [[ "\${FUNCNAME[1]:-}" == "__mcode_run_prompt_command_array" ]]; then
        case "$BASH_COMMAND" in
          '(( __mcode_exit_code == 0 ))'|'__mcode_restore_prompt_status "$__mcode_exit_code"'|'eval "$__mcode_prompt_part"'|'eval "$__mcode_final_prompt_command"'|__mcode_dispatching_user_prompt_command=*|__mcode_osc133_precmd|__mcode_osc133_epilogue) return 0 ;;
        esac
      fi
...
__mcode_append_prompt_command "__mcode_osc133_epilogue"
