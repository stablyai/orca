  ...
  describe('RPC routing', () => {
    ...
    it('refuses an auto-intent kill while a live non-shell foreground process runs', async () => {
      ...
      daemon.clients.set('client-1', {
        ...
        streamSocket: { destroy: vi.fn() } as unknown as Socket,
        authenticatedPairEstablished: true
      })
      await daemon.routeRequest('client-1', {
        id: 'req-1',
        type: 'createOrAttach',
        payload: { sessionId: 'agent-session', cols: 80, rows: 24 }
      })
      subprocess!.getForegroundProcess = vi.fn(() => 'claude')
      ...
      // Why: a bare shell foreground is not an agent at work — the automated
      // kill may proceed.
      subprocess!.getForegroundProcess = vi.fn(() => 'zsh')
      await daemon.routeRequest('client-1', {
        id: 'req-3',
        type: 'kill',
        payload: { sessionId: 'agent-session', intent: 'auto' }
      })
      expect(subprocess!.kill).toHaveBeenCalled()
    })
    ...
    it('keeps a legacy intent-less kill unconditional', async () => {
      ...
      // Why: old clients cannot say why they kill; refusing them would break
      // legitimate explicit closes, so missing intent stays destructive.
      subprocess!.getForegroundProcess = vi.fn(() => 'claude')
