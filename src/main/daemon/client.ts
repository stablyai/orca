...
export class DaemonClient {
  ...
  notify(type: string, payload: unknown): boolean {
    ...
    if (!this.canBufferControlMessage(this.controlSocket, encoded)) {
      // Why: fail loudly instead of dropping the keystroke. A silently discarded notify would
      // desync the terminal invisibly; destroying surfaces a labeled error and routes through the
      // normal disconnect path, which reconnects and restores the session from history. Reporting
      // undelivered lets the caller start recovery now instead of waiting for the close event.
      this.controlSocket.destroy(
