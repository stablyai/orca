...
function parseTokenResponse(stdout: string): CachedToken | null {
  ...
  const expiresAtMs =
    typeof parsed.expires_on === 'number' ? parsed.expires_on * 1000 : Number.MAX_SAFE_INTEGER
  return { token: parsed.accessToken, expiresAtMs }
}

export async function getAzCliAzureDevOpsAccessToken(): Promise<string | null> {
