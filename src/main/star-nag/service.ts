...
export class StarNagService {
  ...
  registerIpcHandlers(): void {
    ...
    ipcMain.handle('star-nag:starMCode', () => this.starMCodeFromNag())
  ...
  private async maybeShow(
    ...
    try {
      // Why: checkMCodeStarred lets us skip users who already starred outside
  ...
  private async starMCodeFromNag(): Promise<boolean> {
    const session = this.promptSession
    if (!session) {
      return false
    }
    ...
    const attempt = this.runStarMCodeAttempt(session)
  ...
  private async runStarMCodeAttempt(session: StarNagPromptSession): Promise<boolean> {
