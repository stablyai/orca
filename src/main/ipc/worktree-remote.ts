...
} from './worktrees/create/created-worktree-lineage'
export {
  assertAttachableParentWorkspace,
  recordWorkspaceLineageForCreatedWorktree
} from './worktrees/create/created-worktree-lineage'
...
export async function createRemoteWorktree(
  ...
  const capturedLineageParent = assertAttachableParentWorkspace(
    store,
  ...
  const { worktree } = timing.timeSync('persist_metadata', () => {
    const meta = writeWorktreeMetaForHost(
      store,
      worktreeId,
      getRepoExecutionHostId(repo),
      metaUpdates
