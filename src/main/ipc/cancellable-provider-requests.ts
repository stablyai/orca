...
const MAX_CANCEL_TOMBSTONES = 1024
 ...
 */
export class CancellableProviderRequests {
  ...
  cancel(requestId: unknown): void {
    ...
    if (this.cancelTombstones.size > MAX_CANCEL_TOMBSTONES) {
      const oldest = this.cancelTombstones.keys().next().value
      if (oldest !== undefined) {
        this.cancelTombstones.delete(oldest)
      }
    }
  }
