...
const testingTargets = new Set();
const explicitRelaySetupTargets = new Set();
export function registerSshHandlers(store, getMainWindow) {
    ...
    const callbacks = {
        onCredentialRequest: (targetId, kind, detail) => requestCredential(getMainWindow, targetId, kind, detail),
        onStateChange: (targetId, state) => {
            ...
            if (state.status === 'connected' &&
                ...
                void reestablishRelayStack(targetId, getMainWindow, connectionManager, activeMultiplexers, portForwardManager);
            }
        }
    };
    ...
    ipcMain.handle('ssh:addTarget', (_event, args) => {
        return sshStore.addTarget(args.target);
    });
    ipcMain.handle('ssh:updateTarget', (_event, args) => {
        return sshStore.updateTarget(args.id, args.updates);
    });
    ipcMain.handle('ssh:removeTarget', (_event, args) => {
    ...
    ipcMain.handle('ssh:connect', async (_event, args) => {
        ...
        try {
            ...
            const status = isAuthError(errObj) ? 'auth-failed' : 'error';
            const win = getMainWindow();
            if (win && !win.isDestroyed()) {
    ...
    ipcMain.handle('ssh:disconnect', async (_event, args) => {
        cleanupConnection(args.targetId, activeMultiplexers, initializedConnections, portForwardManager);
        await connectionManager.disconnect(args.targetId);
    });
    ipcMain.handle('ssh:getState', (_event, args) => {
        return connectionManager.getState(args.targetId);
    });
    ipcMain.handle('ssh:testConnection', async (_event, args) => {
        const target = sshStore.getTarget(args.targetId);
        if (!target) {
            throw new Error(`SSH target "${args.targetId}" not found`);
        }
    ...
    // ── Port forwarding ─────────────────────────────────────────────────
    ipcMain.handle('ssh:addPortForward', async (_event, args) => {
        const conn = connectionManager.getConnection(args.targetId);
        if (!conn) {
            throw new Error(`SSH connection "${args.targetId}" not found`);
        }
        return portForwardManager.addForward(args.targetId, conn, args.localPort, args.remoteHost, args.remotePort, args.label);
    });
    ipcMain.handle('ssh:removePortForward', (_event, args) => {
        return portForwardManager.removeForward(args.id);
    });
    ipcMain.handle('ssh:listPortForwards', (_event, args) => {
        return portForwardManager.listForwards(args?.targetId);
    });
    return { connectionManager, sshStore };
}
export function getSshConnectionManager() {
    return connectionManager;
}
export function getSshConnectionStore() {
    return sshStore;
}
export function getActiveMultiplexer(connectionId) {
