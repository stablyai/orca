...
const { handleMock, onMock, removeHandlerMock, removeAllListenersMock, existsSyncMock, statSyncMock, accessSyncMock, mkdirSyncMock, writeFileSyncMock, chmodSyncMock, getPathMock, spawnMock, openCodeBuildPtyEnvMock, openCodeClearPtyMock, piBuildPtyEnvMock, piClearPtyMock } = vi.hoisted(() => ({
    handleMock: vi.fn(),
    onMock: vi.fn(),
...
describe('registerPtyHandlers', () => {
    ...
    beforeEach(() => {
        ...
        piBuildPtyEnvMock.mockImplementation((_ptyId, existingAgentDir) => ({
    ...
    async function spawnAndGetEnv(argsEnv, processEnvOverrides, getSelectedCodexHomePath) {
        const savedEnv = {};
        ...
        try {
            ...
            handlers.clear();
            registerPtyHandlers(mainWindow, undefined, getSelectedCodexHomePath);
    ...
    describe('spawn environment', () => {
        ...
        it('leaves ambient CODEX_HOME untouched when system default is selected', async () => {
            const env = await spawnAndGetEnv(undefined, { CODEX_HOME: '/tmp/system-codex-home' }, () => null);
    ...
    it('lists sessions from both local and SSH providers', async () => {
        ...
        const sessions = (await handlers.get('pty:listSessions')(null, undefined));
    ...
    describe('Windows UTF-8 code page', () => {
        ...
        it('passes chcp 65001 to cmd.exe for UTF-8 console output', () => {
            ...
            expect(spawnMock).toHaveBeenCalledWith('C:\\Windows\\system32\\cmd.exe', ['/K', 'chcp 65001 > nul'], expect.any(Object));
        });
        it('sets Console encoding for powershell.exe', () => {
            process.env.COMSPEC = 'C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe';
            registerPtyHandlers(mainWindow);
            handlers.get('pty:spawn')(null, { cols: 80, rows: 24 });
            expect(spawnMock).toHaveBeenCalledWith('C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe', [
                '-NoExit',
                '-Command',
        ...
        it('sets Console encoding for pwsh.exe', () => {
            ...
            registerPtyHandlers(mainWindow);
            handlers.get('pty:spawn')(null, { cols: 80, rows: 24 });
            expect(spawnMock).toHaveBeenCalledWith('C:\\Program Files\\PowerShell\\7\\pwsh.exe', [
                '-NoExit',
                '-Command',
                'try { . $PROFILE } catch {}; [Console]::OutputEncoding = [System.Text.Encoding]::UTF8; [Console]::InputEncoding = [System.Text.Encoding]::UTF8'
            ], expect.any(Object));
        ...
        it('passes no encoding args for unrecognized shells', () => {
            ...
            registerPtyHandlers(mainWindow);
            handlers.get('pty:spawn')(null, { cols: 80, rows: 24 });
            expect(spawnMock).toHaveBeenCalledWith('C:\\Program Files\\Git\\bin\\bash.exe', [], expect.any(Object));
        });
    });
    it('rejects missing WSL worktree cwd instead of validating only the fallback Windows cwd', async () => {
        ...
        try {
            ...
            })).rejects.toThrow('Working directory "\\\\wsl.localhost\\Ubuntu\\home\\jin\\missing" does not exist.');
    ...
    it('falls back to a system shell when SHELL points to a missing binary', async () => {
        ...
        existsSyncMock.mockImplementation((targetPath) => targetPath !== '/opt/homebrew/bin/bash');
        try {
            ...
            expect(spawnMock).toHaveBeenCalledWith('/bin/zsh', ['-l'], expect.objectContaining({ cwd: '/tmp' }));
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Primary shell "/opt/homebrew/bin/bash" failed'));
        }
        finally {
            warnSpy.mockRestore();
            if (originalShell === undefined) {
    ...
    it('falls back when SHELL points to a non-executable binary', async () => {
        const originalShell = process.env.SHELL;
        const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => { });
        accessSyncMock.mockImplementation((targetPath) => {
        ...
        try {
            ...
            expect(spawnMock).toHaveBeenCalledTimes(1);
            expect(spawnMock).toHaveBeenCalledWith('/bin/zsh', ['-l'], expect.objectContaining({ cwd: '/tmp' }));
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Shell "/opt/homebrew/bin/bash" is not executable'));
        }
        finally {
            warnSpy.mockRestore();
            if (originalShell === undefined) {
    ...
    it('prefers args.env.SHELL and normalizes the child env after fallback', async () => {
        ...
        try {
            ...
            expect(spawnMock).toHaveBeenCalledTimes(1);
            expect(spawnMock).toHaveBeenCalledWith('/bin/zsh', ['-l'], expect.objectContaining({
                cwd: '/tmp',
    ...
    it('disposes PTY listeners before manual kill IPC', async () => {
        ...
        await handlers.get('pty:kill')(null, { id: spawnResult.id });
        expect(onDataDisposable.dispose.mock.invocationCallOrder[0]).toBeLessThan(proc.kill.mock.invocationCallOrder[0]);
        expect(onExitDisposable.dispose.mock.invocationCallOrder[0]).toBeLessThan(proc.kill.mock.invocationCallOrder[0]);
    });
    it('disposes PTY listeners before runtime controller kill', async () => {
        ...
        const runtimeController = runtime.setPtyController.mock.calls[0]?.[0];
