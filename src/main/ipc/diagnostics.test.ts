...
describe('diagnostics IPC handlers', () => {
  beforeEach(() => {
    ...
    delete (globalThis as { MCODE_BUILD_IDENTITY?: unknown }).MCODE_BUILD_IDENTITY
    delete (globalThis as { MCODE_DIAGNOSTICS_TOKEN_URL?: unknown }).MCODE_DIAGNOSTICS_TOKEN_URL
    process.env.MCODE_DIAGNOSTICS_TOKEN_URL = 'https://diagnostics.example.com/diagnostics/token'
  ...
  it('pins official builds to the compile-time diagnostics endpoint', async () => {
    ...
    const globalOverrides = globalThis as {
      MCODE_BUILD_IDENTITY?: 'stable'
      MCODE_DIAGNOSTICS_TOKEN_URL?: string
    }
    globalOverrides.MCODE_BUILD_IDENTITY = 'stable'
    globalOverrides.MCODE_DIAGNOSTICS_TOKEN_URL = 'https://official.example.com/diagnostics/token'
    process.env.MCODE_DIAGNOSTICS_TOKEN_URL = 'https://attacker.example.com/diagnostics/token'
    collectDiagnosticBundleMock.mockReturnValue(bundle)
    readFileSyncMock.mockReturnValue(bundle.payload)
    const collect = handlers.get('diagnostics:collectBundle')!
    const openPreview = handlers.get('diagnostics:openBundlePreview')!
