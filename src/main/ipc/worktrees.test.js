...
const { handleMock, removeHandlerMock, listWorktreesMock, addWorktreeMock, removeWorktreeMock, getGitUsernameMock, getDefaultBaseRefMock, getBranchConflictKindMock, getPRForBranchMock, getEffectiveHooksMock, createIssueCommandRunnerScriptMock, createSetupRunnerScriptMock, shouldRunSetupForCreateMock, runHookMock, hasHooksFileMock, loadHooksMock, computeWorktreePathMock, ensurePathWithinWorkspaceMock, gitExecFileAsyncMock } = vi.hoisted(() => ({
    handleMock: vi.fn(),
...
describe('registerWorktreeHandlers', () => {
    ...
    it('auto-suffixes the branch name when the first choice collides with a remote branch', async () => {
        ...
        getBranchConflictKindMock.mockImplementation(async (_repoPath, branch) => branch === 'improve-dashboard' ? 'remote' : null);
        listWorktreesMock.mockResolvedValue([
            {
        ...
        expect(addWorktreeMock).toHaveBeenCalledWith('/workspace/repo', '/workspace/improve-dashboard-2', 'improve-dashboard-2', 'origin/main', false);
    ...
    it('creates an issue-command runner for an existing repo/worktree pair', async () => {
        ...
        expect(createIssueCommandRunnerScriptMock).toHaveBeenCalledWith(expect.objectContaining({ id: 'repo-1' }), '/workspace/improve-dashboard', 'codex exec "long command"');
        expect(result).toEqual({
            runnerScriptPath: '/workspace/repo/.git/orca/issue-command-runner.sh',
            envVars: {
                ORCA_ROOT_PATH: '/workspace/repo',
    ...
    it('auto-suffixes the branch name when the first choice already belongs to a PR', async () => {
        ...
        getPRForBranchMock.mockImplementation(async (_repoPath, branch) => branch === 'improve-dashboard'
            ? {
                number: 3127,
                title: 'Existing PR',
                state: 'merged',
    ...
    it('returns a setup launch payload when setup should run', async () => {
        ...
        expect(createSetupRunnerScriptMock).toHaveBeenCalledWith(expect.objectContaining({ id: 'repo-1' }), '/workspace/improve-dashboard', 'pnpm worktree:setup');
        expect(result).toEqual({
            worktree: expect.objectContaining({
                repoId: 'repo-1',
                path: '/workspace/improve-dashboard',
        ...
        expect(addWorktreeMock).toHaveBeenCalledWith('/workspace/repo', '/workspace/improve-dashboard', 'improve-dashboard', 'origin/main', false);
    });
    ...
    it('rejects ask-policy creates before mutating git state when setup decision is missing', async () => {
        ...
        await expect(handlers['worktrees:create'](null, {
            repoId: 'repo-1',
            name: 'improve-dashboard'
