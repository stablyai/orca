...
/** These cases are about the local merge proof, so every read routes locally. */
...
describe('workspace cleanup git evidence merge handling', () => {
  ...
  it('stops calling a squash-merged workspace unpushed', async () => {
    const evidence = await readWorkspaceCleanupGitEvidence(WORKTREE, REPO, LOCAL_ROUTE)
  ...
  it('skips the unpushed count entirely once the merge is proven', async () => {
    await readWorkspaceCleanupGitEvidence(WORKTREE, REPO, LOCAL_ROUTE)
  ...
  it('forwards the project WSL distro to the probe', async () => {
    await readWorkspaceCleanupGitEvidence(WORKTREE, REPO, LOCAL_ROUTE, undefined, {
      wslDistro: 'Ubuntu'
    })
