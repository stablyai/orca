...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('strips MCODE_PANE_KEY/TAB_ID/WORKTREE_ID from SSH spawn env when remote agent hooks are disabled', async () => {
        ...
        const prevFlag = process.env.MCODE_FEATURE_REMOTE_AGENT_HOOKS
        process.env.MCODE_FEATURE_REMOTE_AGENT_HOOKS = '0'
        try {
          ...
          if (prevFlag === undefined) {
            ...
            process.env.MCODE_FEATURE_REMOTE_AGENT_HOOKS = prevFlag
          }
        }
      })
      it('forwards MCODE_PANE_KEY/TAB_ID/WORKTREE_ID over SSH by default', async () => {
        const sshSpawn = vi.fn(async (_opts: { env: Record<string, string> }) => ({
          id: 'ssh-pty'
        }))
        registerSshPtyProvider('ssh-1', {
        ...
        try {
          ...
          const env = sshSpawn.mock.calls.at(-1)![0].env
          expect(env.MCODE_PANE_KEY).toBe(paneKey)
          expect(env.MCODE_TAB_ID).toBe('tab-2')
          expect(env.MCODE_WORKTREE_ID).toBe('wt-2')
