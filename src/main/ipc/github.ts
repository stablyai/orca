...
export function registerGitHubHandlers(store: Store, stats: StatsCollector): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      // This captures PRs opened from any workflow (MCode UI, gh CLI, github.com).
  ...
  // Star operations target the MCode repo itself — no repoPath validation needed
  ipcMain.handle('gh:viewer', () => getAuthenticatedViewer())
  ipcMain.handle('gh:checkMCodeStarred', () => checkMCodeStarred())
  ipcMain.handle('gh:starMCode', async (_event, source: unknown) => {
