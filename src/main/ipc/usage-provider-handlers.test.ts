...
describe('usage provider IPC handlers', () => {
  it('registers every provider route and forwards query arguments', () => {
    ...
    call('claudeUsage', 'getSnapshot', { scope: 'mcode', range: '30d', limit: 7 })
    call('claudeUsage', 'getSummary', { scope: 'all', range: '7d' })
    call('claudeUsage', 'getDaily', { scope: 'mcode', range: '90d' })
    call('claudeUsage', 'getBreakdown', { scope: 'all', range: 'all', kind: 'model' })
    call('claudeUsage', 'getRecentSessions', { scope: 'mcode', range: '30d', limit: 4 })
    ...
    expect(claudeUsage.getSnapshot).toHaveBeenCalledWith('mcode', '30d', 7)
    expect(claudeUsage.getSummary).toHaveBeenCalledWith('all', '7d')
    expect(claudeUsage.getDaily).toHaveBeenCalledWith('mcode', '90d')
    expect(claudeUsage.getBreakdown).toHaveBeenCalledWith('all', 'all', 'model')
    expect(claudeUsage.getRecentSessions).toHaveBeenCalledWith('mcode', '30d', 4)
  })
})
