/* eslint-disable max-lines -- Why: covers every helper exported from
   worktree-logic.ts plus the linkedGitLab* fields added in the merge.
   Splitting into per-helper test files would scatter the same import
   block across many files for no real benefit while the surface
   stays this narrow. */
