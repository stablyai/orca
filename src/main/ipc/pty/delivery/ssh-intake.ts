  ...
  session.sshOutputIntake = new SshPtyOutputIntake(
    {
      ...
      checkpointOwnershipTransferModel: ({ event, projection, modelSequenceEnd }) => {
        const ownershipTransfer = event.source?.ownershipTransfer
        if (!session.runtime || !ownershipTransfer) {
          return Promise.reject(
            new Error('pty_ownership_transfer_model_checkpoint_runtime_unavailable')
          )
        }
        return session.runtime.checkpointPtyOwnershipTransferModel({
          ptyId: event.id,
          ptyIncarnation: event.ptyIncarnation,
          modelSequenceEnd,
          projectionSequenceEnd: projection.identity.sequenceEnd,
          ownershipTransfer,
          data: event.data
        })
      },
    ...
    {
      // Tagged ownership-transfer frames require a durable destination sink. Keep the
      // legacy source-credit path usable when this optional runtime seam is absent.
      ownershipTransferOutputEnabled:
        session.runtime?.getPtyOwnershipTransferDestinationRegistry?.() != null
    }
  )
