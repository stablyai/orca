...
describe('detectWslCommandsOnPath', () => {
  ...
  it('parses detected commands from prefixed stdout', async () => {
    execFileAsyncMock.mockResolvedValue({
      stdout:
        '__MCODE_AGENT_PATH__claude\t/usr/bin/claude\n' +
        '__MCODE_AGENT_PATH__codex\t/home/user/.local/bin/codex\n',
      stderr: ''
    })
  ...
  it('ignores commands whose resolved path is not absolute', async () => {
    execFileAsyncMock.mockResolvedValue({
      stdout: '__MCODE_AGENT_PATH__claude\tclaude\n' + '__MCODE_AGENT_PATH__codex\tC:\\spoof\n',
      stderr: ''
    })
