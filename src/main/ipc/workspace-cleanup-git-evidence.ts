...
export async function readWorkspaceCleanupGitEvidence(
  ...
  const merged = clean ? await readMergeVerdict(worktree, repo, route, localGitOptions) : null
...
async function readMergeVerdict(
  worktree: Worktree,
  repo: Repo,
  route: WorkspaceCleanupWorktreeGitRoute,
  localGitOptions: LocalProjectWorktreeGitOptions
): Promise<boolean | null> {
  // Why: the proof runs Git on this machine, so it may only judge a workspace
  // this machine owns. The resolved route already separated local from ssh and
  // refused `runtime:`, so reading it here beats re-deriving ownership.
  if (route.kind !== 'local') {
    return null
  }
  try {
