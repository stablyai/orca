...
const { removeHandlerMock, handleMock, notificationShowMock, notificationCloseMock, notificationOnMock, notificationCtorMock, notificationIsSupportedMock, getAllWindowsMock } = vi.hoisted(() => {
...
import { registerNotificationHandlers, triggerStartupNotificationRegistration } from './notifications';
describe('registerNotificationHandlers', () => {
    ...
    function getDispatchHandler() {
        const call = handleMock.mock.calls.find((c) => c[0] === 'notifications:dispatch');
        if (!call) {
    ...
    it('delivers a notification when the event is allowed', () => {
        ...
        const handler = getDispatchHandler();
        expect(handler({}, { source: 'agent-task-complete', repoLabel: 'orca', worktreeLabel: 'feat/notis' })).toEqual({ delivered: true });
