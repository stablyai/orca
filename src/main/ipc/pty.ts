...
export function handoffPtyRendererOwnership(
...
const pendingByPaneKey = new Map<string, { gen: number; ownerWebContents: WebContents }>()
const pendingPaneSerializerCleanupRegistered = new Set<WebContents>()
...
function cleanupPendingPaneSerializersForSender(ownerWebContents: WebContents): void {
  pendingPaneSerializerCleanupRegistered.delete(ownerWebContents)
  for (const [paneKey, pending] of pendingByPaneKey) {
    if (pending.ownerWebContents === ownerWebContents) {
...
function registerPendingPaneSerializerCleanup(sender: WebContents): void {
  if (pendingPaneSerializerCleanupRegistered.has(sender)) {
    return
  }
  pendingPaneSerializerCleanupRegistered.add(sender)
  sender.once('destroyed', () => cleanupPendingPaneSerializersForSender(sender))
}

function declarePendingPaneSerializer(paneKey: string, sender: WebContents): number {
  ...
  pendingByPaneKey.set(paneKey, { gen, ownerWebContents: sender })
...
function settlePendingPaneSerializer(paneKey: string, gen: number, sender: WebContents): boolean {
  const pending = pendingByPaneKey.get(paneKey)
...
let resetRendererDeliveryAccountingForLifecycleReset = (
...
export function registerPtyRenderer(webContents: WebContents): () => void {
  ...
  webContents.on('destroyed', remove)
  return () => {
...
type PtyHandlerOptions = {
  ...
  reuseRegisteredState?: boolean
}

type PtyHandlerContext = {
...
let updateRegisteredPtyHandlerContext: ((context: PtyHandlerContext) => void) | null = null
...
export function registerPtyHandlers(
  ...
  if (options?.reuseRegisteredState && updateRegisteredPtyHandlerContext) {
    updateRegisteredPtyHandlerContext({
      mainWindow,
      runtime,
  ...
  updateRegisteredPtyHandlerContext = null
  ...
  const getCurrentPtyEventSender = (
    ...
    return isCurrentRendererMainFrame(event) && ptyRendererOwners.isRegistered(event.sender)
      ? event.sender
      : null
  }

  const claimPtyForRenderer = (event: IpcMainInvokeEvent, id: string): void => {
  ...
  const requirePtyRendererTarget = (id: string): WebContents => {
    const target = getPtyRendererTarget(id)
    if (!target) {
      throw new Error('pty_renderer_unavailable')
    }
    return target
  }

  const isPtyRendererDispatcherReady = (id: string): boolean =>
    ptyRendererOwners.getOwner(id)
      ? ptyRendererOwners.isDispatcherReadyFor(id)
  ...
  const pendingData = new PtyPendingDataDrainQueue(
    (id) => {
      ...
      if (
        !isPtyRendererDispatcherReady(id) ||
  ...
  const rendererInFlightCharsByWebContentsId = new Map<number, number>()
  let pendingDroppedChars = 0
  let deliveryResyncRequestSerial = 0
  const deliveryResyncProbesByWebContentsId = new Map<
    number,
    { requestId: number; timer: ReturnType<typeof setTimeout> }
  >()
  const deliveryResyncUnansweredWarnedWebContentsIds = new Set<number>()
  const lastAckReceivedAtMsByWebContentsId = new Map<number, number>()
  ...
  function adjustRendererInFlightChars(
    ...
    const next = Math.max(0, (rendererInFlightCharsByWebContentsId.get(webContentsId) ?? 0) + delta)
    if (next === 0) {
      rendererInFlightCharsByWebContentsId.delete(webContentsId)
    } else {
      rendererInFlightCharsByWebContentsId.set(webContentsId, next)
    }
  }
  ...
  resetRendererDeliveryAccountingForLifecycleReset = (webContents, ids) => {
    const resetIds = ids ?? [
      ...new Set([...rendererDeliveryAccountingByPty.keys(), ...pendingData.keys()])
    ]
    ...
    lastLifecycleResetClearedChars = resetIds.reduce(
      (sum, id) => sum + getRendererInFlightCharsForPty(id),
      0
    )
    ...
    if (webContents) {
      clearDeliveryResyncProbe(webContents.id)
      deliveryResyncUnansweredWarnedWebContentsIds.delete(webContents.id)
      lastAckReceivedAtMsByWebContentsId.delete(webContents.id)
    }
  ...
  function canSendPtyDataToRenderer(id: string, options: { interactive?: boolean } = {}): boolean {
    ...
    const ownerInFlight =
      webContentsId == null ? 0 : (rendererInFlightCharsByWebContentsId.get(webContentsId) ?? 0)
    return getRendererInFlightCharsForPty(id) < ptyLimit && ownerInFlight < totalLimit
  }
  ...
  function applyCumulativeAck(id: string, processedChars: number): number {
    ...
    adjustRendererInFlightChars(id, -acknowledged)
    if (acknowledged > 0) {
  ...
  function clearDeliveryResyncProbe(webContentsId?: number): void {
    const ids =
      webContentsId == null ? [...deliveryResyncProbesByWebContentsId.keys()] : [webContentsId]
    for (const id of ids) {
      const probe = deliveryResyncProbesByWebContentsId.get(id)
      if (probe) {
        clearTimeout(probe.timer)
        deliveryResyncProbesByWebContentsId.delete(id)
      }
    }
  }
  ...
  function requestDeliveryResyncForGatedPty(id: string): void {
    const target = getPtyRendererTarget(id)
    if (!target || deliveryResyncProbesByWebContentsId.has(target.id)) {
      return
    }
    deliveryResyncRequestSerial += 1
    const requestId = deliveryResyncRequestSerial
    const timer = setTimeout(() => {
      if (deliveryResyncProbesByWebContentsId.get(target.id)?.requestId !== requestId) {
        return
      }
      clearDeliveryResyncProbe(target.id)
      ...
      if (deliveryResyncUnansweredWarnedWebContentsIds.has(target.id)) {
        return
      }
      deliveryResyncUnansweredWarnedWebContentsIds.add(target.id)
      const lastAckReceivedAtMs = lastAckReceivedAtMsByWebContentsId.get(target.id) ?? null
    ...
    timer.unref?.()
    deliveryResyncProbesByWebContentsId.set(target.id, { requestId, timer })
    target.send('pty:requestDeliveryResync', { requestId })
  }
  ...
  function writeOffLostRendererDelivery(
    ...
    ownsPty: (id: string) => boolean = () => true,
    rendererWebContentsId?: number
  ): PtyDeliveryWriteOff[] {
    ...
    if (writtenOff.length > 0) {
      clearDeliveryResyncProbe(rendererWebContentsId)
      if (rendererWebContentsId != null) {
        deliveryResyncUnansweredWarnedWebContentsIds.delete(rendererWebContentsId)
      }
      const lastAckReceivedAtMs =
        rendererWebContentsId == null
          ? null
          : (lastAckReceivedAtMsByWebContentsId.get(rendererWebContentsId) ?? null)
  ...
  function sendPtyDataToRenderer(
    ...
    adjustRendererInFlightChars(id, charCount)
    recordPtyRendererDeliveryPressure(id)
    try {
      requirePtyRendererTarget(id).send('pty:data', payload)
    } catch (error) {
      ...
      if (current) {
        ...
        adjustRendererInFlightChars(id, -(inFlightBeforeRollback - inFlightAfterRollback))
  ...
  function sendModelRestoreNeededMarker(
    ...
    getPtyRendererTarget(id)?.send('pty:modelRestoreNeeded', {
      id,
      reason,
      ...(typeof markerSeq === 'number' ? { markerSeq } : {})
    })
  ...
  function armDispatcherReadyWatchdog(): void {
    ...
    dispatcherReadyWatchdogTimer = setTimeout(() => {
      ...
      if (ptyRendererOwners.isRegistered(mainWindow.webContents)) {
        ptyRendererOwners.markDispatcherReady(mainWindow.webContents)
      }
  ...
  function finalizePtyExitForRenderer(payload: { id: string; code: number }): void {
    ...
    adjustRendererInFlightChars(payload.id, -releasedRendererCredit)
  ...
  function sendPtySpawnedToRenderer(id: string): void {
    getPtyRendererTarget(id)?.send('pty:spawned', { id })
  }
  ...
  function acceptPtyDataForRenderer(
    ...
    if (isInteractiveOutput && isPtyRendererDispatcherReady(payload.id)) {
      if (!canSendPtyDataToRenderer(payload.id, { interactive: true })) {
        ...
        requestDeliveryResyncForGatedPty(payload.id)
        return
      }
      deletePendingPtyData(payload.id)
      clearFlushTimerIfIdle()
  ...
  const bindProviderListeners = (): void => {
    ...
    localWriteUnavailableUnsub =
      localProvider.onWriteUnavailable?.((payload) => {
        getPtyRendererTarget(payload.id)?.send('pty:writeUnavailable', { id: payload.id })
      }) ?? null
  ...
  function requestSerializedBuffer(
    ...
    const target = getPtyRendererTarget(ptyId)
    if (!target) {
    ...
    return new Promise<SerializeResult>((resolve) => {
      ...
      target.send('pty:serializeBuffer:request', payload)
    })
  }
  ...
  const attachControlRenderer = (): void => {
    ...
    const resetRendererPtyDeliveryGateState = (): void => {
      ...
      const ownedIds = ptyRendererOwners.getOwnedIds(webContents)
    ...
    rendererGateResetWebContents = webContents
    webContents.on('did-finish-load', rendererGateResetLoadHandler)
    webContents.on('render-process-gone', rendererGateResetGoneHandler)
  }
  attachControlRenderer()

  if (options?.reuseRegisteredState) {
    updateRegisteredPtyHandlerContext = (context) => {
      clearDispatcherReadyWatchdog()
      mainWindow = context.mainWindow
      runtime = context.runtime
      getSelectedCodexHomePath = context.getSelectedCodexHomePath
      getSettings = context.getSettings
      prepareClaudeAuth = context.prepareClaudeAuth
      store = context.store
      options = context.options
      registerPtyRenderer(mainWindow.webContents)
      rendererPtyDispatcherReady = ptyRendererOwners.isDispatcherReady(mainWindow.webContents)
      attachControlRenderer()
    }
  }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (
        ...
        args.id.length === 0 ||
        !isPtyEventFromOwner(event, args.id)
      ) {
        return null
      }
  ...
  ipcMain.handle(
    ...
    (event): PtyRendererDeliveryDebugSnapshot => {
      if (!getCurrentPtyEventSender(event)) {
        return { ...EMPTY_PTY_RENDERER_DELIVERY_DEBUG_SNAPSHOT }
      }
      return getPtyRendererDeliveryDebugSnapshot()
    }
  )
  ipcMain.handle('pty:resetRendererDeliveryDebug', (event): void => {
    if (!getCurrentPtyEventSender(event)) {
      return
    }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const rendererClaimSender = getCurrentPtyEventSender(event)
      if (!rendererClaimSender) {
        throw new Error('untrusted_ui_renderer')
      }
      const codexHomeLaunchStartedAt = !args.connectionId ? new Date() : undefined
      const codexHomeLaunchStartedSequence = !args.connectionId ? ++ptyLifecycleSequence : undefined
      ...
      const existingPaneSpawn = earlyReservationKey
        ...
        : undefined
      if (existingPaneSpawn) {
        const existing = { ...(await existingPaneSpawn.promise), isReattach: true }
        claimPtyForRenderer(event, existing.id)
        return existing
      }
      ...
      let rejectedRegistrationCandidate: PtySpawnResult | null = null
      let pendingRegistrationPtyId: string | null = null
      let provisionalRendererClaimId: string | null = null
      // Why hoisted to the reply scope: main reconciles the provider sequence
      // deep inside the spawn path, but the pane needs that renderer-domain
      // boundary beside the daemon snapshot's kitty flags.
      let reconciledSnapshotSeq: number | null = null
      ...
      try {
        ...
        const preAllocatedHandle =
          shouldPreAllocateTerminalHandle && currentRuntime
            ? (preAdoptedStablePane?.owner.handle ??
              currentRuntime.createPreAllocatedTerminalHandle())
            : null
        if (shouldRefreshAgentTeamsEnv && preAllocatedHandle && currentRuntime) {
          // Why: Agent Teams ids/tokens are process-local, so the team env must be regenerated for the new leader PTY.
          const prepared = await currentRuntime.prepareClaudeAgentTeamsLeaderForHandle({
        ...
        const provisionalExpectedPtyId = earlyStablePaneOwner?.ptyId ?? effectiveSessionAppId
        if (provisionalExpectedPtyId) {
          if (!ptyRendererOwners.getOwner(provisionalExpectedPtyId)) {
            provisionalRendererClaimId = provisionalExpectedPtyId
          }
          claimPtyForRenderer(event, provisionalExpectedPtyId)
        }
        ...
        try {
          ...
          if (expectedPtyId) {
            if (provisionalRendererClaimId && provisionalRendererClaimId !== expectedPtyId) {
              if (ptyRendererOwners.owns(provisionalRendererClaimId, rendererClaimSender)) {
                ptyRendererOwners.release(provisionalRendererClaimId)
              }
              provisionalRendererClaimId = null
            }
            if (!ptyRendererOwners.getOwner(expectedPtyId)) {
              provisionalRendererClaimId = expectedPtyId
            }
            claimPtyForRenderer(event, expectedPtyId)
            runtime?.beginPtyRegistration?.(expectedPtyId)
            pendingRegistrationPtyId = expectedPtyId
          }
          if (isDaemonHostSpawn && expectedPtyId) {
        ...
        if (provisionalRendererClaimId && provisionalRendererClaimId !== result.id) {
          if (ptyRendererOwners.owns(provisionalRendererClaimId, rendererClaimSender)) {
            ptyRendererOwners.release(provisionalRendererClaimId)
          }
        }
        provisionalRendererClaimId = null
        claimPtyForRenderer(event, result.id)
        sendPtySpawnedToRenderer(result.id)
        if (!args.connectionId) {
          options?.onCodexHomePtySpawned?.({
            id: result.id,
  ...
  const reportUnavailablePtyWrite = (id: string, error: unknown): void => {
    ...
    getPtyRendererTarget(id)?.send('pty:writeUnavailable', { id })
  }
  ...
  const isPtyEventFromOwner = (event: IpcMainEvent | IpcMainInvokeEvent, id: string): boolean => {
    const sender = getCurrentPtyEventSender(event)
  ...
  ipcMain.on('pty:write', (event, args: unknown) => {
    if (!isPtyWritePayload(args) || !isPtyEventFromOwner(event, args.id)) {
      return
    }
  ...
  ipcMain.on('pty:claimViewport', (event, args: unknown) => {
    ...
    const claim = (
      prior
        ? prior.then(
            () => currentRuntime.claimRemoteDesktopHost(args.id, args.cols, args.rows),
            () => currentRuntime.claimRemoteDesktopHost(args.id, args.cols, args.rows)
          )
        : currentRuntime.claimRemoteDesktopHost(args.id, args.cols, args.rows)
    ).catch(() => false)
  ...
  ipcMain.on(
    ...
    (event, args: { id: string; charCount?: number; processedChars?: number }) => {
      const sender = getCurrentPtyEventSender(event)
      if (!sender || !ptyRendererOwners.owns(args?.id, sender)) {
        return
      }
      const rendererWebContentsId = sender.id
      lastAckReceivedAtMsByWebContentsId.set(rendererWebContentsId, Date.now())
  ...
  ipcMain.on(
    ...
    (event, args: { requestId: number; processedCharsByPty: Record<string, number> }) => {
      ...
      for (const [id, processedChars] of Object.entries(args.processedCharsByPty ?? {})) {
        if (!isPtyEventFromOwner(event, id)) {
          continue
        }
  ...
  ipcMain.on('pty:setActiveRendererPty', (event, args: { id: string; active: boolean }) => {
    ...
    if (
      ...
      typeof args.id !== 'string' ||
      !args.id ||
      !ptyRendererOwners.owns(args.id, sender)
    ) {
      return
    }
    ptyRendererOwners.setActive(sender, args.id, args.active === true)
  ...
  ipcMain.on('pty:setRendererPtyVisible', (event, args: { id: string; visible: boolean }) => {
    ...
    ptyRendererOwners.setVisible(sender, args.id, args.visible === true)
  ...
  ipcMain.on('pty:setHiddenRendererPty', (event, args: { id: string; hidden: boolean }) => {
    ...
    ptyRendererOwners.setHidden(sender, args.id, args.hidden === true)
  ...
  ipcMain.on('pty:setPtyDeliveryInterest', (event, args: { id: string; interested: boolean }) => {
    ...
    ptyRendererOwners.setInterested(sender, args.id, args.interested === true)
  ...
  ipcMain.handle('pty:listSessions', async (event): Promise<PtyListedSession[]> => {
    if (!getCurrentPtyEventSender(event)) {
      return []
    }
  ...
  ipcMain.handle(
    ...
    async (event, args: { ids?: unknown }) => {
      ...
      for (const value of ids) {
        ...
        seen.add(value)
        if (!isPtyEventFromOwner(event, value)) {
  ...
  ipcMain.handle('pty:hasChildProcesses', async (event, args: { id: string }): Promise<boolean> => {
    if (!isPtyEventFromOwner(event, args?.id) || !hasPtyProviderForInspection(args.id)) {
      return false
    }
  ...
  ipcMain.handle('pty:inspectProcess', async (event, args: { id: string }) => {
    if (!isPtyEventFromOwner(event, args?.id)) {
      throw new Error('pty_renderer_not_owner')
    }
  ...
  ipcMain.handle(
    ...
    async (event, args: { paneKey?: unknown; gen?: unknown }): Promise<void> => {
      ...
      const settledCurrentGeneration = settlePendingPaneSerializer(args.paneKey, args.gen, sender)
      // Why: the generation-to-PTY binding survives late teardown of a reused id; paneKey reverse maps may already be gone.
      if (settledCurrentGeneration) {
  ...
  ipcMain.handle(
    ...
    async (event, args: { ptyId?: unknown }): Promise<void> => {
      ...
      if (!isPtyEventFromOwner(event, args.ptyId)) {
        return
      }
