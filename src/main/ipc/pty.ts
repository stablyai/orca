...
import type {
  PtyPendingDataDrainRound,
  PtyPendingDataDrainSelection as DrainPtyPendingDataDrainSelection
...
const UnboundedPtyPendingDataDrainQueue = PtyPendingDataDrainQueue
type DrainPendingPtyData = PendingPtyData
...
export function registerPtyHandlers(
  ...
  type PendingPtyData = DrainPendingPtyData & {
    upstreamCreditSpans?: PtyDeliveryCreditSpan[]
  }
  type PendingPtyDataDrainSelection = Omit<DrainPtyPendingDataDrainSelection, 'pending'> & {
    readonly pending: PendingPtyData
  }
  ...
  class PtyPendingDataDrainQueue extends UnboundedPtyPendingDataDrainQueue {
    private readonly retained: PendingPtyDataMap<PendingPtyData>

    constructor(
      classify: ConstructorParameters<typeof UnboundedPtyPendingDataDrainQueue>[0],
      readPolicyToken: ConstructorParameters<typeof UnboundedPtyPendingDataDrainQueue>[1]
    ) {
      super(classify, readPolicyToken)
      this.retained = new PendingPtyDataMap(
        (pending) => ({
    ...
    entries(): IterableIterator<[string, PendingPtyData]> {
      return this.retained.entries()
    }

    [Symbol.iterator](): IterableIterator<[string, PendingPtyData]> {
      return this.entries()
    }

    set(id: string, pending: PendingPtyData): void {
      super.set(id, pending)
      if (!this.retained.admit(id, pending)) {
        super.delete(id)
      }
    }

    delete(id: string): PendingPtyData | undefined {
      const pending = super.delete(id) as PendingPtyData | undefined
    ...
    takeNext(round: PtyPendingDataDrainRound): PendingPtyDataDrainSelection | null {
      return super.takeNext(round) as PendingPtyDataDrainSelection | null
    }

    block(selection: PendingPtyDataDrainSelection): void {
      super.block(selection)
    }

    remove(selection: PendingPtyDataDrainSelection): void {
      this.retained.delete(selection.id)
      super.remove(selection)
    }

    replaceWithRemainder(
      selection: PendingPtyDataDrainSelection,
      pending: PendingPtyData
    ): void {
      if (!this.retained.admit(selection.id, pending)) {
        super.remove(selection)
        return
      }
      super.replaceWithRemainder(selection, pending)
    }
  }
  ...
  const rendererDeliveryRestoreNeededPtyRetention = new PtyRendererDeliveryIdSet()
  ...
  resetRendererDeliveryAccountingForLifecycleReset = () => {
    ...
    rendererDeliveryRestoreNeededPtyRetention.clear()
  ...
  function sendPtyDataToRenderer(id: string, payload: PtyDataPayload): boolean {
    const upstreamCredit = arguments[2] as PtyDeliveryCreditSpan[] | undefined
    ...
    try {
      ...
      settleRejectedPtyRendererDelivery(charCount, upstreamCredit, (chars) =>
        tryGetProviderForPty(id)?.acknowledgeDataEvent(id, chars)
      )
      if (rendererDeliveryRestoreNeededPtyRetention.remember(id)) {
    ...
    if (rendererDeliveryRestoreNeededPtys.has(id)) {
      try {
        ...
        rendererDeliveryRestoreNeededPtyRetention.delete(id)
      } catch (error) {
        console.error(
  ...
  function settleAllPendingUpstreamCredit(): void {
    for (const [id, pending] of pendingData.entries()) {
      settlePendingPtyDataDelivery(id, pending)
    }
  }
  ...
  function dropOversizedPendingPtyData(id: string, pending: PendingPtyData): PendingPtyData {
    ...
    if (isHiddenPtyDeliveryGateEnabled(getSettings?.())) {
      pendingOverflowMarkedPtys.remember(id)
    }
    pendingDroppedChars += pending.data.length
    settlePendingPtyDataDelivery(id, pending)
  ...
  function sendPtyExitToRenderer(payload: { id: string; code: number }): void {
    ...
    try {
      ...
      rendererDeliveryRestoreNeededPtyRetention.delete(payload.id)
  ...
  function routeProviderData(payload: PtyDataEvent, runtimeAlreadyIngested: boolean): void {
    ...
    updateProducerFlowControl(payload.id)
    if (!canSendPtyDataToRenderer(payload.id, { interactive: activeRendererPtys.has(payload.id) })) {
      requestDeliveryResyncForGatedPty()
    }
    if (!flushTimer) {
