...
describe('GitLab IPC handlers', () => {
  ...
  it('rejects source context for a different host', async () => {
    registerGitLabHandlers(
      storeWithRepos([repo({ id: 'repo-local', path: '/local/mcode' })]) as Store
    )
  ...
  it('resolves pasted URL lookups by repoId and source host context', async () => {
    ...
    expect(getWorkItemByProjectRefMock).toHaveBeenCalledWith(
      '/ssh/mcode',
      { host: 'gitlab.com', path: 'mcode-ide/mcode' },
      42,
      'issue',
      'builder'
    )
  ...
  it('routes local WSL project GitLab issue, MR, work-item, and todo IPC through project git options', async () => {
    ...
    getProjectSlugMock.mockResolvedValue({ host: 'gitlab.com', path: 'mcode-ide/mcode' })
    ...
    await ipcHandlers.get('gitlab:projectSlug')?.(null, { repoPath: '/local/mcode' })
    await ipcHandlers.get('gitlab:mrForBranch')?.(null, {
      repoPath: '/local/mcode',
      branch: 'feature/wsl'
    })
    await ipcHandlers.get('gitlab:mr')?.(null, { repoPath: '/local/mcode', iid: 8 })
    await ipcHandlers.get('gitlab:listMRs')?.(null, {
      repoPath: '/local/mcode',
      state: 'opened',
      page: 1,
    ...
    await ipcHandlers.get('gitlab:issue')?.(null, { repoPath: '/local/mcode', number: 7 })
    ...
    await ipcHandlers.get('gitlab:listLabels')?.(null, { repoPath: '/local/mcode' })
    await ipcHandlers.get('gitlab:listAssignableUsers')?.(null, { repoPath: '/local/mcode' })
    await ipcHandlers.get('gitlab:todos')?.(null, { repoPath: '/local/mcode' })

    const hostedReviewOptions = { localGitExecOptions: localGitOptions }
    expect(getProjectSlugMock).toHaveBeenCalledWith('/local/mcode', null, hostedReviewOptions)
    ...
    expect(getMergeRequestMock).toHaveBeenCalledWith('/local/mcode', 8, null, hostedReviewOptions)
    expect(listMergeRequestsMock).toHaveBeenCalledWith(
      '/local/mcode',
      'opened',
      1,
    ...
    expect(getIssueMock).toHaveBeenCalledWith('/local/mcode', 7, null, localGitOptions)
    ...
    expect(listLabelsMock).toHaveBeenCalledWith('/local/mcode', undefined, null, localGitOptions)
    ...
    expect(listTodosMock).toHaveBeenCalledWith('/local/mcode', null, localGitOptions)
  })
  ...
  it('routes local WSL project GitLab MR details, review, job, and pasted URL IPC through project git options', async () => {
    ...
    await ipcHandlers.get('gitlab:closeMR')?.(null, { repoPath: '/local/mcode', iid: 8 })
    await ipcHandlers.get('gitlab:reopenMR')?.(null, { repoPath: '/local/mcode', iid: 8 })
    ...
    await ipcHandlers.get('gitlab:jobTrace')?.(null, { repoPath: '/local/mcode', jobId: 99 })
    await ipcHandlers.get('gitlab:retryJob')?.(null, { repoPath: '/local/mcode', jobId: 99 })
