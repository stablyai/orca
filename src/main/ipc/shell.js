...
export function registerShellHandlers() {
    ...
    ipcMain.handle('shell:openUrl', (_event, rawUrl) => {
        let parsed;
        try {
    ...
    ipcMain.handle('shell:openFilePath', async (_event, filePath) => {
    ...
    ipcMain.handle('shell:openFileUri', async (_event, rawUri) => {
        let parsed;
        try {
    ...
    ipcMain.handle('shell:pathExists', async (_event, filePath) => {
        return pathExists(filePath);
    });
    ipcMain.handle('shell:pickDirectory', async (_event, args) => {
    ...
    ipcMain.handle('shell:pickAttachment', async () => {
        const result = await dialog.showOpenDialog({
            properties: ['openFile']
        });
        if (result.canceled || result.filePaths.length === 0) {
    ...
    ipcMain.handle('shell:copyFile', async (_event, args) => {
