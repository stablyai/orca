...
vi.mock('../git/repo', () => ({
    ...
    getRepoName: vi.fn().mockImplementation((path) => path.split('/').pop()),
...
vi.mock('../providers/ssh-git-dispatch', () => ({
    getSshGitProvider: vi.fn().mockImplementation((id) => {
        if (id === 'conn-1') {
...
describe('repos:addRemote', () => {
    ...
    beforeEach(() => {
        ...
        registerRepoHandlers(mockWindow, mockStore);
    });
    ...
    it('creates a remote repo with connectionId', async () => {
        ...
        expect(mockStore.addRepo).toHaveBeenCalledWith(expect.objectContaining({
