...
import { isGitRepo, getGitUsername, getRepoName, getBaseRefDefault, searchBaseRefs } from '../git/repo';
...
let activeCloneProc = null;
let activeClonePath = null;
export function registerRepoHandlers(mainWindow, store) {
    ...
    ipcMain.handle('repos:add', async (_event, args) => {
    ...
    ipcMain.handle('repos:addRemote', async (_event, args) => {
    ...
    ipcMain.handle('repos:remove', async (_event, args) => {
        store.removeRepo(args.repoId);
        await rebuildAuthorizedRootsCache(store);
        notifyReposChanged(mainWindow);
    });
    ipcMain.handle('repos:update', (_event, args) => {
    ...
    ipcMain.handle('repos:clone', async (_event, args) => {
    ...
    ipcMain.handle('repos:getGitUsername', async (_event, args) => {
    ...
    ipcMain.handle('repos:getBaseRefDefault', async (_event, args) => {
        const repo = store.getRepo(args.repoId);
        if (!repo || isFolderRepo(repo)) {
            return 'origin/main';
        }
        ...
        if (repo.connectionId) {
            ...
            try {
                const result = await provider.exec(['symbolic-ref', '--quiet', 'refs/remotes/origin/HEAD'], repo.path);
    ...
    ipcMain.handle('repos:searchBaseRefs', async (_event, args) => {
        const repo = store.getRepo(args.repoId);
        if (!repo || isFolderRepo(repo)) {
            return [];
        }
...
function notifyReposChanged(mainWindow) {
