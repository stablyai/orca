...
import { getStatus, detectConflictOperation, getDiff, stageFile, unstageFile, bulkStageFiles, bulkUnstageFiles, discardChanges, getBranchCompare, getBranchDiff } from '../git/status';
import { getRemoteFileUrl } from '../git/repo';
import { resolveAuthorizedPath, resolveRegisteredWorktreePath, validateGitRelativeFilePath, isENOENT, authorizeExternalPath, rebuildAuthorizedRootsCache } from './filesystem-auth';
...
export function registerFilesystemHandlers(store) {
    void rebuildAuthorizedRootsCache(store);
    const activeTextSearches = new Map();
    // ─── Filesystem ─────────────────────────────────────────
    ipcMain.handle('fs:readDir', async (_event, args) => {
    ...
    ipcMain.handle('fs:readFile', async (_event, args) => {
        if (args.connectionId) {
            const provider = getSshFilesystemProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No filesystem provider for connection "${args.connectionId}"`);
    ...
    ipcMain.handle('fs:writeFile', async (_event, args) => {
        if (args.connectionId) {
            const provider = getSshFilesystemProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No filesystem provider for connection "${args.connectionId}"`);
    ...
    ipcMain.handle('fs:deletePath', async (_event, args) => {
        if (args.connectionId) {
            const provider = getSshFilesystemProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No filesystem provider for connection "${args.connectionId}"`);
    ...
    ipcMain.handle('fs:authorizeExternalPath', (_event, args) => {
        authorizeExternalPath(args.targetPath);
    });
    ipcMain.handle('fs:stat', async (_event, args) => {
        if (args.connectionId) {
            const provider = getSshFilesystemProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No filesystem provider for connection "${args.connectionId}"`);
    ...
    // ─── Search ────────────────────────────────────────────
    ipcMain.handle('fs:search', async (event, args) => {
        if (args.connectionId) {
            const provider = getSshFilesystemProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No filesystem provider for connection "${args.connectionId}"`);
        ...
        const maxResults = Math.max(1, Math.min(args.maxResults ?? DEFAULT_SEARCH_MAX_RESULTS, DEFAULT_SEARCH_MAX_RESULTS));
    ...
    ipcMain.handle('fs:listFiles', async (_event, args) => {
        if (args.connectionId) {
            const provider = getSshFilesystemProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No filesystem provider for connection "${args.connectionId}"`);
    ...
    ipcMain.handle('git:conflictOperation', async (_event, args) => {
        if (args.connectionId) {
            const provider = getSshGitProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No git provider for connection "${args.connectionId}"`);
    ...
    ipcMain.handle('git:branchCompare', async (_event, args) => {
        if (args.connectionId) {
            const provider = getSshGitProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No git provider for connection "${args.connectionId}"`);
    ...
    ipcMain.handle('git:branchDiff', async (_event, args) => {
        if (args.connectionId) {
            const provider = getSshGitProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No git provider for connection "${args.connectionId}"`);
    ...
    ipcMain.handle('git:bulkStage', async (_event, args) => {
        if (args.connectionId) {
            const provider = getSshGitProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No git provider for connection "${args.connectionId}"`);
    ...
    ipcMain.handle('git:bulkUnstage', async (_event, args) => {
        if (args.connectionId) {
            const provider = getSshGitProvider(args.connectionId);
            if (!provider) {
                throw new Error(`No git provider for connection "${args.connectionId}"`);
    ...
    ipcMain.handle('git:remoteFileUrl', async (_event, args) => {
