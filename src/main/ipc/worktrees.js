...
import { createIssueCommandRunnerScript, getEffectiveHooks, loadHooks, readIssueCommand, runHook, hasHooksFile, hasUnrecognizedOrcaYamlKeys, writeIssueCommand } from '../hooks';
import { mergeWorktree, parseWorktreeId, formatWorktreeRemovalError, isOrphanedWorktreeError } from './worktree-logic';
import { createLocalWorktree, createRemoteWorktree, notifyWorktreesChanged } from './worktree-remote';
import { rebuildAuthorizedRootsCache, ensureAuthorizedRootsCache } from './filesystem-auth';
export function registerWorktreeHandlers(mainWindow, store) {
    ...
    ipcMain.handle('worktrees:list', async (_event, args) => {
        // Why: use ensureAuthorizedRootsCache (not rebuild) to avoid redundantly
        // listing git worktrees when the cache is already fresh — the handler
    ...
    ipcMain.handle('worktrees:create', async (_event, args) => {
        const repo = store.getRepo(args.repoId);
        if (!repo) {
    ...
    ipcMain.handle('worktrees:remove', async (_event, args) => {
    ...
    ipcMain.handle('worktrees:updateMeta', (_event, args) => {
    ...
    ipcMain.handle('worktrees:persistSortOrder', (_event, args) => {
    ...
    ipcMain.handle('hooks:check', (_event, args) => {
        const repo = store.getRepo(args.repoId);
    ...
    ipcMain.handle('hooks:createIssueCommandRunner', (_event, args) => {
        const repo = store.getRepo(args.repoId);
        if (!repo) {
            throw new Error(`Repo not found: ${args.repoId}`);
        }
    ...
    ipcMain.handle('hooks:readIssueCommand', (_event, args) => {
        const repo = store.getRepo(args.repoId);
        if (!repo || isFolderRepo(repo)) {
            return {
                localContent: null,
    ...
    ipcMain.handle('hooks:writeIssueCommand', (_event, args) => {
        const repo = store.getRepo(args.repoId);
        if (!repo || isFolderRepo(repo)) {
            return;
        }
