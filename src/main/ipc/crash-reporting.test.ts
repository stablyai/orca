...
const {
  ...
  resolveDiagnosticMCodeChannelMock,
  spanEndMock,
  startSpanMock,
  submitFeedbackMock
} = vi.hoisted(() => {
  ...
  return {
    ...
    resolveDiagnosticMCodeChannelMock: vi.fn(),
    spanEndMock,
    startSpanMock: vi.fn(() => ({
      traceId: 'trace-id',
      spanId: 'span-id',
...
vi.mock('../observability/diagnostic-upload-endpoint', () => ({
  resolveDiagnosticMCodeChannel: resolveDiagnosticMCodeChannelMock
}))
...
describe('registerCrashReportingHandlers', () => {
  beforeEach(() => {
    ...
    resolveDiagnosticMCodeChannelMock.mockReset()
    resolveDiagnosticMCodeChannelMock.mockReturnValue('stable')
  ...
  it('copies sanitized submission and diagnostic omission failures for a captured report', async () => {
    ...
    const result = await handlers.get('crashReports:copyLatestDiagnostics')?.(null, {
      ...
      notes: 'current notes',
      submissionFailure: {
        error: 'fallback failed at C:\\Users\\alice\\MCode',
        diagnosticContext: {
          status: 'not_uploaded',
  ...
  it('submits an uncaptured Help menu crash report with an attached diagnostic bundle', async () => {
    ...
    expect(collectDiagnosticBundleMock).toHaveBeenCalledWith(
      expect.objectContaining({ lookbackMinutes: 3 * 24 * 60, mcodeChannel: 'stable' })
    )
    expect(submitFeedbackMock).toHaveBeenCalledWith({
