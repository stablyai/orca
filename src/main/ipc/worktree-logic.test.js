...
import { sanitizeWorktreeName, ensurePathWithinWorkspace, computeBranchName, computeWorktreePath, shouldSetDisplayName, mergeWorktree, parseWorktreeId, formatWorktreeRemovalError, isOrphanedWorktreeError, areWorktreePathsEqual } from './worktree-logic';
...
describe('ensurePathWithinWorkspace', () => {
    ...
    it('throws when path traverses outside workspace', () => {
        expect(() => ensurePathWithinWorkspace('/workspace/../outside', '/workspace')).toThrow('Invalid worktree path');
    });
});
describe('computeBranchName', () => {
    it('prefixes with git username when branchPrefix is git-username and username is present', () => {
        expect(computeBranchName('feature', { branchPrefix: 'git-username' }, 'jdoe')).toBe('jdoe/feature');
    });
    ...
    it('prefixes with custom value when branchPrefix is custom', () => {
        expect(computeBranchName('feature', { branchPrefix: 'custom', branchPrefixCustom: 'team' }, null)).toBe('team/feature');
    });
    it('returns bare name when branchPrefix is custom but custom value is empty', () => {
        expect(computeBranchName('feature', { branchPrefix: 'custom', branchPrefixCustom: '' }, null)).toBe('feature');
    });
...
describe('computeWorktreePath', () => {
    it('nests under repo name when nestWorkspaces is true', () => {
        expect(computeWorktreePath('feature', '/repos/my-project', {
            nestWorkspaces: true,
    ...
    it('strips .git suffix from repo path when nesting', () => {
        expect(computeWorktreePath('feature', '/repos/my-project.git', {
            nestWorkspaces: true,
            workspaceDir: '/workspaces'
        })).toBe(join('/workspaces', 'my-project', 'feature'));
    });
...
describe('areWorktreePathsEqual', () => {
    it('treats Windows slash and casing differences as the same path', () => {
        expect(areWorktreePathsEqual('C:\\Workspaces\\Improve-Dashboard', 'c:/workspaces/improve-dashboard', 'win32')).toBe(true);
    });
...
describe('formatWorktreeRemovalError', () => {
    ...
    it('returns fallback for non-Error input', () => {
        expect(formatWorktreeRemovalError('oops', path, false)).toBe(`Failed to delete worktree at ${path}.`);
    });
    it('includes stderr when present on Error', () => {
        const error = Object.assign(new Error('generic'), { stderr: 'branch not clean' });
        expect(formatWorktreeRemovalError(error, path, false)).toBe(`Failed to delete worktree at ${path}. branch not clean`);
    });
    it('falls back to message when no stderr/stdout', () => {
        const error = new Error('something went wrong');
        expect(formatWorktreeRemovalError(error, path, false)).toBe(`Failed to delete worktree at ${path}. something went wrong`);
    });
    it('uses force text when force is true', () => {
        expect(formatWorktreeRemovalError('oops', path, true)).toBe(`Failed to force delete worktree at ${path}.`);
    });
    it('returns fallback when Error has empty message and no streams', () => {
        const error = new Error(' ');
        error.message = '';
        expect(formatWorktreeRemovalError(error, path, false)).toBe(`Failed to delete worktree at ${path}.`);
    });
});
