...
const unwatchableRoots = new Set();
...
function coalesceEvents(raw) {
    ...
    const deleteBeforeCreate = new Set();
...
async function tryStatIsDirectory(filePath) {
    try {
...
// ── Flush and emit ───────────────────────────────────────────────────
async function flushBatch(rootKey, root) {
    ...
    const events = await Promise.all(coalesced.map(async (evt) => {
...
function scheduleBatchFlush(rootKey, root) {
    const now = Date.now();
...
// ── Watcher creation ─────────────────────────────────────────────────
async function createWatcher(rootKey, rootPath) {
    ...
    try {
        ...
        root.subscription = await watcher.subscribe(rootPath, (err, events) => {
            if (err) {
...
async function subscribe(worktreePath, sender) {
            ...
            for (const [key, watchedRoot] of watchedRoots) {
                if (watchedRoot.listeners.has(sender.id)) {
                    ...
                    if (watchedRoot.listeners.size === 0) {
                        ...
                        void watchedRoot.subscription.unsubscribe().catch((err) => {
...
function unsubscribe(worktreePath, senderId) {
    const rootKey = normalizeRootPath(worktreePath);
    ...
    if (root.listeners.size === 0) {
        ...
        const teardownTimer = setTimeout(() => {
            ...
            void currentRoot.subscription.unsubscribe().catch((err) => {
...
const remoteWatchers = new Map();
// ── Public API ───────────────────────────────────────────────────────
export function registerFilesystemWatcherHandlers() {
    ipcMain.handle('fs:watchWorktree', async (event, args) => {
    ...
    ipcMain.handle('fs:unwatchWorktree', (_event, args) => {
        if (args.connectionId) {
            const key = `${args.connectionId}:${args.worktreePath}`;
            const unwatchFn = remoteWatchers.get(key);
            if (unwatchFn) {
