...
describe('registerWorktreeHandlers – Windows path handling', () => {
    ...
    it('accepts a newly created Windows worktree when git lists the same path with different separators', async () => {
        ...
        expect(addWorktreeMock).toHaveBeenCalledWith('C:\\repo', 'C:\\workspaces\\improve-dashboard', 'improve-dashboard', 'origin/main', false);
        expect(store.setWorktreeMeta).toHaveBeenCalledWith('repo-1::C:/workspaces/improve-dashboard', expect.objectContaining({
    ...
    it('preserves create-time metadata on the next list when Windows path formatting differs', async () => {
        ...
        store.getWorktreeMeta.mockImplementation((worktreeId) => worktreeId === 'repo-1::C:/workspaces/improve-dashboard'
            ? {
                lastActivityAt: 123,
                displayName: 'Improve Dashboard'
            }
