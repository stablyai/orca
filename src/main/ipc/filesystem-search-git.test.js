...
describe('filesystem-search-git', () => {
    ...
    it('parses git grep output and finds matches', async () => {
        ...
        setTimeout(() => {
            ;
            proc.stdout.emit('data', 'src/index.ts\x005:  console.log("hello world")\n');
            proc.stdout.emit('data', 'src/main.ts\x0012:  return "hello"\n');
            proc.emit('close');
        }, 10);
    ...
    it('finds multiple matches per line', async () => {
        ...
        setTimeout(() => {
            ;
            proc.stdout.emit('data', 'file.txt\x001:ab cd ab ef ab\n');
            proc.emit('close');
        }, 10);
        const result = await promise;
    ...
    it('respects maxResults and sets truncated', async () => {
        ...
        setTimeout(() => {
            ;
            proc.stdout.emit('data', 'a.ts\x001:x\n' + 'b.ts\x001:x\n' + 'c.ts\x001:x\n');
            proc.emit('close');
        }, 10);
        const result = await promise;
        expect(result.totalMatches).toBe(2);
    ...
    it('passes correct flags for case-insensitive fixed-string search', async () => {
        const proc = createMockProcess();
        spawnMock.mockReturnValue(proc);
        const promise = searchWithGitGrep('/mock/root', { query: 'test', rootPath: '/mock/root', caseSensitive: false, useRegex: false }, 100);
        setTimeout(() => proc.emit('close'), 10);
        await promise;
        const gitArgs = spawnMock.mock.calls[0][1];
    ...
    it('passes correct flags for regex whole-word search', async () => {
        const proc = createMockProcess();
        spawnMock.mockReturnValue(proc);
        const promise = searchWithGitGrep('/mock/root', {
            query: 'foo',
            rootPath: '/mock/root',
            caseSensitive: true,
            useRegex: true,
    ...
    it('skips lines without null separator', async () => {
        ...
        setTimeout(() => {
            ...
            ;
            proc.stdout.emit('data', 'no-null-here:1:ok\n');
            proc.stdout.emit('data', 'valid.ts\x003:ok\n');
            proc.emit('close');
        }, 10);
        const result = await promise;
        expect(result.files).toHaveLength(1);
