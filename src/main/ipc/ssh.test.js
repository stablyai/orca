...
const { handleMock, mockSshStore, mockConnectionManager, mockDeployAndLaunchRelay, mockMux, mockPtyProvider, mockFsProvider, mockGitProvider, mockPortForwardManager } = vi.hoisted(() => ({
    handleMock: vi.fn(),
    mockSshStore: {
...
describe('SSH IPC handlers', () => {
    ...
    it('ssh:connect throws for unknown targetId', async () => {
        ...
        await expect(handlers.get('ssh:connect')(null, { targetId: 'unknown' })).rejects.toThrow('SSH target "unknown" not found');
    });
