...
export class WorktreeCloneInterruptedError extends Error {
  constructor(readonly termination: 'exited' | 'unverifiable') {
    super(`Workspace cloning was interrupted; process termination is ${termination}`)
    this.name = 'WorktreeCloneInterruptedError'
  }
}

export class WorktreeClonePartialError extends Error {
  constructor(target: string, cause: unknown) {
    super(`Workspace clone publication failed; inspect the partial destination at "${target}"`, {
      cause
    })
    this.name = 'WorktreeClonePartialError'
  }
}
