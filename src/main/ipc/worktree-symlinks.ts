...
import {
  resolveCopyOnWriteBackend,
  type CopyOnWriteBackend,
  type CopyOnWriteBackendOptions
} from './worktree-copy-on-write-backend'
import {
...
type WorktreeLinkedPathOptions = CopyOnWriteBackendOptions & {
...
// 'link': symlink when no copy-on-write clone is available (user-configured shared paths).
// 'copy': real copy when no copy-on-write clone is available (.worktreeinclude paths, which
// are per-worktree copies by cross-tool convention — edits must not leak back).
// 'share': always symlink (orca.yaml sharedDirectories). A clone would give each
// worktree an independent node_modules, defeating one-install-serves-all.
...
/** A copy-on-write clone was expected (so its bytes were never charged) but
 *  failed, and the byte-for-byte fallback would escape the budget. */
class WorktreeCopyBudgetFallbackError extends Error {
  constructor(target: string) {
    super(`Clone-copy failed and a real copy of "${target}" would exceed the copy budget`)
...
async function createWorktreeLinkedPath(
  ...
  if (backend && mode !== 'share' && (!sourceIsSymbolicLink || mode === 'copy')) {
    try {
      await backend.clone(copySource, target, sourceIsDirectory)
      return
    } catch (error) {
      if (
        error instanceof WorktreeCloneInterruptedError ||
        error instanceof WorktreeClonePartialError
      ) {
        throw error
      }
      ...
      // Why: a clone can fail across volumes or on a filesystem without the
      // feature. Fall back per mode without touching any target path that may
      // have appeared after our preflight.
      if (!(error instanceof WorktreeCloneUnavailableError)) {
        console.warn(
          `[worktree-symlinks] ${backend.label} clone-copy unavailable for "${target}":`,
          error
        )
      }
      // A later unavailable verdict does not undo an earlier byte exemption.
  ...
  await symlinkWorktreePath(source, target, sourceIsDirectory, platform)
}
...
async function materializeWorktreePaths(
  ...
  const backend = resolveCopyOnWriteBackend(platform, options)
  ...
  for (const rawPath of paths) {
    ...
    try {
      ...
      console.warn(`[worktree-symlinks] Unsafe target for "${safePath.rel}":`, error)
      if (mode === 'copy') {
        skipped.push({ path: safePath.rel, reason: 'failed' })
      }
      continue
    }
    ...
    if (sourceIsDirectory && mode !== 'share') {
      const [realSource, realWorktree] = await Promise.all([
        realpath(source),
        realpath(worktreePath)
      ])
      const realTarget = resolve(realWorktree, relative(resolve(worktreePath), target))
      if (isDescendantOrEqual(realTarget, realSource)) {
        console.warn(`[worktree-symlinks] Refusing recursive materialization of "${safePath.rel}"`)
        if (mode === 'copy') {
          skipped.push({ path: safePath.rel, reason: 'failed' })
        }
        continue
    ...
    if (mode === 'copy') {
      try {
        ...
        // Strict clones share data blocks; the entry budget still bounds metadata work.
        bytesAreCopied = !(backend ? await backend.canClone(copySource, worktreePath) : false)
    ...
    try {
      ...
      if (error instanceof WorktreeCloneInterruptedError) {
        if (error.termination === 'unverifiable' || mode !== 'copy') {
          error.message += `. Workspace remains at "${worktreePath}"; materialization stopped at "${safePath.rel}". Check the destination before retrying.`
          throw error
        }
        skipped.push({
          path: safePath.rel,
        ...
        console.warn(`[worktree-symlinks] Interrupted "${safePath.rel}": ${error.message}`)
        continue
      }
      ...
      if (mode === 'copy') {
        skipped.push({ path: safePath.rel, reason: 'failed', mayBePartial: true })
      }
      console.error(
...
/** Materialize the per-user Worktree Shared Paths into a freshly-created
 *  worktree: a copy-on-write clone where the platform and filesystem offer one
 *  (APFS on macOS, reflink on Linux), otherwise a symlink to the primary
 *  checkout. Failures on individual paths are logged and skipped so a
 *  missing/stale entry never blocks worktree creation.
 *
 ...
 *  createWorktreeLinkedPaths, but the fallback when no clone is available is a
 *  real copy, never a symlink: the convention promises each worktree its own
 *  private copy.
 *
 ...
 *  worktree. Unlike createWorktreeLinkedPaths this never clone-copies: a clone
 *  would give each worktree its own node_modules, and the point of a shared
 *  directory is that one install serves every worktree. */
