  ...
  for (const rawPath of paths) {
    ...
    if (mode === 'copy') {
      try {
        ...
        // Why: a copy-on-write clone shares blocks — a 2.7 GB tree clones in
        // ~20ms on APFS and consumes no disk — so bytes are not the cost there,
        // inodes are. Charging bytes on that path would refuse work that is
        // already free.
