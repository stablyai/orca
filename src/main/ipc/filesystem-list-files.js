...
function shouldIncludeQuickOpenPath(path) {
    let start = 0;
...
export async function listQuickOpenFiles(rootPath, store) {
 ...
 */
function listFilesWithGit(rootPath) {
    const files = new Set();
    const runGitLsFiles = (args) => {
        return new Promise((resolve) => {
            let buf = '';
            let done = false;
            const finish = () => {
