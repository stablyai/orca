...
describe('registerPtyHandlers', () => {
  ...
  it('cleans up fresh runtime-owned SSH spawns when binding persistence fails', async () => {
    ...
    try {
      ...
      ).rejects.toThrow(/MCODE_TERMINAL_SESSION_STATE_SAVE_FAILED/)
  ...
  it('ignores renderer-provided MCODE_TERMINAL_HANDLE for local PTY spawns', async () => {
    const runtime = {
    ...
    await handlers.get('pty:spawn')!(null, {
      cols: 80,
      rows: 24,
      env: { MCODE_TERMINAL_HANDLE: 'term_untrusted' }
    })

    const spawnCall = spawnMock.mock.calls.at(-1)!
    const env = spawnCall[2].env as Record<string, string>
    expect(env.MCODE_TERMINAL_HANDLE).toBe('term_trusted')
    expect(runtime.preAllocateHandleForPty).toHaveBeenCalledWith(expect.any(String))
  })
  it('forwards the trusted MCode terminal handle into managed WSL terminals', async () => {
    ...
    expect(env.MCODE_TERMINAL_HANDLE).toBe('term_wsl')
    expect(env.MCODE_USER_DATA_PATH).toBe('/tmp/mcode-user-data')
    expect(env.MCODE_CLI_COMMAND).toBe('mcode-ide')
    ...
    expect(env.WSLENV?.split(':')).not.toEqual(
      expect.arrayContaining(['MCODE_OMP_SOURCE_AGENT_DIR/p'])
    )
  })
  it('forces managed MCODE_USER_DATA_PATH for WSL spawns even when the caller provides a stale root', async () => {
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    ...
    try {
      ...
      await handlers.get('pty:spawn')!(null, {
        ...
        shellOverride: 'wsl.exe',
        env: {
          MCODE_USER_DATA_PATH: '/tmp/stale-mcode-user-data'
        }
      })
    } finally {
      if (platform) {
