...
describe('registerEphemeralVmHandlers', () => {
  ...
  beforeEach(() => {
    ...
    connectRuntimeOwnedSshTargetMock.mockResolvedValue({
      targetId: 'runtime-ssh-mcode-instance-1',
      target: {
        id: 'runtime-ssh-mcode-instance-1',
        label: 'Sandbox',
  ...
  it('lists recipes from local repo mcode.yaml', async () => {
    const repoPath = makeDir('mcode-ephemeral-vm-ipc-repo-')
    writeFileSync(
  ...
  it('merges approved plugin recipes while repository recipes shadow matching ids', async () => {
    ...
    const pluginService = pluginServiceWithRecipes([
      {
        pluginKey: 'mcode-samples.recipes',
  ...
  it('uses an immutable plugin recipe snapshot after the plugin is removed', async () => {
    const userDataPath = makeDir('mcode-ephemeral-vm-ipc-user-data-')
    const repoPath = makeDir('mcode-ephemeral-vm-ipc-repo-')
  ...
  it('retains the runtime-owned SSH target when destroy fails', async () => {
    ...
    expect(cleaned.sshTargetId).toBe('runtime-ssh-mcode-instance-1')
  })
