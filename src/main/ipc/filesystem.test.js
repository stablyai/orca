...
const { handleMock, trashItemMock, readdirMock, readFileMock, writeFileMock, statMock, realpathMock, lstatMock, getStatusMock, getDiffMock, getBranchCompareMock, getBranchDiffMock, stageFileMock, bulkStageFilesMock, unstageFileMock, bulkUnstageFilesMock, discardChangesMock, listWorktreesMock } = vi.hoisted(() => ({
    handleMock: vi.fn(),
    trashItemMock: vi.fn(),
    readdirMock: vi.fn(),
    readFileMock: vi.fn(),
...
describe('registerFilesystemHandlers', () => {
    ...
    it('rejects readFile when the real path escapes allowed roots', async () => {
        ...
        await expect(handlers.get('fs:readFile')(null, { filePath: linkPath })).rejects.toThrow('Access denied: path resolves outside allowed directories');
        expect(readFileMock).not.toHaveBeenCalled();
    });
    it('rejects writes to directories', async () => {
        lstatMock.mockResolvedValue({ isDirectory: () => true });
        registerFilesystemHandlers(store);
        await expect(handlers.get('fs:writeFile')(null, {
    ...
    it.each([
        ...
        registerFilesystemHandlers(store);
        await expect(handlers.get('fs:readFile')(null, { filePath: path.resolve(`/workspace/repo/file.${ext}`) })).resolves.toEqual({
    ...
    it('keeps non-image binaries hidden from the editor payload', async () => {
        ...
        registerFilesystemHandlers(store);
        await expect(handlers.get('fs:readFile')(null, { filePath: path.resolve('/workspace/repo/archive.zip') })).resolves.toEqual({
            content: '',
            isBinary: true
        });
    });
    ...
    it('rejects git file paths that escape the selected worktree', async () => {
        registerFilesystemHandlers(store);
        await expect(handlers.get('git:discard')(null, {
            worktreePath: WORKTREE_FEATURE_PATH,
    ...
    it('rejects git operations for unknown worktrees', async () => {
        listWorktreesMock.mockResolvedValue([]);
        registerFilesystemHandlers(store);
        await expect(handlers.get('git:status')(null, {
            worktreePath: WORKTREE_FEATURE_PATH
    ...
    it('rejects bulk unstage requests that escape the selected worktree', async () => {
        registerFilesystemHandlers(store);
        await expect(handlers.get('git:bulkUnstage')(null, {
            worktreePath: WORKTREE_FEATURE_PATH,
    ...
    it('rejects branchCompare for a worktree added after cache was built, then succeeds after invalidation', async () => {
        ...
        await expect(handlers.get('git:branchCompare')(null, {
