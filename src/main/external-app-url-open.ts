...
export async function openExternalAppUrlWithUserApproval(
  ...
  const runPrompt = async (): Promise<OpenExternalAppUrlResult> => {
    ...
    // Why: classify again immediately before openExternal so a mutated raw input
    // cannot skip the deny list after the user confirms (#12719).
    const confirmed = classifyExternalAppUrl(classified.url)
    if (!confirmed.ok || confirmed.kind !== 'custom') {
      return !confirmed.ok && confirmed.reason === 'denied' ? 'denied' : 'invalid'
    }
    try {
      await shell.openExternal(confirmed.url)
      return 'opened'
    } catch {
      return 'failed'
    }
