 ...
 * MCode's history tree. That is why deletion needs its own path here instead of
 ...
 */
const SESSION_PREFIX = 'mcode_'
const RELAY_SESSION_PREFIX = 'mcode_relay_'
const SAFE_SESSION_NAME = /^mcode_(?:relay_)?[0-9a-f]{1,64}$/
/** Deliberately does NOT match the relay prefix. A relay host keyed by its
 *  CLIENT's worktree ids shares the one fish data dir with any desktop MCode on
...
const MCODE_HISTORY_FILE = /^mcode_([0-9a-f]{1,64})_history$/
...
/** Drop a `fish_history` this process inherited from an outer MCode.
 *  fish EXPORTS the variable, so an MCode launched from a fish pane hands the
 *  LAUNCHING worktree's session name to every pane of the nested app, which then
 *  writes into that worktree's history file (STA-4682). Only MCode-minted names
...
export function dropInheritedMCodeFishHistory(env: Record<string, string | undefined>): void {
 ...
 *  Resolve this from the PTY's SPAWN env: MCode can be launched with a different
 ...
 *  the two disagree whenever MCode's environment differs from the PTY's. */
...
/**
 * Delete every MCode-minted fish history file whose worktree is gone.
 *
 ...
 * scan of the data dir can find those. Matching on MCode's own `mcode_<hex>_`
...
export function sweepOrphanedFishHistoryFiles(
  ...
  for (const dir of new Set(dirs)) {
    ...
    for (const entry of entries) {
      const hash = MCODE_HISTORY_FILE.exec(entry)?.[1]
