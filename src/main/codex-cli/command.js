...
function getExecutableNames(platform, commandName) {
    if (platform === 'win32') {
...
function parseVersionSegment(raw) {
    return raw
        .replace(/^v/i, '')
        .split('.')
...
function findFirstExecutable(directories, executableNames) {
...
function getVersionManagerDirectories(platform, homePath, executableNames) {
    const directories = [
...
function resolveCommand(commandName, options = {}) {
    ...
    const versionManagerCandidate = findFirstExecutable(getVersionManagerDirectories(platform, homePath, executableNames), executableNames);
    return versionManagerCandidate ?? commandName;
}
export function resolveCodexCommand(options = {}) {
    return resolveCommand('codex', options);
}
export function resolveClaudeCommand(options = {}) {
...
export function getVersionManagerBinPaths(options = {}) {
    const platform = options.platform ?? process.platform;
    const homePath = options.homePath ?? homedir();
