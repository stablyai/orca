...
import { mapCheckRunRESTStatus, mapCheckRunRESTConclusion, mapCheckStatus, mapCheckConclusion, mapPRState, deriveCheckStatus } from './mappers';
 ...
 */
export async function checkOrcaStarred() {
    await acquire();
    try {
 ...
 */
export async function starOrca() {
    await acquire();
    try {
...
export async function getAuthenticatedViewer() {
    await acquire();
    try {
        const { stdout } = await execFileAsync('gh', ['api', 'user', '--jq', '{login: .login, email: .email}'], { encoding: 'utf-8' });
...
function mapIssueWorkItem(item) {
    return {
        ...
        labels: Array.isArray(item.labels)
            ? item.labels
                .map((label) => typeof label === 'object' && label !== null && 'name' in label
                ? String(label.name ?? '')
                : '')
                .filter(Boolean)
            : [],
        updatedAt: String(item.updated_at ?? item.updatedAt ?? ''),
        author: typeof item.user === 'object' && item.user !== null && 'login' in item.user
...
function mapPullRequestWorkItem(item) {
    return {
        ...
        author: typeof item.user === 'object' && item.user !== null && 'login' in item.user
            ...
            : typeof item.author === 'object' && item.author !== null && 'login' in item.author
                ? String(item.author.login ?? '')
                : null,
        branchName: typeof item.head === 'object' && item.head !== null && 'ref' in item.head
            ? String(item.head.ref ?? '')
            : String(item.headRefName ?? ''),
        baseRefName: typeof item.base === 'object' && item.base !== null && 'ref' in item.base
            ? String(item.base.ref ?? '')
            : String(item.baseRefName ?? '')
    };
}
function sortWorkItemsByUpdatedAt(items) {
...
function parseTaskQuery(rawQuery) {
    const query = {
        scope: 'all',
        state: null,
        assignee: null,
...
async function listRecentWorkItems(repoPath, ownerRepo, limit) {
    if (ownerRepo) {
        ...
        const prs = JSON.parse(prsResult.stdout).map(mapPullRequestWorkItem);
    ...
    const issues = JSON.parse(issuesResult.stdout).map(mapIssueWorkItem);
    const prs = JSON.parse(prsResult.stdout).map(mapPullRequestWorkItem);
    return sortWorkItemsByUpdatedAt([...issues, ...prs]).slice(0, limit);
}
async function listQueriedWorkItems(repoPath, ownerRepo, query, limit) {
    const fetchers = [];
    ...
    if (issueScope) {
        fetchers.push((async () => {
            ...
            try {
                ...
                return JSON.parse(stdout).map(mapIssueWorkItem);
            }
            catch {
                return [];
            }
    ...
    if (prScope) {
        fetchers.push((async () => {
            ...
            try {
                const { stdout } = await ghExecFileAsync(args, { cwd: repoPath });
                return JSON.parse(stdout).map(mapPullRequestWorkItem);
            }
            catch {
                return [];
            }
...
export async function listWorkItems(repoPath, limit = 24, query) {
...
export async function getWorkItem(repoPath, number) {
    await acquire();
    try {
        const ownerRepo = await getOwnerRepo(repoPath);
        if (ownerRepo) {
            const { stdout } = await ghExecFileAsync(['api', `repos/${ownerRepo.owner}/${ownerRepo.repo}/issues/${number}`], { cwd: repoPath });
            const item = JSON.parse(stdout);
            if ('pull_request' in item) {
                const prResult = await ghExecFileAsync(['api', `repos/${ownerRepo.owner}/${ownerRepo.repo}/pulls/${number}`], { cwd: repoPath });
                ...
                return {
                    ...
                    author: typeof pr.user === 'object' && pr.user !== null && 'login' in pr.user
                        ? String(pr.user.login ?? '')
                        : null,
                    branchName: typeof pr.head === 'object' && pr.head !== null && 'ref' in pr.head
                        ? String(pr.head.ref ?? '')
                        : undefined,
                    baseRefName: typeof pr.base === 'object' && pr.base !== null && 'ref' in pr.base
                        ? String(pr.base.ref ?? '')
                        : undefined
                };
            }
        ...
        try {
            ...
            return {
                ...
                author: typeof item.author === 'object' && item.author !== null && 'login' in item.author
                    ? String(item.author.login ?? '')
                    : null
            };
        }
 ...
 */
export async function getPRForBranch(repoPath, branch) {
    ...
    try {
        ...
        const conflictSummary = data.mergeable === 'CONFLICTING' && data.baseRefName && data.baseRefOid && data.headRefOid
 ...
 */
export async function getPRChecks(repoPath, prNumber, headSha, options) {
    ...
    try {
        ...
        const { stdout } = await ghExecFileAsync(['pr', 'checks', String(prNumber), '--json', 'name,state,link'], { cwd: repoPath });
        const data = JSON.parse(stdout);
        return data.map((d) => ({
            name: d.name,
 ...
 */
export async function getPRComments(repoPath, prNumber, options) {
    const ownerRepo = await getOwnerRepo(repoPath);
    await acquire();
    try {
        if (ownerRepo) {
            ...
            const [issueResult, threadsResult, reviewsResult] = await Promise.allSettled([
                execFileAsync('gh', ['api', ...cacheArgs, `${base}/issues/${prNumber}/comments?per_page=100`], { cwd: repoPath, encoding: 'utf-8' }),
                execFileAsync('gh', [
                    'api',
                    'graphql',
                    '-f',
                ...
                execFileAsync('gh', ['api', ...cacheArgs, `${base}/pulls/${prNumber}/reviews?per_page=100`], { cwd: repoPath, encoding: 'utf-8' })
            ]);
            let issueComments = [];
            if (issueResult.status === 'fulfilled') {
                issueComments = JSON.parse(issueResult.value.stdout).map((c) => ({
                    id: c.id,
            ...
            if (threadsResult.status === 'fulfilled') {
                const threadsData = JSON.parse(threadsResult.value.stdout);
            ...
            if (reviewsResult.status === 'fulfilled') {
                reviewSummaries = JSON.parse(reviewsResult.value.stdout)
        ...
        const { stdout } = await execFileAsync('gh', ['pr', 'view', String(prNumber), '--json', 'comments'], { cwd: repoPath, encoding: 'utf-8' });
        const data = JSON.parse(stdout);
 ...
 */
export async function resolveReviewThread(repoPath, threadId, resolve) {
    ...
    await acquire();
    try {
        await execFileAsync('gh', ['api', 'graphql', '-f', `query=${query}`, '-f', `threadId=${threadId}`], { cwd: repoPath, encoding: 'utf-8' });
        return true;
    }
    catch (err) {
 ...
 */
export async function mergePR(repoPath, prNumber, method = 'squash') {
    await acquire();
    try {
 ...
 * Update a PR's title.
 */
export async function updatePRTitle(repoPath, prNumber, title) {
    await acquire();
    try {
