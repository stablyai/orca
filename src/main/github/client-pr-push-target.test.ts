...
describe('getPRForBranch', () => {
  ...
  it('resolves fork PR push target using the origin URL protocol', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'stablyai', repo: 'mcode' })
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'prateek/fix-sidebar-agents-toggle',
          repo: {
            full_name: 'prateek/mcode',
            name: 'mcode',
            clone_url: 'https://github.com/prateek/mcode.git',
            ssh_url: 'git@github.com:prateek/mcode.git',
            owner: { login: 'prateek' }
          }
        }
      })
    })
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:mcode-ide/mcode.git')

    const target = await getPullRequestPushTarget('/repo-root', 1738)

    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(['api', 'repos/mcode-ide/mcode/pulls/1738'], {
      cwd: '/repo-root'
    })
  ...
  it('pins explicit origin push-target lookup when upstream has the same PR number', async () => {
    getOwnerRepoMock.mockResolvedValue({ owner: 'fork', repo: 'mcode' })
    resolvePRRepositoryCandidatesMock.mockResolvedValue({
      candidates: [
        { owner: 'upstream', repo: 'mcode' },
        { owner: 'fork', repo: 'mcode' }
      ],
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'contributor/fix',
          repo: {
            full_name: 'contributor/mcode',
            name: 'mcode',
            clone_url: 'https://github.com/contributor/mcode.git',
            ssh_url: 'git@github.com:contributor/mcode.git',
            owner: { login: 'contributor' }
          }
        }
      })
    })
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:fork/mcode.git')

    await getPullRequestPushTarget('/repo-root', 1738, null, {}, 'origin')

    expect(resolvePRRepositoryCandidatesMock).not.toHaveBeenCalled()
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(['api', 'repos/fork/mcode/pulls/1738'], {
      cwd: '/repo-root',
      host: 'github.com'
    })
    expect(ghExecFileAsyncMock).not.toHaveBeenCalledWith(
      ['api', 'repos/upstream/mcode/pulls/1738'],
      expect.anything()
    )
  })
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'fix-sidebar',
          repo: {
            full_name: 'mcode-ide/mcode',
            name: 'mcode',
            clone_url: 'https://github.com/mcode-ide/mcode.git',
            ssh_url: 'git@github.com:mcode-ide/mcode.git',
  ...
  it('keeps getRepoSlug origin-based on a fork checkout (#7331)', async () => {
    ...
    getOwnerRepoMock.mockResolvedValue({ owner: 'fsdwen', repo: 'mcode' })
  ...
  it('resolves a distinct upstream remote as the repo upstream', async () => {
    ...
    getOwnerRepoMock.mockResolvedValue({ owner: 'tmchow', repo: 'mcode' })
    getOwnerRepoForRemoteMock.mockImplementation(async (_repoPath: string, remoteName: string) =>
      remoteName === 'origin'
        ? { owner: 'tmchow', repo: 'mcode' }
        : { owner: 'stablyai', repo: 'mcode' }
    )
  ...
  it('does not treat a same-repository upstream remote as a fork', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ...
      ['repo', 'view', 'github.com/StablyAI/MCode', '--json', 'isFork,parent'],
  ...
  it('falls back to the GitHub parent when no upstream remote is configured', async () => {
    getOwnerRepoForRemoteMock
      .mockResolvedValueOnce({ owner: 'tmchow', repo: 'mcode' })
      .mockResolvedValueOnce(null)
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        isFork: true,
        parent: { name: 'mcode', owner: { login: 'stablyai' } }
      })
    })
  ...
  it('routes GHES push-target probes through the Enterprise host', async () => {
    const ghes = { owner: 'team', repo: 'mcode', host: 'github.acme-corp.com' }
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'feature',
          repo: {
            full_name: 'team/mcode',
            name: 'mcode',
            clone_url: 'https://github.acme-corp.com/team/mcode.git',
            ssh_url: 'git@github.acme-corp.com:team/mcode.git',
            owner: { login: 'team' }
          }
        }
      })
  ...
  it('does not confuse same-slug PR repositories across GitHub hosts', async () => {
    ...
    const dotCom = { owner: 'team', repo: 'mcode', host: 'github.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [enterprise, dotCom],
      headRepo: dotCom
    })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce(dotCom)
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:team/mcode.git')
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'feature',
    ...
    await expect(getPullRequestPushTarget('/repo-root', 7)).resolves.toEqual({
      pushTarget: {
        remoteName: 'pr-team-mcode',
        branchName: 'feature',
        remoteUrl: 'git@github.acme-corp.com:team/mcode.git'
      }
    })
  })
  ...
  it('probes additional PR repo candidates when the first lookup is not found', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork', repo: 'mcode' })
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          head: {
            ref: 'feature/test',
            repo: {
              full_name: 'fork/mcode',
              name: 'mcode',
              clone_url: 'https://github.com/fork/mcode.git',
              ssh_url: 'git@github.com:fork/mcode.git',
              owner: { login: 'fork' }
            }
          }
        })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, ['api', 'repos/fork/mcode/pulls/1849'], {
      cwd: '/repo-root'
    })
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['api', 'repos/mcode-ide/mcode/pulls/1849'],
      { cwd: '/repo-root' }
    )
  })

  it('probes the next PR work-item candidate after a permission denial', async () => {
    const upstream = { owner: 'upstream', repo: 'mcode', host: 'github.com' }
    const origin = { owner: 'fork', repo: 'mcode', host: 'github.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Origin PR',
          state: 'OPEN',
          url: 'https://github.com/fork/mcode/pull/42',
          labels: [],
    ...
    expect(ghExecFileAsyncMock.mock.calls[0][0]).toEqual(
      expect.arrayContaining(['pr', 'view', '--repo', 'upstream/mcode'])
    )
    expect(ghExecFileAsyncMock.mock.calls[1][0]).toEqual(['api', 'repos/upstream/mcode/pulls/42'])
    expect(ghExecFileAsyncMock.mock.calls[2][0]).toEqual(
      expect.arrayContaining(['pr', 'view', '--repo', 'fork/mcode'])
    )
  })
