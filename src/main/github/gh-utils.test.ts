...
describe('github owner/repo resolution', () => {
  ...
  it('parses GitHub HTTPS and SSH remotes', () => {
    ...
    expect(parseGitHubOwnerRepo('git@github.com:mcode-ide/mcode.git')).toEqual({
      owner: 'stablyai',
      repo: 'mcode'
    })
    ...
    expect(parseGitHubOwnerRepo('ssh://git@github.com/mcode-ide/mcode.git')).toEqual({
      owner: 'stablyai',
      repo: 'mcode'
    })
    expect(parseGitHubOwnerRepo('ssh://git@ssh.github.com:443/mcode-ide/mcode.git')).toEqual({
      owner: 'stablyai',
      repo: 'mcode'
    })
    expect(parseGitHubOwnerRepo('git@example.com:mcode-ide/mcode.git')).toBeNull()
  })

  it('parses GitHub Enterprise host identity', () => {
    expect(parseGitHubRemoteIdentity('https://ghe.acme.internal/acme/mcode.git')).toEqual({
      host: 'ghe.acme.internal',
      owner: 'acme',
      repo: 'mcode'
    })
    expect(parseGitHubRemoteIdentity('git@ghe.acme.internal:acme/mcode.git')).toEqual({
      host: 'ghe.acme.internal',
      owner: 'acme',
      repo: 'mcode'
    })
    expect(parseGitHubOwnerRepo('https://ghe.acme.internal/acme/mcode.git')).toBeNull()
  })

  it('prefers upstream for PR owner/repo resolution (#7331)', async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:mcode-ide/mcode.git\n'
    })

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'stablyai', repo: 'mcode' })
  ...
  it('prefers upstream for issue owner/repo resolution', async () => {
    ...
    await expect(getIssueOwnerRepo('/repo')).resolves.toEqual({ owner: 'stablyai', repo: 'mcode' })
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(['remote', 'get-url', 'upstream'], {
      cwd: '/repo',
      timeout: 30_000
    })
  ...
  it('falls back to origin when upstream is missing or non-GitHub', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:mcode-ide/mcode.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/mcode.git\n' })

    await expect(getIssueOwnerRepo('/repo')).resolves.toEqual({ owner: 'fork', repo: 'mcode' })
  ...
  it('does not mix origin and upstream cache entries for the same repo path', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/mcode.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:mcode-ide/mcode.git\n' })
  ...
  it('resolves SSH repo remotes through the registered SSH git provider', async () => {
    const sshProvider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        return { stdout: 'git@github.com:mcode-ide/mcode.git\n', stderr: '' }
      })
    }
    getSshGitProviderMock.mockReturnValue(sshProvider)

    await expect(getOwnerRepo('/home/user/mcode', 'openclaw-2')).resolves.toEqual({
      owner: 'stablyai',
      repo: 'mcode'
    })
  ...
  it('keeps local and SSH owner/repo cache entries separate for the same path', async () => {
    const sshProvider = {
      exec: vi.fn().mockResolvedValue({ stdout: 'git@github.com:remote/mcode.git\n', stderr: '' })
    }
    gitExecFileAsyncMock.mockResolvedValueOnce({ stdout: 'git@github.com:local/mcode.git\n' })
    getSshGitProviderMock.mockReturnValue(sshProvider)

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'local', repo: 'mcode' })
    await expect(getOwnerRepo('/repo', 'ssh-1')).resolves.toEqual({ owner: 'remote', repo: 'mcode' })
  })
      ...
      async (args: string[], options: { wslDistro?: string } = {}) => {
        ...
        return {
          stdout: options.wslDistro
            ? 'git@github.com:wsl/mcode.git\n'
            : 'git@github.com:host/mcode.git\n'
        }
      }
    )

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'host', repo: 'mcode' })
  ...
  it('prunes expired distinct owner/repo cache entries on later lookups', async () => {
    ...
    try {
      ...
      await expect(getOwnerRepo('/repo-a')).resolves.toEqual({ owner: 'stablyai', repo: 'mcode' })
  ...
  it('resolves PR candidates as upstream then origin and de-dupes matching slugs', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:Acme/MCode.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:acme/mcode.git\n' })

    await expect(resolvePRRepositoryCandidates('/repo')).resolves.toEqual({
      candidates: [{ owner: 'Acme', repo: 'MCode' }],
      headRepo: { owner: 'acme', repo: 'mcode' }
    })
  })

  it('ignores non-GitHub upstream while keeping origin as the head repo', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:Acme/MCode.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/mcode.git\n' })

    await expect(resolvePRRepositoryCandidates('/repo')).resolves.toEqual({
      candidates: [{ owner: 'fork', repo: 'mcode' }],
      headRepo: { owner: 'fork', repo: 'mcode' }
    })
  })
  ...
  it('expires cached remote owner/repo entries after the TTL', async () => {
    ...
    try {
      gitExecFileAsyncMock
        .mockResolvedValueOnce({ stdout: 'git@github.com:old/mcode.git\n' })
        .mockResolvedValueOnce({ stdout: 'git@github.com:new/mcode.git\n' })
  ...
  it('keeps local missing-remote probes cached beyond the short positive TTL', async () => {
    const repoPath = await mkdtemp(join(tmpdir(), 'mcode-gh-utils-'))
...
describe('resolveIssueSource', () => {
  ...
  it("'auto' + no upstream → origin, fellBack=false", async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:mcode-ide/mcode.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:solo/mcode.git\n' })

    await expect(resolveIssueSource('/repo', 'auto')).resolves.toEqual({
      source: { owner: 'solo', repo: 'mcode' },
      fellBack: false
    })
  })
  ...
  it("'origin' + upstream exists → origin (ignores upstream), fellBack=false", async () => {
    ...
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:fork/mcode.git\n'
    })

    await expect(resolveIssueSource('/repo', 'origin')).resolves.toEqual({
      source: { owner: 'fork', repo: 'mcode' },
      fellBack: false
    })
    expect(gitExecFileAsyncMock).toHaveBeenCalledTimes(1)
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(['remote', 'get-url', 'origin'], {
  ...
  it("'origin' + no upstream → origin, fellBack=false", async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:solo/mcode.git\n'
    })
...
describe('gh error classification', () => {
  ...
  it('classifies "has disabled issues" stderr as issues_disabled', () => {
    const stderr =
      "Command failed: gh issue list --limit 36 --json number,title,state --repo brennanb2025/mcode --state open\nthe 'brennanb2025/mcode' repository has disabled issues"
