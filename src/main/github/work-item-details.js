...
async function getPRHeadBaseSha(repoPath, prNumber) {
    const ownerRepo = await getOwnerRepo(repoPath);
    try {
        if (ownerRepo) {
            const { stdout } = await ghExecFileAsync(['api', '--cache', '60s', `repos/${ownerRepo.owner}/${ownerRepo.repo}/pulls/${prNumber}`], { cwd: repoPath });
        ...
        const { stdout } = await ghExecFileAsync(['pr', 'view', String(prNumber), '--json', 'headRefOid,baseRefOid'], { cwd: repoPath });
        const data = JSON.parse(stdout);
...
async function getPRFiles(repoPath, prNumber) {
    const ownerRepo = await getOwnerRepo(repoPath);
    if (!ownerRepo) {
        return [];
    }
...
async function getIssueBodyAndComments(repoPath, issueNumber) {
    const ownerRepo = await getOwnerRepo(repoPath);
    try {
        if (ownerRepo) {
            ...
            const comments = JSON.parse(commentsResult.stdout).map((c) => ({
                id: c.id,
        ...
        const { stdout } = await ghExecFileAsync(['issue', 'view', String(issueNumber), '--json', 'body,comments'], { cwd: repoPath });
        const data = JSON.parse(stdout);
...
async function getPRBody(repoPath, prNumber) {
    const ownerRepo = await getOwnerRepo(repoPath);
    try {
        if (ownerRepo) {
            const { stdout } = await ghExecFileAsync(['api', '--cache', '60s', `repos/${ownerRepo.owner}/${ownerRepo.repo}/pulls/${prNumber}`], { cwd: repoPath });
...
export async function getWorkItemDetails(repoPath, number) {
    ...
    const item = await getWorkItem(repoPath, number);
    if (!item) {
        return null;
    }
    await acquire();
