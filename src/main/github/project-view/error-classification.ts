// Why: extracted from internals.ts to stay under the repo's oxlint max-lines
// cap — error classification (interpreting gh/GraphQL stderr+stdout into a
// GitHubProjectViewError) is a cohesive, self-contained concern, separate
// from the slug-validation and low-level gh-invocation plumbing that stays
// in internals.ts. internals.ts re-exports these so no other file's import
// path changes.
