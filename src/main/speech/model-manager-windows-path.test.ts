...
describe('ModelManager Windows model path handling', () => {
  beforeEach(() => {
    ...
    appGetPathMock.mockImplementation(() => join(tmpdir(), 'mcode-speech-models-test'))
  })
  ...
  it('uses an ASCII cache path when the Windows default user data path has non-ASCII characters', () => {
    ...
    try {
      ...
      const userDataDir = join(dir, '用户', 'MCode')
      ...
      expect(manager.getModelsDir()).toContain(join(programDataDir, 'MCode', 'speech-models'))
