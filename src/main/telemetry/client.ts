...
const BUILD_IDENTITY: 'stable' | 'rc' | null =
  typeof MCODE_BUILD_IDENTITY !== 'undefined'
    ...
    : ((globalThis as { MCODE_BUILD_IDENTITY?: 'stable' | 'rc' | null }).MCODE_BUILD_IDENTITY ?? null)
const WRITE_KEY: string | null =
  typeof MCODE_POSTHOG_WRITE_KEY !== 'undefined'
    ? MCODE_POSTHOG_WRITE_KEY
    : ((globalThis as { MCODE_POSTHOG_WRITE_KEY?: string | null }).MCODE_POSTHOG_WRITE_KEY ?? null)
...
// No-op in contributor / non-official builds; only official stable/rc builds (CI-injected `MCODE_BUILD_IDENTITY` + `MCODE_POSTHOG_WRITE_KEY`) transmit.
