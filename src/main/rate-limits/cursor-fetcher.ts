...
const cursorDashboardSchema = z.object({
  planUsage: z
    .object({
      totalPercentUsed: z.unknown().optional(),
      autoPercentUsed: z.unknown().optional(),
      apiPercentUsed: z.unknown().optional()
    })
    .optional(),
  billingCycleStart: z.unknown().optional(),
  billingCycleEnd: z.unknown().optional(),
  planType: z.unknown().optional(),
  plan: z.unknown().optional()
})
type CursorDashboardResponse = z.infer<typeof cursorDashboardSchema>
type CursorPlanUsage = NonNullable<CursorDashboardResponse['planUsage']>
...
function mapDashboardResponse(data: CursorDashboardResponse, source: 'cli'): ProviderRateLimits {
  ...
  const monthly: RateLimitWindow | undefined =
    totalPercent === null
      ? undefined
      : {
...
export async function fetchCursorRateLimits(
  ...
  try {
    ...
    if (res.status === 429) {
      ...
      return result('error', 'Cursor usage is rate limited right now.', {
        usageMetadata: {
    ...
    const parsed = cursorDashboardSchema.safeParse(data)
    if (!parsed.success) {
      return parseFailure(source)
    }
    return mapDashboardResponse(parsed.data, source)
  } catch {
