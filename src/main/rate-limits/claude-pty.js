 ...
 */
function extractPercentAfterLabel(lines, labelRe) {
...
function extractResetAfterLabel(lines, labelRe) {
    for (let i = 0; i < lines.length; i++) {
        if (!labelRe.test(lines[i])) {
            continue;
        }
...
function describeClaudeUsageFailure(output) {
...
export async function fetchViaPty() {
