...
import { CURSOR_ACCESS_TOKEN_KEY } from '../../shared/cursor-session-paths'

const netFetchMock = vi.hoisted(() => vi.fn())
const dbState = vi.hoisted<{ token: string | null }>(() => ({ token: null }))
...
vi.mock('../sqlite/sync-database', () => ({
  default: class MockDatabase {
    prepare(sql: string) {
      return {
        get: (key: string) => {
          if (sql.includes('ItemTable') && key === CURSOR_ACCESS_TOKEN_KEY && dbState.token) {
...
describe('fetchCursorRateLimits', () => {
  ...
  it('maps usage-summary JSON into provider windows', async () => {
