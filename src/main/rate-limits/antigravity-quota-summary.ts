...
function isWeeklyBucket(bucketId: string, window: string, displayName: string): boolean {
  const text = `${bucketId} ${window} ${displayName}`.toLowerCase()
  return window === 'weekly' || /weekly|7d|7-day/.test(text)
}
...
export function parseAntigravityQuotaSummary(data: unknown): AntigravityQuotaSummary | null {
  ...
  for (const groupValue of readGroups(data)) {
    ...
    for (const bucketValue of groupValue.buckets) {
      if (!isRecord(bucketValue) || bucketValue.disabled === true) {
        continue
      }
      ...
      const displayName =
        typeof bucketValue.displayName === 'string' ? bucketValue.displayName : bucketId
      const weekly = isWeeklyBucket(bucketId, window, displayName)
      buckets.push({
        name: `${groupName} · ${weekly ? 'Weekly' : 'Five hour'}`,
