...
describe('createAuthFilesystemOperation', () => {
  ...
  it('caps retained operations, coalesces existing paths at saturation, and recovers', async () => {
    // Why: seeding relative to the constant passes at any value, including one large enough to be
    // no bound at all. 129 is the literal ceiling plus one, so raising the constant fails here.
    const registry = new AuthFilesystemOperationRegistry<string>()
    expect(MAX_AUTH_FILESYSTEM_REGISTRY_ENTRIES).toBe(128)
    const deferredReads = Array.from({ length: 129 }, () => deferred<string>())
    ...
    const recovered = registry.getOrCreate('/auth/recovered', () => deferredReads[128]!.promise)
    ...
    deferredReads[128]!.resolve('recovered')
    await Promise.all([
  ...
  it('accepts an exact-limit auth path and rejects one byte more without starting it', async () => {
    expect(MAX_AUTH_FILESYSTEM_OPERATION_PATH_BYTES).toBe(64 * 1024)
    const exactPath = 'x'.repeat(64 * 1024)
  ...
  it('caps coalesced waiters and admits another after one leaves', async () => {
    expect(MAX_AUTH_FILESYSTEM_OPERATION_WAITERS).toBe(256)
  ...
  it('caps the WSL wait queue and recovers as soon as a queued operation leaves', async () => {
    ...
    expect(MAX_QUEUED_WSL_AUTH_OPERATIONS).toBe(128)
