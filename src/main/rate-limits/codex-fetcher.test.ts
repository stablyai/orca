...
describe('fetchCodexRateLimits', () => {
  ...
  it('runs rate-limit RPC through WSL when the Codex home is a WSL managed account', async () => {
    ...
    try {
      ...
      expect(shellCommand).toContain('_mcode_wsl_shell=$(getent passwd')
      expect(shellCommand).toContain('bash|zsh|ksh|mksh|ash) exec "$_mcode_wsl_shell" -ilc')
      expect(shellCommand).toContain(
        'exec 3<&0\nexec 4>&1\nexec </dev/null\nexec >/dev/null\n_mcode_wsl_shell='
      )
      expect(shellCommand).toContain('mkdir -p "$mcode_rate_limit_cwd"')
      expect(shellCommand).toContain('cd "$mcode_rate_limit_cwd"')
      expect(shellCommand).toContain(
        "export CODEX_HOME='\\''/home/alice/.local/share/mcode/account/home'\\''"
      )
      expect(shellCommand).toContain(
      ...
      expect(shellCommand).not.toContain('_mcode_codex')
