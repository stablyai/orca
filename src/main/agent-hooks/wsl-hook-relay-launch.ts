...
export function resolveWslHookRelayBundle(): WslHookRelayBundle | null {
  ...
  if (process.env.MCODE_RELAY_PATH) {
    candidates.push(join(process.env.MCODE_RELAY_PATH, 'wsl'))
  }
...
// Why: the install dir is namespaced by bundle version so concurrent MCode
...
export function buildGuestInstallScript(bundleJs: Buffer, version: string): string {
  ...
  return [
    ...
    'mkdir -p "$d"',
    `base64 -d > "$d/bundle.$$.tmp" << 'MCODE_EOF_BUNDLE'`,
    b64.trimEnd(),
    'MCODE_EOF_BUNDLE',
    `mv "$d/bundle.$$.tmp" "$d/${WSL_HOOK_RELAY_BUNDLE_NAME}"`,
    `cat > "$d/launch.$$.tmp" << 'MCODE_EOF_LAUNCH'`,
...
export function buildWslRelaySpawnEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    WSL_UTF8: '1',
    MCODE_AGENT_HOOK_PORT: coords.MCODE_AGENT_HOOK_PORT,
    MCODE_AGENT_HOOK_TOKEN: coords.MCODE_AGENT_HOOK_TOKEN,
    MCODE_AGENT_HOOK_ENV: coords.MCODE_AGENT_HOOK_ENV,
    MCODE_AGENT_HOOK_VERSION: coords.MCODE_AGENT_HOOK_VERSION,
  ...
  addMCodeWslInteropEnv(env as Record<string, string>)
  return env
}
