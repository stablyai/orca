...
export function createPretoolMutationResolver(
  ...
  return (request) => {
    ...
    if (!dispatch) {
      // An ordinary session. It has no Dispatch, so there is no supervised work
      // to protect and blocking it would strand real work on any hiccup.
      return { deny: false }
    }
    let placement
    try {
    ...
    if (!placement?.terminalHandle || !placement.worktreeId) {
      return { deny: true, reason: UNPLACEABLE_SUPERVISED }
    }
    ...
    if (!placement.processIncarnation || !dispatch.process_incarnation) {
      return { deny: true, reason: UNATTRIBUTABLE_SUPERVISED }
    }
    try {
