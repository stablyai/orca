...
export function wrapRuntimeHomeHookCommand(
  ...
  const windowsScript = `"\${HOME-}/.mcode/agent-hooks/${scriptBaseName}.cmd"`
  const posixScript = `"\${HOME-}/.mcode/agent-hooks/${scriptBaseName}.sh"`
  ...
  const powershellCommand = `$homePath = $env:HOME -replace '^/([A-Za-z])/', '$1:/'; $scriptPath = Join-Path $homePath '.mcode\\agent-hooks\\${scriptBaseName}.cmd'; if (Test-Path -LiteralPath $scriptPath -PathType Leaf) { & $scriptPath; exit $LASTEXITCODE }; [Console]::In.ReadToEnd() | Out-Null${powershellFallback}; exit 0`
