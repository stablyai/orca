...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for MCode-created worktrees whose branch MCode is allowed to rename. */
  canRenameMCodeCreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (!deps.canRenameMCodeCreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`, true)
  }
  // Only rename a branch MCode auto-named — never one the user chose.
