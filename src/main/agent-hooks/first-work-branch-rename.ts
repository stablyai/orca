...
function sourceControlAiAgentFromStatus(agentType: string | undefined): TuiAgent | null {
  if (!isTuiAgent(agentType)) {
    return null
  }
  return getCommitMessageAgentSpec(agentType) ? agentType : null
}
...
async function runAutoRename(
  ...
  const contextualDefaultAgent = sourceControlAiAgentFromStatus(agentType)
