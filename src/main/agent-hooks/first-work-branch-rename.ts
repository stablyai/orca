...
/** Test seam: clear the per-process dedupe sets and the usage-limit backoff. */
...
export async function maybeAutoRenameBranchOnFirstWork(
  ...
  // Issue #9705: a session/usage-limited agent fails every attempt identically,
  // so wait out the limit instead of retrying on each assistant message.
  if (usageLimitBackoffRemainingMs() > 0) {
    return
  }
...
async function runAutoRename(
  ...
  if (!generated.success) {
    ...
    if (!generated.canceled) {
      ...
      const backoffUntil = noteGenerationFailureForUsageLimit(
        generated.error,
        generated.failureOutput
      )
      if (backoffUntil !== null) {
        console.info(
          `[auto-branch-rename] usage-limit response; backing off until ${new Date(backoffUntil).toISOString()}`
        )
      }
    }
