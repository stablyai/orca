...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mcode-installer-utils-test-'))
...
describe('createManagedCommandMatcher', () => {
  ...
  it('matches commands containing the agent-hooks/<scriptFileName> path', () => {
    expect(
      match('/bin/sh "/Users/alice/Library/Application Support/MCode/agent-hooks/claude-hook.sh"')
    ).toBe(true)
  ...
  it('normalizes Windows backslashes so cmd-style paths still match', () => {
    expect(match('C:\\Users\\alice\\AppData\\Roaming\\MCode\\agent-hooks\\claude-hook.sh')).toBe(
      true
    )
  })
  ...
  it('matches the guarded launcher form so wrapped commands sweep correctly', () => {
    ...
    expect(
      match(
        'if [ -x "/Users/alice/Library/Application Support/MCode/agent-hooks/claude-hook.sh" ]; then /bin/sh "/Users/alice/Library/Application Support/MCode/agent-hooks/claude-hook.sh"; fi'
      )
    ).toBe(true)
  })

  it('matches encoded Windows launcher commands by decoding their script path', () => {
    const command = wrapWindowsHookCommand('C:\\Users\\alice\\.mcode\\agent-hooks\\claude-hook.cmd')
  ...
  it('matches the pre-default-form launcher so upgrades replace it instead of duplicating', () => {
    ...
    expect(
      match(
        'if [ -z "$HOME" ]; then :; else if [ -f "$HOME/.mcode/agent-hooks/claude-hook.sh" ]; then /bin/sh "$HOME/.mcode/agent-hooks/claude-hook.sh"; fi; fi'
      )
    ).toBe(true)
  })
  ...
  it('matches PowerShell and POSIX variants across Copilot platform switches', () => {
    ...
    expect(matchPosix("& 'C:\\Users\\alice\\.mcode\\agent-hooks\\copilot-hook.ps1'")).toBe(true)
    expect(
      matchPosix(wrapWindowsHookCommand('C:\\Users\\alice\\.mcode\\agent-hooks\\copilot-hook.ps1'))
    ).toBe(true)
    expect(matchPowerShell("/bin/sh '/home/alice/.mcode/agent-hooks/copilot-hook.sh'")).toBe(true)
  })

  it('matches the legacy per-userData script path AND the new shared ~/.mcode path', () => {
    // Why: install() must sweep old per-userData commands when migrating to
    // the shared ~/.mcode script path, or stale launchers keep failing.
    expect(
      match("/bin/sh '/Users/alice/Library/Application Support/mcode/agent-hooks/claude-hook.sh'")
    ).toBe(true)
    expect(match("/bin/sh '/Users/alice/.mcode/agent-hooks/claude-hook.sh'")).toBe(true)
  })
})
  ...
  it('removes managed direct bash/powershell/command fields', () => {
    const cleaned = removeManagedCommands(
      [
        {
          type: 'command',
          bash: '/bin/sh "/Users/alice/MCode/agent-hooks/copilot-hook.sh"',
          timeoutSec: 5
        },
        {
          type: 'command',
          powershell: "& 'C:\\Users\\alice\\MCode\\agent-hooks\\copilot-hook.sh'",
          timeoutSec: 5
        },
        {
          type: 'command',
          ...
          hooks: [
            {
              ...
              args: [
                ...
                '/d',
                '/c',
                'C:\\Users\\alice\\.mcode\\agent-hooks\\copilot-hook.cmd'
              ]
            },
            { type: 'command', command: 'echo keep me' }
          ]
          ...
          hooks: [
            {
              type: 'command',
              command: 'C:\\Windows\\System32\\conhost.exe',
              args: ['--headless', 'C:\\Users\\alice\\.mcode\\agent-hooks\\copilot-hook.cmd']
            }
          ]
        },
        match
...
describe('getSharedManagedScriptPath', () => {
  it("returns ~/.mcode/agent-hooks/<scriptFileName> rooted at the user's home", () => {
    expect(getSharedManagedScriptPath('claude-hook.sh')).toBe(
      join(homedir(), '.mcode', 'agent-hooks', 'claude-hook.sh')
    )
  })

  it('does not depend on Electron app.getPath, so two MCode instances resolve to the same path', () => {
...
describe('wrapPosixHookCommand', () => {
  ...
  it('preserves spaces in the script path (Library/Application Support case)', () => {
    ...
    const cmd = wrapPosixHookCommand('/Users/a/Library/Application Support/MCode/agent-hooks/x.sh')
    expect(cmd).toContain("'/Users/a/Library/Application Support/MCode/agent-hooks/x.sh'")
  })
  ...
  it('can scope environment variables to the guarded script invocation', () => {
    const cmd = wrapPosixHookCommand('/does/not/exist.sh', {
      MCODE_COPILOT_HOOK_EVENT: 'UserPromptSubmit'
    })
    expect(cmd).toBe(
      `if [ -f '/does/not/exist.sh' ] && [ -r '/does/not/exist.sh' ] && [ -x '/does/not/exist.sh' ]; then MCODE_COPILOT_HOOK_EVENT='UserPromptSubmit' /bin/sh '/does/not/exist.sh'; else ${POSIX_HOOK_STDIN_DRAIN_COMMAND}; fi`
    )
  })
...
describe('wrapWindowsHookCommand', () => {
  it('invokes the .cmd through an encoded PowerShell command', () => {
    const command = wrapWindowsHookCommand('C:\\Users\\alice\\.mcode\\agent-hooks\\codex-hook.cmd')
    ...
    expect(decodeWindowsHookCommand(command)).toBe(
      expectedDecodedWindowsHookCommand('C:\\Users\\alice\\.mcode\\agent-hooks\\codex-hook.cmd')
    )
  })
  ...
  it('scopes environment variables inside the encoded launcher', () => {
    ...
    expect(decodeWindowsHookCommand(command)).toContain(
      "$env:MCODE_COPILOT_HOOK_EVENT = 'UserPromptSubmit'; if (Test-Path"
    )
  })
  ...
  it('preserves spaces in the script path (user profile with space case)', () => {
    const cmd = wrapWindowsHookCommand('C:\\Users\\Jorge Silva\\.mcode\\agent-hooks\\codex-hook.cmd')
    ...
    expect(decodeWindowsHookCommand(cmd)).toBe(
      expectedDecodedWindowsHookCommand(
        'C:\\Users\\Jorge Silva\\.mcode\\agent-hooks\\codex-hook.cmd'
      )
    )
  })

  it('keeps cmd.exe percent expansion and caret escapes out of the command line', () => {
    const cmd = wrapWindowsHookCommand('C:\\Users\\%MCODE_TEST%\\a^b\\codex-hook.cmd')
    expect(cmd).not.toContain('%MCODE_TEST%')
    expect(cmd).not.toContain('^')
    expect(decodeWindowsHookCommand(cmd)).toBe(
      expectedDecodedWindowsHookCommand('C:\\Users\\%MCODE_TEST%\\a^b\\codex-hook.cmd')
    )
  })
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const scriptDir = join(tmpDir, 'home with ^ caret', '.mcode', 'agent-hooks')
      ...
      const result = spawnSync('cmd.exe', ['/d', '/c', wrapWindowsHookCommand(scriptPath)], {
        env: { ...process.env, MCODE_WRAP_TEST: 'expanded' }
      })
...
describe('wrapWindowsCmdHookCommand', () => {
  it('returns the bare, directly-spawnable path for a cmd-safe managed script', () => {
    ...
    const scriptPath = 'C:\\Users\\alice\\.mcode\\agent-hooks\\codex-hook.cmd'
  ...
  it('falls back to the encoded launcher when cmd.exe would split or expand the path', () => {
    const scriptPath = 'C:\\Users\\Jane Doe\\%MCODE_TEST%\\codex-hook.cmd'
    const command = wrapWindowsCmdHookCommand(scriptPath)
    expect(command).toMatch(qualifiedWindowsPowerShellCommand)
...
describe('wrapRuntimeHomeHookCommand', () => {
  it('selects the runtime platform variant under HOME', () => {
    ...
    expect(command).toContain('"${HOME-}/.mcode/agent-hooks/claude-hook.cmd"')
    expect(command).toContain('/bin/sh "${HOME-}/.mcode/agent-hooks/claude-hook.sh"')
  ...
  it('executes the destination HOME script for the current runtime', () => {
    ...
    const sourceScriptDir = join(sourceHome, '.mcode', 'agent-hooks')
    const destinationScriptDir = join(destinationHome, '.mcode', 'agent-hooks')
  ...
  it.skipIf(process.platform !== 'win32')('keeps common Windows profiles on the fast path', () => {
    ...
    const scriptDir = join(destinationHome, '.mcode', 'agent-hooks')
    mkdirSync(scriptDir, { recursive: true })
  ...
  it('emits neutral JSON when a lifecycle script is missing', () => {
    ...
    const result = spawnSync(
      shell,
      ['-c', wrapRuntimeHomeHookCommand('missing-mcode-hook', { neutralJsonWhenMissing: true })],
      {
...
describe('buildWindowsAgentHookPostCommand', () => {
  it('posts hook stdin through bounded curl without spawning PowerShell', () => {
    ...
    expect(command).toContain('-H "X-MCode-Agent-Hook-Token: %MCODE_AGENT_HOOK_TOKEN%"')
    expect(command).toContain('--data-urlencode "paneKey=%MCODE_PANE_KEY%"')
...
describe('buildWindowsAgentHookCurlPostCommand', () => {
  it('posts form fields via curl.exe and reads the payload from stdin', () => {
    ...
    expect(command).toContain('http://127.0.0.1:%MCODE_AGENT_HOOK_PORT%/hook/codex')
    expect(command).toContain('-H "Content-Type: application/x-www-form-urlencoded"')
    expect(command).toContain('-H "X-MCode-Agent-Hook-Token: %MCODE_AGENT_HOOK_TOKEN%"')
    expect(command).toContain('--data-urlencode "paneKey=%MCODE_PANE_KEY%"')
    expect(command).toContain('--data-urlencode "worktreeId=%MCODE_WORKTREE_ID%"')
