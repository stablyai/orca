...
type AgentHookSource = 'claude' | 'codex' | 'gemini' | 'opencode' | 'cursor' | 'pi' | 'droid'
...
const TOOL_INPUT_KEYS_BY_TOOL: Record<string, readonly string[]> = {
  // Claude + Droid shared tools (PascalCase).
  ...
  WebFetch: ['url'],
  WebSearch: ['query'],
  // Droid-specific tool labels (Factory CLI uses these names instead of
  // Claude's nearest equivalents: Write/Bash/WebFetch).
...
function normalizeDroidEvent(
  ...
  const snapshot = resolveToolState(paneKey, extractToolFields('droid', eventName, hookPayload), {
        ...
        : source === 'gemini'
          ...
          : source === 'cursor'
            ...
            : source === 'pi'
              ...
              : source === 'droid'
                ? normalizeDroidEvent(eventName, promptText, paneKey, hookPayloadRecord)
