...
export function buildPosixAgentHookPostCommand(
  ...
  return [
    ...
    `  orca_hook_run_id=$(printf '%s' "\${ORCA_AGENT_STATUS_RUN_ID:-}" | base64 | tr -d '\\n') && \\`,
    `  orca_hook_execution_id=$(printf '%s' "\${ORCA_AGENT_STATUS_EXECUTION_ID:-}" | base64 | tr -d '\\n') && \\`,
    `  orca_hook_emitter_pid=$(printf '%s' "$PPID" | base64 | tr -d '\\n') && \\`,
    ...
    `  ${indent}-H "X-Orca-Agent-Status-Run-Id: \${orca_hook_run_id}" \\`,
    `  ${indent}-H "X-Orca-Agent-Status-Execution-Id: \${orca_hook_execution_id}" \\`,
    `  ${indent}-H "X-Orca-Agent-Hook-Emitter-Pid: \${orca_hook_emitter_pid}" \\`,
    `  ${indent}--data-binary @-`,
    'else',
    `  printf '%s' "$payload" | ${curlCommand} -sS -X POST "http://127.0.0.1:\${ORCA_AGENT_HOOK_PORT}/hook/${source}" \\`,
    `  ${indent}--connect-timeout "\${connect_timeout:-0.5}" --max-time "\${max_time:-1.5}" \\`,
    ...
    `  ${indent}--data-urlencode "runId=\${ORCA_AGENT_STATUS_RUN_ID}" \\`,
    `  ${indent}--data-urlencode "executionId=\${ORCA_AGENT_STATUS_EXECUTION_ID}" \\`,
    `  ${indent}--data-urlencode "emitterProcessId=$PPID" \\`,
