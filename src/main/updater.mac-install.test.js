...
const { appMock, browserWindowMock, nativeUpdaterMock, autoUpdaterMock, shellMock, isMock, killAllPtyMock } = vi.hoisted(() => {
...
describe('updater mac install handoff', () => {
    ...
    it.runIf(process.platform === 'darwin')('waits for Squirrel.Mac before honoring a manual quit that should install the update', async () => {
        ...
        const nativeDownloadedHandler = nativeUpdaterMock.on.mock.calls.find(([eventName]) => eventName === 'update-downloaded')?.[1];
    ...
    it.runIf(process.platform === 'darwin')('falls back to a normal quit if Squirrel.Mac never becomes ready', async () => {
        vi.useFakeTimers();
        const sendMock = vi.fn();
        const mainWindow = { webContents: { send: sendMock } };
        autoUpdaterMock.checkForUpdates.mockResolvedValue(undefined);
