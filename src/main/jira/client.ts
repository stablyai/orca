...
export function normalizeJiraSiteUrl(siteUrl: string): string {
  ...
  // Why: credentials belong in encrypted token storage, never site metadata or UI.
  url.username = ''
  url.password = ''
...
function getServerSiteId(
  siteUrl: string,
  authMode: JiraSite['authMode'],
...
export async function connect(
  ...
  try {
    ...
    if (args.deploymentType === 'server') {
      ...
      const id = getServerSiteId(siteUrl, authMode, viewer.userId)
      site = {
        id,
        siteUrl,
