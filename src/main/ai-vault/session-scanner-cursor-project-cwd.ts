 ...
 */
export async function resolveCursorTranscriptCwd(filePath: string): Promise<string | null> {
  const projectDir = cursorProjectDirFromTranscriptPath(filePath)
  if (!projectDir) {
    return null
  }
  const slug = pathLeaf(projectDir)
  const trusted = await readCursorWorkspaceTrustedPath(projectDir)
  return trusted ?? decodeCursorProjectSlug(slug)
}
...
export async function readCursorWorkspaceTrustedPath(projectDir: string): Promise<string | null> {
  const trustFile = join(projectDir, '.workspace-trusted')
  const record = await readJsonObjectIfExists(trustFile)
...
export function decodeCursorProjectSlug(slug: string): string | null {
  ...
  // POSIX absolute paths slugged by Cursor have at least two segments (e.g. Users-ada-code-orca)
  if (!trimmed.includes('-')) {
    return null
  }
