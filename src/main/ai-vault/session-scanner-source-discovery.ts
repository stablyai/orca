...
export async function discoverAiVaultSessionSources(args: {
  ...
  const [opencode, ...rest] = await Promise.all([
...
/**
 * Discover OpenCode sessions from both legacy file storage and the SQLite DB.
 * Expands across WSL home dirs for the file scan; SQLite DB paths are resolved
 * once (local only) via `listOpenCodeDatabases` unless the caller injects them
 * via `options.opencodeDbPaths` (for test isolation).
 */
async function opencodeDiscoveries(
  options: AiVaultScanOptions,
  wslHomeDirs: readonly string[],
  limit: number,
  issues: AiVaultScanIssue[]
): Promise<SessionFileDiscovery[]> {
  const storageDirs = sessionRootDirs(
    options.opencodeStorageDir ?? OPENCODE_STORAGE_DIR,
    wslHomeDirs,
    ['.local', 'share', 'opencode', 'storage']
  )
  // Why: resolve DB paths once so we don't scan the same SQLite DB once per
  // WSL storage dir. When the caller injects paths (tests), use those directly.
  const resolvedDbPaths = options.opencodeDbPaths ?? (await listOpenCodeDatabases())
  return Promise.all(
    storageDirs.map((storageDir, index) =>
      discoverOpenCodeSessions({
        storageDir,
        // Why: only the first (local) storage dir gets SQLite scanning; WSL
        // dirs get an empty array so they only scan legacy files.
        dbPaths: index === 0 ? resolvedDbPaths : [],
        limitPerAgent: limit,
        issues
      })
    )
