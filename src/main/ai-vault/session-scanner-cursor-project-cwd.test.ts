...
describe('resolveCursorTranscriptCwd', () => {
  it('prefers .workspace-trusted workspacePath over slug decode', async () => {
    const root = tempDir()
    ...
    expect(await resolveCursorTranscriptCwd(filePath)).toBe('C:\\Dev\\simulations\\opc')
  })

  it('falls back to slug decode when the trust marker is absent', async () => {
    const root = tempDir()
    ...
    writeFileSync(filePath, '')
    expect(await resolveCursorTranscriptCwd(filePath)).toBe('/Users/ada/code/orca')
  })

  it('returns null for single-word non-path slugs', async () => {
    const root = tempDir()
    ...
    const filePath = join(transcripts, 'session.jsonl')
    writeFileSync(filePath, '')
    expect(await resolveCursorTranscriptCwd(filePath)).toBeNull()
  })
})
