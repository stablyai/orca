...
import { createNewTerminalTab, closeTerminalTab, closeOtherTerminalTabs, closeTerminalTabsToRight, activateTerminalTab, activateEditorFile, toggleTerminalPaneExpand } from './terminal-tab-actions';
export function useTerminalTabs() {
    const { activeWorktreeId, activeView, worktreesByRepo, tabsByWorktree, activeTabId, tabBarOrderByWorktree, setActiveTab, setTabCustomTitle, setTabColor, consumeSuppressedPtyExit, expandedPaneByTabId, workspaceSessionReady, openFiles, activeFileId, activeTabType, setTabBarOrder, closeAllFiles } = useAppStore(useShallow((s) => ({
    ...
    const worktreeFiles = useMemo(() => (activeWorktreeId ? openFiles.filter((f) => f.worktreeId === activeWorktreeId) : []), [activeWorktreeId, openFiles]);
    ...
    const unifiedTabs = useMemo(() => {
        ...
        const orderedIds = reconcileTabOrder(tabBarOrder, terminalIds, worktreeFiles.map((f) => f.id));
        return orderedIds.map((id) => ({
            type: (terminalIdSet.has(id) ? 'terminal' : 'editor'),
            id
        }));
    }, [tabs, worktreeFiles, tabBarOrder]);
    const [mountedWorktreeIds, setMountedWorktreeIds] = useState([]);
    const [initialTabCreationGuard, setInitialTabCreationGuard] = useState(null);
    ...
    if (activeWorktreeId !== prevActiveWorktreeIdRef.current ||
    ...
    const mountedWorktrees = allWorktrees.filter((worktree) => mountedWorktreeIds.includes(worktree.id));
    useEffect(() => {
        if (tabs.length === 0) {
            return;
        }
    ...
    const handleCloseOthers = useCallback((tabId) => closeOtherTerminalTabs(tabId, activeWorktreeId), [activeWorktreeId]);
    const handleCloseTabsToRight = useCallback((tabId) => closeTerminalTabsToRight(tabId, activeWorktreeId), [activeWorktreeId]);
    return {
        activeWorktreeId,
        activeView,
        tabsByWorktree,
