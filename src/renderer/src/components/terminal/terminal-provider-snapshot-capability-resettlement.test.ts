...
import {
  ...
  type EvictionExemptTerminalPtyRoute
} from '../terminal-pane/terminal-hidden-worktree-retention'
import type { TerminalParkWorktreeOwner } from '../terminal-pane/terminal-park-pty-restore-eligibility'
import {
...
const LOCAL_OWNER = { kind: 'local' } as const
const PANE_IDENTITY = { tabId: 'tab-1', leafId: 'leaf-1' } as const
...
describe('terminal provider snapshot capability re-settlement', () => {
  ...
  it('keeps an unknown-capability pty exempt during the outage (safety)', async () => {
    ...
    expect(isEvictionExemptTerminalPty(PTY_ID, WORKTREE_ID, LOCAL_OWNER, PANE_IDENTITY)).toBe(true)
  })
  ...
  it('re-consults a recovered resolver after the retry budget and clears the exemption', async () => {
    ...
    expect(isEvictionExemptTerminalPty(PTY_ID, WORKTREE_ID, LOCAL_OWNER, PANE_IDENTITY)).toBe(false)
  })
  ...
  it('classifies exemption routes in parity with the predicate', async () => {
    ...
    const sshId = 'ssh:conn-1@@pty-1'
    const sshOwner: TerminalParkWorktreeOwner = { kind: 'ssh', connectionId: 'conn-1' }
    const classify = (
      id: string | null,
      owner: TerminalParkWorktreeOwner = LOCAL_OWNER
    ): EvictionExemptTerminalPtyRoute | null =>
      classifyEvictionExemptTerminalPty(id, WORKTREE_ID, owner, PANE_IDENTITY)

    expect(classify(failOpenId)).toBe('fail-open')
    expect(classify(foreignId)).toBe('foreign-worktree')
    expect(classify(PTY_ID)).toBe('capability-unknown')
    // An SSH pty is exempt exactly when this worktree cannot prove it owns the connection.
    expect(classify(sshId, sshOwner)).toBeNull()
    expect(classify(sshId)).toBe('remote-unowned')
    ...
    expect(classify(PTY_ID)).toBeNull()

    for (const id of [failOpenId, foreignId, sshId, PTY_ID, null]) {
      expect(isEvictionExemptTerminalPty(id, WORKTREE_ID, LOCAL_OWNER, PANE_IDENTITY)).toBe(
        classify(id) !== null
      )
    }
  })
  ...
  it('buckets route counts per classification for the force-park breadcrumb', () => {
    const counts = countEvictionExemptTabRoutes(
      [
        { id: 'tab-1', ptyId: 'no-separator-daemon-fail-open' },
        { id: 'tab-2', ptyId: `other-worktree${PTY_SESSION_ID_SEPARATOR}session-9` },
        { id: 'tab-3', ptyId: PTY_ID },
        { id: 'tab-4', ptyId: 'ssh:conn-9@@pty-1' },
        { id: 'tab-5', ptyId: null }
      ],
      WORKTREE_ID,
      LOCAL_OWNER
    ...
    expect(formatEvictionExemptRouteCounts(counts)).toBe(
      'routes=fail-open:1,foreign:1,capability:1,remote-unowned:1,split-pane:1'
    )
  })
})
