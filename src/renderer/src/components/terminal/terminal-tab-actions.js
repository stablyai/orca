...
export function createNewTerminalTab(activeWorktreeId) {
    if (!activeWorktreeId) {
        return;
    }
    ...
    const editorIds = freshState.openFiles
        ...
        .map((f) => f.id);
    const base = reconcileTabOrder(freshState.tabBarOrderByWorktree[activeWorktreeId], termIds, editorIds);
...
export function closeOtherTerminalTabs(tabId, activeWorktreeId) {
    if (!activeWorktreeId) {
        return;
    }
    const state = useAppStore.getState();
...
export function closeTerminalTabsToRight(tabId, activeWorktreeId) {
    if (!activeWorktreeId) {
        return;
    }
    const state = useAppStore.getState();
    ...
    const orderedIds = reconcileTabOrder(state.tabBarOrderByWorktree[activeWorktreeId], terminalIds, currentEditorFiles.map((f) => f.id));
...
export function activateTerminalTab(tabId) {
    const s = useAppStore.getState();
    s.setActiveTab(tabId);
    s.setActiveTabType('terminal');
}
export function activateEditorFile(fileId) {
    const s = useAppStore.getState();
    s.setActiveFile(fileId);
    s.setActiveTabType('editor');
}
export function toggleTerminalPaneExpand(tabId) {
