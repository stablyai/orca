...
export type VisibleTabRef = {
  type: 'terminal' | 'editor' | 'browser' | 'simulator' | 'database'
  id: string
  tabId?: string
}
...
export function getGroupVisibleTabOrder(
  ...
  databaseTabIds: ReadonlySet<string> = new Set()
): VisibleTabRef[] {
  ...
  for (const unifiedId of group.tabOrder) {
    ...
    if (tab.contentType === 'terminal') {
      ...
      if (!databaseTabIds.has(tab.id) || seenDatabases.has(tab.id)) {
        continue
      }
      seenDatabases.add(tab.id)
      result.push({ type: 'database', id: tab.id, tabId: tab.id })
    } else {
...
export function getActiveTabNavOrder(
  ...
  for (const id of visibleIds) {
    ...
    } else if (databaseIdSet.has(id)) {
      result.push({ type: 'database', id })
    }
  }
  return result
}
