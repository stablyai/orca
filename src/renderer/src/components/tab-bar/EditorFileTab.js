...
import { X, FileCode, GitCompareArrows, Copy, ShieldAlert, ExternalLink, Columns2, Rows2 } from 'lucide-react';
...
export default function EditorFileTab({ file, isActive, hasTabsToRight, statusByRelativePath, onActivate, onClose, onCloseToRight, onCloseAll, onPin, onSplitGroup, dragData }) {
    ...
    const tabStatus = file.relativePath === 'All Changes'
        ? null
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { onContextMenuCapture: (event) => {
                    ...
                    setMenuOpen(true);
                        ...
                        if (e.button === 1) {
                            ...
                            onClose();
                    ...
                    }, children: [isConflictReview ? (_jsx(ShieldAlert, { className: `w-3.5 h-3.5 mr-1.5 shrink-0 ${isActive ? 'text-orange-400' : 'text-orange-400/70'}` })) : isDiff ? (_jsx(GitCompareArrows, { className: `w-3.5 h-3.5 mr-1.5 shrink-0 ${isActive ? 'text-foreground' : 'text-muted-foreground'}` })) : (_jsx(FileCode, { className: `w-3.5 h-3.5 mr-1.5 shrink-0 ${isActive ? 'text-foreground' : 'text-muted-foreground'}` })), _jsxs("span", { className: "mr-1.5 flex min-w-0 items-baseline gap-1.5", children: [isRenaming ? (_jsx("input", { ref: renameInputRef, defaultValue: basename(file.filePath), 
                                    // Tiny border to make the edit affordance obvious without
                                    // changing overall tab height. Size matches the label span.
                                    className: "truncate max-w-[130px] bg-transparent text-sm text-foreground outline-none border border-ring rounded-sm px-1 py-0", onPointerDown: (e) => e.stopPropagation(), onMouseDown: (e) => e.stopPropagation(), onClick: (e) => e.stopPropagation(), onDoubleClick: (e) => e.stopPropagation(), onKeyDown: (e) => {
                                        if (e.key === 'Enter') {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            commitRename();
                                    ...
                                    }, onBlur: commitRename })) : (_jsx("span", { className: `truncate max-w-[130px]${file.isPreview ? ' italic' : ''}`, style: tabStatusColor ? { color: tabStatusColor } : undefined, onDoubleClick: (e) => {
                                        ...
                                        e.stopPropagation();
                                        setIsRenaming(true);
                                    }, children: getEditorDisplayLabel(file) })), tabStatus && !isRenaming && (_jsx("span", { className: "shrink-0 text-[10px] leading-none font-semibold tracking-wide", style: { color: tabStatusColor }, children: STATUS_LABELS[tabStatus] }))] }), _jsxs("div", { className: "relative flex items-center justify-center w-4 h-4 shrink-0", children: [file.isDirty && (_jsx("span", { className: "absolute size-1.5 rounded-full bg-foreground/60 group-hover:hidden" })), _jsx("button", { className: `flex items-center justify-center w-4 h-4 rounded-sm ${file.isDirty
                                        ...
                                        e.stopPropagation();
                                        onClose();
                                    }, children: _jsx(X, { className: "w-3 h-3" }) })] })] }) }), _jsxs(DropdownMenu, { open: menuOpen, onOpenChange: setMenuOpen, modal: false, children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsx("button", { "aria-hidden": true, tabIndex: -1, className: "pointer-events-none fixed size-px opacity-0", style: { left: menuPoint.x, top: menuPoint.y } }) }), _jsxs(DropdownMenuContent, { className: "w-48", sideOffset: 0, align: "start", children: [_jsxs(DropdownMenuItem, { onSelect: () => onSplitGroup('up', file.tabId ?? file.id), children: [_jsx(Rows2, { className: "mr-1.5 size-3.5" }), "Split Up"] }), _jsxs(DropdownMenuItem, { onSelect: () => onSplitGroup('down', file.tabId ?? file.id), children: [_jsx(Rows2, { className: "mr-1.5 size-3.5" }), "Split Down"] }), _jsxs(DropdownMenuItem, { onSelect: () => onSplitGroup('left', file.tabId ?? file.id), children: [_jsx(Columns2, { className: "mr-1.5 size-3.5" }), "Split Left"] }), _jsxs(DropdownMenuItem, { onSelect: () => onSplitGroup('right', file.tabId ?? file.id), children: [_jsx(Columns2, { className: "mr-1.5 size-3.5" }), "Split Right"] }), _jsx(DropdownMenuSeparator, {}), _jsx(DropdownMenuItem, { onSelect: onClose, children: "Close" }), _jsx(DropdownMenuItem, { onSelect: onCloseAll, children: "Close All Editor Tabs" }), _jsx(DropdownMenuItem, { onSelect: onCloseToRight, disabled: !hasTabsToRight, children: "Close Tabs To The Right" }), _jsx(DropdownMenuSeparator, {}), _jsxs(DropdownMenuItem, { onSelect: () => {
                                ...
                                }, children: [_jsx(ExternalLink, { className: "w-3.5 h-3.5 mr-1.5" }), revealLabel] })] })] })] }));
}
