...
export function DatabaseResults({
  ...
  const rowKeys = useMemo(() => resultKeys(result?.rows ?? []), [result])
  const columnKeys = useMemo(() => resultKeys(result?.columns ?? []), [result])
  if (!result) {
    return (
      ...
      <table className="min-w-full border-separate border-spacing-0 text-xs">
        ...
        <tbody>
          {result.rows.map((row, rowIndex) => (
            <tr key={rowKeys[rowIndex]} className="hover:bg-accent/50">
...
function resultKeys(values: readonly unknown[]): string[] {
