...
describe('task page cache selectors', () => {
  ...
  it('scopes repo source rows by source cache scope for retry ownership', () => {
    const localRepo = {
      id: 'repo-1',
      path: '/same/path',
      sourceCacheScope: 'source:local:github:mcode-ide/mcode'
    }
    const sshRepo = {
      id: 'repo-1',
      path: '/same/path',
      sourceCacheScope: 'source:ssh:devbox:github:mcode-ide/mcode'
    }

    expect(buildTaskPageRepoSourceState([localRepo, sshRepo], [])).toMatchObject([
      {
        sourceKey: 'repo-1::source:local:github:mcode-ide/mcode'
      },
      {
        sourceKey: 'repo-1::source:ssh:devbox:github:mcode-ide/mcode'
      }
    ])
  })
