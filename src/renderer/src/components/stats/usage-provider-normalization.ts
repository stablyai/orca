...
import type { DevinUsageSummary } from '../../../../shared/devin-usage-types'
...
function getDevinNewInputTokens(summary: DevinUsageSummary | null): number {
  if (!summary) {
    return 0
  }
  return Math.max(summary.inputTokens - summary.cachedInputTokens, 0)
...
export function createDevinProvider(input: UsageOverviewInput['devin']): UsageProviderOverview {
  ...
  return {
    id: 'devin',
    label: translate('auto.components.stats.usage.overview.model.devin.label', 'Devin'),
    enabled: input.scanState?.enabled ?? false,
    isScanning: input.scanState?.isScanning ?? false,
    ...
    activityCount: summary?.events ?? 0,
    totalTokens: summary?.totalTokens ?? 0,
    newInputTokens: getDevinNewInputTokens(summary),
    outputTokens: summary?.outputTokens ?? 0,
    cacheTokens: summary?.cachedInputTokens ?? 0,
    reasoningTokens: summary?.reasoningOutputTokens ?? 0,
    estimatedCostUsd: summary?.estimatedCostUsd ?? null,
