...
export function useStatusBarController(floatingTerminalOpen: boolean) {
  ...
  const { claude, codex, gemini, opencodeGo, kimi, antigravity, minimax, grok, devin } = rateLimits
  ...
  const usageSettings = {
    ...
    devinAuthConfigured: rateLimits.devinAuthConfigured
  }
  ...
  const visibleDevin = getVisibleUsageProvider('devin', devin, usageSettings)
  const showClaude =
    visibleClaude !== null &&
  ...
  const isEmptyUsageState = isUsageEmptyState(
    { claude, codex, gemini, opencodeGo, kimi, antigravity, minimax, grok, devin },
    usageSettings
  )
