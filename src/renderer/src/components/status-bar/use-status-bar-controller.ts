...
export function useStatusBarController(floatingTerminalOpen: boolean) {
  ...
  const usageSettings = {
    ...
    openrouterApiKeyConfigured: rateLimits.openrouterApiKeyConfigured
  }
  ...
  const visibleOpenRouter = getVisibleUsageProvider('openrouter', openrouter, usageSettings)
  const showClaude =
    visibleClaude !== null &&
  ...
  // Why: OpenRouter is API-key auth, not a CLI on PATH, so detection-gating
  // doesn't apply — same as MiniMax and OpenCode Go.
  const showOpenRouter = visibleOpenRouter !== null && statusBarItems.includes('openrouter')
  ...
  const isEmptyUsageState = isUsageEmptyState(
    { claude, codex, cursor, openrouter, gemini, opencodeGo, kimi, antigravity, minimax, grok },
    usageSettings
  )
  ...
  const rosterProviders = [
    ...
    showOpenRouter ? visibleOpenRouter : null
