...
> & {
  // Why: main probes Windows Credential Manager `gemini:antigravity` each fetch.
...
export function hasUsageProviderSettingsForProvider(
  ...
  if (providerId === 'antigravity') {
    // Why: only auth-on-disk is durable; a checked item alone would pin a permanent "--" bar.
    return settings.antigravityAuthConfigured === true
  }
