...
describe('mergeSnapshotAndSessions', () => {
  ...
  it('includes browser-only workspaces in their repo', () => {
    const worktree = {
      id: 'mcode::/Users/me/browser-only',
      repoId: 'mcode',
    ...
    const out = mergeSnapshotAndSessions(
      ...
      [],
      baseCtx({
        repoDisplayNameById: new Map([['mcode', 'MCODE']]),
  ...
  it('passes through snapshot worktrees with numeric metrics and hasLocalSamples', () => {
    const wt: WorktreeMemory = {
      worktreeId: 'mcode::/Users/me/Triton',
      worktreeName: 'Triton',
      repoId: 'mcode',
      repoName: 'MCODE',
      cpu: 1.5,
  ...
  it('binds a deferred SSH row so its single-row kill cannot skip confirmation', () => {
    ...
    const sessionId = 'mcode::/remote/Stingray@@deferred1'
    const out = mergeSnapshotAndSessions(
      null,
      [{ id: sessionId, cwd: '', title: 'mcode/Stingray', agentOwnership: 'absent' as const }],
      baseCtx({
        tabsByWorktree: { 'mcode::/remote/Stingray': [makeTab('tab-ssh')] },
        deferredSshSessionIdsByTabId: { 'tab-ssh': sessionId },
        repoConnectionIdById: new Map([['mcode', 'ssh-conn-1']])
      })
    )
  ...
  it('@@ parse: an SSH-style session id resolves to its worktree group', () => {
    const ds: DaemonSession[] = [
      {
        id: 'mcode::/remote/Stingray@@abcd1234',
        cwd: '',
        title: 'mcode/Stingray',
    ...
    expect(out[0].worktrees[0]).toMatchObject({
      worktreeId: 'mcode::/remote/Stingray',
    ...
    expect(out[0].worktrees[0].sessions[0]).toMatchObject({
      sessionId: 'mcode::/remote/Stingray@@abcd1234',
      hasLocalSamples: false,
      cpu: null,
      memory: null,
      bound: false
  ...
  it('warm-reattach local PTY: chip stays off when repo has no connectionId', () => {
    ...
    const ds: DaemonSession[] = [
      {
        id: 'mcode::/local/Triton@@deadbeef',
        cwd: '/local/Triton',
        title: 'mcode/Triton',
        agentOwnership: 'absent' as const
      }
    ...
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['mcode', null]])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0]).toMatchObject({
      repoId: 'mcode',
  ...
  it('tab walk wins over @@ parse when they disagree', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: {
        'mcode::/correct/path': [makeTab(tabId, 'My Tab')]
      },
      ptyIdsByTabId: { [tabId]: ['mcode::/wrong/path@@feedface'] }
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].worktrees[0].worktreeId).toBe('mcode::/correct/path')
  ...
  it('treats startup deferred reattach tab ptyId wake hints as bound sessions', () => {
    ...
    const sessionId = 'mcode::/Users/me/Triton@@deferred'
    const ds: DaemonSession[] = [
      {
        id: sessionId,
        cwd: '/Users/me/Triton',
    ...
    const ctx = baseCtx({
      tabsByWorktree: {
        'mcode::/Users/me/Triton': [restoredTab]
      },
      ptyIdsByTabId: { [tabId]: [] }
    })
  ...
  it('local-bound interaction state: numeric metrics + bound=true + tabId set', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: { 'mcode::/Users/me/Triton': [makeTab(tabId)] },
  ...
  it('uses repoDisplayNameById to humanize new project groups when available', () => {
    const ds: DaemonSession[] = [
      { id: 'stably-ai/mcode::/remote/Wt@@1', cwd: '', title: '', agentOwnership: 'absent' as const }
    ]
    const ctx = baseCtx({
      repoDisplayNameById: new Map([['stably-ai/mcode', 'MCODE']])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].repoName).toBe('MCODE')
  })
