...
const FRAME_COPY: readonly FrameCopy[] = [
  {
    id: 1,
    title: 'Workspaces',
    caption:
...
function WorkingSpinner({ size = 'sm' }: { size?: 'sm' | 'xs' }): JSX.Element {
  // Why: matches AgentStateDot's 'working' indicator so the tour preview
  // shows the same yellow spinning ring users see in the real app.
        ...
        <div className="grid flex-1 grid-cols-2 divide-x divide-border font-mono text-[10px] leading-[1.4] text-foreground">
          <div className="p-2">
            <div className="flex items-center gap-1">
              <span className="text-emerald-500">~/orca</span>
              <span className="text-muted-foreground">$</span>
              <span className="feature-tour-terminal-line relative inline-block overflow-hidden whitespace-nowrap text-foreground">
                pnpm test --filter web
                <span className="feature-tour-terminal-caret absolute -right-[3px] top-0 inline-block h-[11px] w-[1.5px] bg-foreground" />
              </span>
            </div>
            <div className="mt-1.5 flex flex-col gap-1">
              <div className="feature-tour-terminal-output flex items-center gap-2" data-line="1">
                <span className="size-1 rounded-full bg-emerald-500/80" />
                <span className="h-1.5 w-[60%] rounded-full bg-foreground/20" />
              </div>
              <div className="feature-tour-terminal-output flex items-center gap-2" data-line="2">
                <span className="size-1 rounded-full bg-emerald-500/80" />
                <span className="h-1.5 w-[78%] rounded-full bg-foreground/20" />
              </div>
            </div>
          </div>
          <div className="p-2">
            ...
            <div className="mt-1.5 flex flex-col gap-1">
              <div className="feature-tour-terminal-output flex items-center gap-2" data-line="2">
                <span className="size-1 rounded-full bg-sky-500/80" />
                <span className="h-1.5 w-[70%] rounded-full bg-foreground/20" />
              </div>
              <div className="feature-tour-terminal-output flex items-center gap-2" data-line="3">
                <span className="size-1 rounded-full bg-sky-500/80" />
                <span className="h-1.5 w-[55%] rounded-full bg-foreground/20" />
              </div>
            </div>
          </div>
        </div>
    ...
    >
      ...
      <Card className="gap-0 overflow-hidden py-0" role="complementary" aria-label="Explore Orca">
        <div className="flex flex-col gap-3 p-3.5">
          <div
            className="relative h-[180px] w-full overflow-hidden rounded-lg border border-border bg-muted/40"
            aria-hidden
          ...
          <div className="min-w-0 space-y-0.5">
            ...
            {/* Why: pre-reserve the height of the tallest caption so the card
                doesn't reflow as the per-frame copy cross-fades. */}
            <div className="relative min-h-[56px]" data-feature-tour-nudge-caption>
              {FRAME_COPY.map((frame) => (
                <div key={frame.id} className="feature-tour-copy-slide" data-frame={frame.id}>
                  <h3 className="truncate text-sm font-semibold leading-tight">{frame.title}</h3>
                  <p className="text-xs leading-snug text-muted-foreground">{frame.caption}</p>
                </div>
              ))}
            </div>
          </div>
