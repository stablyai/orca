...
export function VoloSetupSteps(
  props: ConnectStepProps & { onConnected: () => void; onOpenIntegrations: () => void }
): React.JSX.Element {
  const [dialogOpen, setDialogOpen] = useState(false)
    ...
    <>
      <ol className="divide-y divide-border/50">
        <TaskSourceStepRow
          index={1}
          state={getConnectStepState(props)}
          title={translate('auto.components.settings.TasksPane.connectVoloTitle', 'Connect Volo')}
          description={translate(
            'auto.components.settings.TasksPane.connectVoloDescription',
            'Sign in to Volo with Google, then show boards in Tasks.'
          )}
          action={
            <Button
              type="button"
            ...
            >
              {props.connected
                ? translate('auto.components.settings.TasksPane.manageVolo', 'Manage Volo')
                : translate('auto.components.settings.TasksPane.addVolo', 'Add Volo access')}
            </Button>
          }
        />
        <TaskSourceShowInTasksStep
          index={2}
          providerLabel={translate('auto.components.settings.TasksPane.voloLabel', 'Volo')}
          visible={props.visible}
          canHide={props.canHide}
          onToggleVisible={props.onToggleVisible}
        />
