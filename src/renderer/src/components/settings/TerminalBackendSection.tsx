      ...
      <div className="divide-y divide-border/40">
        ...
        <SettingsRow
          ...
          description={translate(
            ...
            "Managed uses Orca's verified sidecar. System resolves Herdr on the execution host PATH."
          )}
          control={
            <SettingsSegmentedControl
              ariaLabel={translate(
              ...
              value={settings.herdrBinarySource?.kind ?? 'managed'}
              onChange={(value) => {
                if (value === 'custom') {
                  updateSettings({ herdrBinarySource: { kind: 'custom', path: '' } })
                  return
                }
                updateSettings({ herdrBinarySource: { kind: value as 'managed' | 'system' } })
              }}
              options={[
                {
                  value: 'managed',
                  label: translate(
                    'auto.components.settings.TerminalBackendSection.managed',
                    'Managed'
                  )
                },
                {
              ...
              <Input
                ...
                onBlur={(event) => {
                  if (!event.target.value.trim()) {
                    updateSettings({ herdrBinarySource: { kind: 'managed' } })
                  }
                }}
              />
            }
