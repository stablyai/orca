  ...
  return [
    ...
    },
    {
      ...
      description: translate(
        ...
        'Ignore mcode.yaml; run only your local commands.'
      )
    },
    {
      policy: 'run-both',
      ...
      description: translate(
        ...
        'mcode.yaml first, then your local commands.'
      )
    }
  ]
}
  ...
  return [
    {
      ...
      placeholder: translate(
        ...
        '# e.g. pnpm install cp "$MCODE_ROOT_PATH/.env" "$MCODE_WORKTREE_PATH/.env"'
      )
    },
    {
      name: 'archive',
      ...
      placeholder: translate(
        ...
        '# e.g. echo "Cleaning up $MCODE_WORKSPACE_NAME"'
      )
    }
  ]
}
    ...
    case 'update-available':
      return {
        heading: translate(
          ...
          '`mcode.yaml` could not be parsed'
        ),
        description: translate(
          'auto.components.settings.RepositoryHooksSection.aba825233f',
          'The file contains configuration keys that this version of MCode does not recognize. You may need to update MCode, or check the file for typos.'
        )
      }
    case 'invalid':
      return {
        ...
        description: translate(
          ...
          'The core configuration file exists in the repo root, but MCode could not parse the supported hook definitions yet.'
        )
      }
    default:
      return {
        heading: translate(
          ...
          'No `mcode.yaml` detected'
        ),
        description: translate(
          'auto.components.settings.RepositoryHooksSection.b20c5df6ca',
          'Add an `mcode.yaml` file to enable shared setup, archive, or issue-automation defaults for this repo. Example template:'
        )
      }
  }
}
    ...
    <div className="space-y-2">
      <p className="text-[10px] tracking-[0.18em] text-muted-foreground">
        ...
        <code className="rounded bg-muted px-1 py-0.5">
          {translate('auto.components.settings.RepositoryHooksSection.39da2ae12f', 'mcode.yaml')}
        </code>{' '}
          ...
          <p className="text-xs leading-5 text-muted-foreground">
            {isChecking
              ? translate(
                  ...
                  'Local scripts are saved. MCode is still checking mcode.yaml before it can recommend which script source to use.'
                )
              : translate(
                  'auto.components.settings.RepositoryHooksSection.0ce113fd7b',
                  'Local scripts are saved, but Script Source is set to mcode.yaml only.'
                )}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          {translate(
            ...
            'Scripts that run when worktrees are created or archived. Local scripts are stored on this machine; `mcode.yaml` scripts are shared with your team.'
          )}
        </p>
      </div>
                    ...
                    <div className="space-y-2 text-xs text-muted-foreground">
                      <p>
                        {translate(
                          ...
                          'The file is present, but MCode could not find valid `scripts` or `issueCommand` definitions.'
                        )}
                      </p>
...
function renderYamlScriptPreview(hooks: MCodeHooks | null): string {
