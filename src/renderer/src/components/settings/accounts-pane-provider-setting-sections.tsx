...
import { GeminiIcon, GlmIcon, OpenCodeGoIcon } from '../status-bar/icons'
...
export function renderGlmAccountsSection(model: AccountsPaneSectionModel): React.JSX.Element {
  const { settings, updateSettings, glmRateLimits } = model
  return (
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <SettingsSegmentedControl<'zai' | 'zhipu'>
            ...
            ariaLabel={translate('auto.components.settings.AccountsPane.glmPlatform', 'GLM platform')}
            size="sm"
            options={[
        ...
        {glmRateLimits?.status === 'ok' && (glmRateLimits?.session || glmRateLimits?.weekly) ? (
          <div className="flex items-center gap-2 text-xs">
            <Badge variant="secondary" className="tabular-nums">
              {glmRateLimits.session ? `${Math.round(glmRateLimits.session.usedPercent)}% (5h)` : null}
              {glmRateLimits.session && glmRateLimits.weekly ? ' · ' : null}
              {glmRateLimits.weekly ? `${Math.round(glmRateLimits.weekly.usedPercent)}% (wk)` : null}
            </Badge>
          </div>
        ) : null}
      </div>
