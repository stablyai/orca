...
const SHORTCUT_GROUP_DEFINITIONS = [
    {
        title: 'Global',
        items: [
            {
...
export const SHORTCUTS_PANE_SEARCH_ENTRIES = SHORTCUT_GROUP_DEFINITIONS.flatMap((group) => group.items.map((item) => ({
    title: item.action,
...
export function ShortcutsPane() {
    ...
    const groups = useMemo(() => SHORTCUT_GROUP_DEFINITIONS.map((group) => ({
        title: group.title,
    ...
    const groupEntries = useMemo(() => Object.fromEntries(SHORTCUT_GROUP_DEFINITIONS.map((groupDef) => [
        groupDef.title,
    ...
    return (_jsx("div", { className: "space-y-8", children: _jsxs("section", { className: "space-y-4", children: [_jsxs("div", { className: "space-y-1", children: [_jsx("h2", { className: "text-sm font-semibold", children: "Keyboard Shortcuts" }), _jsx("p", { className: "text-xs text-muted-foreground", children: "View common hotkeys used across the application. Shortcuts customization is not currently supported." })] }), _jsx("div", { className: "grid gap-8", children: groups
                        .filter((group) => matchesSettingsSearch(searchQuery, groupEntries[group.title] ?? []))
                        .map((group) => (_jsxs("div", { className: "space-y-3", children: [_jsx("h3", { className: "border-b border-border/50 pb-2 text-sm font-medium text-muted-foreground", children: group.title }), _jsx("div", { className: "grid gap-2", children: group.items.map((item, idx) => {
                                    ...
                                    return (_jsxs(SearchableSetting, { title: item.action, description: `${group.title} shortcut`, keywords: keywords, className: "flex items-center justify-between py-1", children: [_jsx("span", { className: "text-sm text-foreground", children: item.action }), _jsx(ShortcutKeyCombo, { keys: item.keys })] }, idx));
                                }) })] }, group.title))) })] }) }));
}
