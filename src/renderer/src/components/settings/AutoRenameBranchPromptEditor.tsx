  ...
  return (
    ...
    <div className="space-y-2 py-2">
      <div className="space-y-0.5">
        <Label htmlFor="git-auto-rename-branch-name-prompt">{translate("auto.components.settings.AutoRenameBranchPromptEditor.7d6176f506", "Prompt")}</Label>
        <p className="text-xs text-muted-foreground">
          ...
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                {translate("auto.components.settings.AutoRenameBranchPromptEditor.182d419b97", "built-in branch-name prompt")}</button>
            </PopoverTrigger>
            <PopoverContent
              align="start"
              side="bottom"
          ...
          <code className="font-mono">{translate("auto.components.settings.AutoRenameBranchPromptEditor.ebb942a2ec", "fix-login-flow")}</code>{translate("auto.components.settings.AutoRenameBranchPromptEditor.39278f4411", "; your branch prefix setting still applies.")}</p>
      </div>
      <textarea
        ...
        <div className="flex items-center gap-2">
          {dirty ? (
            <Button type="button" variant="ghost" size="xs" onClick={onDiscard} disabled={saving}>
              {translate("auto.components.settings.AutoRenameBranchPromptEditor.63121132c0", "Discard")}</Button>
          ) : null}
          <Button
            type="button"
            variant="secondary"
