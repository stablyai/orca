  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card/50 p-4">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-medium">{translate("auto.components.settings.BrowserUseExamples.2a180694f7", "Try it — example prompts")}</p>
      </div>
      <p className="mt-1 text-xs text-muted-foreground">
        {translate("auto.components.settings.BrowserUseExamples.c5325e91f6", "Paste any of these into Claude Code, Codex, or another agent in a project where the skill is installed.")}</p>
        ...
        {EXAMPLE_PROMPTS.map((prompt) => (
          ...
          >
            <p className="flex-1 text-[11px] leading-relaxed text-foreground/90">
              {translate("auto.components.settings.BrowserUseExamples.59722f31b4", "\"")}{prompt}{translate("auto.components.settings.BrowserUseExamples.b84807f228", "\"")}</p>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    aria-label={translate("auto.components.settings.BrowserUseExamples.1188e56af4", "Copy example prompt")}
                  >
                    <Copy className="size-3.5" />
                  </Button>
                </TooltipTrigger>
                <TooltipContent side="left" sideOffset={6}>
                  {translate("auto.components.settings.BrowserUseExamples.1199258ace", "Copy")}</TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </li>
        ))}
