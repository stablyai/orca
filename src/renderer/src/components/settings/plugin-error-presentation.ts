...
export function pluginInstallErrorMessage(cause: unknown): string {
  ...
  if (detail.includes('mcode-plugin.json') && /(missing|unreadable|no )/.test(detail)) {
    return translate(
      'auto.components.settings.pluginError.installManifestMissing',
      "No readable mcode-plugin.json was found. Choose the plugin's root folder."
    )
  }
  ...
  if (detail.includes('requires mcode')) {
    return translate(
      'auto.components.settings.pluginError.incompatible',
      'This plugin requires a different MCode version.'
    )
  }
  ...
  if (/(exceeds|too many)/.test(detail)) {
    return translate(
      ...
      "The plugin exceeds MCode's install size or file-count limits."
    )
  }
  if (/(git|repository|fetch|clone|checkout|remote)/.test(detail)) {
    return translate(
      'auto.components.settings.pluginError.installGit',
      'MCode could not fetch the pinned Git revision. Check the URL, #ref, access, and system Git setup.'
    )
  }
  return translate(
...
export function invalidPluginErrorMessage(detailValue: string): string {
  ...
  if (detail.includes('missing mcode-plugin.json')) {
    return translate(
      'auto.components.settings.pluginError.invalidManifestMissing',
      'The plugin root is missing mcode-plugin.json. Add it, then refresh plugins.'
    )
  }
  if (detail.includes('invalid manifest')) {
    return translate(
      'auto.components.settings.pluginError.invalidManifest',
      'mcode-plugin.json is invalid. Fix it, then refresh plugins.'
    )
  }
