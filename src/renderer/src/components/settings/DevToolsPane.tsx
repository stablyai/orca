...
// Dev-only preview of the first-party MCode Cloud sign-in. The sidebar/titlebar
...
function MCodeCloudDevSubsection(): React.JSX.Element {
  const authStatus = useAppStore((s) => s.mcodeProfileAuthStatus)
  const connecting = useAppStore((s) => s.mcodeProfileConnecting)
  const connect = useAppStore((s) => s.connectCurrentMCodeProfile)
  const signOut = useAppStore((s) => s.signOutCurrentMCodeProfile)
  const refresh = useAppStore((s) => s.fetchMCodeProfileAuthStatus)
    ...
    <section className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        <SettingsSubsectionHeader
          title={translate('auto.components.settings.DevToolsPane.mcodeCloud', 'MCode Cloud')}
          description={translate(
            'auto.components.settings.DevToolsPane.mcodeCloudDescription',
            'Dev-only preview of first-party cloud sign-in. Hidden in production; in dev it also appears in the sidebar account switcher once MCODE_CLOUD_API_URL and MCODE_CLOUD_CLIENT_ID are set.'
          )}
        />
      ...
      {configured ? (
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground">
            {translate('auto.components.settings.DevToolsPane.mcodeCloudStatus', 'Status')}:{' '}
          ...
          <div className="flex flex-wrap gap-2">
            {connected ? (
              ...
              >
                {translate('auto.components.settings.DevToolsPane.mcodeCloudSignOut', 'Sign out')}
              </Button>
            ) : (
              <Button
                type="button"
              ...
              >
                {translate(
                  'auto.components.settings.DevToolsPane.mcodeCloudConnect',
                  'Connect profile'
                )}
              </Button>
            )}
            <Button type="button" variant="ghost" size="sm" onClick={() => void refresh()}>
              {translate(
                'auto.components.settings.DevToolsPane.mcodeCloudRefresh',
                'Refresh status'
              )}
            </Button>
          </div>
        ...
        <p className="text-xs text-muted-foreground">
          {authStatus?.setupMessage ??
            translate(
              'auto.components.settings.DevToolsPane.mcodeCloudNotConfigured',
              'Set MCODE_CLOUD_API_URL and MCODE_CLOUD_CLIENT_ID to preview MCode Cloud sign-in in this dev build.'
            )}
        </p>
      )}
    </section>
