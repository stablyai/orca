    ...
    <div className="rounded-2xl border border-border/60 bg-card/30 p-4">
      <div className="flex items-center justify-between gap-3">
        <div className="space-y-0.5">
          <p className="text-sm font-semibold">{translate("auto.components.settings.MobileEmulatorAgentControlRow.2a674aa810", "Agent Mobile Emulator Control")}</p>
          <p className="text-xs text-muted-foreground">
            {translate("auto.components.settings.MobileEmulatorAgentControlRow.ff4b7e65d6", "Let coding agents control the active mobile emulator with Orca CLI commands.")}</p>
        </div>
        <span
      ...
      <div className="mt-3 divide-y divide-border/40">
        <div className="flex items-start gap-3 py-3">
          ...
          <div className="min-w-0 flex-1 space-y-1">
            <p className="text-sm font-medium">{translate("auto.components.settings.MobileEmulatorAgentControlRow.4f2205f3b6", "Enable Orca CLI")}</p>
            <p className="text-xs text-muted-foreground">
              {translate("auto.components.settings.MobileEmulatorAgentControlRow.2fef055608", "Registers the Orca CLI command so agents can control the active emulator from their shell.")}</p>
        ...
        <div className={cn('py-3', step2Blocked && 'opacity-60')}>
          <AgentSkillSetupPanel
            variant="inline"
            title={translate("auto.components.settings.MobileEmulatorAgentControlRow.67e19ee03c", "Orca CLI skill")}
            description={translate("auto.components.settings.MobileEmulatorAgentControlRow.d94ca6a623", "Enables agents to use Orca CLI commands, including mobile emulator control.")}
        ...
        <div className="py-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium">{translate("auto.components.settings.MobileEmulatorAgentControlRow.c7f3fe0a6e", "Common emulator commands")}</p>
          </div>
          <p className="mt-1 text-xs text-muted-foreground">
            {translate("auto.components.settings.MobileEmulatorAgentControlRow.8af7a8bc38", "Commands target the active emulator for the current worktree. Coordinates are normalized from 0..1.")}</p>
