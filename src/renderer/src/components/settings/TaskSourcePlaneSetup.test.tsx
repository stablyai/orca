...
vi.mock('@/lib/plane-agent-skill-update-command', () => ({
  getPlaneAgentSkillUpdateTarget: () => ({
    command: 'npx skills update orca-plane --global',
    skillName: 'orca-plane'
  })
}))
...
vi.mock('@/store', () => ({
  useAppStore: (selector: (state: { checkPlaneConnection: () => void }) => unknown) =>
    selector({ checkPlaneConnection: mocks.checkPlaneConnection })
}))
...
describe('TaskSourcePlaneSetup', () => {
  ...
  it('blocks first-time skill install until Plane is connected', () => {
    const markup = renderSetup({ connected: false })

    expect(markup).toContain('Connect Plane first')
  ...
  it('shows the skill panel as done when the skill is already installed without a connection', () => {
    ...
    expect(markup).not.toContain('Connect Plane first')
  ...
  it('routes connected credential management to Integrations', () => {
    ...
    fireEvent.click(rendered.getByRole('button', { name: 'Manage access' }))
