    ...
    <div className="flex items-center justify-between gap-4 py-2">
      <div className="min-w-0 space-y-0.5">
        <div className="flex items-center gap-2">
          ...
          <Label>{translate("auto.components.settings.OpenAiTranscriptionSettingsRow.27e0cb656d", "OpenAI Transcription")}</Label>
          {configured && (
            <span className="flex items-center gap-1 text-xs text-muted-foreground">
              <CheckCircle2 className="size-3.5" />
              {translate("auto.components.settings.OpenAiTranscriptionSettingsRow.3b0ab3fc0b", "Connected")}</span>
          )}
        </div>
      ...
      {configured ? (
        <div className="flex shrink-0 items-center gap-1.5">
          <Button variant="outline" size="sm" disabled={disabled} onClick={onConfigure}>
            {translate("auto.components.settings.OpenAiTranscriptionSettingsRow.a622bc3b37", "Replace key")}</Button>
          <button
            onClick={onClear}
            aria-label={translate("auto.components.settings.OpenAiTranscriptionSettingsRow.ae2df8f511", "Disconnect OpenAI API key")}
            disabled={disabled}
        ...
        <Button variant="outline" size="sm" disabled={disabled} onClick={onConfigure}>
          {translate("auto.components.settings.OpenAiTranscriptionSettingsRow.85c589cd61", "Add API key")}</Button>
      )}
    </div>
  )
}
