...
import { usePeerCollabSavedPairing } from './use-peer-collab-saved-pairing'
...
import {
  ...
  type RemoteTerminalDialogTarget
} from '@/components/peer-collab/RemoteTerminalDialog'
...
export function PeerCollabSettingsPane(): React.JSX.Element {
  ...
  // Why: distinct from `hostTerminals` above, which is the *remote* host's
  // terminals seen while connected as a peer client; this is this desktop's
  // own terminals, used for the grant picker when acting as the host.
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => void loadHostTerminals(), CONNECTED_CLIENTS_POLL_MS)
    return () => window.clearInterval(timer)
