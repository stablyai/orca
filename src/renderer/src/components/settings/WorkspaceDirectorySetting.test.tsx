...
describe('WorkspaceDirectorySetting', () => {
  it('keeps typed workspace paths local until blur', () => {
    ...
    expect(updateSettings).toHaveBeenCalledWith({ workspaceDir: 'mcode-workspaces' })
  })
  ...
  it('does not commit Enter while IME composition is active', () => {
    ...
    expect(getInput().value).toBe('mcode-workspaces')
    expect(updateSettings).not.toHaveBeenCalled()
  })
  ...
  it('allows retrying the same draft when the persisted value has not changed', () => {
    ...
    expect(updateSettings).toHaveBeenNthCalledWith(1, { workspaceDir: 'mcode-workspaces' })
    expect(updateSettings).toHaveBeenNthCalledWith(2, { workspaceDir: 'mcode-workspaces' })
  })
})
