...
export function DeepgramTranscriptionKeyDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Deepgram Transcription</DialogTitle>
          <DialogDescription>
            Audio is sent to Deepgram only when Deepgram Nova-3 is selected.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-2">
          <Label htmlFor="deepgram-speech-api-key">API Key</Label>
          <Input
            id="deepgram-speech-api-key"
            type="password"
            value={apiKeyDraft}
            placeholder={configured ? 'API key configured' : 'Deepgram API key'}
            disabled={pending}
