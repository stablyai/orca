...
describe('provider rate-limit panels account scope', () => {
  it('shows the local host scope for GitHub API budget', () => {
    ...
    expect(markup).toContain(
      'GitHub API budget is fetched from the CLI on this desktop client. Use Settings &gt; Remote MCode Servers &gt; Advanced to view server-owned budgets.'
    )
  ...
  it('shows the remote server scope for GitLab API budget', () => {
    ...
    expect(markup).toContain(
      'GitLab API budget is fetched from the CLI on this remote server. Use Settings &gt; Remote MCode Servers &gt; Advanced to view another default runtime budget.'
    )
  })
})
