...
describe('browser notice formatting', () => {
    ...
    it('formats download completion and byte counts', () => {
        expect(formatDownloadFinishedNotice({
    ...
    it('formats load failure copy for localhost and remote pages', () => {
        expect(formatLoadFailureDescription({
            code: -102,
        ...
        expect(formatLoadFailureRecoveryHint({
            host: 'localhost:3000',
            isLocalhostLike: true
        })).toBe('If this should be a local app, make sure the server is running and listening on the expected port.');
        expect(formatLoadFailureDescription({
            code: -105,
