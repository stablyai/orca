...
import { ArrowLeft, ArrowRight, CircleCheck, Copy, Crosshair, ExternalLink, Globe, Image, Import, Loader2, OctagonX, RefreshCw, Settings, SquareCode } from 'lucide-react';
...
import { clearLiveBrowserUrl, consumeEvictedBrowserTab, markEvictedBrowserTab, rememberLiveBrowserUrl } from './browser-runtime';
import { useGrabMode } from './useGrabMode';
import { formatGrabPayloadAsText } from './GrabConfirmationSheet';
import { isEditableKeyboardTarget } from './browser-keyboard';
import BrowserAddressBar from './BrowserAddressBar';
import BrowserFind from './BrowserFind';
import { consumeBrowserFocusRequest, ORCA_BROWSER_FOCUS_REQUEST_EVENT } from './browser-focus';
import { formatByteCount, formatDownloadFinishedNotice, formatLoadFailureDescription, formatLoadFailureRecoveryHint, formatPermissionNotice, formatPopupNotice } from './browser-notices';
const webviewRegistry = new Map();
const registeredWebContentsIds = new Map();
const parkedAtByTabId = new Map();
let hiddenContainer = null;
const DRAG_LISTENER_KEY = '__orcaBrowserPaneDragListeners';
const MAX_PARKED_WEBVIEWS = 6;
const EMPTY_BROWSER_PAGES = [];
...
function setWebviewsDragPassthrough(passthrough) {
...
if (typeof window !== 'undefined') {
    ...
    const dragstart = () => setWebviewsDragPassthrough(true);
    const dragend = () => setWebviewsDragPassthrough(false);
    const drop = () => setWebviewsDragPassthrough(false);
...
export function destroyPersistentWebview(browserTabId) {
...
function getBrowserDisplayTitle(title, url) {
    if (url === 'about:blank' ||
...
function getLoadErrorMetadata(loadError) {
    ...
    try {
        ...
        const isLocalhostLike = hostname === 'localhost' ||
            hostname === '127.0.0.1' ||
            hostname === '0.0.0.0' ||
            hostname === '::1';
...
function getOpenableExternalUrl(webview, fallbackUrl) {
...
function getCurrentBrowserUrl(webview, fallbackUrl) {
    let currentUrl = fallbackUrl;
    if (webview) {
        try {
            currentUrl = webview.getURL() || fallbackUrl;
...
function retryBrowserTabLoad(webview, browserTab, onUpdatePageState) {
    if (!webview) {
        return;
    }
    const retryUrl = normalizeBrowserNavigationUrl(browserTab.loadError?.validatedUrl ?? browserTab.url);
    if (!retryUrl) {
        return;
    }
...
function evictParkedWebviews(excludedTabId = null) {
    ...
    const parkedBrowserTabIds = [...webviewRegistry.entries()]
        .filter(([browserTabId, webview]) => browserTabId !== excludedTabId && webview.parentElement === hidden)
...
export default function BrowserPane({ browserTab, isActive }) {
    ...
    const activeBrowserPage = browserPages.find((page) => page.id === browserTab.activePageId) ?? browserPages[0] ?? null;
    ...
    return (_jsx("div", { className: "relative flex h-full min-h-0 flex-1 flex-col", children: activeBrowserPage ? (_jsx("div", { className: "relative flex min-h-0 flex-1", children: _jsx(BrowserPagePane, { browserTab: activeBrowserPage, workspaceId: browserTab.id, worktreeId: browserTab.worktreeId, sessionProfileId: browserTab.sessionProfileId ?? null, isActive: isActive, onUpdatePageState: updateBrowserPageState, onSetUrl: setBrowserPageUrl }) })) : null }));
}
function BrowserPagePane({ browserTab, workspaceId, worktreeId, sessionProfileId, isActive, onUpdatePageState, onSetUrl }) {
    const containerRef = useRef(null);
    const addressBarInputRef = useRef(null);
    const webviewRef = useRef(null);
    const browserTabIdRef = useRef(browserTab.id);
    browserTabIdRef.current = browserTab.id;
    const faviconUrlRef = useRef(browserTab.faviconUrl);
    ...
    const lastKnownWebviewUrlRef = useRef(null);
    ...
    const downloadStateRef = useRef(null);
    ...
    useEffect(() => {
        ...
        if (browserSessionImportState.status === 'success' && browserSessionImportState.summary) {
            ...
            setResourceNotice(`Imported ${importedCookies} cookies for ${domainPreview}${more}. Reload the page to use them.`);
    ...
    const [grabToast, setGrabToast] = useState(null);
    const grabToastTimerRef = useRef(undefined);
    ...
    const showGrabToast = useCallback((message, type, payload) => {
        let x = 0;
        let y = 0;
        let below = true;
    ...
    useEffect(() => {
        setResourceNotice(consumeEvictedBrowserTab(browserTab.id)
    ...
    useEffect(() => {
        return window.api.browser.onContextMenuRequested((event) => {
            ...
            console.debug('[context-menu] screen=(%d,%d) window=(%d,%d) zoom=%.2f → viewport=(%d,%d)', event.screenX, event.screenY, window.screenX, window.screenY, zoomFactor, x, y);
            setContextMenu({
                x,
                y,
    ...
    useEffect(() => {
        return window.api.browser.onDownloadProgress((event) => {
    ...
    useEffect(() => {
        return window.api.browser.onDownloadFinished((event) => {
    ...
    useEffect(() => {
        ...
        const handleBrowserFocusRequest = (event) => {
    ...
    const syncNavigationState = useCallback((webview) => {
        try {
            onUpdatePageStateRef.current(browserTab.id, {
                title: getBrowserDisplayTitle(webview.getTitle(), webview.getURL() || browserTabUrlRef.current),
    ...
    useEffect(() => {
        ...
        const handleDidStopLoading = () => {
            ...
            if (activeLoadFailure) {
                const normalizedAttemptedUrl = normalizeBrowserNavigationUrl(activeLoadFailure.validatedUrl) ??
        ...
        const handleFindCloseOnNavigate = () => {
            setFindOpen(false);
        };
        ...
        if (needsInitialNavigation) {
            ...
            const initialUrl = normalizeBrowserNavigationUrl(initialBrowserUrlRef.current) ?? ORCA_BROWSER_BLANK_URL;
    ...
    useEffect(() => {
        ...
        if (normalizedLiveUrl !== normalizedUrl &&
    ...
    const handleGrabActionShortcut = useCallback((key) => {
        const copyFromPayload = (payload) => {
            if (key === 'c') {
            ...
            void (async () => {
                ...
                if (key === 's') {
                    try {
                        ...
                        if (ssResult.ok) {
                            payload.screenshot = ssResult.screenshot;
                        }
                    }
                    catch {
    ...
    return (_jsxs("div", { className: cn('absolute inset-0 flex min-h-0 flex-1 flex-col', isActive ? 'z-10' : 'pointer-events-none hidden'), children: [contextMenu
                ? createPortal(_jsxs(_Fragment, { children: [_jsx("div", { className: "fixed inset-0 z-50", onPointerDown: () => setContextMenu(null) }), _jsxs("div", { ref: contextMenuRef, role: "menu", "data-testid": "browser-context-menu", style: { left: contextMenu.x, top: contextMenu.y }, className: "fixed z-50 min-w-[13rem] overflow-hidden rounded-[11px] border border-black/14 bg-[rgba(255,255,255,0.82)] p-1 text-black shadow-[0_16px_36px_rgba(0,0,0,0.24),inset_0_1px_0_rgba(255,255,255,0.14)] backdrop-blur-2xl dark:border-white/14 dark:bg-[rgba(0,0,0,0.72)] dark:text-white dark:shadow-[0_20px_44px_rgba(0,0,0,0.42),inset_0_1px_0_rgba(255,255,255,0.04)]", children: [contextMenu.linkUrl ? (_jsxs(_Fragment, { children: [_jsx("button", { role: "menuitem", className: "relative flex w-full cursor-default items-center gap-2 rounded-[7px] px-2 py-0.5 text-[12px] leading-5 font-medium outline-none select-none hover:bg-black/8 dark:hover:bg-white/14", onClick: () => {
                                                createBrowserTab(worktreeId, contextMenu.linkUrl, {
                                                    title: contextMenu.linkUrl
                                                });
                                                setContextMenu(null);
                                            }, children: "Open Link In Orca Browser" }), _jsx("button", { role: "menuitem", className: "relative flex w-full cursor-default items-center gap-2 rounded-[7px] px-2 py-0.5 text-[12px] leading-5 font-medium outline-none select-none hover:bg-black/8 dark:hover:bg-white/14", onClick: () => {
                                                const targetUrl = normalizeExternalBrowserUrl(contextMenu.linkUrl);
                                                if (targetUrl) {
                                                    void window.api.shell.openUrl(targetUrl);
                                                }
                                                setContextMenu(null);
                                            }, children: "Open Link In Default Browser" }), _jsx("button", { role: "menuitem", className: "relative flex w-full cursor-default items-center gap-2 rounded-[7px] px-2 py-0.5 text-[12px] leading-5 font-medium outline-none select-none hover:bg-black/8 dark:hover:bg-white/14", onClick: () => {
                                                void window.api.ui.writeClipboardText(contextMenu.linkUrl ?? '');
                                                setContextMenu(null);
                                            }, children: "Copy Link Address" }), _jsx("div", { className: "my-1 h-px bg-border/70" })] })) : null, _jsx("button", { role: "menuitem", disabled: !browserTab.canGoBack, className: "relative flex w-full cursor-default items-center gap-2 rounded-[7px] px-2 py-0.5 text-[12px] leading-5 font-medium outline-none select-none hover:bg-black/8 disabled:pointer-events-none disabled:opacity-50 dark:hover:bg-white/14", onClick: () => {
                                        webviewRef.current?.goBack();
                                        setContextMenu(null);
                                    }, children: "Back" }), _jsx("button", { role: "menuitem", disabled: !browserTab.canGoForward, className: "relative flex w-full cursor-default items-center gap-2 rounded-[7px] px-2 py-0.5 text-[12px] leading-5 font-medium outline-none select-none hover:bg-black/8 disabled:pointer-events-none disabled:opacity-50 dark:hover:bg-white/14", onClick: () => {
                                        webviewRef.current?.goForward();
                                        setContextMenu(null);
                                    }, children: "Forward" }), _jsx("button", { role: "menuitem", className: "relative flex w-full cursor-default items-center gap-2 rounded-[7px] px-2 py-0.5 text-[12px] leading-5 font-medium outline-none select-none hover:bg-black/8 dark:hover:bg-white/14", onClick: () => {
                                        webviewRef.current?.reload();
                                        setContextMenu(null);
                                    }, children: "Reload" }), _jsx("div", { className: "my-1 h-px bg-border/70" }), _jsx("button", { role: "menuitem", className: "relative flex w-full cursor-default items-center gap-2 rounded-[7px] px-2 py-0.5 text-[12px] leading-5 font-medium outline-none select-none hover:bg-black/8 dark:hover:bg-white/14", onClick: () => {
                                        const targetUrl = normalizeExternalBrowserUrl(contextMenu.pageUrl);
                                        if (targetUrl) {
                                            void window.api.shell.openUrl(targetUrl);
                                        }
                                        setContextMenu(null);
                                    }, children: "Open Page In Default Browser" }), _jsx("button", { role: "menuitem", className: "relative flex w-full cursor-default items-center gap-2 rounded-[7px] px-2 py-0.5 text-[12px] leading-5 font-medium outline-none select-none hover:bg-black/8 dark:hover:bg-white/14", onClick: () => {
                                        void window.api.ui.writeClipboardText(contextMenu.pageUrl);
                                        setContextMenu(null);
                                    }, children: "Copy Page URL" }), _jsx("div", { className: "my-1 h-px bg-border/70" }), _jsx("button", { role: "menuitem", className: "relative flex w-full cursor-default items-center gap-2 rounded-[7px] px-2 py-0.5 text-[12px] leading-5 font-medium outline-none select-none hover:bg-black/8 dark:hover:bg-white/14", onClick: () => {
                                        void window.api.browser.openDevTools({ browserPageId: browserTab.id });
                                        setContextMenu(null);
                                    }, children: "Inspect Page" })] })] }), document.body)
                : null, _jsxs("div", { className: "relative z-10 flex items-center gap-2 border-b border-border/70 bg-background/95 px-3 py-1.5", children: [_jsx(Button, { size: "icon", variant: "ghost", className: "h-7 w-7", onClick: () => webviewRef.current?.goBack(), disabled: !browserTab.canGoBack, children: _jsx(ArrowLeft, { className: "size-4" }) }), _jsx(Button, { size: "icon", variant: "ghost", className: "h-7 w-7", onClick: () => webviewRef.current?.goForward(), disabled: !browserTab.canGoForward, children: _jsx(ArrowRight, { className: "size-4" }) }), _jsx(Button, { size: "icon", variant: "ghost", className: "h-7 w-7", onClick: () => {
                            const webview = webviewRef.current;
                            if (!webview) {
                                return;
                            }
                        ...
                        }, children: browserTab.loading ? (_jsx(Loader2, { className: "size-4 animate-spin" })) : (_jsx(RefreshCw, { className: "size-4" })) }), _jsx(BrowserAddressBar, { value: addressBarValue, onChange: setAddressBarValue, onSubmit: submitAddressBar, onNavigate: navigateToUrl, inputRef: addressBarInputRef }), _jsx(Button, { size: "icon", variant: grab.state !== 'idle' ? 'default' : 'ghost', className: `h-8 w-8 ${grab.state !== 'idle' ? 'bg-foreground/80 text-background hover:bg-foreground/90' : ''}`, onClick: grab.toggle, title: `Grab page element (${navigator.userAgent.includes('Mac') ? '⌘C' : 'Ctrl+C'})`, disabled: isBlankTab, children: _jsx(Crosshair, { className: "size-4" }) }), _jsx(Button, { size: "icon", variant: "ghost", className: "h-7 w-7", onClick: () => void window.api.browser.openDevTools({ browserPageId: browserTab.id }), title: "Open browser devtools", children: _jsx(SquareCode, { className: "size-4" }) }), _jsx(Button, { size: "icon", variant: "ghost", className: "h-7 w-7", onClick: () => {
                            if (!externalUrl) {
                                return;
                            }
                            void window.api.shell.openUrl(externalUrl);
                        }, title: "Open in default browser", disabled: !externalUrl, children: _jsx(ExternalLink, { className: "size-4" }) }), sessionProfile && (_jsxs("div", { className: "flex h-6 items-center gap-1 rounded-md bg-accent/60 px-2 text-[10px] font-medium text-muted-foreground", title: `Session: ${sessionProfile.label}${sessionProfile.source?.browserFamily ? ` (${sessionProfile.source.browserFamily})` : ''}`, children: [_jsx(Import, { className: "size-3" }), sessionProfile.label] })), _jsx(Button, { size: "icon", variant: "ghost", className: "h-8 w-8", title: "Browser Settings", onClick: () => {
                            useAppStore.getState().openSettingsTarget({ pane: 'browser', repoId: null });
                            useAppStore.getState().openSettingsPage();
                        }, children: _jsx(Settings, { className: "size-4" }) })] }), downloadState ? (_jsxs("div", { className: "flex items-center gap-3 border-b border-border/60 bg-amber-500/10 px-3 py-2 text-xs text-foreground/90", children: [_jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("div", { className: "truncate font-medium text-foreground", children: downloadState.filename }), _jsx("div", { className: "truncate text-muted-foreground", children: downloadState.status === 'requested'
                                    ? `Download from ${downloadState.origin}`
                                    : `Downloading from ${downloadState.origin}${downloadProgressLabel ? ` • ${downloadProgressLabel}` : ''}` })] }), downloadState.status === 'requested' ? (_jsxs(_Fragment, { children: [_jsx(Button, { size: "sm", variant: "outline", className: "h-7", onClick: () => {
                                    void window.api.browser.acceptDownload({
                                        downloadId: downloadState.downloadId
                                    });
                                }, children: "Save" }), _jsx(Button, { size: "sm", variant: "ghost", className: "h-7", onClick: () => {
                                    void window.api.browser.cancelDownload({
                                        downloadId: downloadState.downloadId
                                    });
                                }, children: "Cancel" })] })) : (_jsx("span", { className: "shrink-0 text-muted-foreground", children: downloadProgressLabel ?? 'Downloading' }))] })) : null, resourceNotice ? (_jsxs("div", { className: "flex items-center justify-between gap-2 border-b border-border/60 bg-background px-3 py-1.5 text-xs text-muted-foreground", children: [_jsx("span", { children: resourceNotice }), _jsx("button", { type: "button", onClick: () => setResourceNotice(null), className: "shrink-0 text-muted-foreground/60 hover:text-foreground", "aria-label": "Dismiss", children: "\u2715" })] })) : null, grab.state !== 'idle' ? (_jsxs("div", { className: cn('flex items-center gap-2 border-b border-border/60 px-3 py-1.5 text-xs text-foreground/90', grab.state === 'error'
                    ? 'bg-destructive/10'
                    : grab.state === 'confirming'
                        ? 'bg-green-500/10'
                        : 'bg-blue-500/10'), children: [_jsx(Crosshair, { className: cn('size-3 shrink-0', grab.state === 'error'
                            ? 'text-destructive'
                            : grab.state === 'confirming'
                                ? 'text-green-500'
                                : 'text-blue-500') }), _jsx("span", { children: grab.state === 'error'
                            ? `Grab failed: ${grab.error ?? 'Unknown error'}`
                            : grab.state === 'confirming'
                                ? 'Copied — press S to screenshot, or select another element'
                                : 'Click or hover an element, then press C to copy or S to screenshot.' }), _jsx("button", { className: "ml-auto shrink-0 rounded px-2 py-0.5 text-muted-foreground transition-colors hover:text-foreground", onClick: grab.cancel, children: "Cancel" })] })) : null, _jsxs("div", { ref: containerRef, className: "relative flex min-h-0 flex-1 overflow-hidden bg-background", children: [_jsx(BrowserFind, { isOpen: findOpen, onClose: () => setFindOpen(false), webviewRef: webviewRef }), showFailureOverlay ? (_jsx("div", { className: "absolute inset-0 z-10 flex items-center justify-center bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.02),transparent_58%)] px-6", children: _jsxs("div", { className: "flex max-w-sm flex-col items-center px-8 py-8 text-center opacity-70", children: [_jsx("div", { className: "mb-4 rounded-full border border-border/70 bg-muted/30 p-3", children: _jsx(Globe, { className: "size-5 text-muted-foreground" }) }), _jsx("h2", { className: "text-base font-semibold text-foreground/85", children: loadErrorMeta.host ? `Can't reach ${loadErrorMeta.host}` : "Can't load this page" }), _jsx("p", { className: "mt-2 text-sm text-muted-foreground", children: formatLoadFailureDescription(browserTab.loadError, loadErrorMeta) }), loadErrorHint ? (_jsx("p", { className: "mt-2 text-xs text-muted-foreground/80", children: loadErrorHint })) : null, _jsxs("div", { className: "mt-5 flex items-center gap-2", children: [_jsxs(Button, { size: "sm", variant: "outline", className: "h-9 gap-2 px-3", title: "Retry", onClick: () => {
                                                const webview = webviewRef.current;
                                                if (!webview) {
                                                    return;
                                                }
                                                ...
                                                retryBrowserTabLoad(webview, browserTab, onUpdatePageStateRef.current);
                                            }, children: [_jsx(RefreshCw, { className: "size-4" }), _jsx("span", { children: "Refresh" })] }), _jsxs(Button, { size: "sm", variant: "ghost", className: "h-9 gap-2 px-3", title: "Copy failed page URL", onClick: () => {
                                            ...
                                            }, children: [_jsx(Copy, { className: "size-4" }), _jsx("span", { children: "Copy Address" })] }), externalUrl ? (_jsxs(Button, { size: "sm", variant: "ghost", className: "h-9 gap-2 px-3", title: "Open failed page in default browser", onClick: () => {
                                                ...
                                                void window.api.shell.openUrl(externalUrl);
                                            }, children: [_jsx(ExternalLink, { className: "size-4" }), _jsx("span", { children: "Open Externally" })] })) : null] })] }) })) : null, isBlankTab ? (_jsx("div", { className: "pointer-events-none absolute inset-0 z-10 flex items-center justify-center bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.02),transparent_58%)] px-6", children: _jsxs("div", { className: "flex flex-col items-center px-8 py-8 text-center opacity-70", children: [_jsx("div", { className: "mb-4 rounded-full border border-border/70 bg-muted/30 p-3", children: _jsx(Globe, { className: "size-5 text-muted-foreground" }) }), _jsxs("div", { className: "text-center", children: [_jsx("p", { className: "text-base font-semibold text-foreground/85", children: "New Tab" }), _jsx("p", { className: "mt-2 text-sm text-muted-foreground", children: "Type a URL above to start browsing." })] })] }) })) : null, _jsxs(DropdownMenu, { open: grab.state === 'confirming' && grab.contextMenu, onOpenChange: (open) => {
                            if (!open && grab.state === 'confirming') {
                                ...
                                grab.rearm();
                        ...
                        }, children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsx("button", { "aria-hidden": true, tabIndex: -1, className: "pointer-events-none absolute size-px opacity-0", style: (() => {
                                        if (!grab.payload) {
                                            return { left: 0, top: 0 };
                                        }
                                    ...
                                    })() }) }), _jsxs(DropdownMenuContent, { align: "start", sideOffset: 4, children: [_jsxs(DropdownMenuItem, { onSelect: handleGrabCopy, children: [_jsx(Copy, { className: "size-3.5" }), "Copy Contents", _jsx(DropdownMenuShortcut, { children: "C" })] }), grab.payload?.screenshot?.dataUrl?.startsWith('data:image/png;base64,') ? (_jsxs(DropdownMenuItem, { onSelect: handleGrabCopyScreenshot, children: [_jsx(Image, { className: "size-3.5" }), "Copy Screenshot", _jsx(DropdownMenuShortcut, { children: "S" })] })) : null, _jsx(DropdownMenuSeparator, {}), _jsx(DropdownMenuItem, { onSelect: () => {
                                            grabMenuActionTakenRef.current = true;
                                            grab.cancel();
                                        }, children: "Cancel" })] })] }), grabToast ? (_jsxs("div", { className: "absolute z-30 flex items-center animate-in fade-in zoom-in-95 duration-150", style: {
                            left: grabToast.x,
                            top: grabToast.y,
                        ...
                        }, children: [_jsx("div", { className: "h-2 w-4 shrink-0", style: {
                                    clipPath: grabToast.below
                                        ? 'polygon(50% 0%, 0% 100%, 100% 100%)'
                                        : 'polygon(0% 0%, 100% 0%, 50% 100%)',
                                    background: 'white'
                                } }), _jsxs("div", { className: `flex items-center gap-1.5 rounded-full py-1.5 pl-3 pr-1.5 shadow-lg ${grabToast.type === 'success' ? 'bg-white text-gray-900' : 'bg-white text-red-600'}`, children: [grabToast.type === 'success' ? (_jsx(CircleCheck, { className: "size-4 fill-blue-600 text-white" })) : (_jsx(OctagonX, { className: "size-4 text-red-500" })), _jsx("span", { className: "text-sm font-semibold", children: grabToast.message }), _jsxs(DropdownMenu, { onOpenChange: (open) => {
                                            if (open) {
                                                clearTimeout(grabToastTimerRef.current);
                                            }
                                            else {
                                        ...
                                        }, children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsx("button", { className: "flex size-6 items-center justify-center rounded-full text-gray-500 transition-colors hover:bg-black/10 hover:text-gray-700", children: _jsx("span", { className: "text-sm font-bold leading-none", children: "\u00B7\u00B7\u00B7" }) }) }), _jsx(DropdownMenuContent, { align: "start", sideOffset: 4, children: _jsxs(DropdownMenuItem, { onSelect: () => {
                                                        const dataUrl = grabToast.payload?.screenshot?.dataUrl;
                                                        if (dataUrl?.startsWith('data:image/png;base64,')) {
                                                            void window.api.ui.writeClipboardImage(dataUrl);
                                                            setGrabToast((prev) => prev ? { ...prev, message: 'Screenshotted' } : null);
                                                        }
                                                    }, disabled: !grabToast.payload?.screenshot?.dataUrl?.startsWith('data:image/png;base64,'), children: [_jsx(Image, { className: "size-3.5" }), "Copy Screenshot", _jsx(DropdownMenuShortcut, { children: "S" })] }) })] })] })] })) : null] })] }));
}
