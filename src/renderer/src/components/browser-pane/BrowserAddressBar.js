...
export default function BrowserAddressBar({ value, onChange, onSubmit, onNavigate, inputRef }) {
    ...
    return (_jsxs(Popover, { open: open, onOpenChange: (next) => {
            ...
            setOpen(next);
        }, children: [_jsx(PopoverTrigger, { asChild: true, children: _jsxs("form", { className: "flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-border bg-background px-3 py-1 shadow-sm", onSubmit: (event) => {
                        event.preventDefault();
                        setOpen(false);
                        onSubmit();
                    }, children: [_jsx(Globe, { className: "size-4 shrink-0 text-muted-foreground" }), _jsx(Input, { ref: inputRef, value: value, onChange: (event) => onChange(event.target.value), onFocus: handleFocus, onBlur: handleBlur, onKeyDown: handleKeyDown, "data-orca-browser-address-bar": "true", className: "h-auto border-0 bg-transparent px-0 text-sm shadow-none focus-visible:ring-0", spellCheck: false, autoCapitalize: "none", autoCorrect: "off", role: "combobox", "aria-expanded": open, "aria-controls": "browser-history-listbox", "aria-autocomplete": "list" })] }) }), suggestions.length > 0 && (_jsx(PopoverContent, { align: "start", sideOffset: 4, className: "w-[var(--radix-popover-trigger-width)] p-0", onOpenAutoFocus: (e) => {
                    ...
                    e.preventDefault();
                }, children: _jsx(Command, { shouldFilter: false, value: selectedValue, onValueChange: setSelectedValue, children: _jsx(CommandList, { id: "browser-history-listbox", role: "listbox", children: _jsx(CommandGroup, { children: suggestions.map((entry) => (_jsxs(CommandItem, { value: entry.url, onSelect: () => handleSelect(entry.url), className: "flex items-center gap-2 px-3 py-2", children: [_jsx(Globe, { className: "size-3.5 shrink-0 text-muted-foreground" }), _jsxs("div", { className: "flex min-w-0 flex-1 flex-col", children: [_jsx("span", { className: "truncate text-sm", children: entry.title }), _jsx("span", { className: "truncate text-xs text-muted-foreground", children: entry.url })] })] }, entry.url))) }) }) }) }))] }));
}
