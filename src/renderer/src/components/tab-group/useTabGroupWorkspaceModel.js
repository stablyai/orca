...
const EMPTY_GROUPS = [];
const EMPTY_UNIFIED_TABS = [];
const EMPTY_BROWSER_TABS = [];
const EMPTY_RUNTIME_TERMINAL_TABS = [];
export function useTabGroupWorkspaceModel({ groupId, worktreeId }) {
    const worktreeState = useAppStore(useShallow((state) => ({
        ...
        worktree: Object.values(state.worktreesByRepo)
            .flat()
    ...
    const group = useMemo(() => worktreeState.groups.find((item) => item.id === groupId) ?? null, [groupId, worktreeState.groups]);
    const groupTabs = useMemo(() => worktreeState.unifiedTabs.filter((item) => item.groupId === groupId), [groupId, worktreeState.unifiedTabs]);
    const activeItemId = group?.activeTabId ?? null;
    const activeTab = groupTabs.find((item) => item.id === activeItemId) ?? null;
    const terminalTabs = useMemo(() => groupTabs
    ...
    const editorItems = useMemo(() => groupTabs
        ...
        .filter((item) => item !== null), [groupTabs, worktreeState.openFiles]);
    const browserItems = useMemo(() => groupTabs
        ...
        .filter((item) => item !== null), [groupTabs, worktreeState.browserTabs]);
    const activeBrowserTab = useMemo(() => activeTab?.contentType === 'browser'
        ? (worktreeState.browserTabs.find((bt) => bt.id === activeTab.entityId) ?? null)
        : null, [activeTab, worktreeState.browserTabs]);
    const runtimeTerminalTabById = useMemo(() => new Map(worktreeState.runtimeTerminalTabs.map((tab) => [tab.id, tab])), [worktreeState.runtimeTerminalTabs]);
    const closeEditorIfUnreferenced = useCallback((entityId, closingTabId) => {
        const otherReference = (useAppStore.getState().unifiedTabsByWorktree[worktreeId] ?? []).some((item) => item.id !== closingTabId &&
    ...
    const closeItem = useCallback((itemId, opts) => {
    ...
    const closeMany = useCallback((itemIds) => {
    ...
    const activateTerminal = useCallback((terminalId) => {
        const item = groupTabs.find((candidate) => candidate.entityId === terminalId && candidate.contentType === 'terminal');
        if (!item) {
            return;
        }
    ...
    const activateEditor = useCallback((tabId) => {
    ...
    const activateBrowser = useCallback((browserTabId) => {
        const item = groupTabs.find((candidate) => candidate.entityId === browserTabId && candidate.contentType === 'browser');
        if (!item) {
            return;
        }
        focusGroup(worktreeId, groupId);
    ...
    const createSplitGroup = useCallback((direction, sourceVisibleTabId) => {
        const sourceTab = groupTabs.find((candidate) => candidate.contentType === 'terminal' || candidate.contentType === 'browser'
            ? candidate.entityId === sourceVisibleTabId
            : candidate.id === sourceVisibleTabId) ?? activeTab;
        focusGroup(worktreeId, groupId);
        ...
        if (sourceTab.contentType === 'browser') {
            const browserTab = worktreeState.browserTabs.find((candidate) => candidate.id === sourceTab.entityId);
            if (!browserTab) {
                return;
            }
    ...
    const closeGroup = useCallback(() => {
        const items = [...(useAppStore.getState().unifiedTabsByWorktree[worktreeId] ?? [])].filter((item) => item.groupId === groupId);
    ...
    const tabBarOrder = useMemo(() => (group?.tabOrder ?? []).map((itemId) => {
        const item = groupTabs.find((candidate) => candidate.id === itemId);
        if (!item) {
            return itemId;
        }
    ...
    return {
        ...
        commands: {
            ...
            closeGroup,
            closeItem,
            closeOthers: (itemId) => closeMany(closeOtherTabs(itemId)),
            closeToRight: (itemId) => closeMany(closeTabsToRight(itemId)),
