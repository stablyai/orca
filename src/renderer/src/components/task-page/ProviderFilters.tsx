...
import { TaskPageMantisBTFilters } from './mantisbt/Filters'
...
export function TaskPageProviderFilters({
  ...
  const { linearConnected, jiraConnected, mantisBTConnected, taskSource, githubMode } = model
  return taskSource === 'github' && githubMode === 'items' ? (
    ...
    <TaskPageMantisBTFilters model={model} />
  ) : taskSource === 'gitlab' ? (
