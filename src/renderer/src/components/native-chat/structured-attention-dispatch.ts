 ...
 *
 * EVERY SETTLED TURN NOTIFIES, matching the CLI lane: success says "finished", failure says
 * "failed" and cancellation says "stopped", through the same turn-ending field. A turn with no outcome is UNKNOWN — the host sends no event for one, and nothing
      ...
      deliverAgentAttentionNotification(
        {
          ...
          // main words a 'working' notification as "working". The outcome picks the wording.
          agentState: 'done',
          ...(completion.outcome !== 'success' ? { agentTurnEnding: completion.outcome } : {}),
