...
export const EMULATOR_LOCAL_SHUTDOWN_EVENT = 'mcode:emulator-shutdown'
...
export function useEmulatorPaneSessionEvents({
  ...
  useEffect(() => {
    ...
    window.addEventListener('mcode:emulator-auto-attach', onAuto)
    return () => window.removeEventListener('mcode:emulator-auto-attach', onAuto)
