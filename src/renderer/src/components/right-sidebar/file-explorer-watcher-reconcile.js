...
export function purgeDirCacheSubtree(setDirCache, deletedPath) {
...
export function purgeExpandedDirsSubtree(worktreeId, deletedPath) {
    const normalized = normalizeAbsolutePath(deletedPath);
...
export function clearStalePendingReveal(deletedPath) {
    const normalized = normalizeAbsolutePath(deletedPath);
    useAppStore.setState((state) => {
        if (state.pendingExplorerReveal &&
            isPathEqualOrDescendant(normalizeAbsolutePath(state.pendingExplorerReveal.filePath), normalized)) {
