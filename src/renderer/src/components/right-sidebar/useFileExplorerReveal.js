...
export function useFileExplorerReveal({ activeWorktreeId, worktreePath, pendingExplorerReveal, clearPendingExplorerReveal, expanded, dirCache, rootCache, rowsByPath, flatRows, loadDir, setSelectedPath, setFlashingPath, flashTimeoutRef, virtualizer }) {
    ...
    useEffect(() => {
        ...
        const parentDirPath = pendingRevealAncestorDirs.length > 0 ? pendingRevealAncestorDirs.at(-1) : worktreePath;
        const parentDirCache = dirCache[parentDirPath];
        const missingExpandedAncestor = pendingRevealAncestorDirs.find((dirPath) => !expanded.has(dirPath));
        const missingAncestor = pendingRevealAncestorDirs.find((dirPath) => !rowsByPath.has(dirPath));
        const parentDirStillLoading = parentDirPath === worktreePath
