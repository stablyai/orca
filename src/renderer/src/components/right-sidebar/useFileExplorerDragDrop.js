...
export function useFileExplorerDragDrop({ worktreePath, activeWorktreeId, expanded, toggleDir, refreshDir, scrollRef }) {
    ...
    const [nativeDropTargetDir, setNativeDropTargetDir] = useState(null);
    const lastDragClientYRef = useRef(null);
    ...
    const handleMoveDrop = useCallback((sourcePath, destDir) => {
        if (!worktreePath || !activeWorktreeId) {
            return;
        }
        const fileName = basename(sourcePath), sourceDir = dirname(sourcePath);
        ...
        const remapOpenTabsForMovedPath = (fromPath, toPath) => {
        ...
        const run = async () => {
            const filesToMove = openFiles.filter((file) => {
                ...
                return (file.filePath.startsWith(`${sourcePath}/`) ||
    ...
    const handleDragExpandDir = useCallback((dirPath) => {
    ...
    const handleNativeDragExpandDir = useCallback((dirPath) => {
        if (!activeWorktreeId) {
            return;
        }
        useAppStore.setState((state) => {
            const current = state.expandedDirs[activeWorktreeId] ?? new Set();
