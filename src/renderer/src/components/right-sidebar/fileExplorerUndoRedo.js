...
const past = [];
const future = [];
export function commitFileExplorerOp(op) {
    past.push(op);
...
export function clearFileExplorerUndoHistory() {
    past.length = 0;
    future.length = 0;
}
export async function undoFileExplorer() {
    const op = past.pop();
    if (!op) {
        return false;
    }
...
export async function redoFileExplorer() {
    const op = future.pop();
    if (!op) {
        return false;
    }
...
export function fileExplorerHasUndo() {
    return past.length > 0;
}
export function fileExplorerHasRedo() {
