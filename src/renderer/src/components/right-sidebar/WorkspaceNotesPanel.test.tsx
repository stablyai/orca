  ...
  return {
    ...
    useState: <T,>(initialState: T) => {
      ...
      if (initialState !== 'idle') {
        const result: [T, typeof setValue] = [value, setValue]
        return result
      }
      const trackedSetValue: typeof setValue = (next: SetStateAction<T>) => {
        stateTracking.idleStateUpdates += 1
        return setValue(next)
      }
      const result: [T, typeof setValue] = [value, trackedSetValue]
      return result
    }
  }
})
...
describe('WorkspaceNotesPanel', () => {
  ...
  it('persists a queued draft after the originating panel remounts', async () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Workspace note' })).toMatchObject({
      value: 'Existing note first second'
    })
  ...
  it('flushes a debounced draft when the panel unmounts before the timer fires', async () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Workspace note' })).toMatchObject({
      value: 'Existing note before close'
    })
  ...
  it('reconciles an external comment change for the same workspace', () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Workspace note' })).toMatchObject({
      value: 'Updated outside the panel'
    })
  })
  ...
  it('does not replace an active local draft with an external comment change', async () => {
    ...
    expect(note).toMatchObject({ value: 'Existing note local draft' })
  })
  ...
  it('resets and cancels a draft when the host changes for the same workspace id', async () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Workspace note' })).toMatchObject({ value: 'SSH note' })
