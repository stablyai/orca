...
import { getSelectionRangeKeys, reconcileSelectionKeys } from './useSourceControlSelection';
function makeEntry(key, area, path, status = 'modified') {
    return {
        key,
        area,
        entry: {
...
describe('reconcileSelectionKeys', () => {
    it('drops selections that no longer exist in the visible list', () => {
        ...
        expect(reconcileSelectionKeys(new Set(['unstaged::a.ts', 'untracked::gone.ts']), flatEntries)).toEqual(new Set(['unstaged::a.ts']));
    });
});
describe('getSelectionRangeKeys', () => {
    it('selects an inclusive range across visible sections', () => {
        ...
        expect(getSelectionRangeKeys(flatEntries, 'unstaged::b.ts', 'untracked::d.ts')).toEqual(new Set(['unstaged::b.ts', 'staged::c.ts', 'untracked::d.ts']));
    });
