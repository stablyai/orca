...
import {
  ...
  repo,
  type PollState,
  GitStatusPollingHarness,
  usePollingOnce
} from './git-status-polling-test-fixture'

describe('useGitStatusPolling', () => {
  it('does not run Git against a session placeholder before the owner catalog arrives', async () => {
    const placeholders = buildRuntimeSessionPlaceholders({
      repos: [],
      worktreesByRepo: {},
      runtimeHostIdByWorkspaceSessionKey: { [worktree.id]: 'runtime:delayed' }
    })
    const { state, gitStatus } = await usePollingOnce(
      { entries: [], conflictOperation: 'unknown' },
      {
        repo: placeholders.repos[0],
