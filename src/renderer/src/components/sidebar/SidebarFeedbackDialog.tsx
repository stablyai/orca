  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="text-sm">{translate("auto.components.sidebar.SidebarFeedbackDialog.0eb643f07f", "Send Feedback")}</DialogTitle>
          <DialogDescription className="text-xs">
            {translate("auto.components.sidebar.SidebarFeedbackDialog.a828fa4aee", "Share what's working, what's broken, or what Orca should do next.")}</DialogDescription>
        </DialogHeader>

        <div className="space-y-2 rounded-md border border-border/70 bg-muted/30 p-3">
          <div className="text-xs font-medium text-foreground">{translate("auto.components.sidebar.SidebarFeedbackDialog.9b33530b3d", "Other ways to reach us")}</div>
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              {translate("auto.components.sidebar.SidebarFeedbackDialog.d245c4ef6c", "GitHub issues")}<ExternalLink className="size-3.5" />
            </Button>
            <Button
              type="button"
              variant="outline"
            ...
            >
              ...
              {translate("auto.components.sidebar.SidebarFeedbackDialog.26108d3699", "Join Discord")}<ExternalLink className="size-3.5" />
            </Button>
            <Button
              type="button"
              variant="outline"
            ...
            >
              ...
              {translate("auto.components.sidebar.SidebarFeedbackDialog.3460258a54", "Follow on X")}<ExternalLink className="size-3.5" />
            </Button>
          </div>
        </div>
        ...
        <div className="min-h-9 rounded-md border border-border/70 bg-muted/30 px-3 py-2">
          {viewer ? (
            ...
            <div className="text-xs text-muted-foreground">{translate("auto.components.sidebar.SidebarFeedbackDialog.d20439c560", "Checking GitHub identity…")}</div>
          ) : (
            <div className="text-xs text-muted-foreground">
              {translate("auto.components.sidebar.SidebarFeedbackDialog.8de03e23c5", "Submit with your typed feedback only, or connect `gh` to include GitHub identity.")}</div>
          )}
        </div>
        <DialogFooter>
