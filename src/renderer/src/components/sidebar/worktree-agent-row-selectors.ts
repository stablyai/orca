...
function getSleepingEntriesByWorktree(
  ...
  const cached = sleepingEntriesByWorktreeCache
  if (cached && cached.sleepingAgentSessionsByPaneKey === state.sleepingAgentSessionsByPaneKey) {
    return cached.entriesByWorktree
  }

  const previous = cached?.entriesByWorktree
  ...
  for (const sleeping of Object.values(state.sleepingAgentSessionsByPaneKey ?? {})) {
    const bucket = entriesByWorktree.get(sleeping.worktreeId)
    if (bucket) {
      bucket.push(sleeping)
    } else {
      entriesByWorktree.set(sleeping.worktreeId, [sleeping])
    }
  }
  for (const [worktreeId, entries] of entriesByWorktree) {
    entriesByWorktree.set(worktreeId, reuseArrayIfEqual(previous?.get(worktreeId), entries))
