...
function CodexSubagentProgressBody({
  ...
  // Why: a shared 30s tick re-evaluates parent freshness while the sheet stays open.
  const now = useNow(30_000)
  const freshParentEntry =
    parentEntry && isExplicitAgentStatusFresh(parentEntry, now, AGENT_STATUS_STALE_AFTER_MS)
      ? parentEntry
      : undefined
