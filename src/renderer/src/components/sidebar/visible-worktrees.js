 ...
 */
export function computeVisibleWorktreeIds(worktreesByRepo, sortedIds, opts) {
    let all = Object.values(worktreesByRepo).flat();
    // Filter archived
 ...
 */
export function setVisibleWorktreeIds(ids) {
 ...
 */
export function getVisibleWorktreeIds() {
    ...
    if (state.sortBy === 'smart') {
        sortedIds = sortWorktreesSmart(allWorktrees, state.tabsByWorktree, repoMap, state.prCache).map((w) => w.id);
    }
    else {
        const sorted = [...allWorktrees].sort(buildWorktreeComparator(state.sortBy, state.tabsByWorktree, repoMap, state.prCache, Date.now()));
