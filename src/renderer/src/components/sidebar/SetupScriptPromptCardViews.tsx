  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="top" sideOffset={4}>
        {translate("auto.components.sidebar.SetupScriptPromptCardViews.822ff300ad", "Dismiss")}</TooltipContent>
    </Tooltip>
  )
}
  ...
  return (
    <div className="mt-3 border-t border-worktree-sidebar-border pt-3">
      <div className="mb-2 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
        ...
        {translate("auto.components.sidebar.SetupScriptPromptCardViews.7275f674cc", "Detected setup")}</div>
      <textarea
        value={setup}
        aria-label={translate("auto.components.sidebar.SetupScriptPromptCardViews.fdbc6cb064", "Detected setup script")}
      ...
      {provenance ? (
        <p className="mt-1.5 text-[11px] text-muted-foreground">
          {translate("auto.components.sidebar.SetupScriptPromptCardViews.d02e6a42b1", "Detected from")}<code className="rounded bg-muted px-1 py-0.5">{provenance}</code>
        </p>
      ) : null}
    </div>
  )
  ...
  return (
    <div className="mt-3 flex flex-col gap-2">
      ...
      >
        ...
        <span className={cn('truncate', isSaving && 'text-muted-foreground')}>{translate("auto.components.sidebar.SetupScriptPromptCardViews.ca4efcbc25", "Save")}</span>
      </Button>
      <Button
        type="button"
        variant="ghost"
      ...
      >
        ...
        <span className="truncate">{translate("auto.components.sidebar.SetupScriptPromptCardViews.eefa756190", "Configure manually")}</span>
      </Button>
    </div>
  )
}
...
export function SetupScriptPromptBody({
  ...
  if (sharedSetupIgnored) {
    return (
      <>
        {translate("auto.components.sidebar.SetupScriptPromptCardViews.bb879db364", "This repo ignores shared")}<code>{translate("auto.components.sidebar.SetupScriptPromptCardViews.8f6be51aa1", "orca.yaml")}</code> {translate("auto.components.sidebar.SetupScriptPromptCardViews.660cdc17f8", "setup scripts. Add a local command, or change the source in Settings.")}</>
    )
  }
  ...
  if (candidateSource) {
    return (
      <>
        {translate("auto.components.sidebar.SetupScriptPromptCardViews.b56d1322f7", "Found a setup command in")}<span className="break-words">{candidateSource}</span>{translate("auto.components.sidebar.SetupScriptPromptCardViews.8349e3fa4c", ". Save it to run for new worktrees.")}</>
    )
  }
  ...
  return (
    <div className="mt-3 flex gap-2">
      ...
      >
        ...
        <span className="truncate">{translate("auto.components.sidebar.SetupScriptPromptCardViews.4a98f907ae", "Retry")}</span>
      </Button>
      <Button
        type="button"
        variant="ghost"
      ...
      >
        <Settings className="size-3.5" />
        <span className="sr-only">{translate("auto.components.sidebar.SetupScriptPromptCardViews.31b8b01a45", "Settings")}</span>
      </Button>
    </div>
  )
}
...
export function SaveLocalSetupAction({
  ...
  return (
    ...
    >
      ...
      <span className={cn('truncate', isSaving && 'text-muted-foreground')}>{translate("auto.components.sidebar.SetupScriptPromptCardViews.96a7f4198c", "Save local setup")}</span>
    </Button>
  )
}
