...
vi.mock('@/lib/agent-status', () => ({
    detectAgentStatusFromTitle: vi.fn((title) => {
...
function makeTerminalTab(title) {
    return {
        id: 'tab-1',
...
describe('getWorktreeStatus', () => {
    ...
    it('keeps terminal agent states higher priority than browser presence', () => {
        expect(getWorktreeStatus([makeTerminalTab('permission needed')], [{ id: 'browser-1' }])).toBe('permission');
        expect(getWorktreeStatus([makeTerminalTab('working hard')], [{ id: 'browser-1' }])).toBe('working');
    });
});
