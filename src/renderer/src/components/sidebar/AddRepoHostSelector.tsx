...
import type { AddProjectSource } from './use-add-repo-source-selection'

type AddRepoHostSelectorProps = {
  hosts: SidebarHostOption[]
  selectedHostId: ExecutionHostId
  // Why: WSL is a non-host picker section (Task 7 design) — it never becomes
  // an ExecutionHostId, so the trigger label and checkmark key off this instead.
...
export function AddRepoHostSelector({
  ...
  if (!shouldShowHostScopeControls(hosts) && !showHostSetupActions && !onSelectWsl) {
    return null
  }
  ...
  const isWslSelected = selectedSource?.kind === 'wsl'
  return (
        ...
        <PopoverTrigger asChild>
          ...
          >
            <span className="min-w-0 truncate">
              {isWslSelected
                ? translate('auto.components.sidebar.AddRepoHostSelector.wsl', 'WSL')
                : selectedHost.label}
            </span>
            {!isWslSelected && selectedHost.health !== 'local' ? (
              <span
              ...
              {onSelectWsl ? (
                ...
                >
                  <Check
                    className={cn(
                      'mt-0.5 size-3 text-muted-foreground',
                      isWslSelected ? 'opacity-70' : 'opacity-0'
                    )}
                  />
                  <ShellIcon shell="wsl.exe" size={14} />
                  <span className="min-w-0 flex-1">
                    <span className="flex min-w-0 items-center gap-2">
                      <span className="truncate font-medium">
                        {translate('auto.components.sidebar.AddRepoHostSelector.wsl', 'WSL')}
                    ...
                    <span className="mt-0.5 block truncate text-[11px] text-muted-foreground">
                      {translate(
                        'auto.components.sidebar.AddRepoHostSelector.wslDetail',
                        'Open a project from a WSL distro'
                      )}
                    </span>
                  </span>
                </CommandItem>
