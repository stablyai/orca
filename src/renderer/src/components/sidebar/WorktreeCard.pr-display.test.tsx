...
describe('WorktreeCard linked PR display', () => {
  ...
  it('applies experimental unread status and read-title visuals only when enabled', async () => {
    ...
    expect(unreadMarkup).toContain('Active · PR checks: Failed · Unread')
    expect(unreadMarkup).not.toContain('Mark read')
    expect(unreadMarkup).toContain('data-worktree-activity-status="active"')
    expect(unreadMarkup).toContain('data-worktree-review-status=""')
    expect(unreadMarkup).toContain('size-2.5')
    expect(unreadMarkup).toContain('mr-1 w-5 items-center')
    expect(unreadMarkup).toContain('bg-emerald-500')
