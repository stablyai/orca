...
import { Bell, CalendarClock, Database, Search, Smartphone } from 'lucide-react'
...
const SidebarNav = React.memo(function SidebarNav() {
  ...
  const openDatabasePage = useAppStore((s) => s.openDatabasePage)
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-current={databaseActive ? 'page' : undefined}
        className={cn(
          'flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-[13px] font-medium tracking-tight transition-colors',
          databaseActive
            ? 'bg-worktree-sidebar-accent text-worktree-sidebar-accent-foreground'
      ...
      >
        <Database
          className={cn(
            'size-4 shrink-0',
            !databaseActive && 'text-worktree-sidebar-foreground/30'
          )}
          strokeWidth={databaseActive ? 2.25 : 1.75}
        />
        <span className="flex-1">
          {translate('auto.components.sidebar.SidebarNav.database', 'Database')}
        </span>
      </button>
      <button
        type="button"
