 ...
 *
 * A notice row can render outside its project's own section, so ambiguity is
 * measured across that project instead of its current section.
 *
...
/** Non-local host labels keyed by host-qualified worktree identity. */
export function getNonLocalWorktreeHostContextLabels(
  ...
  return labelsByIdentity.size > 0 ? labelsByIdentity : undefined
}
