  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from MCode. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from MCode. Git worktrees will also be removed from git and disk; folder workspaces will only remove the MCode workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from MCode. The project folder on disk will not be deleted.'
