...
/** True when an existing MCode host already owns this config alias / label. */
export function isDuplicateSshTargetAlias({
  ...
  // Why: the config picker's `alreadyInMCode` flag compares lowercased aliases; match it or the
 ...
 *  otherwise an alias it greys out as "In MCode" is still savable as a second target. */
