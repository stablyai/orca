...
describe('submitFolderWorkspaceCreate typed prompt', () => {
  ...
  afterEach(() => {
    ...
    mocks.startStructuredAgentLaunch.mockReset()
  ...
  it.each(['codex', 'claude'] as const)(
    'hands the typed prompt to a structured %s launch',
    async (agent) => {
      ...
      // Delivery mode is pinned too: a typed prompt with nothing linked must submit, not draft.
      expect(mocks.startStructuredAgentLaunch).toHaveBeenCalledWith(expect.any(String), agent, {
