...
describe('createSessionGridWheelGesture', () => {
  ...
  it('does not discard a physical mouse notch smaller than the trackpad threshold', () => {
    const moves = feedTrain(makeGesture(), [3, 12, 24], { discrete: true, gapMs: 200 })
    expect(moves.map((move) => move.offset)).toEqual([1, 1, 1])
  })
