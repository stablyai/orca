...
export function useSessionsGridItems(lenses: SessionGridLenses = {}): {
  ...
  const worktreeCatalog = useMemo(
    ...
    // oxlint-disable-next-line react-hooks/exhaustive-deps -- The builder's translate() calls read the active language implicitly.
    [workspaceCatalogs, i18n.language]
  )
  ...
  const listing = useMemo(
    ...
    // oxlint-disable-next-line react-hooks/exhaustive-deps -- Translated filter and fallback labels must refresh without session activity.
    [listingState, worktreeCatalog, reuseCache, i18n.language]
  )
