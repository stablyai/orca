...
/** Shift can move a mouse wheel's vertical travel onto the horizontal axis. */
export function getSessionGridWheelDelta(
  event: Pick<WheelEvent, 'deltaX' | 'deltaY' | 'shiftKey'>
): number {
  if (event.shiftKey && event.deltaY === 0) {
    return event.deltaX
  }
  return Math.abs(event.deltaY) >= Math.abs(event.deltaX) ? event.deltaY : 0
}

/** DOM line/page deltas need a pixel distance before grid scrolling or snapping. */
export function normalizeSessionGridWheelPixels(
  delta: number,
  deltaMode: number,
  pageHeight: number
): number {
  const lineHeight = 16
  return delta * (deltaMode === 1 ? lineHeight : deltaMode === 2 ? pageHeight : 1)
}
...
export function createSessionGridWheelGesture(config: {
  ...
  const feed = (sample: SessionGridWheelSample): SessionGridWheelMove | null => {
    ...
    if (discrete) {
      if (sample.at - lastStepAt < DISCRETE_STEP_COALESCE_MS) {
        return null
      }
