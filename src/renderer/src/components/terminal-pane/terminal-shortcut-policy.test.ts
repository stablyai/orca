...
describe('kitty keyboard protocol panes', () => {
  ...
  it('types Option-composed letters on a compose side instead of chords (#20171)', () => {
    // macOS composition reports key='π' for Option+P on ABC/compose layouts; OMP binds alt+p
    // and alt+m. #8031 encoded these as chords for such hotkeys; #20171 flipped that — the
    // effective setting promises composition, so the text wins and chord users opt into
    // macOptionAsAlt 'true' (see the Alt-side expectations below).
