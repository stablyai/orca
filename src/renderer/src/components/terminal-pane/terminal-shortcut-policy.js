...
export function resolveTerminalShortcutAction(event, isMac, macOptionAsAlt = 'false', optionKeyLocation = 0) {
    ...
    if (isMac && !event.metaKey && !event.ctrlKey && event.altKey && !event.shiftKey) {
        ...
        const shouldActAsMeta = (macOptionAsAlt === 'left' && isLeftOption) || (macOptionAsAlt === 'right' && isRightOption);
        if (shouldActAsMeta) {
