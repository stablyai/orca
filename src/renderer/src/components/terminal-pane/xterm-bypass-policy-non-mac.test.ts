...
describe('shouldSuppressTerminalImeKeyboardEvent — Windows/Linux', () => {
  ...
  it('lets Windows Sogou Shift-to-English keydowns reach CompositionHelper', () => {
    // Why: native Shift toggle is key=Process code=ShiftLeft keyCode=229, then
    // an ordinary Shift. Swallowing that keydown drops the later latin input.
    expect(
      shouldSuppressTerminalImeKeyboardEvent(
  ...
  it('suppresses a Windows Sogou Shift-coded keyup so kitty cannot encode the release', () => {
    // Why: the held-commit fix only needs the keydown. A Shift-coded Process/229
    // keyup otherwise reaches KittyKeyboard and emits a release CSI-u.
    expect(
      shouldSuppressTerminalImeKeyboardEvent(
        event({
          type: 'keyup',
    ...
    expect(
      shouldSuppressTerminalImeKeyboardEvent(
        event({ type: 'keyup', key: 'Process', code: 'ShiftLeft', keyCode: 229 }),
        windowsIdle
      )
    ).toBe(true)
  })

  it('keeps Shift-coded IME events on the previous policy for macOS and Linux', () => {
    const macComposing = {
      isMac: true,
      isLinux: false,
      compositionActive: true,
    ...
    for (const options of [macComposing, linuxComposing]) {
      expect(
        shouldSuppressTerminalImeKeyboardEvent(
          event({
            key: 'Shift',
