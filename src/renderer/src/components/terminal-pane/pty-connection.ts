...
export function connectPanePty(
  ...
  let disposeCodexBackfillPaneHold: (() => void) | null = null
  ...
  const runDeferredConnect = (): void => {
    ...
    disposeCodexBackfillPaneHold ??= subscribeToCodexBackfillPaneHold(cacheKey, (state) => {
      deps.onCodexIndexingStateRef?.current?.(pane.id, state)
    })
  ...
  return {
    ...
    dispose() {
      disposed = true
      disposeCodexBackfillPaneHold?.()
      disposeCodexBackfillPaneHold = null
