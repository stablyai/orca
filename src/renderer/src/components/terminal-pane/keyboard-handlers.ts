...
type ShortcutActionEvent = {
  ...
  code?: string
  repeat: boolean
  // The Option release tracker reads these; a snapshotted chord carries the press's values, which
  // is the point of snapshotting.
  metaKey: boolean
  ctrlKey: boolean
  altKey: boolean
  shiftKey: boolean
...
export function useTerminalKeyboardShortcuts({
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent): void => {
      ...
      const composing = e.isComposing || hasPendingImeComposition
      runShortcutAction(e, action, manager, composing, (pane, sendResolvedInput) => {
        if (!(composing && (e.key === 'Enter' || imeProcessEnter))) {
          return false
        }
        if (isWindows) {
    ...
    function runShortcutAction(
      ...
      manager: PaneManager,
      // Passed in because a chord recovered from a release has no live event to read it from,
      // and is composing by definition — that is what armed the recovery.
      composing: boolean,
      ...
      if (action.type === 'sendInput') {
        ...
        // Keydown only: the Option policy returns null for a composing event, and a chord
        // recovered from a release is composing by definition, so neither field reaches here.
