...
export function useTerminalPaneGlobalEffects({ tabId, isActive, isVisible, managerRef, containerRef, paneTransportsRef, pendingWritesRef, isActiveRef, isVisibleRef, toggleExpandPane }) {
    ...
    useEffect(() => {
        ...
        if (isVisible) {
            ...
            const entries = Array.from(pendingWritesRef.current.entries()).filter(([, buf]) => buf.length > 0);
