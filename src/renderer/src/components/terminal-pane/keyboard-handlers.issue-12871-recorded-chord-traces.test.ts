...
// Every row below is captured, not authored.
//
...
// apart at the moment the handler has to decide:
//
//   - Korean 2-Set commits the syllable, emits compositionend, and the platform replays the
//     chord unmarked after keyup. Acting on both copies sends Option+Left's \x1bb twice, which
//     jumps two words instead of one.
//   - Japanese conversion swallows the chord outright: no compositionend, no replay, still
//     composing at keyup. Before the exemption these three chords produced no bytes at all.
//
// So both halves are pinned together: the fix is only correct if Korean stays at one firing
// while Japanese goes from zero to one each.
...
const CASES: RecordedCase[] = [
  ...
  {
    "name": "Japanese, three chords across one live preedit",
    "expectCalls": [
      "\u0001",
      "\u001bb",
      "\u0015"
