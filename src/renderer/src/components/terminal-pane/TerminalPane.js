...
import { DEFAULT_TERMINAL_DIVIDER_DARK, normalizeColor, resolveEffectiveTerminalAppearance } from '@/lib/terminal-theme';
...
export const shutdownBufferCaptures = new Set();
const MAX_BUFFER_BYTES = 512 * 1024;
export default function TerminalPane({ tabId, worktreeId, cwd, isActive, isVisible = true, onPtyExit, onCloseTab }) {
    const containerRef = useRef(null);
    const managerRef = useRef(null);
    const paneFontSizesRef = useRef(new Map());
    const expandedPaneIdRef = useRef(null);
    const expandedStyleSnapshotRef = useRef(new Map());
    const paneTransportsRef = useRef(new Map());
    const panePtyBindingsRef = useRef(new Map());
    ...
    const [paneTitles, setPaneTitles] = useState({});
    const paneTitlesRef = useRef({});
    ...
    const onPtyErrorRef = useRef((_paneId, message) => {
    ...
    const consumePendingCodexPaneRestart = useAppStore((store) => store.consumePendingCodexPaneRestart);
    ...
    const [issueCommandSplit] = useState(() => useAppStore.getState().pendingIssueCommandSplitByTabId[tabId]);
    ...
    const macOptionAsAltRef = useRef(settings?.terminalMacOptionAsAlt ?? 'false');
    ...
    const persistLayoutSnapshot = useCallback(() => {
        ...
        if (existing?.buffersByLeafId) {
            ...
            layout.buffersByLeafId = Object.fromEntries(Object.entries(existing.buffersByLeafId).filter(([id]) => currentLeafIds.has(id)));
        }
        ...
        if (existing?.ptyIdsByLeafId) {
            const currentLeafIds = new Set(manager.getPanes().map((p) => paneLeafId(p.id)));
            layout.ptyIdsByLeafId = Object.fromEntries(Object.entries(existing.ptyIdsByLeafId).filter(([id]) => currentLeafIds.has(id)));
        }
        ...
        const ptyEntries = currentPanes
            .map((p) => [paneLeafId(p.id), paneTransportsRef.current.get(p.id)?.getPtyId() ?? null])
        ...
        const titleEntries = currentPanes
            ...
            .map((p) => [paneLeafId(p.id), titles[p.id]]);
        if (titleEntries.length > 0) {
            layout.titlesByLeafId = Object.fromEntries(titleEntries);
        }
        setTabLayout(tabId, layout);
    }, [tabId, setTabLayout]);
    const syncPanePtyLayoutBinding = useCallback((paneId, ptyId) => {
        const existingLayout = useAppStore.getState().terminalLayoutsByTabId[tabId] ?? EMPTY_LAYOUT;
        const { ptyIdsByLeafId: _existingPtyIdsByLeafId, ...layoutWithoutPtyBindings } = existingLayout;
    ...
    const { setExpandedPane, restoreExpandedLayout, refreshPaneSizes, syncExpandedLayout, toggleExpandPane } = createExpandCollapseActions({
        expandedPaneIdRef,
    ...
    const executeClosePane = useCallback((paneId) => {
        const manager = managerRef.current;
        if (!manager) {
            return;
        }
    ...
    const handleRequestClosePane = useCallback((paneId) => {
    ...
    const handleRestartCodexPane = useCallback((paneId) => {
        const manager = managerRef.current;
    ...
    useEffect(() => {
        const captureBuffers = () => {
            ...
            const titleEntries = panes
                ...
                .map((p) => [paneLeafId(p.id), paneTitlesRef.current[p.id]]);
            if (titleEntries.length > 0) {
                layout.titlesByLeafId = Object.fromEntries(titleEntries);
            }
            setTabLayout(tabId, layout);
    ...
    const handleStartRename = useCallback((paneId) => {
    ...
    const handleRemoveTitle = useCallback((paneId) => {
        setPaneTitles((prev) => {
            if (!(paneId in prev)) {
                return prev;
            }
    ...
    const terminalContainerStyle = {
        ...
        ['--orca-terminal-divider-color']: effectiveAppearance?.dividerColor ?? DEFAULT_TERMINAL_DIVIDER_DARK,
        ['--orca-terminal-divider-color-strong']: normalizeColor(effectiveAppearance?.dividerColor, DEFAULT_TERMINAL_DIVIDER_DARK)
    };
    const activePane = managerRef.current?.getActivePane();
    return (_jsxs(_Fragment, { children: [_jsx("div", { ref: containerRef, className: "absolute inset-0 min-h-0 min-w-0", "data-native-file-drop-target": "terminal", "data-terminal-tab-id": tabId, style: terminalContainerStyle, onContextMenuCapture: contextMenu.onContextMenuCapture, onDragOver: (e) => {
                ...
                } }), terminalError && isActive && (_jsx(TerminalErrorToast, { error: terminalError, onDismiss: () => setTerminalError(null) })), activePane?.container &&
                createPortal(_jsx(TerminalSearch, { isOpen: searchOpen, onClose: () => setSearchOpen(false), searchAddon: activePane.searchAddon ?? null, searchStateRef: searchStateRef }), activePane.container), _jsx(TerminalContextMenu, { open: contextMenu.open, onOpenChange: contextMenu.setOpen, menuPoint: contextMenu.point, menuOpenedAtRef: contextMenu.menuOpenedAtRef, canClosePane: contextMenu.paneCount > 1, canExpandPane: contextMenu.paneCount > 1, menuPaneIsExpanded: contextMenu.menuPaneId !== null && contextMenu.menuPaneId === expandedPaneId, onCopy: () => void contextMenu.onCopy(), onPaste: () => void contextMenu.onPaste(), onSplitRight: contextMenu.onSplitRight, onSplitDown: contextMenu.onSplitDown, onClosePane: contextMenu.onClosePane, onClearScreen: contextMenu.onClearScreen, onToggleExpand: contextMenu.onToggleExpand, onSetTitle: contextMenu.onSetTitle }), managerRef.current?.getPanes().map((pane) => {
                const title = paneTitles[pane.id];
                const isEditing = renamingPaneId === pane.id;
                if (!title && !isEditing) {
                    return null;
                }
                return createPortal(_jsx("div", { className: "pane-title-bar", ...(isEditing ? { 'data-editing': '' } : {}), children: isEditing ? (_jsx("input", { ref: renameInputRef, className: "pane-title-input", value: renameValue, onChange: (e) => setRenameValue(e.target.value), onKeyDown: (e) => {
                            if (e.key === 'Enter') {
                                handleRenameSubmit();
                            }
                        ...
                        }, onBlur: handleRenameSubmit })) : (_jsxs(_Fragment, { children: [_jsx("span", { className: "pane-title-text", onClick: () => handleStartRename(pane.id), children: title }), _jsx("button", { className: "pane-title-close", onClick: (e) => {
                                    e.stopPropagation();
                                    handleRemoveTitle(pane.id);
                                }, "aria-label": "Remove title", children: "\u00D7" })] })) }), pane.container, `pane-title-${pane.id}`);
            }), _jsx(CloseTerminalDialog, { open: closeConfirmPaneId !== null, onCancel: () => setCloseConfirmPaneId(null), onConfirm: handleConfirmClose })] }));
}
