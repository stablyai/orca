...
import { detectAgentStatusFromTitle, clearWorkingIndicators, createAgentStatusTracker, normalizeTerminalTitle, extractLastOscTitle } from '../../../../shared/agent-detection';
import { ptyDataHandlers, ptyExitHandlers, openCodeStatusHandlers, ptyTeardownHandlers, ensurePtyDispatcher, getEagerPtyBufferHandle } from './pty-dispatcher';
...
export function createIpcPtyTransport(opts = {}) {
    const { cwd, env, command, connectionId, worktreeId, onPtyExit, onTitleChange, onPtySpawn, onBell, onAgentBecameIdle, onAgentBecameWorking, onAgentExited } = opts;
    let connected = false;
    let destroyed = false;
    let ptyId = null;
    ...
    let lastObservedTerminalTitle = null;
    let openCodeStatus = null;
    let staleTitleTimer = null;
    const agentTracker = onAgentBecameIdle || onAgentBecameWorking || onAgentExited
        ? createAgentStatusTracker((title) => {
            if (!suppressAttentionEvents) {
                onAgentBecameIdle?.(title);
            }
        }, onAgentBecameWorking, onAgentExited)
        : null;
    const STALE_TITLE_TIMEOUT = 3000; // ms before stale working title is cleared
    let storedCallbacks = {};
    function unregisterPtyHandlers(id) {
        ptyDataHandlers.delete(id);
        ptyExitHandlers.delete(id);
        openCodeStatusHandlers.delete(id);
        ptyTeardownHandlers.delete(id);
    }
    function unregisterPtyDataAndStatusHandlers(id) {
        ptyDataHandlers.delete(id);
        openCodeStatusHandlers.delete(id);
    }
    function getSyntheticOpenCodeTitle(status) {
        const baseTitle = lastObservedTerminalTitle && lastObservedTerminalTitle !== 'OpenCode'
    ...
    function applyOpenCodeStatus(event) {
    ...
    function applyObservedTerminalTitle(title) {
    ...
    function registerPtyDataHandler(id) {
    ...
    function clearAccumulatedState() {
        if (staleTitleTimer) {
            clearTimeout(staleTitleTimer);
            staleTitleTimer = null;
        }
    ...
    function registerPtyExitHandler(id) {
        ...
        async connect(options) {
            ...
            try {
                ...
                if (connectionId && msg.includes('No PTY provider for connection')) {
                    storedCallbacks.onError?.('SSH connection is not active. Use the reconnect dialog or Settings to connect.');
                }
                else {
