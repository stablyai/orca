...
const pendingSpawnByTabId = new Map();
export function connectPanePty(pane, manager, deps) {
    let disposed = false;
    let connectFrame = null;
    let startupInjectTimer = null;
    ...
    const onTitleChange = (title, rawTitle) => {
        ...
        if (!hasConsideredInitialCacheTimerSeed) {
            ...
            if (shouldSeedCacheTimerOnInitialTitle({
                rawTitle,
    ...
    connectFrame = requestAnimationFrame(() => {
        ...
        if (cols === 0 || rows === 0) {
            deps.onPtyErrorRef?.current?.(pane.id, `Terminal has zero dimensions (${cols}×${rows}). The pane container may not be visible.`);
        }
        ...
        const startFreshSpawn = () => {
            const spawnPromise = Promise.resolve(transport.connect({
                url: '',
                cols,
                rows,
                callbacks: {
                ...
                .then((spawnedPtyId) => typeof spawnedPtyId === 'string' ? spawnedPtyId : transport.getPtyId())
                .catch(() => null)
                .finally(() => {
        ...
        const reattachSessionId = restoredPtyId ?? (existingPtyId && !hasExistingPaneTransport ? existingPtyId : null);
        if (reattachSessionId) {
            ...
            void Promise.resolve(reattachPromise)
                ...
                const connectResult = result && typeof result === 'object' && 'id' in result
                    ? result
                    : null;
                const ptyId = connectResult?.id ?? (typeof result === 'string' ? result : transport.getPtyId());
                if (ptyId) {
                    deps.syncPanePtyLayoutBinding(pane.id, ptyId);
                    deps.updateTabPtyId(deps.tabId, ptyId);
                }
            ...
            if (pendingSpawn) {
                void pendingSpawn
                    ...
                    if (!spawnedPtyId) {
                        ...
                        console.warn(`Pending PTY spawn for tab ${deps.tabId} resolved without a PTY id, retrying fresh spawn`);
                        startFreshSpawn();
                        return;
                    }
