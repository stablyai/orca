...
import {
  isMCodeWindowForegroundFocused,
...
export function dispatchTerminalNotification(
  ...
  if (event.source === 'agent-task-complete') {
    ...
    const shouldMarkUnread = event.paneKey
      ...
      : state.activeWorktreeId !== worktreeId || !isMCodeWindowForegroundFocused()
    if (shouldMarkUnread) {
      // Why: activeWorktreeId is only in-app selection. If MCode is backgrounded,
