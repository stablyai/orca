...
function encodeHtmlPayload(raw) {
    return encodeURIComponent(raw);
}
function decodeHtmlPayload(payload) {
    try {
...
function createPlaceholder(kind, raw) {
...
function matchPlaceholder(src, kind) {
    const prefix = kind === 'inline' ? INLINE_PLACEHOLDER_PREFIX : BLOCK_PLACEHOLDER_PREFIX;
...
export function encodeRawMarkdownHtmlForRichEditor(content) {
    let index = 0;
