...
import { canAutoSaveOpenFile, getOpenFilesForExternalFileChange, normalizeAutoSaveDelayMs, ORCA_EDITOR_QUIESCE_FILE_SAVES_EVENT, requestEditorFileSave, requestEditorSaveQuiesce } from './editor-autosave';
function makeOpenFile(overrides = {}) {
    return {
        id: '/repo/file.ts',
...
describe('canAutoSaveOpenFile', () => {
    it('allows normal edit tabs and unstaged single-file diffs', () => {
        ...
        expect(canAutoSaveOpenFile(makeOpenFile({
...
describe('requestEditorSaveQuiesce', () => {
    ...
    it('waits for a claiming listener to finish quiescing', async () => {
        ...
        window.addEventListener(ORCA_EDITOR_QUIESCE_FILE_SAVES_EVENT, handler);
        try {
            ...
            window.removeEventListener(ORCA_EDITOR_QUIESCE_FILE_SAVES_EVENT, handler);
        }
    });
});
describe('requestEditorFileSave', () => {
    it('rejects when no save controller claims the request', async () => {
        await expect(requestEditorFileSave({ fileId: 'file-1' })).rejects.toThrow('Editor save controller is unavailable.');
    });
});
describe('getOpenFilesForExternalFileChange', () => {
    it('matches edit tabs and unstaged diff tabs for the same worktree file', () => {
        ...
        expect(getOpenFilesForExternalFileChange([matchingEdit, matchingUnstagedDiff, stagedDiff, otherWorktree], {
            worktreeId: 'wt-1',
            worktreePath: '/repo',
            relativePath: 'file.ts'
        }).map((file) => file.id)).toEqual(['/repo/file.ts', 'wt-1::diff::unstaged::file.ts']);
    });
});
