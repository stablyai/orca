...
let roundTripExtensions = null;
const roundTripCache = new Map();
const MAX_CACHE_ENTRIES = 20;
export function canRoundTripRichMarkdown(content) {
    const output = getRichMarkdownRoundTripOutput(content);
    return output !== null && normalizeMarkdown(content) === normalizeMarkdown(output);
}
export function getRichMarkdownRoundTripOutput(content) {
