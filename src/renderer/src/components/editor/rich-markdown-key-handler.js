 ...
 */
export function createRichMarkdownKeyHandler(ctx) {
    return (_view, event) => {
        ...
        if (event.key === 'ArrowDown') {
            event.preventDefault();
            ctx.setSelectedCommandIndex((currentIndex) => (currentIndex + 1) % currentFilteredSlashCommands.length);
            return true;
        }
        ...
        if (event.key === 'Enter' || event.key === 'Tab') {
            ...
            if (selectedCommand) {
                runSlashCommand(activeEditor, currentSlashMenu, selectedCommand, () => ctx.handleLocalImagePickRef.current());
            }
            return true;
        }
        if (event.key === 'Escape') {
