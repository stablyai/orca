...
import { canAutoSaveOpenFile, getOpenFilesForExternalFileChange, normalizeAutoSaveDelayMs, ORCA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, ORCA_EDITOR_FILE_SAVED_EVENT, ORCA_EDITOR_QUIESCE_FILE_SAVES_EVENT, ORCA_EDITOR_SAVE_AND_CLOSE_EVENT, ORCA_EDITOR_SAVE_FILE_EVENT } from './editor-autosave';
import { flushPendingEditorChange } from './editor-pending-flush';
import { ORCA_EDITOR_SAVE_DIRTY_FILES_EVENT } from '../../../../shared/editor-save-events';
function getDuplicateDirtySavePaths(files) {
...
export function attachEditorAutosaveController(store) {
    const autoSaveTimers = new Map();
    const autoSaveScheduledContent = new Map();
    const saveQueue = new Map();
    const saveGeneration = new Map();
    const clearAutoSaveTimer = (fileId) => {
    ...
    const bumpSaveGeneration = (fileId) => {
        saveGeneration.set(fileId, (saveGeneration.get(fileId) ?? 0) + 1);
    };
    const queueSave = (file, fallbackContent) => {
        ...
        const queuedSave = previousSave
            ...
            window.dispatchEvent(new CustomEvent(ORCA_EDITOR_FILE_SAVED_EVENT, {
    ...
    const quiesceFileSave = async (fileId) => {
    ...
    const getLatestWritableContent = (file) => {
    ...
    const syncAutoSave = () => {
        ...
        for (const fileId of Array.from(autoSaveTimers.keys())) {
            ...
            const shouldKeepTimer = state.settings?.editorAutoSave &&
                file &&
                file.isDirty &&
    ...
    const handleSaveDirtyFiles = async (event) => {
        ...
        try {
            ...
            detail.reject(String(error?.message ?? error));
        }
    };
    ...
    const handleQuiesce = async (event) => {
        const detail = event.detail;
        if (!detail) {
            return;
        }
        detail.claim();
        const matchingFiles = 'fileId' in detail
    ...
    syncAutoSave();
    window.addEventListener(ORCA_EDITOR_SAVE_DIRTY_FILES_EVENT, handleSaveDirtyFiles);
    window.addEventListener(ORCA_EDITOR_SAVE_AND_CLOSE_EVENT, handleSaveAndClose);
    window.addEventListener(ORCA_EDITOR_SAVE_FILE_EVENT, handleSaveFile);
    window.addEventListener(ORCA_EDITOR_QUIESCE_FILE_SAVES_EVENT, handleQuiesce);
    window.addEventListener(ORCA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, handleExternalFileChange);
    return () => {
        unsubscribe();
        window.removeEventListener(ORCA_EDITOR_SAVE_DIRTY_FILES_EVENT, handleSaveDirtyFiles);
        window.removeEventListener(ORCA_EDITOR_SAVE_AND_CLOSE_EVENT, handleSaveAndClose);
        window.removeEventListener(ORCA_EDITOR_SAVE_FILE_EVENT, handleSaveFile);
        window.removeEventListener(ORCA_EDITOR_QUIESCE_FILE_SAVES_EVENT, handleQuiesce);
        window.removeEventListener(ORCA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, handleExternalFileChange);
...
export function attachAppEditorAutosaveController() {
