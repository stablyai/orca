...
import { getLinkBubblePosition, RichMarkdownLinkBubble } from './RichMarkdownLinkBubble';
...
export default function RichMarkdownEditor({ fileId, content, filePath, scrollCacheKey, onContentChange, onDirtyStateHint, onSave }) {
    ...
    const editor = useEditor({
        ...
        onSelectionUpdate: ({ editor: nextEditor }) => {
            ...
            if (nextEditor.isActive('link')) {
                ...
                if (pos) {
                    setLinkBubble({ href: attrs.href || '', ...pos });
                }
            }
            else {
                setLinkBubble(null);
    ...
    useEffect(() => {
        if (editor) {
            ...
            ;
            editor.storage.image.filePath = filePath;
    ...
    const { handleLinkSave, handleLinkRemove, handleLinkEditCancel, handleLinkOpen, toggleLinkFromToolbar } = useLinkBubble(editor, rootRef, linkBubble, setLinkBubble, setIsEditingLink);
    const { activeMatchIndex, closeSearch, isSearchOpen, matchCount, moveToMatch, openSearch, searchInputRef, searchQuery, setSearchQuery } = useRichMarkdownSearch({
        editor,
        isMac,
        rootRef
    });
    ...
    useEffect(() => {
        ...
        setSelectedCommandIndex((currentIndex) => Math.min(currentIndex, filteredSlashCommands.length - 1));
    ...
    return (_jsxs("div", { ref: rootRef, className: "rich-markdown-editor-shell", style: { '--editor-font-zoom-level': editorFontZoomLevel }, children: [_jsx(RichMarkdownToolbar, { editor: editor, onToggleLink: toggleLinkFromToolbar, onImagePick: handleLocalImagePick }), _jsx(RichMarkdownSearchBar, { activeMatchIndex: activeMatchIndex, isOpen: isSearchOpen, matchCount: matchCount, onClose: closeSearch, onMoveToMatch: moveToMatch, onQueryChange: setSearchQuery, query: searchQuery, searchInputRef: searchInputRef }), _jsx("div", { ref: scrollContainerRef, className: "min-h-0 flex-1 overflow-auto", children: _jsx(EditorContent, { editor: editor }) }), linkBubble ? (_jsx(RichMarkdownLinkBubble, { linkBubble: linkBubble, isEditing: isEditingLink, onSave: handleLinkSave, onRemove: handleLinkRemove, onEditStart: () => setIsEditingLink(true), onEditCancel: handleLinkEditCancel, onOpen: handleLinkOpen })) : null, slashMenu && filteredSlashCommands.length > 0 ? (_jsx(RichMarkdownSlashMenu, { editor: editor, slashMenu: slashMenu, filteredCommands: filteredSlashCommands, selectedIndex: selectedCommandIndex, onImagePick: handleLocalImagePick })) : null] }));
}
