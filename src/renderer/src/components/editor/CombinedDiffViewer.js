...
export default function CombinedDiffViewer({ file, viewStateKey }) {
    ...
    const snapshotEntries = React.useMemo(() => file.uncommittedEntriesSnapshot?.filter((e) => e.conflictStatus !== 'unresolved'), [file.uncommittedEntriesSnapshot]);
    const uncommittedEntries = React.useMemo(() => snapshotEntries ??
        getCombinedUncommittedEntries(gitStatusByWorktree[file.worktreeId] ?? [], file.combinedAreaFilter), [snapshotEntries, file.worktreeId, file.combinedAreaFilter, gitStatusByWorktree]);
    ...
    useEffect(() => {
        ...
        const canRestoreCachedSections = cached &&
    ...
    const loadSection = useCallback(async (index) => {
    ...
    const handleSectionSave = useCallback(async (index) => {
        ...
        try {
            ...
            setSections((prev) => prev.map((s, i) => (i === index ? { ...s, modifiedContent: content, dirty: false } : s)));
        }
        catch (err) {
    ...
    useEffect(() => {
        ...
        const preservedScrollTop = combinedDiffScrollTopCache.get(viewStateKey) ?? scrollContainerRef.current?.scrollTop ?? 0;
    ...
    if (sections.length === 0 && (file.skippedConflicts?.length ?? 0) > 0) {
        return (_jsx("div", { className: "flex h-full items-center justify-center px-6 text-center", children: _jsxs("div", { className: "max-w-md space-y-3", children: [_jsx("div", { className: "text-sm font-medium text-foreground", children: "Conflicted files are reviewed separately" }), _jsx("div", { className: "text-xs text-muted-foreground", children: "This diff view excludes unresolved conflicts because the normal two-way diff pipeline is not conflict-safe." }), _jsx("div", { className: "text-xs text-muted-foreground", children: file.skippedConflicts.map((entry) => entry.path).join(', ') }), _jsx("div", { className: "flex justify-center", children: _jsx(Button, { type: "button", size: "sm", variant: "outline", onClick: () => openConflictReview(file.worktreeId, file.filePath, file.skippedConflicts.map((entry) => ({
                                path: entry.path,
                                conflictKind: entry.conflictKind
                            })), 'combined-diff-exclusion'), children: "Review conflicts" }) })] }) }));
    }
    if (sections.length === 0) {
        return (_jsx("div", { className: "flex items-center justify-center h-full text-muted-foreground text-sm", children: "No changes to display" }));
    }
    const skippedConflictNotice = (file.skippedConflicts?.length ?? 0) > 0 ? (_jsxs("div", { className: "mx-4 mt-3 rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs", children: [_jsx("div", { className: "font-medium text-foreground", children: "Conflicted files are reviewed separately" }), _jsxs("div", { className: "mt-1 text-muted-foreground", children: [file.skippedConflicts.length, " unresolved conflict", file.skippedConflicts.length === 1 ? '' : 's', " were excluded from this diff view."] }), _jsx("div", { className: "mt-2 flex items-center gap-2", children: _jsx(Button, { type: "button", size: "sm", variant: "outline", className: "h-7 text-xs", onClick: () => openConflictReview(file.worktreeId, file.filePath, file.skippedConflicts.map((entry) => ({
                        path: entry.path,
                        conflictKind: entry.conflictKind
                    })), 'combined-diff-exclusion'), children: "Review conflicts" }) })] })) : null;
    return (_jsxs("div", { className: "flex flex-col flex-1 min-h-0", children: [_jsxs("div", { className: "flex items-center justify-between px-3 py-1.5 border-b border-border bg-background/50 shrink-0", children: [_jsxs("span", { className: "text-xs text-muted-foreground", children: [sections.length, " changed files", isBranchMode && branchCompare ? ` vs ${branchCompare.baseRef}` : ''] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsxs("button", { className: "flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors", onClick: () => openDiffCommentsTab(file.worktreeId, file.filePath), title: "Open diff comments tab", children: [_jsx(MessageSquare, { className: "size-3" }), "Comments (", diffCommentCount, ")"] }), file.combinedAlternate && (_jsx("button", { className: "text-xs text-muted-foreground hover:text-foreground transition-colors", onClick: openAlternateDiff, children: file.combinedAlternate.source === 'combined-branch'
                                    ? 'Open Branch Diff'
                                    : 'Open Uncommitted Diff' })), _jsx("button", { className: "text-xs text-muted-foreground hover:text-foreground transition-colors", onClick: () => setSections((prev) => prev.map((s) => ({ ...s, collapsed: true }))), children: "Collapse All" }), _jsx("button", { className: "text-xs text-muted-foreground hover:text-foreground transition-colors", onClick: () => setSections((prev) => prev.map((s) => ({ ...s, collapsed: false }))), children: "Expand All" }), _jsx("button", { className: "px-2 py-0.5 text-xs rounded border border-border text-muted-foreground hover:text-foreground transition-colors", onClick: () => setSideBySide((prev) => !prev), children: sideBySide ? 'Inline' : 'Side by Side' })] })] }), _jsxs("div", { ref: scrollContainerRef, className: "flex-1 overflow-auto scrollbar-editor", children: [skippedConflictNotice, sections.map((section, index) => (_jsx(DiffSectionItem, { section: section, index: index, isBranchMode: isBranchMode, sideBySide: sideBySide, isDark: isDark, settings: settings, sectionHeight: sectionHeights[index], worktreeId: file.worktreeId, worktreeRoot: file.filePath, loadSection: loadSection, toggleSection: toggleSection, setSectionHeights: setSectionHeights, setSections: setSections, modifiedEditorsRef: modifiedEditorsRef, handleSectionSaveRef: handleSectionSaveRef }, `${section.key}:${generation}`)))] })] }));
}
