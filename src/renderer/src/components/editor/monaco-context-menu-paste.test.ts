...
import { runMCodeContextMenuPaste, type MCodeContextMenuPasteDeps } from './monaco-context-menu-paste'
...
function makeDeps(
  overrides: Partial<MCodeContextMenuPasteDeps> & { editor?: ReturnType<typeof makeEditor> }
): MCodeContextMenuPasteDeps {
...
describe('runMCodeContextMenuPaste', () => {
  ...
  it('reads via IPC and dispatches a native paste for writable focused editors', async () => {
    ...
    const outcome = runMCodeContextMenuPaste(deps)
  ...
  it('applies in-app copy metadata (empty-selection + multicursor) when enabled', async () => {
    ...
    await runMCodeContextMenuPaste(deps)
    expect(handle.trigger).toHaveBeenCalledWith('keyboard', 'paste', {
      text: 'line text',
      pasteOnNewLine: true,
      multicursorText: ['a', 'b'],
  ...
  it('does nothing on an empty clipboard', async () => {
    ...
    await expect(runMCodeContextMenuPaste(deps)).resolves.toEqual({
      status: 'noop',
      reason: 'empty'
    })
  ...
  it('routes oversized pastes through the chunked inserter instead of a native trigger', async () => {
    ...
    const outcome = await runMCodeContextMenuPaste(deps)
