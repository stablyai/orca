...
const MCodeDetails = Details.extend({
  ...
  addAttributes() {
    return {
      ...
      variant: {
        default: null,
        parseHTML: (element) => parseToggleHeadingVariant(element.getAttribute('data-mcode-toggle')),
        renderHTML: ({ variant }) => {
          const parsed = parseToggleHeadingVariant(variant)
          return parsed ? { 'data-mcode-toggle': parsed } : {}
        }
      }
    }
  },
...
const MCodeDetailsContent = DetailsContent.extend({
...
export function createMCodeDetailsExtensions(): AnyExtension[] {
  return [
    MCodeDetails.configure({
      persist: true,
      HTMLAttributes: {
        class: 'mcode-details'
      }
