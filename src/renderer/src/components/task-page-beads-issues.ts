...
function compareBeadsUpdatedAtDesc(a: BeadsIssue, b: BeadsIssue): number {
...
export function useTaskPageBeadsIssues(args: {
  ...
  useEffect(() => {
    ...
    for (const context of contexts) {
      ...
      void fetchBeadsIssues(context, preset, force ? { force: true } : undefined).catch(() => {})
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- contextsKey stands in for the contexts array identity
  }, [enabled, contextsKey, preset, refreshNonce, fetchBeadsIssues])
...
/** Client-side facets applied on top of the fetched list, ANDed with the text query. */
export type TaskPageBeadsFacetFilters = {
...
export const EMPTY_BEADS_FACET_FILTERS: TaskPageBeadsFacetFilters = {
  statuses: [],
  priorities: [],
  types: [],
  labels: [],
...
export function hasActiveBeadsFacetFilters(filters: TaskPageBeadsFacetFilters): boolean {
  return (
...
function matchesBeadsFacetFilters(issue: BeadsIssue, filters: TaskPageBeadsFacetFilters): boolean {
  ...
  if (filters.types.length > 0 && !filters.types.includes(issue.issueType)) {
    return false
  }
  // Why: labels AND together like GitHub's label qualifiers; the other facets are any-of.
  if (filters.labels.length > 0 && !filters.labels.every((label) => issue.labels.includes(label))) {
    return false
  }
  if (filters.assignee !== null && issue.assignee !== filters.assignee) {
    return false
  }
  return true
}

/** Case-insensitive client-side filter by title, id, or label substring plus facets (beads has no server query syntax). */
export function filterBeadsIssueRows(
  rows: readonly TaskPageBeadsIssueRow[],
  query: string,
  facets: TaskPageBeadsFacetFilters = EMPTY_BEADS_FACET_FILTERS
): TaskPageBeadsIssueRow[] {
  const needle = query.trim().toLowerCase()
  return rows.filter(({ issue }) => {
    if (!matchesBeadsFacetFilters(issue, facets)) {
      return false
    }
    if (!needle) {
      return true
...
export const BEADS_DEFAULT_ISSUE_TYPES: readonly string[] = [
  'bug',
  'feature',
  'task',
  'epic',
