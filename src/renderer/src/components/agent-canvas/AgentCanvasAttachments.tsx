...
export function AgentCanvasAttachments({
  ...
  if (!notes.length && context.nodes[nodeId]?.state !== 'session-changed') {
    return null
  }
  return (
    <div
    ...
    >
        ...
        <Popover open={open} onOpenChange={setOpen}>
          ...
          <PopoverContent
            ...
            onCloseAutoFocus={(event) => {
              const noteId = pendingReview.current
              if (noteId) {
                event.preventDefault()
                pendingReview.current = null
                onConnect(noteId, nodeId)
              }
            }}
          >
      ...
      <div className="pb-1 text-[11px] text-muted-foreground">
        <AgentCanvasContextStatus
          nodeId={nodeId}
          onAdoptSession={onAdoptSession}
          onlySessionChanged={!notes.length}
        />
      </div>
    </div>
  )
