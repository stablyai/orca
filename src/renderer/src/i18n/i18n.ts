...
// multiple megabytes to the startup chunk (parsed on every launch) even though the
...
void i18n
  ...
  .init({
    ...
    lng: DEFAULT_LOCALE,
    // Every requested language is normalized to a supported catalog first, so
    // loading a base language for zh-TW only duplicates the full Simplified catalog.
    load: 'currentOnly',
