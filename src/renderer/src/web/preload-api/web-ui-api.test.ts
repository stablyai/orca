...
describe('web ui.set host compatibility', () => {
  ...
  it('also filters acknowledged writes while retaining the local Cursor preference', async () => {
    await createWebUiApi().setWithAck!({ statusBarItems: ['claude', 'cursor'] })
    expect(runtime.callRuntimeResult).toHaveBeenCalledWith(
      'ui.set',
      { statusBarItems: ['claude'] },
      15_000
    )
    expect(readLocalWebUIState().statusBarItems).toEqual(['claude', 'cursor'])
  })
})
