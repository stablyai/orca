import { createDivider, applyDividerStyles, applyPaneOpacity, applyRootBackground } from './pane-divider';
import { createDragReorderState, hideDropOverlay, handlePaneDrop, updateMultiPaneState } from './pane-drag-reorder';
...
import { findPaneChildren, removeDividers, promoteSibling, wrapInSplit, safeFit, refitPanesUnder } from './pane-tree-ops';
export class PaneManager {
    ...
    dragState = createDragReorderState();
    ...
    splitPane(paneId, direction, opts) {
    ...
    createPaneInternal() {
        ...
        const pane = createPaneDOM(id, this.options, this.dragState, this.getDragCallbacks(), (paneId) => {
            if (!this.destroyed) {
     ...
     */
    handlePaneMouseEnter(paneId, event) {
    ...
    createDividerWrapped(isVertical) {
    ...
    getDragCallbacks() {
        return {
            ...
            safeFit: (pane) => safeFit(pane),
            applyPaneOpacity: () => applyPaneOpacity(this.panes.values(), this.activePaneId, this.styleOptions),
