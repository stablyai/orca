...
export function buildDefaultTerminalOptions() {
    return {
        ...
        fontFamily: '"SF Mono", "Menlo", "Monaco", "Cascadia Mono", "Consolas", "DejaVu Sans Mono", "Liberation Mono", monospace',
        fontWeight: '300',
        fontWeightBold: '500',
        scrollback: 10000,
...
function getTerminalUrlOpenHint() {
    return navigator.userAgent.includes('Mac')
        ? '⌘+click to open or ⇧⌘+click for system browser'
        : 'Ctrl+click to open or Shift+Ctrl+click for system browser';
}
export function createPaneDOM(id, options, dragState, dragCallbacks, onPointerDown, onMouseEnter) {
    ...
    const webLinksAddon = new WebLinksAddon(options.onLinkClick ? (event, uri) => options.onLinkClick(event, uri) : undefined, {
        hover: (_event, uri) => {
            if (uri) {
...
export function openTerminal(pane) {
    const { terminal, xtermContainer, linkTooltip, fitAddon, searchAddon, serializeAddon, unicode11Addon, webLinksAddon } = pane;
...
export function attachWebgl(pane) {
...
export function disposePane(pane, panes) {
    try {
