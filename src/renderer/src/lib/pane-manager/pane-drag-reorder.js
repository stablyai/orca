...
export function createDragReorderState() {
    return {
...
export function attachPaneDrag(handle, paneId, state, callbacks) {
    let dragging = false;
    let startX = 0;
    let startY = 0;
        ...
        const onPointerUpOuter = (ev) => {
            ...
            if (dragging) {
                ...
                if (state.currentDropTarget && state.dragSourcePaneId !== null) {
                    handlePaneDrop(state.dragSourcePaneId, state.currentDropTarget.paneId, state.currentDropTarget.zone, state, callbacks);
                }
...
export function handlePaneDrop(sourcePaneId, targetPaneId, zone, _state, callbacks) {
...
export function showDropOverlay(state) {
...
export function hideDropOverlay(state) {
    if (state.dropOverlay) {
...
export function updateMultiPaneState(callbacks) {
    if (callbacks.getPanes().size >= 2) {
...
function updateDropTarget(clientX, clientY, state, callbacks) {
