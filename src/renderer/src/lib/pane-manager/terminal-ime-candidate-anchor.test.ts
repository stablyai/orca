...
describe('installTerminalImeCandidateAnchor', () => {
  ...
  it('wins the clamp back after xterm re-positions the textarea on its own timer', () => {
    const harness = createHarness()
    installTerminalImeCandidateAnchor(harness.terminal)
    harness.style.width = `${2 * CELL_WIDTH}px`
    ...
    // xterm's updateCompositionElements re-arms itself once through setTimeout(0) and writes the
    // uncorrected cursorLeft back. Between events that value is what sits on the element, and
    // composition-event time is exactly when the OS samples it — so ours has to land after.
    harness.style.left = `${(COLS - 1) * CELL_WIDTH}px`
