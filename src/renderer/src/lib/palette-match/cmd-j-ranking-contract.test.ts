...
describe('Cmd+J semantic proof contract', () => {
  ...
  it('uses a complete container phrase for placement and rendered quality', () => {
    const match = matchPaletteTabDocument(
      buildPaletteTabDocument({
        id: 'tab',
        title: 'atlas',
    ...
    expect(match?.rank).toMatchObject({ coverage: 2, placement: 0 })
    expect(match?.qualityClass).toBe('exact-evidence')
    expect(match?.titleRanges).toEqual([])
    expect(match?.worktreeRanges).toHaveLength(2)
  })
