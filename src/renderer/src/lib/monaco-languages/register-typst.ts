...
export async function loadTypstTextMateGrammar(scopeName: string): Promise<IRawGrammar | null> {
  ...
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: the JSON module's inferred literal shape cannot structurally satisfy IRawGrammar's recursive rule unions; the vendored grammar is validated by the loadTypstTextMateGrammar tests.
