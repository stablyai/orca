...
describe('launchAgentBackgroundSession', () => {
  ...
  it('keeps empty Codex override launches on the fast SSH path (no mythical --prefill)', async () => {
    // Why: Codex has no --prefill flag. Empty-prompt launches use the override
    // command as-is without shell-ready unless the startup plan opts in.
    vi.useFakeTimers()
    try {
      state.repos = [{ id: 'repo-1', connectionId: 'ssh-1', path: '/repo' }]
      state.settings = {
