...
export function shouldOpenWorkspacePortInMCodeBrowser(
...
export function resolvePortOpenInMCodeBrowser({
  settings,
  event,
  isMac
}: {
  ...
  return shouldOpenWorkspacePortInMCodeBrowser(settings)
}
...
export async function openWorkspacePortInBrowser(args: {
  ...
  if (args.openInMCodeBrowser === false && args.runtimeTarget.kind === 'local') {
    try {
