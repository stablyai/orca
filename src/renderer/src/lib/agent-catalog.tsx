...
export const getAgentCatalog = createLocalizedCatalog(
  (): AgentCatalogEntry[] => [
    {
      id: 'claude',
    ...
    },
    {
      ...
      label: translate(
        'auto.lib.agent.catalog.bf53f09bf8',
        'Claude Agent Teams',
      ),
      cmd: getTuiAgentLaunchCommand(
        TUI_AGENT_CONFIG['claude-agent-teams'],
        getCatalogPlatform(),
      ),
...
export function getAgentCatalogWithProfiles(
  ...
  return catalog.flatMap((entry): AgentCatalogEntry[] => [
    entry,
    ...(profilesByBase.get(entry.id) ?? []).flatMap(
      (profile): AgentCatalogEntry[] => {
...
export function getAgentLabel(
  ...
  return (
    profile?.label ??
    getAgentCatalog().find((entry) => entry.id === agent)?.label ??
    agent
  )
}
