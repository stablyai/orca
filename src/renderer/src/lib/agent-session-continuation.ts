...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `MCode pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior MCode session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest MCode status hints:', ...statusHints] : []),
    '',
