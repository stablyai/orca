const FILE_LINK_CANDIDATE_REGEX = /(?:\/|\.{1,2}\/|[A-Za-z0-9._-]+\/)[A-Za-z0-9._~\-/]*(?::\d+)?(?::\d+)?/g;
...
function parsePathWithOptionalLineColumn(value) {
...
export function resolveTerminalFileLink(parsed, cwd) {
    const absolutePath = normalizeAbsolutePath(parsed.pathText)?.normalized ?? joinAbsolutePath(cwd, parsed.pathText);
    if (!absolutePath) {
        return null;
    }
    return {
