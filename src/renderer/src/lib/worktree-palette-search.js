...
function extractCommentSnippet(comment, matchStart, matchEnd) {
...
export function searchWorktrees(worktrees, query, repoMap, prCache, issueCache) {
    if (!query) {
    ...
    for (const worktree of worktrees) {
        ...
        if (nameIndex !== -1) {
            results.push(makeResult(worktree.id, 'displayName', {
        ...
        if (repoIndex !== -1) {
            results.push(makeResult(worktree.id, 'repo', {
        ...
        if (worktree.comment) {
            ...
            if (commentIndex !== -1) {
                const snippet = extractCommentSnippet(worktree.comment, commentIndex, commentIndex + q.length);
                results.push(makeResult(worktree.id, 'comment', {
                    supportingText: {
                        label: 'Comment',
                        text: snippet.text,
        ...
        if (pr) {
            ...
            if (prNumberIndex !== -1) {
                results.push(makeResult(worktree.id, 'pr', {
                    supportingText: {
                        label: 'PR',
                        text: prText,
                        matchRange: {
        ...
        if (issueNumberIndex !== -1) {
            results.push(makeResult(worktree.id, 'issue', {
                supportingText: {
                    label: 'Issue',
                    text: issueText,
                    matchRange: {
