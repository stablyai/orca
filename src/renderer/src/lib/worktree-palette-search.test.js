...
describe('worktree-palette-search', () => {
    ...
    it('returns a truncated comment snippet with the highlighted match range', () => {
        const results = searchWorktrees([
            makeWorktree({
                comment: 'This worktree carries the quick jump refresh implementation details for the new palette.'
            })
        ], 'implementation', repoMap, null, null);
        expect(results).toHaveLength(1);
        expect(results[0].supportingText?.label).toBe('Comment');
        expect(results[0].supportingText?.text).toContain('implementation');
        expect(results[0].supportingText?.text.slice(results[0].supportingText.matchRange.start, results[0].supportingText.matchRange.end)).toBe('implementation');
    });
    it('keeps PR title matches in the search result model instead of inferring them during render', () => {
        const results = searchWorktrees([makeWorktree({ branch: 'refs/heads/feature/palette-refresh', linkedPR: 426 })], 'quick jump', repoMap, {
    ...
    it('matches issue numbers with a leading hash and returns issue render context', () => {
        const results = searchWorktrees([makeWorktree({ linkedIssue: 304 })], '#304', repoMap, null, null);
        expect(results).toHaveLength(1);
        expect(results[0].supportingText).toEqual({
            label: 'Issue',
            text: 'Issue #304',
