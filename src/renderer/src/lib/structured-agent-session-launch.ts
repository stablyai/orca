...
import {
  deletePendingStructuredAgentLaunch,
  listPendingStructuredAgentLaunches,
  notifyStructuredAgentLaunchStatusChanged,
  readPendingStructuredAgentLaunch,
  recordPendingStructuredAgentLaunch,
  structuredAgentLaunchIdentity,
...
export type { StructuredAgentLaunchStatus } from './structured-agent-session-launch-status'
...
function cleanupLaunchState(state: StructuredLaunchState): void {
  if (deletePendingStructuredAgentLaunch(state)) {
    notifyStructuredAgentLaunchStatusChanged()
  }
}
...
function structuredAgentLaunchState(
  ...
  const identity = structuredAgentLaunchIdentity(worktreeId, agent, options.resumeFrom)
  const existing = readPendingStructuredAgentLaunch(identity)
  if (existing) {
  ...
  const state: StructuredLaunchState = {
    ...
    cancelled: false,
    onVisibilityChanged: notifyStructuredAgentLaunchStatusChanged,
    callers
  }
  ...
  recordPendingStructuredAgentLaunch(state)
  notifyStructuredAgentLaunchStatusChanged()
...
export function cancelStructuredAgentLaunch(worktreeId: string, sessionId: string): boolean {
  const state = listPendingStructuredAgentLaunches().find(
    (candidate) =>
