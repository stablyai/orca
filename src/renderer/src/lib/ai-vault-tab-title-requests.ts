...
export function structuredAgentSessionTitleTabs(state: AppState): Tab[] {
  // Tolerates a state with no unified tabs at all: a workspace can hold only terminal tabs.
...
/**
 * Where a tab's stored provider name lives, across both tab models. The sync compares against this
 * to decide whether a request still needs a scan, so it has to see chat tabs too.
 */
...
export function collectAiVaultTitleRequests(state: AppState): AiVaultTitleRequest[] {
  ...
  for (const tab of structuredAgentSessionTitleTabs(state)) {
    requests.push({
      ...
      // Why: a chat held open is live, so its provider name is re-read on the same cadence a
      // terminal-backed session gets — that is how a later rename reaches the tab.
      refresh: true,
      tabId: tab.id,
