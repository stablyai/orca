...
describe('hashMCodeHookScript', () => {
  it('produces a stable hex digest via crypto.subtle', async () => {
    const hash = await hashMCodeHookScript('echo hi')
    expect(hash).toMatch(/^[0-9a-f]+$/)
    expect(await hashMCodeHookScript('  echo hi  ')).toBe(hash)
  })
  ...
  it('falls back to a deterministic hash when crypto.subtle is unavailable', async () => {
    ...
    expect(hash).toMatch(/^[0-9a-f]+$/)
    expect(await hashMCodeHookScript('echo hi')).toBe(hash)
    expect(await hashMCodeHookScript('echo bye')).not.toBe(hash)
  })
})
