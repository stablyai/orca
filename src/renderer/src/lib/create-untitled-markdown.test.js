...
describe('createUntitledMarkdownFile', () => {
    ...
    it('retries with the next untitled name when createFile loses the EEXIST race', async () => {
        const pathExists = vi.fn(async (filePath) => filePath.endsWith('untitled.md'));
        const createFile = vi
            .fn()
    ...
    it('throws a descriptive error when untitled names are exhausted', async () => {
        ...
        await expect(createUntitledMarkdownFile('/repo', 'wt-1')).rejects.toThrow('Unable to create untitled markdown file after 100 attempts.');
