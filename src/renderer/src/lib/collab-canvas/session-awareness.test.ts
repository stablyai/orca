...
describe('buildSessionBoardAwarenessText', () => {
  it('names board and worktree as operator note (not system/MCP style)', () => {
    const text = buildSessionBoardAwarenessText({
      boardId: 'board-x',
      worktreeId: 'wt-y'
    })
    expect(text).toContain('OPERATOR — collab board is open')
    ...
    expect(text).not.toContain('[collab-canvas awareness]')
  })
})
