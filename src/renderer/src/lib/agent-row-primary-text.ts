...
import {
  findMCodeDispatchTaskMarkerIndex,
  MCODE_DISPATCH_STATUS_PREAMBLE_PREFIX,
  MCODE_DISPATCH_STATUS_TASK_MARKER
} from '../../../shared/mcode-dispatch-status-prompt'

export const MCODE_DISPATCH_PREAMBLE_PREFIX = MCODE_DISPATCH_STATUS_PREAMBLE_PREFIX
const MCODE_DISPATCH_TASK_MARKER = MCODE_DISPATCH_STATUS_TASK_MARKER
const MCODE_DISPATCH_TASK_ID_MARKER = 'Your task ID is:'
...
const MCODE_DISPATCH_TASK_PREVIEW_SCAN_LIMIT = 512
// Why: task id lives near the top of the preamble; keep that scan tight.
const MCODE_DISPATCH_TASK_ID_SCAN_LIMIT = 1024
...
const MCODE_DISPATCH_TASK_MARKER_SCAN_LIMIT = 32_768

/** True when the live prompt is still an MCode dispatch turn (not sticky metadata alone). */
export function isMCodeDispatchPrompt(prompt: string): boolean {
  return prompt.trimStart().startsWith(MCODE_DISPATCH_PREAMBLE_PREFIX)
}
...
export function orchestrationLabelsMatchLiveDispatch(
  ...
  const liveTaskId = getMCodeDispatchTaskId(entry.prompt)
  if (!liveTaskId) {
    return true
  }
...
export function getAgentRowPrimaryText(
  ...
  // first characters are boilerplate ("You are working inside MCode…").
  if (orchestrationLabelsMatchLiveDispatch(entry)) {
    return (
      entry.orchestration?.displayName?.trim() ||
      entry.orchestration?.taskTitle?.trim() ||
      getMCodeDispatchTaskPreview(entry.prompt)
    )
  }
  if (isMCodeDispatchPrompt(entry.prompt)) {
    return getMCodeDispatchTaskPreview(entry.prompt)
  }
...
export function getMCodeDispatchTaskId(prompt: string): string | null {
  if (!isMCodeDispatchPrompt(prompt)) {
    return null
  }
  const scan = prompt.trimStart().slice(0, MCODE_DISPATCH_TASK_ID_SCAN_LIMIT)
  const markerIndex = scan.indexOf(MCODE_DISPATCH_TASK_ID_MARKER)
  if (markerIndex === -1) {
    return null
  }
  ...
  const afterMarker = scan.slice(markerIndex + MCODE_DISPATCH_TASK_ID_MARKER.length).trimStart()
  const idEnd = afterMarker.search(/\s/)
  const idLine = idEnd === -1 ? afterMarker : afterMarker.slice(0, idEnd)
  return idLine || null
}

function getMCodeDispatchTaskPreview(prompt: string): string {
  ...
  const scan = prompt
    .trimStart()
    .slice(0, MCODE_DISPATCH_TASK_MARKER_SCAN_LIMIT + MCODE_DISPATCH_TASK_PREVIEW_SCAN_LIMIT)
  ...
  const taskBodyStart = taskMarkerIndex + MCODE_DISPATCH_TASK_MARKER.length
  const taskBody = scan.slice(taskBodyStart, taskBodyStart + MCODE_DISPATCH_TASK_PREVIEW_SCAN_LIMIT)
