...
export type WorkspaceFilePreviewPlan =
  ...
  | { status: 'doc-preview' }
  /** Rendered on the owning host by officecli and shown in the document-preview partition. */
  | { status: 'office-preview'; kind: OfficeDocKind }
  /**
   * A format Orca recognises and does not render. A first-class outcome, not an error: the panel
   * names the format and offers the system app. It must never mention officecli.
   */
  | { status: 'office-unrenderable'; extension: string }
...
export function getWorkspaceFilePreviewPlan(
  ...
  // Answered before any channel question: an unrenderable Office format needs no host and no
  // preview channel, and routing it through the doc-preview arms would end in a blank page.
  const officeRouting = resolveOfficePreviewRouting(filePath)
  if (officeRouting.status === 'unrenderable') {
    return { status: 'office-unrenderable', extension: officeRouting.extension }
  }
  ...
  if (officeRouting.status === 'renderable') {
    // Every host renders the same way — officecli runs where the document lives and the client
    // serves the result from memory — so this arm needs none of the per-owner branching below.
    return { status: 'office-preview', kind: officeRouting.kind }
  }
...
export function openFileInBrowserTab(params: {
  ...
  if (plan.status === 'doc-preview' || plan.status === 'office-preview') {
    openDocPreviewTab(state, { ...params, activate: true })
    return plan
  }
  if (plan.status === 'office-unrenderable') {
    // The panel that names the format lives in the preview pane, so the tab still opens; what it
    // must not do is claim the toolchain is at fault.
    openDocPreviewTab(state, { ...params, activate: true })
    return plan
  }
...
export function openFilePreviewToSide(params: {
  ...
  if (
    plan.status === 'doc-preview' ||
    plan.status === 'office-preview' ||
    plan.status === 'office-unrenderable'
  ) {
