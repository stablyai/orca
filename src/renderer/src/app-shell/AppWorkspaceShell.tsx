...
import { RoomInspectorPortalContext } from '../components/rooms/room-inspector-portal'
...
export function AppWorkspaceShell(props: {
  ...
  const [roomInspectorTarget, setRoomInspectorTarget] = useState<HTMLElement | null>(null)

  return (
    <RoomInspectorPortalContext.Provider value={roomInspectorTarget}>
      ...
      >
        <div className="flex flex-row flex-1 min-h-0 overflow-hidden">
          ...
          {layout.showRightSidebarControls ? (
            ...
            >
              <RightSidebar
                key={layout.roomMode ? 'room' : 'workspace'}
                roomMode={layout.roomMode}
                roomPanelRef={setRoomInspectorTarget}
              />
            </RecoverableRenderErrorBoundary>
          ) : null}
        </div>
      </RecoverableRenderErrorBoundary>
    </RoomInspectorPortalContext.Provider>
  )
}
