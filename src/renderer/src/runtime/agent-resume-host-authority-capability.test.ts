...
describe('agentResumeHostAuthorityCapability', () => {
  ...
  it('gates Kimchi resume behind its own capability', () => {
    expect(agentResumeHostAuthorityCapability('kimchi')).toBe(
      AGENT_SESSION_KIMCHI_RESUME_RUNTIME_CAPABILITY
    )
  })
  ...
  it('advertises the Kimchi resume capability from the host', () => {
    expect(RUNTIME_CAPABILITIES).toContain(AGENT_SESSION_KIMCHI_RESUME_RUNTIME_CAPABILITY)
  })
