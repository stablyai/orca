...
vi.mock('@/lib/worktree-operation-route', () => ({
  resolveActiveWorkspaceRoute: () => null,
  resolveWorktreeOperationRouteResult: () => mocks.resolveRoute()
}))
...
describe('runtime database client project routing', () => {
  beforeEach(() => {
    mocks.resolveRoute.mockReset().mockReturnValue({ kind: 'resolved' })
  ...
  it.each(['ambiguous', 'missing'])(
    'rejects %s project ownership before sending credentials',
    async (kind) => {
      mocks.resolveRoute.mockReturnValue({ kind })
      await expect(testDatabaseConnection('aps::worktree', request)).rejects.toThrow('host')
      expect(mocks.callRuntimeRpc).not.toHaveBeenCalled()
    }
  )

  it('cancels on the original runtime after the workspace owner changes', async () => {
    const deferred = Promise.withResolvers<never>()
    mocks.callRuntimeRpc.mockImplementation((_target, method) =>
      method === 'database.execute' ? deferred.promise : Promise.resolve({ canceled: true })
    )
    const running = executeDatabaseQuery('aps::worktree', {
      ...request,
    ...
    mocks.getRuntimeOwner.mockReturnValue('new-runtime')
    mocks.resolveRoute.mockReturnValue({ kind: 'missing' })
    await expect(
      cancelDatabaseQuery('another-workspace', 'postgres', 'pending-query')
    ).resolves.toBe(false)
    await expect(cancelDatabaseQuery('aps::worktree', 'postgres', 'pending-query')).resolves.toBe(
      true
    )
    expect(mocks.callRuntimeRpc).toHaveBeenLastCalledWith(
      { kind: 'environment', environmentId: 'linux-jae' },
      'database.cancel',
      { providerId: 'postgres', queryId: 'pending-query' },
      { timeoutMs: 5000 }
    )
