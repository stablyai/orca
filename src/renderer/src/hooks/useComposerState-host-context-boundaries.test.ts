...
describe('useComposerState host-context boundaries', () => {
  ...
  it('plans new workspace agent startup from the selected repo runtime', () => {
    expect(HOOK_SOURCE).toContain('const selectedRepoAgentStartupTarget = useMemo')
    expect(HOOK_SOURCE).toContain('getLocalRepoProjectExecutionRuntimeContext')
    expect(HOOK_SOURCE).toContain('resolveAgentStartupTarget')
    expect(HOOK_SOURCE).toContain('projectRuntime: selectedRepoProjectRuntime')
