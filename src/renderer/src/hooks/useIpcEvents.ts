  ...
  useEffect(() => {
    ...
    function applyAgentStatusBatch(
      ...
      return useAppStore.getState().transactAgentStatuses((transaction) => {
        ...
        const results = events.map(({ data, replay, retry, allowBeforeWorkspaceReady }) =>
          applyAgentStatus(data, { batch, replay, retry, allowBeforeWorkspaceReady })
        )
    ...
    // Why: once the startup barrier applied the hook cache, a live push (e.g. a
    // remote Stop landing during reconnect) must not be dropped pre-ready, or the
    // stale working row would drive cold restore. Read the flag at apply time so a
    // burst queued before the barrier still lands once it opens.
    const applyLiveAgentStatus = (data: AgentStatusIpcPayload): AgentStatusApplyResult =>
      applyAgentStatus(
        data,
    ...
    function applyLiveAgentStatusBatch(batch: readonly AgentStatusIpcPayload[]): boolean {
      const allowBeforeWorkspaceReady = startupSnapshotBarrierStarted || undefined
      return applyAgentStatusBatch(batch.map((data) => ({ data, allowBeforeWorkspaceReady }))).some(
    ...
    function enqueueLiveAgentStatus(data: AgentStatusIpcPayload): void {
      ...
      if (
        ...
        if (applyLiveAgentStatus(data) !== 'applied') {
          lastLiveAgentStatusApplyAt = 0
        }
        return
      }
