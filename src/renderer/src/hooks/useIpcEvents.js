...
export function useIpcEvents() {
    useEffect(() => {
        const unsubs = [];
        unsubs.push(window.api.repos.onChanged(() => {
            useAppStore.getState().fetchRepos();
        }));
        unsubs.push(window.api.worktrees.onChanged((data) => {
            useAppStore.getState().fetchWorktrees(data.repoId);
        }));
        unsubs.push(window.api.ui.onOpenSettings(() => {
            useAppStore.getState().openSettingsPage();
        }));
        unsubs.push(window.api.ui.onToggleLeftSidebar(() => {
            dispatchClearModifierHints();
            useAppStore.getState().toggleSidebar();
        }));
        unsubs.push(window.api.ui.onToggleRightSidebar(() => {
            dispatchClearModifierHints();
            useAppStore.getState().toggleRightSidebar();
        }));
        unsubs.push(window.api.ui.onToggleWorktreePalette(() => {
            dispatchClearModifierHints();
        ...
        unsubs.push(window.api.ui.onOpenQuickOpen(() => {
            dispatchClearModifierHints();
            const store = useAppStore.getState();
        ...
        unsubs.push(window.api.ui.onJumpToWorktreeIndex((index) => {
            dispatchClearModifierHints();
            const store = useAppStore.getState();
        ...
        unsubs.push(window.api.ui.onToggleStatusBar(() => {
            const store = useAppStore.getState();
            store.setStatusBarVisible(!store.statusBarVisible);
        }));
        unsubs.push(window.api.ui.onActivateWorktree(({ repoId, worktreeId, setup }) => {
            void (async () => {
                const store = useAppStore.getState();
        ...
        unsubs.push(window.api.updater.onStatus((raw) => {
            const status = raw;
            useAppStore.getState().setUpdateStatus(status);
        }));
        unsubs.push(window.api.updater.onClearDismissal(() => {
            useAppStore.getState().clearDismissedUpdateVersion();
        }));
        unsubs.push(window.api.ui.onFullscreenChanged((isFullScreen) => {
            useAppStore.getState().setIsFullScreen(isFullScreen);
        }));
        unsubs.push(window.api.browser.onGuestLoadFailed(({ browserPageId, loadError }) => {
        ...
        unsubs.push(window.api.browser.onOpenLinkInOrcaTab(({ browserPageId, url }) => {
            const store = useAppStore.getState();
        ...
        unsubs.push(window.api.ui.onNewBrowserTab(() => {
            const store = useAppStore.getState();
        ...
        unsubs.push(window.api.ui.onNewTerminalTab(() => {
            const store = useAppStore.getState();
            const worktreeId = store.activeWorktreeId;
            if (!worktreeId) {
                return;
        ...
        unsubs.push(window.api.ui.onCloseActiveTab(() => {
            const store = useAppStore.getState();
        ...
        unsubs.push(window.api.rateLimits.onUpdate((state) => {
            useAppStore.getState().setRateLimitsFromPush(state);
        }));
        ...
        void (async () => {
            try {
                ...
                for (const target of targets) {
                    ...
                    if (state) {
                        useAppStore.getState().setSshConnectionState(target.id, state);
                    }
                }
        ...
        unsubs.push(window.api.ssh.onCredentialRequest((data) => {
            useAppStore.getState().enqueueSshCredentialRequest(data);
        }));
        unsubs.push(window.api.ssh.onCredentialResolved(({ requestId }) => {
            useAppStore.getState().removeSshCredentialRequest(requestId);
        }));
        unsubs.push(window.api.ssh.onStateChanged((data) => {
            const store = useAppStore.getState();
            ...
            if (['disconnected', 'auth-failed', 'reconnection-failed', 'error'].includes(state.status)) {
                ...
                const remoteWorktreeIds = new Set(Object.values(store.worktreesByRepo)
                    .flat()
                    ...
                    for (const worktreeId of worktreeIds) {
                        ...
                        if (hasDead) {
                            useAppStore.setState((s) => ({
                                tabsByWorktree: {
                                    ...s.tabsByWorktree,
                                    [worktreeId]: (s.tabsByWorktree[worktreeId] ?? []).map((t) => t.ptyId ? t : { ...t, generation: (t.generation ?? 0) + 1 })
                                }
                            }));
                        }
                    }
        ...
        unsubs.push(window.api.ui.onTerminalZoom((direction) => {
            const { activeView, activeTabType, editorFontZoomLevel, setEditorFontZoomLevel, settings } = useAppStore.getState();
            ...
            const rawNext = direction === 'in' ? current + ZOOM_STEP : direction === 'out' ? current - ZOOM_STEP : 0;
