...
describe('useIpcEvents agent status snapshot integration', () => {
  ...
  it('applies an authoritative live restart for a retired pane', async () => {
    const setAgentStatus = vi.fn()
    const onSetListenerRef: { current: ((data: AgentStatusSetData) => void) | null } = {
      current: null
    }
    const storeState: StoreLike = buildStoreState({
      setAgentStatus,
      workspaceSessionReady: true,
      recentlyRetiredAgentStatusPaneKeys: { [FUTURE_PANE_KEY]: true },
      settings: { terminalFontSize: 13, notifications: { enabled: false } },
      tabsByWorktree: {
    ...
    onSetListenerRef.current({
      paneKey: FUTURE_PANE_KEY,
      state: 'done',
      prompt: 'late retired completion',
      agentType: 'omp',
    ...
    expect(setAgentStatus).toHaveBeenCalledWith(
      FUTURE_PANE_KEY,
      expect.objectContaining({ state: 'working', prompt: 'OMP new turn', agentType: 'omp' }),
      'Future Tab',
