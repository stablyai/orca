...
describe('closeTab and the session grid hidden tabs', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockApi.worktrees.updateMeta.mockResolvedValue({})
  })

  it('drops the closed tab from the hidden list', () => {
    const store = storeWithTabs()
    store.setState({ sessionsGridHiddenTabIds: ['tab-a', 'other-workspace-tab'] })

    store.getState().closeTab('tab-a')

    expect(store.getState().sessionsGridHiddenTabIds).toEqual(['other-workspace-tab'])
  })

  it('leaves the hidden list untouched — same reference — when the tab was never hidden', () => {
    const store = storeWithTabs()
    const hidden = ['other-workspace-tab']
    store.setState({ sessionsGridHiddenTabIds: hidden })
