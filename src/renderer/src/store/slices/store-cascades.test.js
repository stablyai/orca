...
import { createTestStore, makeLayout, makeOpenFile, makeTab, makeTabGroup, makeUnifiedTab, makeWorktree, seedStore } from './store-test-helpers';
// ─── Tests ────────────────────────────────────────────────────────────
...
describe('setActiveWorktree', () => {
    ...
    it('creates a root tab group when the first terminal opens in a worktree', () => {
        ...
        expect(unifiedTabs).toEqual(expect.arrayContaining([
    ...
    it('falls back to the stable terminal label when a live title clears', () => {
        ...
        expect(store
            .getState()
            .unifiedTabsByWorktree[wt]?.find((tab) => tab.contentType === 'terminal' && tab.entityId === first.id)).toMatchObject({
            label: 'Terminal 1'
        });
    });
