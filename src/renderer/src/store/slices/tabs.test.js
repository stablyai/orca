...
describe('TabsSlice', () => {
    ...
    describe('move/copy/merge group operations', () => {
        it('moves a unified tab into another group', () => {
            ...
            expect(state.groupsByWorktree[WT].find((group) => group.id === sourceGroupId)?.tabOrder).toEqual([]);
            expect(state.groupsByWorktree[WT].find((group) => group.id === targetGroupId)?.tabOrder).toEqual([tab.id]);
        });
        it('copies a unified tab into another group', () => {
            ...
            const state = store.getState();
            expect(state.unifiedTabsByWorktree[WT]).toHaveLength(2);
            expect(state.groupsByWorktree[WT].find((group) => group.id === sourceGroupId)?.tabOrder).toEqual([tab.id]);
            expect(state.groupsByWorktree[WT].find((group) => group.id === targetGroupId)?.tabOrder).toEqual([copied.id]);
        ...
        it('drops a unified tab onto a pane edge to create a sibling split', () => {
            ...
            expect(state.groupsByWorktree[WT].find((group) => group.id === newGroupId)?.tabOrder).toEqual([second.id]);
