...
function normalizeBrowserTitle(title, url) {
    if (url === 'about:blank' ||
...
function buildBrowserPage(workspaceId, worktreeId, url, title) {
...
function buildWorkspaceFromPage(id, worktreeId, page, pageIds, sessionProfileId) {
    return {
        id,
        worktreeId,
...
function mirrorWorkspaceFromActivePage(workspace, pages) {
...
function getFallbackTabTypeForWorktree(worktreeId, openFiles, terminalTabsByWorktree, browserTabsByWorktree) {
...
function findWorkspace(browserTabsByWorktree, workspaceId) {
    return (Object.values(browserTabsByWorktree)
        .flat()
        .find((workspace) => workspace.id === workspaceId) ?? null);
}
function findPage(browserPagesByWorkspace, pageId) {
    return (Object.values(browserPagesByWorkspace)
        .flat()
        .find((page) => page.id === pageId) ?? null);
}
export const createBrowserSlice = (set, get) => ({
    ...
    createBrowserTab: (worktreeId, url, options) => {
        ...
        const browserTab = buildWorkspaceFromPage(workspaceId, worktreeId, page, [page.id], options?.sessionProfileId);
        set((s) => {
            ...
            const shouldFocusAddressBar = shouldUpdateGlobalActiveSurface &&
        ...
        const state = get();
        const alreadyHasUnifiedTab = (state.unifiedTabsByWorktree[worktreeId] ?? []).some((t) => t.contentType === 'browser' && t.entityId === workspaceId);
    ...
    closeBrowserTab: (tabId) => {
        set((s) => {
            let owningWorktreeId = null;
            let closedWorkspace = null;
            const nextBrowserTabsByWorktree = {};
            ...
            const nextTabBarOrder = {
                ...s.tabBarOrderByWorktree,
                [owningWorktreeId]: (s.tabBarOrderByWorktree[owningWorktreeId] ?? []).filter((entryId) => entryId !== tabId)
            };
            const isActiveTabInOwningWorktree = s.activeWorktreeId === owningWorktreeId && s.activeBrowserTabId === tabId;
            const nextActiveTabTypeByWorktree = { ...s.activeTabTypeByWorktree };
            let nextActiveTabType = s.activeTabType;
            if (remainingBrowserTabs.length === 0) {
                const fallbackTabType = getFallbackTabTypeForWorktree(owningWorktreeId, s.openFiles, s.tabsByWorktree);
            ...
            const nextPendingAddressBarFocusByPageId = Object.fromEntries(Object.entries(s.pendingAddressBarFocusByPageId).filter(([pageId]) => !closedPages.some((page) => page.id === pageId)));
            const nextPendingAddressBarFocusByTabId = Object.fromEntries(Object.entries(s.pendingAddressBarFocusByTabId).filter(([focusId]) => focusId !== tabId && !closedPages.some((page) => page.id === focusId)));
            return {
                browserTabsByWorktree: nextBrowserTabsByWorktree,
                browserPagesByWorkspace: nextBrowserPagesByWorkspace,
                activeBrowserTabId: s.activeBrowserTabId === tabId
                    ? (neighborTabId ?? remainingBrowserTabs[0]?.id ?? null)
        ...
        for (const tabs of Object.values(get().unifiedTabsByWorktree)) {
            const workspaceItem = tabs.find((entry) => entry.contentType === 'browser' && entry.entityId === tabId);
            if (workspaceItem) {
    ...
    reopenClosedBrowserTab: (worktreeId) => {
        ...
        if (activePageId) {
            ...
            const targetPage = restoredPages.find((p) => p.url === pages.find((orig) => orig.id === activePageId)?.url);
    ...
    createBrowserPage: (workspaceId, url, options) => {
        ...
        set((s) => {
            ...
            const shouldUpdateGlobalActiveSurface = shouldActivate &&
            ...
            return {
                ...
                browserTabsByWorktree: {
                    ...s.browserTabsByWorktree,
                    [workspace.worktreeId]: (s.browserTabsByWorktree[workspace.worktreeId] ?? []).map((tab) => tab.id === workspaceId ? nextWorkspace : tab)
                },
                pendingAddressBarFocusByPageId: shouldFocusAddressBar
                    ? {
                        ...s.pendingAddressBarFocusByPageId,
    ...
    closeBrowserPage: (pageId) => {
        set((s) => {
            ...
            const nextActivePageId = workspace.activePageId === pageId
            ...
            return {
                ...
                browserTabsByWorktree: {
                    ...s.browserTabsByWorktree,
                    [workspace.worktreeId]: (s.browserTabsByWorktree[workspace.worktreeId] ?? []).map((tab) => tab.id === workspace.id ? nextWorkspace : tab)
                },
                recentlyClosedBrowserPagesByWorkspace: {
                    ...s.recentlyClosedBrowserPagesByWorkspace,
                    [workspace.id]: [
                        page,
                        ...(s.recentlyClosedBrowserPagesByWorkspace[workspace.id] ?? []).filter((entry) => entry.id !== page.id)
                    ].slice(0, 10)
                },
                pendingAddressBarFocusByPageId: Object.fromEntries(Object.entries(s.pendingAddressBarFocusByPageId).filter(([pendingPageId]) => pendingPageId !== pageId)),
                pendingAddressBarFocusByTabId: Object.fromEntries(Object.entries(s.pendingAddressBarFocusByTabId).filter(([pendingPageId]) => pendingPageId !== pageId))
            };
        });
    ...
    consumeAddressBarFocusRequest: (pageId) => {
        const state = get();
        if (!state.pendingAddressBarFocusByPageId[pageId] &&
    ...
    updateBrowserPageState: (pageId, updates) => {
        set((s) => {
            ...
            const nextPages = (s.browserPagesByWorkspace[workspace.id] ?? []).map((entry) => entry.id === pageId
                ? {
                    ...entry,
    ...
    setBrowserPageUrl: (pageId, url) => set((s) => {
        const page = findPage(s.browserPagesByWorkspace, pageId);
        if (!page) {
            return s;
        }
    ...
    hydrateBrowserSession: (session) => {
        set((s) => {
            ...
            for (const [worktreeId, tabs] of Object.entries(persistedTabsByWorktree)) {
                ...
                for (const tab of tabs) {
                    ...
                    hydratedTabs.push(mirrorWorkspaceFromActivePage({
                        ...tab,
            ...
            const validBrowserTabIds = new Set(Object.values(browserTabsByWorktree)
                .flat()
                .map((tab) => tab.id));
            ...
            const activeBrowserTabId = activeWorktreeId && activeBrowserTabIdByWorktree[activeWorktreeId]
            ...
            for (const worktreeId of validWorktreeIds) {
                ...
                if (persistedActiveTabTypeByWorktree[worktreeId] === 'browser' &&
                    hasBrowserTabs &&
                ...
                if (nextActiveTabTypeByWorktree[worktreeId] === 'browser' && !hasBrowserTabs) {
                    nextActiveTabTypeByWorktree[worktreeId] = getFallbackTabTypeForWorktree(worktreeId, s.openFiles, s.tabsByWorktree, browserTabsByWorktree);
                }
            }
            const activeTabType = (() => {
                if (!activeWorktreeId) {
                ...
                if (restoredTabType === 'editor' &&
                    s.openFiles.some((file) => file.worktreeId === activeWorktreeId)) {
                    return 'editor';
                }
                return getFallbackTabTypeForWorktree(activeWorktreeId, s.openFiles, s.tabsByWorktree, browserTabsByWorktree);
            })();
            return {
                browserTabsByWorktree,
                browserPagesByWorkspace,
        ...
        for (const [worktreeId, browserTabs] of Object.entries(state.browserTabsByWorktree)) {
            for (const bt of browserTabs) {
                const exists = (state.unifiedTabsByWorktree[worktreeId] ?? []).some((t) => t.contentType === 'browser' && t.entityId === bt.id);
                if (!exists) {
                    state.createUnifiedTab(worktreeId, 'browser', {
                        entityId: bt.id,
                        label: bt.title
    ...
    fetchDetectedBrowsers: async () => {
        try {
            const browsers = (await window.api.browser.sessionDetectBrowsers());
    ...
    addBrowserHistoryEntry: (url, title) => {
        ...
        set((s) => {
            ...
            let next = existing
                ? s.browserUrlHistory.map((entry) => entry === existing
