...
import { ensurePtyDispatcher, unregisterPtyDataHandlers } from '@/components/terminal-pane/pty-transport';
function getNextTerminalOrdinal(tabs) {
...
function getFallbackTabTitle(tab, index) {
...
export const createTerminalSlice = (set, get) => ({
    ...
    createTab: (worktreeId, targetGroupId) => {
        ...
        set((s) => {
            ...
            const existing = (s.tabsByWorktree[worktreeId] ?? []).filter((entry) => !orphanTerminalIds.has(entry.id));
            ...
            return {
                ...
                activeGroupIdByWorktree: targetGroupId &&
    ...
    closeTab: (tabId) => {
        ...
        for (const tabs of Object.values(get().unifiedTabsByWorktree)) {
            const workspaceItem = tabs.find((entry) => entry.contentType === 'terminal' && entry.entityId === tabId);
            if (workspaceItem) {
    ...
    updateTabTitle: (tabId, title) => {
        ...
        const item = Object.values(get().unifiedTabsByWorktree)
            ...
            .find((entry) => entry.contentType === 'terminal' && entry.entityId === tabId);
        if (item) {
            const resolvedTitle = Object.values(get().tabsByWorktree)
                .flat()
    ...
    shutdownWorktreeTerminals: async (worktreeId) => {
        ...
        set((s) => {
            const nextTabsByWorktree = {
                ...s.tabsByWorktree,
                [worktreeId]: (s.tabsByWorktree[worktreeId] ?? []).map((tab, index) => clearTransientTerminalState(tab, index))
            };
            ...
            const nextSuppressedPtyExitIds = {
                ...
                ...Object.fromEntries(ptyIds.map((ptyId) => [ptyId, true]))
            };
            const nextPendingCodexPaneRestartIds = { ...s.pendingCodexPaneRestartIds };
            const nextCodexRestartNoticeByPtyId = { ...s.codexRestartNoticeByPtyId };
            ...
            return {
                ...
                ...(shouldResetGlobalBrowser
                    ? { activeBrowserTabId: null, activeTabType: 'terminal' }
                    : {})
            };
        });
        if (ptyIds.length === 0) {
    ...
    hydrateWorkspaceSession: (session) => {
        set((s) => {
            ...
            const tabsByWorktree = Object.fromEntries(Object.entries(session.tabsByWorktree)
            ...
            const validTabIds = new Set(Object.values(tabsByWorktree)
                .flat()
                .map((tab) => tab.id));
            const activeWorktreeId = session.activeWorktreeId && validWorktreeIds.has(session.activeWorktreeId)
                ? session.activeWorktreeId
                : null;
            const activeTabId = session.activeTabId && validTabIds.has(session.activeTabId) ? session.activeTabId : null;
            const activeRepoId = session.activeRepoId && s.repos.some((repo) => repo.id === session.activeRepoId)
            ...
            const pendingReconnectTabByWorktree = {};
            ...
            const pendingReconnectPtyIdByTabId = {};
            for (const worktreeId of pendingReconnectWorktreeIds) {
            ...
            return {
                ...
                pendingReconnectTabByWorktree,
                pendingReconnectPtyIdByTabId,
                ptyIdsByTabId: Object.fromEntries(Object.values(tabsByWorktree)
                    .flat()
                    .map((tab) => [tab.id, []])),
                ...
                terminalLayoutsByTabId: Object.fromEntries(Object.entries(session.terminalLayoutsByTabId).filter(([tabId]) => validTabIds.has(tabId)))
            };
        });
    },
    reconnectPersistedTerminals: async (_signal) => {
        const { pendingReconnectWorktreeIds, pendingReconnectTabByWorktree, pendingReconnectPtyIdByTabId, terminalLayoutsByTabId, tabsByWorktree } = get();
        ...
        for (const worktreeId of ids) {
            ...
            const tabsToReconnect = targetTabIds.length > 0
                ? targetTabIds
            ...
            for (const tab of tabsToReconnect) {
                ...
                if (supportsDeferredReattach && tabLevelPtyId) {
                    set((s) => {
                        ...
                        next[worktreeId] = next[worktreeId].map((t) => t.id === tabId ? { ...t, ptyId: tabLevelPtyId } : t);
                        ...
                        const allPtyIds = hasLeafMappings
                            ? Object.values(leafPtyMap).filter(Boolean)
                            : [tabLevelPtyId];
                        return {
                            tabsByWorktree: next,
                            ptyIdsByTabId: {
