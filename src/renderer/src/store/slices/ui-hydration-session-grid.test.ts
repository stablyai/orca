...
describe('session grid hydration', () => {
  ...
  it('ignores a preset, scroll mode, wheel target or state filter the layout has no case for', () => {
    ...
    expect(store.getState().sessionsGridWheelTarget).toBe('focus')
...
describe('Focus wheel preference compatibility', () => {
  it('defaults to Focus and migrates legacy Auto without producing a write echo', () => {
    const store = createUIStore()
    expect(store.getState().sessionsGridWheelTarget).toBe('focus')
    store
      .getState()
      .hydratePersistedUI(makePersistedUI({ sessionsGridWheelTarget: 'auto' }), 'startup')
    const state = store.getState()
    expect(state.sessionsGridWheelTarget).toBe('focus')
    expect(
      diffPersistedUIWriteFields(
  ...
  it.each(['focus', 'terminal', 'grid'] as const)(
    'round-trips %s using the shipped wire enum',
    (mode) => {
      const store = createUIStore()
      store
        .getState()
        .hydratePersistedUI(makePersistedUI({ sessionsGridWheelTarget: 'terminal' }), 'startup')
      store.getState().setSessionsGridWheelTarget(mode)
      const patch = persistedUIWriteFieldsToWireUpdate({
        sessionsGridWheelTarget: mode,
        sidebarWidth: 321
      })
      expect(patch).toEqual({
        sessionsGridWheelTarget: mode === 'focus' ? 'auto' : mode,
        sidebarWidth: 321
      })
      const other = createUIStore()
      other.getState().hydratePersistedUI(makePersistedUI(patch), 'startup')
      expect(other.getState().sessionsGridWheelTarget).toBe(mode)
      expect(other.getState().sidebarWidth).toBe(321)
    }
  )

  it('preserves an unflushed Focus choice across a stale Grid broadcast', () => {
    const store = createUIStore()
    store
      .getState()
      .hydratePersistedUI(makePersistedUI({ sessionsGridWheelTarget: 'grid' }), 'startup')
    store.getState().setSessionsGridWheelTarget('focus')
    store
      .getState()
      .hydratePersistedUI(makePersistedUI({ sessionsGridWheelTarget: 'grid' }), 'sync')
    expect(store.getState().sessionsGridWheelTarget).toBe('focus')
  })
})
