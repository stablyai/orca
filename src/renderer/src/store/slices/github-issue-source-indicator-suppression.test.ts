...
describe('IssueSourceIndicator suppression', () => {
  it('hides when sources deep-equal, shows when they differ, hides when either is null', async () => {
    ...
    // Case-insensitive equality — the parent design doc calls out that `StablyAI/MCode`
    // and `mcode-ide/mcode` resolve to the same repo and must suppress.
    expect(
      sameGitHubOwnerRepo({ owner: 'StablyAI', repo: 'MCode' }, { owner: 'stablyai', repo: 'mcode' })
    ).toBe(true)
    expect(
      sameGitHubOwnerRepo(
        { owner: 'stablyai', repo: 'mcode', host: 'github.com' },
        { owner: 'stablyai', repo: 'mcode', host: 'ghe.example.test' }
      )
    ).toBe(false)
