import { findWorktreeById, applyWorktreeUpdates, getRepoIdFromWorktreeId } from './worktree-helpers';
function areWorktreesEqual(current, next) {
    if (!current || current.length !== next.length) {
        return false;
    }
    return current.every((worktree, index) => {
        const candidate = next[index];
        return (worktree.id === candidate.id &&
...
export const createWorktreeSlice = (set, get) => ({
    worktreesByRepo: {},
    ...
    createWorktree: async (repoId, name, baseBranch, setupDecision = 'inherit') => {
        ...
        const nextCandidateName = (current, attempt) => attempt === 0 ? current : `${current}-${attempt + 1}`;
        try {
    ...
    removeWorktree: async (worktreeId, force) => {
        ...
        try {
            ...
            set((s) => {
                ...
                const removedFileIds = new Set(s.openFiles.filter((f) => f.worktreeId === worktreeId).map((f) => f.id));
                const nextEditorDrafts = removedFileIds.size > 0 ? { ...s.editorDrafts } : s.editorDrafts;
                const nextMarkdownViewMode = removedFileIds.size > 0 ? { ...s.markdownViewMode } : s.markdownViewMode;
    ...
    setActiveWorktree: (worktreeId) => {
        ...
        set((s) => {
            ...
            const activeGroupId = s.activeGroupIdByWorktree[worktreeId] ?? s.groupsByWorktree[worktreeId]?.[0]?.id ?? null;
            ...
            const activeUnifiedTab = activeUnifiedTabId != null
                ? ((s.unifiedTabsByWorktree[worktreeId] ?? []).find((tab) => tab.id === activeUnifiedTabId && (!activeGroup || tab.groupId === activeGroup.id)) ?? null)
                : null;
            ...
            const browserTabStillOpen = restoredBrowserTabId
                ...
                : false;
            const hasGroupOwnedSurface = (s.groupsByWorktree[worktreeId]?.length ?? 0) > 0 || Boolean(s.layoutByWorktree[worktreeId]);
            ...
            const tabStillExists = restoredTabId
                ...
                : false;
            const activeTabId = activeUnifiedTab?.contentType === 'terminal'
                ? activeUnifiedTab.entityId
                : tabStillExists
                    ? restoredTabId
            ...
            const metaUpdates = shouldClearUnread ? { isUnread: false } : {};
            return {
