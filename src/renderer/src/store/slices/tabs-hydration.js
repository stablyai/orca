import { dedupeTabOrder, getPersistedEditFileIdsByWorktree, isTransientEditorContentType, selectHydratedActiveGroupId } from './tabs-helpers';
function pruneLayoutForGroups(root, validGroupIds) {
    if (root.type === 'leaf') {
...
function hydrateUnifiedFormat(session, validWorktreeIds) {
    ...
    for (const [worktreeId, tabs] of Object.entries(session.unifiedTabs)) {
        ...
        const persistedEditFileIds = persistedEditFileIdsByWorktree[worktreeId] ?? new Set();
    ...
    for (const [worktreeId, groups] of Object.entries(session.tabGroups)) {
        ...
        const activeGroupId = selectHydratedActiveGroupId(hydratedGroups, session.activeGroupIdByWorktree?.[worktreeId]);
        if (activeGroupId) {
...
function hydrateLegacyFormat(session, validWorktreeIds) {
    const tabsByWorktree = {};
    const groupsByWorktree = {};
    const activeGroupIdByWorktree = {};
    const layoutByWorktree = {};
...
export function buildHydratedTabState(session, validWorktreeIds) {
