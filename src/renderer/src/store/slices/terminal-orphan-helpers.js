export function getOrphanTerminalIds(state, worktreeId) {
    const runtimeTabs = state.tabsByWorktree[worktreeId] ?? [];
    const unifiedTerminalEntityIds = new Set((state.unifiedTabsByWorktree[worktreeId] ?? [])
        .filter((tab) => tab.contentType === 'terminal')
        .map((tab) => tab.entityId));
    return new Set(runtimeTabs
        .filter((tab) => {
...
export function buildOrphanTerminalCleanupPatch(state, worktreeId, orphanTerminalIds) {
    const nextTabs = (state.tabsByWorktree[worktreeId] ?? []).filter((tab) => !orphanTerminalIds.has(tab.id));
    ...
    const nextTabBarOrderByWorktree = {
        ...
        [worktreeId]: (state.tabBarOrderByWorktree[worktreeId] ?? []).filter((tabId) => !orphanTerminalIds.has(tabId))
    };
    ...
    return {
        ...
        activeTabId: state.activeTabId && orphanTerminalIds.has(state.activeTabId) ? null : state.activeTabId
    };
}
