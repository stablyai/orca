...
function openWorkspaceEditorItem(state, fileId, worktreeId, label, contentType, isPreview, targetGroupId) {
...
export const createEditorSlice = (set, get) => ({
    editorDrafts: {},
    setEditorDraft: (fileId, content) => set((s) => ({
        editorDrafts: { ...s.editorDrafts, [fileId]: content }
    })),
    clearEditorDraft: (fileId) => set((s) => {
    ...
    clearEditorDrafts: (fileIds) => set((s) => {
        if (fileIds.length === 0) {
            return s;
        }
        const next = { ...s.editorDrafts };
    ...
    // Markdown view mode
    markdownViewMode: {},
    setMarkdownViewMode: (fileId, mode) => set((s) => ({
    ...
    // File explorer
    expandedDirs: {},
    toggleDir: (worktreeId, dirPath) => set((s) => {
        const current = s.expandedDirs[worktreeId] ?? new Set();
    ...
    revealInExplorer: (worktreeId, filePath) => set({
    ...
    openFile: (file, options) => {
        set((s) => {
            ...
            if (existing) {
                ...
                return {
                    openFiles: s.openFiles.map((f) => f.id === id
                        ? {
                            ...f,
                            mode: file.mode,
            ...
            if (isPreview) {
                const existingPreviewIdx = s.openFiles.findIndex((f) => f.worktreeId === worktreeId && f.isPreview);
                if (existingPreviewIdx !== -1) {
                    const replacedPreview = s.openFiles[existingPreviewIdx];
                    const nextEditorDrafts = replacedPreview.id === id
                        ? s.editorDrafts
                        : Object.fromEntries(Object.entries(s.editorDrafts).filter(([fileId]) => fileId !== replacedPreview.id));
                    const nextMarkdownViewMode = replacedPreview.id === id
                        ? s.markdownViewMode
                        : Object.fromEntries(Object.entries(s.markdownViewMode).filter(([fileId]) => fileId !== replacedPreview.id));
                    ...
                    const nextEditorCursorLine = replacedPreview.id === id
                        ? s.editorCursorLine
                        : Object.fromEntries(Object.entries(s.editorCursorLine).filter(([fileId]) => fileId !== replacedPreview.id));
                    // Replace in-place to preserve tab position
                    newFiles = s.openFiles.map((f, i) => i === existingPreviewIdx ? { ...file, id, isDirty: false, isPreview: true } : f);
        ...
        void openWorkspaceEditorItem(get(), file.filePath, file.worktreeId, file.relativePath, file.mode === 'conflict-review'
            ? 'conflict-review'
            : file.mode === 'diff'
                ? 'diff'
                : 'editor', options?.preview ?? false, options?.targetGroupId);
    },
    ...
    closeFile: (fileId) => {
        set((s) => {
            ...
            const terminalTabsForWorktree = activeWorktreeId
                ...
                : [];
            const fallbackBrowserTabId = activeWorktreeId && browserTabsForWorktree.length > 0
                ? (s.activeBrowserTabIdByWorktree[activeWorktreeId] ??
                    browserTabsForWorktree[0]?.id ??
                    null)
                : s.activeBrowserTabId;
            const newActiveTabType = remainingForWorktree.length > 0
                ? s.activeTabType
            ...
            const shouldDeactivateWorktree = activeWorktreeId !== null &&
            ...
            const nextTabBarOrderByWorktree = worktreeId && s.tabBarOrderByWorktree
                ? {
                    ...s.tabBarOrderByWorktree,
                    [worktreeId]: (s.tabBarOrderByWorktree[worktreeId] ?? []).filter((entryId) => entryId !== fileId)
                }
                : s.tabBarOrderByWorktree;
            ...
            if (closedFile && wtRecent) {
                ...
                nextRecentlyClosed = {
                    ...
                    [wtRecent]: [snap, ...stack].slice(0, MAX_RECENT_CLOSED_EDITOR_TABS)
                };
            }
            return {
                openFiles: newFiles,
        ...
        for (const tabs of Object.values(get().unifiedTabsByWorktree ?? {})) {
            const unifiedTab = tabs.find((entry) => entry.entityId === fileId &&
    ...
    closeAllFiles: () => {
        ...
        set((s) => {
            ...
            const newEditorDrafts = Object.fromEntries(Object.entries(s.editorDrafts).filter(([fileId]) => remainingFileIds.has(fileId)));
            const newMarkdownViewMode = Object.fromEntries(Object.entries(s.markdownViewMode).filter(([fileId]) => remainingFileIds.has(fileId)));
            const newEditorCursorLine = Object.fromEntries(Object.entries(s.editorCursorLine).filter(([fileId]) => remainingFileIds.has(fileId)));
            const newActiveFileIdByWorktree = { ...s.activeFileIdByWorktree };
            ...
            newActiveTabTypeByWorktree[activeWorktreeId] =
                browserTabsForWorktree.length > 0 ? 'browser' : 'terminal';
            const shouldDeactivateWorktree = browserTabsForWorktree.length === 0 && terminalTabsForWorktree.length === 0;
            ...
            const closedFileIds = new Set(s.openFiles.filter((f) => f.worktreeId === activeWorktreeId).map((f) => f.id));
            const nextTabBarOrderByWorktree = s.tabBarOrderByWorktree
                ? {
                    ...s.tabBarOrderByWorktree,
                    [activeWorktreeId]: (s.tabBarOrderByWorktree[activeWorktreeId] ?? []).filter((entryId) => !closedFileIds.has(entryId))
                }
                : s.tabBarOrderByWorktree;
            ...
            for (const f of [...closingFiles].reverse()) {
                ...
                nextRecentClosed = [snap, ...nextRecentClosed].slice(0, MAX_RECENT_CLOSED_EDITOR_TABS);
            }
            return {
                openFiles: newFiles,
                editorDrafts: newEditorDrafts,
    ...
    setActiveFile: (fileId) => {
        ...
        const groupId = state.activeGroupIdByWorktree?.[worktreeId] ?? state.groupsByWorktree?.[worktreeId]?.[0]?.id;
        if (!groupId) {
            return;
        }
        const item = state.findTabForEntityInGroup?.(worktreeId, groupId, fileId, 'editor') ??
    ...
    reorderFiles: (fileIds) => set((s) => {
        const reorderedSet = new Set(fileIds);
        const byId = new Map(s.openFiles.map((f) => [f.id, f]));
        const reordered = fileIds.map((id) => byId.get(id)).filter(Boolean);
    ...
    markFileDirty: (fileId, dirty) => set((s) => ({
        openFiles: s.openFiles.map((f) => f.id === fileId
            ? { ...f, isDirty: dirty, ...(dirty && f.isPreview ? { isPreview: undefined } : {}) }
            : f)
    })),
    clearUntitled: (fileId) => set((s) => ({
    ...
    openDiff: (worktreeId, filePath, relativePath, language, staged) => {
        set((s) => {
            const diffSource = staged ? 'staged' : 'unstaged';
        ...
        void openWorkspaceEditorItem(get(), `${worktreeId}::diff::${staged ? 'staged' : 'unstaged'}::${relativePath}`, worktreeId, relativePath, 'diff');
    },
    ...
    openAllDiffs: (worktreeId, worktreePath, alternate, areaFilter) => {
        ...
        const label = areaFilter
            ? ({ staged: 'Staged Changes', unstaged: 'Changes', untracked: 'Untracked Files' }[areaFilter] ?? 'All Changes')
            : 'All Changes';
        set((s) => {
    ...
    openConflictFile: (worktreeId, worktreePath, entry, language) => {
        ...
        set((s) => {
            ...
            const existing = s.openFiles.find((f) => f.id === id);
            const nextTracked = entry.conflictStatus === 'unresolved' && entry.conflictKind
                ? {
            ...
            if (existing) {
                return {
                    ...
                    activeFileIdByWorktree: { ...s.activeFileIdByWorktree, [worktreeId]: id },
                    activeTabTypeByWorktree: { ...s.activeTabTypeByWorktree, [worktreeId]: 'editor' },
                    trackedConflictPathsByWorktree: nextTracked === s.trackedConflictPathsByWorktree[worktreeId]
    ...
    openBranchAllDiffs: (worktreeId, worktreePath, compare, alternate) => {
        ...
        void openWorkspaceEditorItem(get(), id, worktreeId, `Branch Changes (${compare.baseRef})`, 'diff');
    },
    // Cursor line tracking
    editorCursorLine: {},
    setEditorCursorLine: (fileId, line) => set((s) => ({
    ...
    trackConflictPath: (worktreeId, path, conflictKind) => set((s) => {
        const nextTracked = {
            ...s.trackedConflictPathsByWorktree[worktreeId],
            [path]: conflictKind
        };
    ...
    setGitStatus: (worktreeId, status) => set((s) => {
        ...
        const normalizedEntries = status.entries.map((entry) => entry.conflictStatus === 'unresolved'
            ? { ...entry, conflictStatusSource: 'git' }
            : entry);
        const unresolvedEntries = normalizedEntries.filter((entry) => entry.conflictStatus === 'unresolved' && entry.conflictKind);
        ...
        const trackedUnchanged = areTrackedConflictMapsEqual(s.trackedConflictPathsByWorktree[worktreeId] ?? {}, currentTracked);
    ...
    setConflictOperation: (worktreeId, operation) => set((s) => {
        ...
        const nextTracked = operation === 'unknown' && prev !== 'unknown'
            ? {}
            : s.trackedConflictPathsByWorktree[worktreeId];
    ...
    beginGitBranchCompareRequest: (worktreeId, requestKey, baseRef) => set((s) => ({
        gitBranchCompareRequestKeyByWorktree: {
    ...
    setGitBranchCompareResult: (worktreeId, requestKey, result) => set((s) => {
        ...
        const entriesUnchanged = prevEntries &&
            prevEntries.length === result.entries.length &&
            prevEntries.every((e, i) => e.path === result.entries[i].path &&
    ...
    updateFileSearchState: (worktreeId, updates) => set((s) => {
    ...
    toggleFileSearchCollapsedFile: (worktreeId, filePath) => set((s) => {
        const current = s.fileSearchStateByWorktree[worktreeId];
        if (!current) {
            return s;
        }
    ...
    clearFileSearch: (worktreeId) => set((s) => {
        const current = s.fileSearchStateByWorktree[worktreeId];
        if (!current) {
            return s;
        }
    ...
    hydrateEditorSession: (session) => {
        set((s) => {
            ...
            const validWorktreeIds = new Set(Object.values(s.worktreesByRepo)
                .flat()
                .map((w) => w.id));
            const openFiles = [];
            ...
            const activeTabType = activeWorktreeId && persistedActiveTabTypeByWorktree[activeWorktreeId]
            ...
            const filteredActiveFileIdByWorktree = Object.fromEntries([...validWorktreeIds].flatMap((wId) => {
            ...
            const filteredActiveTabTypeByWorktree = Object.fromEntries(Object.entries(persistedActiveTabTypeByWorktree).filter(([wId, tabType]) => {
            ...
            const nextActiveTabType = nextActiveFileId || activeTabType !== 'editor' ? activeTabType : 'terminal';
            return {
                openFiles,
...
function getCompareVersion(compare) {
    return [
...
function toBranchCompareSnapshot(compare) {
    return {
...
function toOpenConflictMetadata(entry) {
...
function areGitStatusEntriesEqual(prev, next) {
    return (prev.length === next.length &&
        prev.every((entry, index) => entry.path === next[index].path &&
...
function areTrackedConflictMapsEqual(prev, next) {
    const prevKeys = Object.keys(prev);
    const nextKeys = Object.keys(next);
    return prevKeys.length === nextKeys.length && prevKeys.every((key) => prev[key] === next[key]);
}
function reconcileOpenFilesForStatus(openFiles, worktreeId, nextEntries) {
    ...
    const nextOpenFiles = openFiles.flatMap((file) => {
        ...
        if (file.conflict.kind === nextConflict.kind &&
