      ...
      try {
        ...
        if (
          ...
          (args.linkedTask?.provider === 'jira' ||
            ...
            args.linkedTask?.provider === 'mantisBT' ||
            args.linkedTaskSourceContext?.provider === 'mantisBT')
        ) {
    ...
    updateFolderWorkspace: async (folderWorkspaceId, updates, options) => {
      ...
      if (
        ...
        (updates.linkedTask?.provider === 'jira' ||
          ...
          updates.linkedTask?.provider === 'mantisBT' ||
          updates.linkedTaskSourceContext?.provider === 'mantisBT')
      ) {
        await assertRuntimeEnvironmentCapability(
          target.environmentId,
          WORKTREE_LINKED_WORK_ITEM_CONTEXT_RUNTIME_CAPABILITY,
