...
export function buildAgentContextQuery(
  ...
  const visibleSpecs = specs.filter((spec) => spec.hidden !== true)
  if (query.view === 'roots') {
    return buildRootsSchema(visibleSpecs)
  }
  if (query.view === 'command') {
    const spec = findExactSpec(visibleSpecs, query.value)
    if (!spec) {
      throw unknownAgentContextCommand(visibleSpecs, query.value)
    }
    return buildCommandsSchema(visibleSpecs, query, [spec], 1)
  }
  if (query.view === 'prefix') {
    const matches = findPrefixSpecs(visibleSpecs, query.value)
    if (matches.length === 0) {
      throw unknownAgentContextPrefix(visibleSpecs, query.value)
    }
    return buildCommandsSchema(visibleSpecs, query, matches, matches.length)
  }

  const matches = findSearchSpecs(visibleSpecs, query.value)
  return buildCommandsSchema(visibleSpecs, query, matches.slice(0, query.limit), matches.length)
}
