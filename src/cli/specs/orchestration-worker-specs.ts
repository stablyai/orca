...
export const ORCHESTRATION_WORKER_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    flagHelp: workerFlagHelp({
      ...
      spec: '<text> Full instructions for a task created alongside the worker',
      'task-title': '<text> Concise title for the task created from --spec',
      deps: '<json_array> Prerequisite task ids for the created task',
      parent: '<task_id> Parent task for the created task',
  ...
  },
  {
    ...
    flagHelp: workerFlagHelp({
      ...
      limit: '<1-100> Maximum number of rows to return'
    }),
    notes: [
