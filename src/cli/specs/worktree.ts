...
export const WORKTREE_COMMAND_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    usage:
      'orca worktree create --name <name> [--repo <selector>|--project <id> [--host <host-id>]|--project-host-setup <id>] [--recipe <recipe-id>] [--agent <id>] [--prompt <text>] [--setup run|skip|inherit] [--base-branch <ref>] [--issue <number>] [--linear-issue <identifier-or-url>] [--comment <text>] [--parent-worktree <selector>] [--no-parent] [--run-hooks] [--activate] [--json]',
    allowedFlags: [
      ...GLOBAL_FLAGS,
      'repo',
      'project',
    ...
    notes: [
      ...
      "--recipe <recipe-id> provisions the repo's per-workspace environment recipe (environmentRecipes in orca.yaml) and creates the workspace on it — the CLI counterpart of the composer's \"Run on\" target. Only provisioned-root (SSH) recipes are supported; the recipe's create script runs on this runtime's machine, and deleting the worktree tears the environment down.",
    ...
    examples: [
      ...
      'orca worktree create --name vm-task --recipe cloud-sandbox --agent claude --prompt "hi" --json',
