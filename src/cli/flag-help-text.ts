...
export const FLAG_HELP_TEXT: Record<string, string> = {
  ...
  force: '--force                Force the operation only where the command documents it',
  ...
  'include-provider-state':
    '--include-provider-state Include redacted, last-known recipe lifecycle state',
  ...
  name: '--name <name>          Name for the newly created resource',
  ...
  recipe: '--recipe <id>         Environment recipe id from orca.yaml',
  ref: '--ref <ref>            Base ref to persist for the repo',
  repo: '--repo <selector>      Repo selector such as id:<id>, name:<name>, or path:<path>',
  'repo-path': '--repo-path <path>    Repository path containing orca.yaml',
