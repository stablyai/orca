  ...
  return await new Promise((resolve, reject) => {
    ...
    if (!transport) {
      reject(
        new RuntimeClientError(
          'runtime_unavailable',
          'No compatible transport found in MCode runtime metadata.'
        )
      )
      return
    }
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new RuntimeClientError(
          'runtime_timeout',
          'Timed out waiting for the MCode runtime to respond.'
        )
      )
    }, timeoutMs)
    ...
    socket.once('error', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'Could not connect to the running MCode app. Restart MCode and try again.'
        )
      })
    })
    ...
    socket.on('data', (chunk: string) => {
      ...
      while (newlineIndex !== -1 && !settled) {
        ...
        try {
          ...
          finish({
            ...
            error: new RuntimeClientError(
              ...
              'The MCode runtime returned an invalid response frame.'
            )
          })
          return
        }
        ...
        // setTimeout().refresh() is stable since Node 10 (MCode ships on
        ...
        // the decode boundary so version skew between the CLI and the MCode
        ...
        if (response.id !== requestId) {
          finish({
            ...
            error: new RuntimeClientError(
              'invalid_runtime_response',
              'The MCode runtime returned a mismatched response id.'
            )
          })
          return
        }
        if (response._meta?.runtimeId && response._meta.runtimeId !== metadata.runtimeId) {
          finish({
            ...
            error: new RuntimeClientError(
              'runtime_unavailable',
              'The MCode runtime changed while the request was in flight. Retry the command.'
            )
          })
          return
        }
