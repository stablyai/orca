...
import { RuntimeClientError, serveMCodeApp } from '../runtime-client'
import { stripElectronRunAsNode } from '../runtime/launch'

function envRecord(): Record<string, string> {
  // Why: the `mcode` launcher runs MCode's Electron binary as Node, so this CLI
...
export const CORE_HANDLERS: Record<string, CommandHandler> = {
  'claude-teams': async ({ client, rawArgs }) => {
    ...
    if (!paneKey) {
      throw new RuntimeClientError(
        'invalid_environment',
        'mcode claude-teams must be run inside an MCode terminal.'
      )
    }
  ...
  open: async ({ client, json }) => {
    const result = await client.openMCode()
  ...
  serve: async ({ flags, json }) => {
    ...
    const exitCode = await serveMCodeApp({
      json,
      port,
      pairingAddress:
