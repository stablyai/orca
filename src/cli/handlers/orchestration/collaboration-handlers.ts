...
function isCollaborationPriority(value: unknown): value is CollaborationPriority {
  return COLLABORATION_PRIORITIES.some((priority) => priority === value)
}
...
function getCollaborationPriority(flags: Map<string, string | boolean>): CollaborationPriority {
  ...
  if (!isCollaborationPriority(priority)) {
...
function validateCollaborationAdmission(admission: unknown): void {
  ...
  if (!isCollaborationPriority(admission.minPriority)) {
    throw new RuntimeClientError(
      'invalid_argument',
...
function isCollaborationMessageIds(value: unknown): value is string[] {
  return (
    Array.isArray(value) &&
    value.length >= 1 &&
    value.length <= COLLABORATION_ACK_MESSAGE_IDS_MAX &&
    value.every((entry) => typeof entry === 'string' && entry.length > 0)
  )
}
...
function getCollaborationMessageIds(flags: Map<string, string | boolean>): string[] {
  ...
  if (!isCollaborationMessageIds(parsed)) {
    throw new RuntimeClientError(
      'invalid_argument',
