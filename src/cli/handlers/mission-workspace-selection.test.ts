...
function testClient(call: unknown, isRemote = false): RuntimeClient {
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: these workspace-selection cases exercise only call and isRemote; RuntimeClient is a class so this focused double is not structurally assignable.
...
describe('mission start workspace selection', () => {
  ...
  it('does not resolve the local directory against a remote runtime', async () => {
    const call = vi.fn()
    const client = testClient(call, true)
