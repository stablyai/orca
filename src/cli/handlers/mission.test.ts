...
function testClient(
  call: unknown,
  options: { openOrca?: unknown; isRemote?: boolean } = {}
): RuntimeClient {
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: mission handlers in this suite read only call, isRemote, and the optional openOrca spy supplied here; RuntimeClient is a class so the focused double is not structurally assignable.
  return {
    call,
    isRemote: options.isRemote ?? false,
    ...(options.openOrca ? { openOrca: options.openOrca } : {})
...
describe('mission start supervisor', () => {
  ...
  it('does not activate or launch Orca before planning a mission from an existing pane', async () => {
    ...
    const client = testClient(call, { openOrca })
  ...
  it('falls back to the next mission agent when worker startup fails before task delivery', async () => {
    ...
    const workerStarts = call.mock.calls.filter(
  ...
  it('materializes collaboration topology, starts independent workers concurrently, waits by Run mailbox, and acknowledges lifecycle delivery', async () => {
    ...
    const call = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'orchestration.taskCreate') {
        ...
        const id = `task_${key}`
        // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: task deps are JSON.stringify output produced by this same fixture and are consumed only as a string-array dependency list.
