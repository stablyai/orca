...
function testClient(call: unknown, isRemote: boolean): RuntimeClient {
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: this coordinator fixture exercises only call and isRemote on RuntimeClient; the class is not structurally assignable.
  return { call, isRemote } as unknown as RuntimeClient
}
...
function fixture(options: ProbeOptions = {}) {
  ...
  const call = vi.fn(async (method: string, params?: Record<string, unknown>) => {
    ...
    switch (method) {
      ...
      case 'terminal.resolveIdentity':
        return response({ identity: { live: options.staleHandle !== true } })
      case 'terminal.show':
        return response({
          terminal: {
  ...
  const client = testClient(call, options.isRemote ?? false)
...
describe('Mission coordinator ownership', () => {
  ...
  it('cleans its pre-Run anchor on Ctrl+C and prevents the pending plan from creating a Run', async () => {
    ...
    // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: process.exit is intercepted in this test and deliberately returns so cancellation cleanup can be asserted without terminating Vitest.
