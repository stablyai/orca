...
describe('buildAgentContextQuery', () => {
  ...
  it('keeps hidden commands off every query surface', () => {
    const specs: CommandSpec[] = [
      ...SPECS,
      {
        path: ['internal', 'repair'],
        hidden: true,
        summary: 'Repair internal state',
        usage: 'orca internal repair',
        allowedFlags: []
      }
    ]
    const roots = buildAgentContextQuery(specs, { view: 'roots' })
    const search = buildAgentContextQuery(specs, {
      view: 'search',
      value: 'internal repair',
      detail: 'summary',
      limit: 20
    })

    expect(roots).toMatchObject({ registryCommandCount: SPECS.length })
    expect(roots.view === 'roots' ? roots.roots.map((entry) => entry.root) : []).not.toContain(
      'internal'
    )
    expect(search.view === 'roots' ? [] : search.commands).toEqual([])
    expect(() =>
    ...
    ).toThrow('Unknown Orca command: internal repair')
  })
