...
function expectBlocked(args, message) {
...
describe('validateGitExecArgs', () => {
    ...
    describe('git config', () => {
        ...
        it.each([
            ...
            expectBlocked(['config', '--list', flag, 'val'], 'git config write operations are not allowed');
        });
    });
    describe('git branch', () => {
        ...
        it.each(['-d', '-D', '--delete', '-m', '-M', '--move', '-c', '-C', '--copy'])('rejects branch %s', (flag) => {
    ...
    describe('git symbolic-ref', () => {
        ...
        it('rejects two positional args (write form)', () => {
            expectBlocked(['symbolic-ref', 'HEAD', 'refs/heads/main'], 'git symbolic-ref write operations');
        });
        it('catches --delete=value compound syntax', () => {
