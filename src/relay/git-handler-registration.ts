...
export function registerGitHandlers(
  ...
  dispatcher.onRequest('git.history', (p) => handlers.read.history(p))
	dispatcher.onRequest('git.blame', (p, context) => handlers.read.blame(p, context))
  dispatcher.onRequest('git.stashList', (p, context) => handlers.stash.list(p, context))
  dispatcher.onRequest('git.stashFiles', (p, context) => handlers.stash.files(p, context))
  dispatcher.onRequest('git.stashCreate', (p) => handlers.stash.create(p))
  dispatcher.onRequest('git.stashApply', (p) => handlers.stash.mutate('apply', p))
  dispatcher.onRequest('git.stashPop', (p) => handlers.stash.mutate('pop', p))
  dispatcher.onRequest('git.stashDrop', (p) => handlers.stash.mutate('drop', p))
