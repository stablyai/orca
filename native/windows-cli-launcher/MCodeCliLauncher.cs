...
internal static class MCodeCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            string launcherDirectory = Path.GetDirectoryName(typeof(MCodeCliLauncher).Assembly.Location);
            ...
            string electronPath = Path.Combine(appDirectory, "MCode.exe");
            ...
            {
                Console.Error.WriteLine("Unable to locate MCode.exe next to \"{0}\"", resourcesDirectory);
                return 1;
            }

            if (!File.Exists(cliPath))
            {
                Console.Error.WriteLine("Unable to locate the MCode CLI entrypoint at \"{0}\"", cliPath);
                return 1;
            }
            ...
            MoveEnvironmentVariable("NODE_OPTIONS", "MCODE_NODE_OPTIONS");
            MoveEnvironmentVariable("NODE_REPL_EXTERNAL_MODULE", "MCODE_NODE_REPL_EXTERNAL_MODULE");
            ...
            Environment.SetEnvironmentVariable(
                "MCODE_CLI_COMMAND",
                requestedCliCommand == "mcode-ide" ? "mcode-ide" : "mcode"
            );
        ...
        {
            Console.Error.WriteLine("Unable to start the MCode CLI: {0}", error.Message);
            return 1;
        }
    }
