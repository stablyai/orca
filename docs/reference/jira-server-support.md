...
- Server/DC site ids should be derived from
  `siteUrl + deploymentType + authMode + viewerIdentityId`, where
...
- Populate `viewerUserId` with the deployment-specific identity. Keep
  `accountId` populated for backward compatibility, but treat it as a legacy
  alias outside Cloud-specific adapter code.
- Populate `JiraUser.userId` with the deployment-specific identity. Keep
  `JiraUser.accountId` as a deprecated alias so old Cloud-oriented call sites
  continue to render while they migrate.
...
- Map viewer id from `name ?? key ?? emailAddress ?? displayName`.
...
| --- | --- | --- |
| Verify connection | `GET /rest/api/2/myself` | User id from `name`, `key`, or fallback fields. |
...
| Assignable users | `GET /rest/api/2/user/assignable/search` | Use `issueKey`, `username`, and `maxResults`; do not send Cloud `query`. |
...
- Server/DC `userId`: `name ?? key ?? emailAddress ?? displayName`.
...
- Server/DC viewer maps `name` or `key` into the stored user id.
...
- Server assignable users send `issueKey`, `username`, and `maxResults`, not
  Cloud `query`.
