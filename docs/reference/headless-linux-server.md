...
ports, and falls back to an OS-assigned port. Orca **persists** that fallback
(STA-1511) so later restarts can rebind the same fallback. An explicit
`orca serve --port` pin binds the requested port first (#8535); if that pin
is still busy, it falls through to the persisted fallback, then to a new
OS-assigned port. The ready block still prints a pairing URL on the **actual**
bound port, so clients, firewalls, health checks, and a systemd unit that
hard-code `6768` go stale after the first collision even though the service
itself starts.

Prefer a dedicated port that the desktop app will not take, for example
`6769` (also the `pnpm dev` pin), and put the same value in both `ExecStart`
and any firewall rules:
...
3. Sidebar → **Add remote host** → **Add remote server**.
