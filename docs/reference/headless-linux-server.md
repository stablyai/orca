...
`orca-daemon-*.scope` on that host. The one-click flow refuses to install while
terminals are live, so nothing is killed mid-conversation; without that scope,
a manual upgrade kills every terminal and agent in the service's cgroup the
...
If you use the manual upgrade path, the census discipline still applies. When
`cgroupUnit` is `null` or unverifiable, immediately before stopping the
