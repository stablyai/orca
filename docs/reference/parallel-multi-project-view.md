...
**Progressive disclosure — no always-on second tab layer.** The project super-tab strip must **not** be permanently visible: two stacked tab strips (super-tabs above the existing per-group `TabBar`) would be visual noise for the common single-project case. The strip mounts *only* when parallel is actually in use — more than one workbench view, **or** a multi-pane active view — otherwise a lone single-leaf view shows zero extra chrome (identical to today). A `View → Show Workbench Tabs` toggle lets users who want it pin the strip on. This is encoded as `shouldShowProjectTabStrip(state, { pinned })` in the workbench-view model, not left to ad-hoc component logic.
