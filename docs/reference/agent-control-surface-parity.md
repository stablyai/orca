...
| Feature discovery and education | Implemented as UI telemetry | Feature tracking records first meaningful interaction and local counts for education-targeted features, including browser setup/use, computer use, orchestration, automations, SSH, usage, account switching, panes/splits, notifications, and workspace surfaces. | This is not control-surface parity. Do not count tours, tips, labels, or interaction counters as parity unless the underlying action is agent-operable. |
