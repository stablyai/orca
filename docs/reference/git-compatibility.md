...
| `hook-run`                  | Create a worktree with `--no-checkout`, then check out and run `post-checkout` through `git hook run` outside the repo's worktree admin lock | Run the plain `worktree add`, checkout and hook included, inside the lock before Git 2.36                                                                                                                  |
