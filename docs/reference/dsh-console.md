...
The original macOS Electron verification baseline is `@cofy-x/dsh-console@0.1.0-alpha.13` with `@deepseek-ai/dsh@0.1.5-rc.1`; installing newer releases does not rerun that acceptance. The integration also targets Console `0.1.0-alpha.16` with DSH `0.1.6-alpha.1`: focused tests cover both lifecycle names and persistent fork IDs, and a probe using the installed published DSH AgentRegistry and Cordis verifies creation events reach the emitted bridge. This is not a new full Electron/model acceptance run. The DSH packages use caret dependency ranges. Pinning the top-level CLI alone does **not** pin all of its transitive DSH packages; use a lockfile and explicit overrides when reproducing a coherent prerelease baseline. See the [published launcher](https://github.com/cofy-x/dsh-console) for upstream installation requirements.
...
The plugin observes public `session/event`, `agent/created` (DSH 0.1.6), legacy `agent/session-start`, `approval/request` and `user-questions/request` events. Creation announcements are deduplicated per live session. It forwards bounded snapshots through Orca's authenticated loopback hook endpoint into the existing unified agent status store. Native approval/question handlers retain their answers, errors and cancellation behavior. Root Console sessions and persistent `dsh-console-fork-` sessions are tracked; completion helpers and side sessions do not overwrite the main pane's status. No transcript polling or separate durable status database is introduced.
...
The original alpha.13/rc.1 combination has a question-answer UI and service, but its default bundle does not register the model-facing `ask_user_question` tool. Real question testing therefore requires the public `@deepseek-ai/dsh-tool-ask-user` plugin in the **test/user profile**. Orca deliberately does not change the user's tool composition. Console 0.1.0-alpha.16 mounts agent presets during creation/resume and inherits their composition for forks; that composition remains Console/DSH-owned.

## Original alpha.13 acceptance limits
