...
The publication before graph now matches prerequisite commit **718ead7ea9f6296d69ceb7d1460a3579116fe85f** exactly. Both variants include the prerequisite's two-argument session registration. The review fixes are applied to both fixed graphs, including async admission error handling and snapshot release after profile persistence. `fixtureVariants` and the reports pin the current permanent fixture. The unrelated unused renderer test import that blocked CI was removed.
