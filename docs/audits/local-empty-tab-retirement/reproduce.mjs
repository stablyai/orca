...
for (const path of [
  'docs/audits/local-empty-tab-retirement/renderer-store-fixture.ts',
  'docs/audits/local-empty-tab-retirement/fixture.test.ts'
]) {
  sourceHashes[path] = {
...
const scratch = await mkdtemp(join(tmpdir(), 'orca-local-empty-tab-retirement-'))
...
try {
  ...
  const includes = ['docs/audits/local-empty-tab-retirement/fixture.test.ts']
...
export default {...base, test: {...base.test, include: ${JSON.stringify(includes)}}, plugins: [{
  name: 'local-empty-tab-retirement-before-fix', enforce: 'pre',
  transform(_code, id) {
  ...
  const result = {
    comparison:
      'Actual renderer close, preload, registered main IPC, Store and runtime; before reverses only fix.patch in memory',
    sourceHashes,
    before,
    after,
    passed
