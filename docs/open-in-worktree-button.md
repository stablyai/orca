# Open In Button on Worktree Cards

## Scope
Add a visible `Open in` control on each worktree card for local worktrees only.

Actions:
- Open in external editor (directory target)
...
Do not use specific editor brand names in UI strings, code identifiers, or docs.

## Current State (Code-Verified)
- `WorktreeCard.tsx` has no visible `Open in` control yet.
...
Launch a generic editor CLI command constant with the normalized directory path as an argv entry, without brand-specific UI or documentation.
...
If using reveal semantics, document that behavior explicitly and keep it consistent across context menu and card button.

### 5) UI integration
- Add reusable sidebar open-in control/menu component used by both:
- `WorktreeCard` visible control (header-right)
- `WorktreeContextMenu` item(s)

Interaction constraints:
- Stop propagation on trigger/menu interactions so card click/double-click/drag handlers are not triggered.
- Disable while deleting (`isDeleting`).
- Preserve existing keyboard/focus behavior of card and menu primitives.
...
- event propagation isolation (open-in clicks do not activate card)
...
2. Add shared sidebar open-in UI component and wire into card + context menu.
