...
| (post-rebase UAT) | **Post-rebase live UAT (2026-09-07).** Chat-first flow bug: a fresh agent tab toggled to Chat before any prompt resolves identity to null (no session file exists yet — correct), and NO identity-hook input changes when OMP lazily creates the session on the first send, so `resolvedSessionId` stayed null forever and the transcript never hydrated (sends still worked; only the render was empty). `use-omp-pane-session-identity.ts` now re-probes a visible, unresolved pane on a fixed cadence (`OMP_PANE_IDENTITY_REPROBE_INTERVAL_MS`) until an identity lands — self-terminating on first resolution, suspended while hidden (resumed by a visibility-transition nonce), and never started by a null re-probe on an already-resolved pane. Sticky-merge semantics unchanged |
