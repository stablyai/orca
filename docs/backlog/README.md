...
| [BACKLOG-015](./BACKLOG-015-task-graph-rpcs-not-wired-to-wscompat.md) | OrcaTask `AddEdge`/`Grant`/`ResolvePermission` exist but aren't wired to `wscompat`; `AddComment`/`ListComments` don't exist at any layer — 3 already-built frontend UIs (add-edge, access panel, comments) sit on fallback | No blocker — unscheduled wiring/design work | Medium |
| [BACKLOG-016](./BACKLOG-016-task-execute-prompt-and-workflow-template-persistence.md) | `task.proto` has no `prompt` field on Execute and no way to persist `workflow_template_id` via `UpdateTask` — 2 frontend UIs run optimistic-only | No blocker — small proto/usecase addition | Medium |
| [BACKLOG-017](./BACKLOG-017-orchestration-service-new-rpcs-not-exposed-to-frontend.md) | 6 new `orchestration-service` coordinator-lifecycle RPCs (real autonomous dispatch now) aren't exposed via `wscompat` — no real Task Execute dashboard possible yet | No blocker — unscheduled wiring + new UI design | Medium |
| [BACKLOG-018](./BACKLOG-018-pre-existing-api-gateway-infra-fleet-build-breakage.md) | `api-gateway`/`infra-fleet-service` fail `go build` at HEAD (`registerInfraFleetChannels` etc., growing list) — unrelated to flow-task but blocks full `make build` verification | Locating the uncommitted code for these functions (likely sitting in another concurrent working tree) | Medium-High |
| [BACKLOG-019](./BACKLOG-019-relay-ssh-exec-output-streaming-gating-question.md) | New `StreamExecOutput` RPC intentionally mirrors `StreamPty`'s relay-ssh gating — but `StreamPty`'s own doc comment is self-contradictory about what that gating should be | Reading `StreamPty`'s design history to resolve the contradiction | Low |
| [BACKLOG-020](./BACKLOG-020-workflow-template-create-rpc-shape-mismatch.md) | `workflow.template.create` payload still doesn't match `channels_workflow.go`'s real shape (sibling `execute`/`template.update` calls were fixed, this one wasn't — out of scope at the time) | No blocker — same fix pattern as the sibling calls | Low |
