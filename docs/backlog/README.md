...
| ~~BACKLOG-016~~ | ~~`task.proto` had no `prompt` field on Execute and no way to persist `workflow_template_id`~~ — **resolved 2026-09-08**, both threaded end-to-end (proto/migration/domain/usecase/wscompat/frontend), 2 frontend UIs no longer optimistic-only | — | — |
| ~~BACKLOG-018~~ | ~~`api-gateway`/`infra-fleet-service` failed `go build` at HEAD~~ — **resolved**, confirmed already fixed by a concurrent working tree | — | — |
| ~~BACKLOG-019~~ | ~~`StreamExecOutput` intentionally mirrors `StreamPty`'s relay-ssh gating, but `StreamPty`'s own doc comment was self-contradictory about it~~ — **resolved 2026-09-08**, traced to a stale/wrong doc comment (not a real design constraint); fixed in `client.go`, gate itself left in place pending live relay-ssh verification — see `SOL-AG-FLOWTASK-001` §3's update | — | — |
| ~~BACKLOG-020~~ | ~~`workflow.template.create` payload didn't match `channels_workflow.go`'s real shape~~ — **resolved**, fixed alongside the sibling `execute`/`template.update` calls | — | — |
