...
- Page links that use `target=_blank` and ordinary unnamed popups open in new Orca tabs (in the pane already showing a browser, when there is one), using the opener tab's browser profile. Named or OAuth-style popups can still open in a separate window.
...
When a browser tab is already showing in the worktree's layout, a link opened in Orca becomes a new tab in that browser's pane instead of covering the pane you clicked in; the previous page stays open as its own tab. If several browser panes are showing, the focused one wins, otherwise the one you used last. A browser hidden behind another tab does not count, and with none showing, links open where they did before.
