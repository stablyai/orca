...
## Structured chat on a paired server

With **Structured Native Chat** and **Chat UI as the default for new agent tabs** enabled,
new Claude and Codex chats can run on a paired Remote Orca Server while the desktop displays
the native chat surface. Select the workspace owned by that server, then open a new agent chat.
The server must also have **Structured Native Chat** enabled in its own profile, advertise
structured-session support, and accept the workspace's create-support probe. Enabling the
client setting alone does not enable server admission. This applies to Orca Web clients too. Model and session options come from the agent running on that server.

A plain SSH terminal connection does not provide structured-session RPCs. Pair the remote Orca
runtime through [Remote Orca Servers](/docs/remote-servers) to use this flow. Existing terminal
sessions keep their current transport; open a new chat to use structured mode.

Creating, retrying, restoring and closing a launch remain tied to its original runtime. Losing
contact leaves the launch outcome unknown until that host answers again; it does not create a
replacement session locally. Pairing the saved server entry again cannot retarget an old launch.
