...
description: Optional chat surface for supported agent sessions — skills, model pickers, and transcript view.
---

Chat UI is a chat surface for supported agent sessions. New local **Claude** and **Codex** chats run as chat sessions Orca drives directly, with no terminal behind them. Other agents, and Claude or Codex chats Orca can't start that way (for example on WSL, over SSH, or on a remote server), show chat over their agent terminal, which stays the source of truth. Transcript decoding covers **Claude**, **Codex**, **Grok**, and **OMP** — OMP sessions open in Chat UI like the others instead of staying raw-terminal-only.

## Enable

1. Open **Settings → Chat UI** (under **AI Capabilities**).
1. Turn **Chat UI** on.
1. Optionally set **Default view** to **Chat UI** so new supported agent tabs open in chat; keep **Terminal chat** to open the TUI first.

For chats shown over an agent terminal, toggle Chat UI ↔ terminal from the agent pane.
