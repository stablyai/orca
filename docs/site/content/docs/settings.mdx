...
- **Worktree Shared Paths** — gitignored paths to materialize from the primary checkout into each new worktree (a copy-on-write clone when the filesystem supports one — APFS clone-copy on macOS, reflink on Linux — otherwise a symlink). Complements repo-checked-in `worktree.sharedDirectories` in `orca.yaml` and `.worktreeinclude` (see [Worktrees](/docs/model/worktrees)).
