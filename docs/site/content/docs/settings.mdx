...
- **Speech Model** — download/select an on-device model, or use cloud OpenAI models after pasting an API key:
  - **Apple Speech** (macOS 26+ only) — the transcriber built into macOS, in your Mac dictation language. Nothing to download from Orca: the first time you pick it, macOS installs its own language files, which it shares with every other app and keeps after Orca is removed. Not offered on older macOS, on Windows, or on Linux.
