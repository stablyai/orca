...
- **Single Terminal Width** — a tab holding one unsplit terminal is capped at this width and centered, so a wide window does not stretch a single terminal edge to edge. Splitting the tab uses the full width again. Set it to `0` to always use the full width.
- Ghostty import.
