...
- Plugin panels and commands follow the selected workspace's Orca runtime. For a paired remote server, install, consent to, and enable the plugin on that server; its worker stays there and its panel appears on the connected desktop. Local workspaces and direct SSH workspaces keep using this computer's plugins.
- Remote plugin catalogs and open panels refresh 10 seconds after each completed request while the window is visible, and on focus. A disconnected or unsupported server does not fall back to local plugins. Marketplace installation, permission management, language packs, and VM recipes in desktop Settings remain local.
