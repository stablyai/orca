...
- **Chat UI** — optional chat view on supported agent sessions; off by default. See [Chat UI](/docs/agents/native-chat).
- **Default view** — open new supported agent tabs in **Terminal chat** or **Chat UI**.
- **Resume working chats automatically after a restart** and **Use your shell environment** — shown while Chat UI is on; they apply to every chat open in Chat UI, whatever the default view. Hidden on paired web clients.
