...
<Callout title="Beta">
  ...
  On Android, the app checks GitHub Releases once a day and shows a banner on the home screen
  when a newer APK is available; tap it to download, or dismiss it to skip that version.
</Callout>
...
- **Update Orca Mobile / Update Orca on your computer** — the phone and desktop speak a versioned mobile protocol. If either side is too old, mobile blocks that host and points you at the App Store (iOS), the latest Android APK, or [GitHub Releases](https://github.com/stablyai/orca/releases) (desktop), then back to hosts. After updating, refresh the connection; if the message stays, remove the host and pair again.
