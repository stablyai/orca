...
`{linkedIssue}` expands to the workspace-linked **GitHub or GitLab issue number**, or empty when none is linked (including Linear-only workspaces). Prefer instructional phrasing — a bare `Fixes #{linkedIssue}` becomes `Fixes #` when unlinked.
