...
- Token type:
  ...
  - **Scoped API token** — a token created with scopes, also paired with your **Atlassian email**. Atlassian only accepts these on the `api.atlassian.com` gateway, so Orca resolves your site's cloud id once and routes every request through `https://api.atlassian.com/ex/jira/<cloudId>`; the site URL is still used for browse links. Grant `read:jira-work`, `write:jira-work`, and `read:jira-user`.
