...
## Files for new worktrees

Git brings the chosen branch’s tracked code. Orca can also copy selected ignored files from the **primary checkout on the execution host** into each new worktree.

### Repository settings → Files to copy

Open **Settings**, choose the **repository and host**, then find **Files to copy**. Add personal paths such as `.env.local` here. These choices apply only to this repository/host entry in Orca, are not written to the project’s files, and affect future worktrees only. Other clients of the same Orca runtime can see that runtime’s settings; this is not a per-account privacy boundary.

The project can also list files in `.worktreeinclude`:

```text
# .worktreeinclude, in the primary checkout
.env
.vscode/
```

Orca combines the personal and project lists, removes duplicate or covered paths, and copies them once. Removing a personal entry does not exclude a project entry. Commit the project list if everyone should use it; the list names `.env`, without committing its ignored contents.

Use literal repository-relative paths to ignored files or folders. Blank lines and whole-line `#` comments are allowed; globs and negation are not. The manifest is limited to 256 KiB and 1,000 entries. Tracked files already come from Git. Missing extras are reported and skipped. Personal entries that are unsafe or nonignored need correction; unsupported project manifest patterns are skipped with a warning for compatibility. Conflicting selections need correction before preparation continues.

### Fast copying is automatic

There is no APFS switch. Orca uses strict APFS copies on supported Macs, reflinks on supported Linux filesystems, and bounded ordinary copies otherwise, including native Windows. WSL copies inside its distro; SSH copies on its host. Older SSH hosts report unsupported personal selections rather than interpreting them as shared links.

**A private copy never falls back to a shared link.** Later edits to ordinary copied files stay in the new worktree. Nested links retain their targets, so external linked data can still be shared. The source is not an atomic snapshot.

The whole copy list shares a budget of **2 GiB of ordinary copying and 50,000 entries**. CoW avoids the byte charge but still counts entries, so large `node_modules` trees may not fit. Use the normal setup install command for dependencies; optionally copy first and install afterwards to reconcile the destination branch’s lockfile. Setup configuration comes from the new worktree, while the extra-file lists and contents come from the primary checkout.

Clean copy refusals are reported and normal setup can proceed. Possible partial output or unverifiable completion pauses dependent setup; inspect the retained worktree before retrying. Opening an existing folder workspace does not copy anything.

### Explicit sharing and legacy paths

Advanced projects can share ignored directories through `orca.yaml`:
...
Sharing always means a link: edits affect the primary checkout too. Copy paths must not overlap shared or legacy paths. Deleting a worktree removes shared links, not their targets.

Historical Settings entries appear under **Legacy paths** and keep their old behavior: a private clone when available, otherwise a shared link. Choose **Use copies…** to preview and confirm conversion of an eligible ignored path. Existing worktrees are never rewritten.
