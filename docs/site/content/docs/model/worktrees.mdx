...
1. **Worktree Shared Paths** (per repo, in Settings → Repository) — paths materialize from the primary checkout into each new worktree (a copy-on-write clone when the filesystem supports one — APFS clone-copy on macOS, reflink on Linux for btrfs, XFS and OpenZFS 2.2+ — otherwise a symlink).
