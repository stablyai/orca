...
orca orchestration worker-payload --dispatch <dispatchId> --digest <sha256> --json
...
Do not leave completed worker terminals open just to re-read output — use `worker-read` after `worker-release`. When `worker-read` clips a block it prints `… (truncated) [full text N bytes, digest <sha256>]` if the host retained the original; `worker-payload --dispatch <id> --digest <sha256>` returns the exact bytes in pages (`--offset`), only for digests clipped while reading that Dispatch. Do not substitute a broad `terminal close` when release returns `release_pending` or `release_unknown`; follow the receipt's recovery action.
