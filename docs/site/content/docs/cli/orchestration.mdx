...
Without `--inject`, `dispatch` only records the attempt: the task turns `dispatched`, the terminal is not told, and the response reports `injected: false` (newer runtimes also add a `warning` saying so). Deliver the task yourself, or reset it (`orca orchestration task-update --id <taskId> --status failed`, then the same command with `--status ready`) and dispatch again with `--inject`.
