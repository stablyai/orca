...
List and revoke runtime pairing grants directly on the server, under the same OS account and user-data directory as Orca:
...
The list includes unused links without exposing credentials. Revoking a grant disconnects everyone sharing it and prevents reconnection after restart. Other grants and mobile pairings remain intact. Remote selectors and forwarded SSH/WSL CLI commands are rejected; run in a host terminal, inside the server's container if applicable.
