...
To fill a password field without the secret reaching the agent's context, the terminal, or any
process arguments, pass a password-manager reference instead of a value:

```bash
orca fill --element @e2 --secret-ref "op://Private/Acme staging/password" --worktree active --json
orca fill --element @e2 --secret-ref "bw://acme-staging/password" --worktree active --json
```

Orca runs the vendor CLI (`op` for `op://`, `bw` for `bw://`) itself, sends the value straight to the
page, and prints only the reference. The CLI must already be installed and unlocked; set
`ORCA_OP_CLI` or `ORCA_BW_CLI` if it is not on `PATH`.

The secret stays out of the terminal, shell history, and the `orca` process arguments in every case.
One caveat on where it goes next: with the desktop browser the value is delivered in-process, but a
host driving external Chromium through `ORCA_BROWSER_EXECUTABLE` passes it on the `agent-browser`
driver's command line, so on that host it is readable from the process table for the life of the
command.
