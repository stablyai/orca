...
`--host` also takes the bare name shown in the `name` column, on either axis — `--host trader-local` reaches the same machine as `--host ssh:ssh-1783599077486-fyasux`. Names resolve only when they are unique: a name held by two SSH targets, by two paired servers, or by one of each is refused with the prefixed flag for each candidate, so a command never reaches a machine you did not name. Use the IDs from `host list` to settle it.
