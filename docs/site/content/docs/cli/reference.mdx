...
`repo hooks show` prints the setup and archive scripts for a repo side by side: the local ones stored on this machine, the ones committed in `orca.yaml`, and the effective ones the host would actually run. `repo hooks set` edits the local side, the same settings the Repository settings pane edits, and never rewrites `orca.yaml`. Pass a multi-line script with `--setup-script-file <path|->`, clear one with `--setup-script null`, and change the defaults for new worktrees with `--setup-run-policy`, `--agent-startup`, and `--command-source`.
