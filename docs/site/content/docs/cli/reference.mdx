...
`terminal history` is the simpler alternative when you just need the scrollback as one string — a stack trace, a build log — without cursor bookkeeping. It takes `--tail-lines` (default 200) in place of a cursor and supports `--screen` the same way `terminal read` does.
