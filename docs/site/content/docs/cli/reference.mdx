...
Tag workspaces to group them across repos in the sidebar (Group by → Tag):

```bash
orca worktree set --worktree active --tag billing --tag api --json
orca worktree set --worktree active --untag api --json
orca worktree set --worktree active --tags null --json
```

`--tag` and `--untag` repeat and keep the workspace's other tags; `--tags a,b` replaces the whole set, and `--tags null` clears it. Tags match case-insensitively, and `worktree ps` lists them.
