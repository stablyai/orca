...
Tailcat links pin the runtime port, so `orca serve --tailcat` binds exactly `--port` (default 6768) or
fails rather than moving to another port. Orca checks the installed `tailcat` before using it and needs
release 0.4 or newer. Only the Orca desktop app can open a Tailcat link today: Orca Mobile and the
browser client refuse it, and `--tailcat` cannot be combined with `--mobile-pairing`. Orca versions
that predate Tailcat refuse the link as invalid instead of dialing an unreachable address.
