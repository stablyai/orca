...
### Pin the desktop listener to one address

By default the desktop app listens on `127.0.0.1` until you share access, then on every interface (`0.0.0.0`) — including at later launches once a network client has connected. If port `6768` is taken it moves to a saved or OS-assigned port.

If you front Orca with your own forwarder (for example Tailscale Serve raw TCP or an SSH local forward to a loopback port), pin the listener instead. Create `runtime-ws-bind.json` in Orca's data folder — the folder that holds `orca-data.json` (by default `~/Library/Application Support/orca` on macOS, `%APPDATA%\orca` on Windows, `~/.config/orca` on Linux) — then restart Orca:

```json
{ "host": "127.0.0.1", "port": 6768 }
```

While the file is present, the desktop app:

- listens on exactly that host and port, on every launch — no saved or OS-assigned fallback port;
- never widens to other interfaces: creating a network access link or mobile QR reports network exposure as unavailable, and a previously connected client does not widen the next launch;
- logs `[runtime] WebSocket listener pinned to ws://127.0.0.1:6768 …`; the same endpoint is the `websocket` entry in `orca-runtime.json`. Neither contains a credential.

`host` must be a loopback address — `127.0.0.1` (or another `127.x.x.x` address) or `::1` — written as a literal, not `localhost`. Wildcard (`0.0.0.0`, `::`), LAN, Tailscale, link-local, zoned, and IPv4-mapped addresses are rejected: expose the loopback port through your forwarder instead. `port` must be an integer from 1 to 65535; other keys are rejected. The file may be saved with or without a UTF-8 byte order mark. If the file is invalid or unreadable, or the port is in use, the listener stays down for that session and Orca shows **Orca remote access unavailable** with the cause. It never falls back to another port or interface. Until you fix the file or free the port and restart Orca, paired devices and remote clients — including relay connections — cannot reach this desktop. Saved grants and the paired-device list are kept, and the local `orca` CLI keeps working. Delete the file to return to the default behavior.

The file applies to the desktop app only; `orca serve` keeps its `--port` behavior.
