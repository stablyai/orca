...
- On a headless host, run `orca runtime-access list --json`, then `orca runtime-access revoke --device <deviceId> --json` directly on the server. A shared grant is revoked for everyone using it; other grants are preserved.
...
Without a desktop window, use the `runtime-access` commands above from a terminal on the server, under Orca's OS account. Run inside its container when applicable; Orca's forwarded SSH/WSL CLI bridge and remote selectors cannot administer grants.
