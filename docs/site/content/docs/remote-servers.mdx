...
`host` must be a literal IPv4 or IPv6 address (not a hostname, not `localhost`), and `port` an integer from 1 to 65535; other keys are rejected. If the file is invalid or unreadable, or the port is in use, the listener stays down for that session and Orca shows **Orca remote access unavailable** with the cause. It never falls back to another port or interface. Fix the file or free the port and restart Orca. Existing grants, paired devices, and the local `orca` CLI are unaffected. Delete the file to return to the default behavior.
