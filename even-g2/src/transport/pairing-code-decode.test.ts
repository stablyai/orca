...
import type { PairingOffer } from '@orca-shared/mobile-relay-pairing-offer'
import {
  decodePairingUrl,
  extractPairingCodeFromUrl,
  parsePairingCode
} from './pairing-code-decode'
...
describe('pairing-code-decode', () => {
  ...
  it('rejects garbage input', () => {
    ...
    expect(parsePairingCode('orca://pair?code=not-valid-base64-json')).toBeNull()
  })

  it('rejects a base64 payload that is valid JSON but fails schema validation', () => {
    const badOffer = btoa(JSON.stringify({ v: 1, endpoint: 'x' }))
      .replace(/\+/g, '-')
      .replace(/\//g, '_')
      .replace(/=+$/, '')
    expect(parsePairingCode(badOffer)).toBeNull()
  })
})
