...
const ITEMS_PER_PAGE = 20 // firmware list hard cap

export function worktreeListPageCount(worktreeCount: number): number {
  return Math.max(1, Math.ceil(worktreeCount / ITEMS_PER_PAGE))
}
...
export function renderWorktreeListScreen(
  ...
  const pageRows = rows.slice(page * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE + ITEMS_PER_PAGE)
