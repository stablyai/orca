...
async function callMCode(cli, args, options = {}) {
...
function buildLongSpec(sizeKb, marker) {
  ...
  const header = [
    `MCODE_LONG_PROMPT_REPRO_START ${marker}`,
  ...
  let index = 0
  while (
    Buffer.byteLength(`${lines.join('\n')}\nMCODE_LONG_PROMPT_REPRO_END ${marker}`, 'utf8') <
    targetBytes
  ) {
    lines.push(
  ...
  lines.push(`MCODE_LONG_PROMPT_REPRO_END ${marker}`)
  return lines.join('\n')
}
...
async function parentMain() {
  ...
  const tempDir = path.join(tmpdir(), `mcode-orchestration-long-prompt-${process.pid}-${Date.now()}`)
  await mkdir(tempDir, { recursive: true })
  const workerReportPath = path.join(tempDir, 'worker-report.json')

  console.log(`runtime: ${cli} status`)
  await callMCode(cli, ['status'], { cwd })

  const coordinator = await callMCode(
    cli,
    [
      'terminal',
      'create',
  ...
  try {
    ...
    const dispatch = await callMCode(
      cli,
      [
        'orchestration',
        'dispatch',
...
async function fakeWorkerMain() {
  ...
  const finish = async (reason) => {
    ...
    process.stdout.write(
      `\nMCODE_REPRO_REPORT ${report.contractOk ? 'ok' : 'failed'} ${reportPath}\n`
    )
...
The parent mode requires a running MCode runtime and creates temporary MCode
terminals. The fake worker records whether orchestration dispatch delivered a
long prompt in a safe agent-input contract.

Options:
  --cli <path>         MCode CLI command (default: MCODE_REPRO_CLI or mcode)
