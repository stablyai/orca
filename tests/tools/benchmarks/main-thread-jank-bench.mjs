#!/usr/bin/env node
/**
 * MCode main-thread jank / subprocess-churn benchmark (issue #7576).
 *
 ...
 * launched with MCODE_MAIN_THREAD_DIAGNOSTICS=1 so the main process emits one
 ...
 *     [--exe <path-to-packaged-MCode>] [--headless] [--no-log-stream]
 *
 ...
 * the shape MCode worktrees commonly have, which forces the effective-upstream
...
function ensureFixture(fixtureDir) {
  ...
  writeFileSync(join(fixtureDir, 'mcode-data.json'), JSON.stringify(state, null, 2))
  return repoPath
}
