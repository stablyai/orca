...
1. **Desktop MCode running** (`mcode status --json`).
...
mcode worktree list --environment <name> --json | head
mcode terminal list --environment <name> --json | head
```
...
MCODE_FREEZE_ENV=paired-remote \
MCODE_FREEZE_SCENARIO=idle-backlog-open \
MCODE_FREEZE_CREATE=8 \
MCODE_FREEZE_IDLE_MS=45000 \
MCODE_FREEZE_OPEN_COUNT=24 \
pnpm run repro:live-remote-realistic-freeze

# Wake-like: idle + reconnect metadata storm + open  ← hard freeze in lab
MCODE_FREEZE_ENV=paired-remote \
MCODE_FREEZE_SCENARIO=idle-backlog-reconnect-open \
MCODE_FREEZE_CREATE=10 \
MCODE_FREEZE_IDLE_MS=60000 \
MCODE_FREEZE_OPEN_COUNT=40 \
pnpm run repro:live-remote-realistic-freeze

# Restart-proxy: idle + mcode open + refresh storm + open (does not kill desktop)
MCODE_FREEZE_ENV=paired-remote \
MCODE_FREEZE_SCENARIO=restart-proxy \
MCODE_FREEZE_CREATE=0 \
MCODE_FREEZE_IDLE_MS=20000 \
MCODE_FREEZE_OPEN_COUNT=30 \
pnpm run repro:live-remote-realistic-freeze
```
...
| `restart-proxy`               | `mcode open` + refresh storm + open (post-restart discovery; no process kill)                  |
| `lockup-storm`                | Idle + flood + reconnect + **concurrent** open fan-out + **mid-storm `mcode status` watchdog** |

### Realistic knobs

| Variable                          | Default             | Meaning                                                           |
| --------------------------------- | ------------------- | ----------------------------------------------------------------- |
| `MCODE_FREEZE_ENV`                 | `paired-remote`     | Paired remote environment name                                    |
| `MCODE_FREEZE_SCENARIO`            | `idle-backlog-open` | See table above                                                   |
| `MCODE_FREEZE_CREATE`              | `0`                 | New flood terminals; mutation requires an explicit positive value |
| `MCODE_FREEZE_IDLE_MS`             | `45000`             | Time “away” while floods run                                      |
| `MCODE_FREEZE_OPEN_COUNT`          | `20`                | Sessions to open after return                                     |
| `MCODE_FREEZE_PACE_MS`             | `250`               | Base delay between opens (human pace)                             |
| `MCODE_FREEZE_PACE_JITTER_MS`      | `150`               | Random extra delay                                                |
| `MCODE_FREEZE_SOFT_MS` / `HARD_MS` | 2000 / 5000         | Thresholds                                                        |
...
| **lockup-storm** (parallel open + overlap refresh) | 12–16  | 45–60s | 64–80 @ p20–32 | **27–35s** batches; some `Terminal reveal timed out` | **HARD stalls + reveal timeouts; app still answers `mcode status` ~150ms** |
...
| Post-storm `mcode status`    | **~113ms**                          |
| Force Quit required         | **no**                              |

Bar for full-app freeze in the harness: continuous **≥30s** window where `mcode status` hangs/fails or stays ≥15s slow (`evaluateFullAppFreeze` / `foreverUiLockupObserved`).
...
MCODE_FREEZE_ENV=paired-remote MCODE_FREEZE_SCENARIO=lockup-storm \
  MCODE_FREEZE_CREATE=12 MCODE_FREEZE_IDLE_MS=30000 MCODE_FREEZE_OPEN_COUNT=80 \
  MCODE_FREEZE_STORM_PARALLEL=28 MCODE_FREEZE_FOREVER_WINDOW_MS=30000 \
  pnpm run repro:live-remote-realistic-freeze
...
MCODE_FREEZE_ENV=paired-remote \
MCODE_FREEZE_CREATE=0 \
MCODE_FREEZE_SWITCH_PASSES=3 \
MCODE_FREEZE_PARALLEL=16 \
...
| Layer    | Module                                                                             |
| -------- | ---------------------------------------------------------------------------------- |
| Runtime  | `TerminalFocusNavigationCoalescer` via `MCodeRuntimeService.focusTerminal`          |
...
- Both harnesses default to `MCODE_FREEZE_CREATE=0`. A positive value creates persistent, high-output remote terminals; use it only on an isolated target you can clean up.
- `restart-proxy` does **not** kill MCode; it runs `mcode open` + refresh RPCs only.
- Manual capture if UI fully freezes: `sample MCode 5 -file ~/Desktop/mcode-freeze-sample.txt`

The scripts honor `MCODE_CLI_COMMAND`, then use `mcode-dev` in a dev runtime, `mcode-ide` on Linux, and `mcode` elsewhere.

PowerShell equivalent for the first example:

```powershell
$env:MCODE_FREEZE_ENV = 'paired-remote'
$env:MCODE_FREEZE_SCENARIO = 'idle-backlog-open'
$env:MCODE_FREEZE_CREATE = '8'
$env:MCODE_FREEZE_IDLE_MS = '45000'
$env:MCODE_FREEZE_OPEN_COUNT = '24'
pnpm run repro:live-remote-realistic-freeze
```
