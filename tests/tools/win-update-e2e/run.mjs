...
//
// Given two MCode Windows installers (version N and N+1), proves what happens to
...
async function main() {
  ...
  const canary = `MCODE-E2E-CANARY-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `mcode-win-update-e2e-${runId}-`))
...
function resolveScopedDaemon(userDataDir) {
  ...
  return {
    ...
    // install-dir MCode.exe — the key survival signal.
...
async function teardown({
  ...
  if (hadPreexistingInstall) {
    console.log(
      '\n*** NOTE: an MCode install existed before this run. It was OVERWRITTEN and the\n' +
