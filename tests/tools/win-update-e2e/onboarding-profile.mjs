// Persistence seed for a fresh packaged MCode profile: dismisses onboarding and
...
// mcode-data.json` BEFORE launch dismisses it. The app derives the Projects list
...
//
// IMPORTANT: seed only BEFORE the first launch. The app rewrites mcode-data.json
 ...
 * for it. A real checkout (init + one commit) is required — MCode treats a
...
export function createSeededRepo(dir) {
  ...
  git('init', '-b', 'main')
  git('config', 'user.email', 'win-update-e2e@mcode.test')
...
export function seedFreshProfile(userDataDir, profile) {
  ...
  writeFileSync(path.join(userDataDir, 'mcode-data.json'), `${JSON.stringify(profile, null, 2)}\n`)
}
