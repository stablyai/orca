...
//
// MCode ships a per-user oneClick NSIS installer (electron-builder defaults:
// oneClick=true, perMachine=false) named mcode-windows-setup.exe. One-click
// silent mode is `<setup.exe> /S`; the app installs under
// %LOCALAPPDATA%\Programs\<dir> and the exe is MCode.exe. The install dir casing
// is not guaranteed (observed lowercase "mcode" on a dev box), so the exe is
...
const PRODUCT_NAME = 'MCode'
const EXE_NAME = 'MCode.exe'
...
export function resolveInstaller({ localPath, releaseTag, assetPattern }) {
  ...
  const outDir = mkdtempSync(path.join(tmpdir(), 'mcode-e2e-installer-'))
...
export function silentInstall(setupExe, { timeoutMs = 180_000, installDir = null } = {}) {
  ...
  // On update runs the old MCode.exe already exists, so wait for the exe whose
...
/**
 * Locate the installed MCode.exe. In isolated mode (`installDir` set), the exe is
 * at a known fixed path (<installDir>\MCode.exe). Otherwise it is discovered
 ...
 * developer's REAL MCode. It additionally refuses to run against the default
  ...
  if (!allowDefaultLocation && pathsEqual(resolved, path.join(programsRoot(), PRODUCT_NAME))) {
    throw new Error(
      `Refusing to uninstall the default install location "${resolved}" — this is where a ` +
        `developer's REAL MCode lives. Isolated mode must target a separate --install-dir.`
    )
  }
