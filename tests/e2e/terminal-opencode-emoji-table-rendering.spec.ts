...
const EMOJI_TABLE_MARKER = 'MCODE_EMOJI_TABLE_RENDER_DONE'
...
test.describe('OpenCode emoji table terminal rendering', () => {
  test('keeps emoji table output visually sane and restores the cursor', async ({
    ...
    await waitForPtyShellEcho(mcodePage, ptyId, 20_000)
    const runId = randomUUID()
    const marker = `${EMOJI_TABLE_MARKER}_${runId}`
    const scriptPath = path.join(testRepoPath, `.mcode-opencode-emoji-table-${runId}.mjs`)
    writeFileSync(scriptPath, emojiTableScript(marker))
    try {
      await sendToTerminal(mcodePage, ptyId, `${nodeTerminalCommand([scriptPath])}\r`)
      await waitForTerminalOutput(mcodePage, marker, 10_000)
      await mcodePage.waitForTimeout(250)
      await forceCursorProbeTheme(mcodePage)
      await mcodePage.waitForTimeout(50)

      const renderState = await readActiveTerminalRenderState(mcodePage)
      const blinkSamples = await sampleCursorBlink(mcodePage)
  ...
  test('local real OpenCode demo keeps table rendering and cursor visible', async ({
    ...
    test.skip(
      process.env.MCODE_E2E_REAL_OPENCODE !== '1',
      'Set MCODE_E2E_REAL_OPENCODE=1 to exercise the locally installed OpenCode TUI'
    )
    ...
    try {
      await waitForTerminalOutput(mcodePage, 'Give me markdown table', 15_000)
      await waitForTerminalOutput(mcodePage, 'Emoji', 60_000)
      await waitForTerminalOutput(mcodePage, 'Alice', 60_000)
      await mcodePage.waitForTimeout(1_500)

      await testInfo.attach('real-opencode-demo-table', {
        body: await mcodePage.screenshot({ fullPage: true }),
