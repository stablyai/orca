...
async function captureProof(page: Page, testInfo: TestInfo, name: string): Promise<void> {
  if (process.env.MCODE_E2E_RECORD_VIDEO === '1') {
    return
  }
...
test.describe('Workspace emoji picker', () => {
  ...
  test('inserts emoji in sidebar rename, worktree details, and Cmd+J', async ({
    ...
    const title = mcodePage.locator('[data-worktree-title-inline-rename=""]').first()
    await expect(title).toBeVisible()
    await title.dblclick()

    const inlineInput = mcodePage.locator('[data-worktree-title-rename-input="true"]')
    await expect(inlineInput).toBeVisible()
    await inlineInput.fill('Sidebar proof')
    await captureProof(mcodePage, testInfo, 'sidebar-rename-before.png')
    await inlineInput.pressSequentially(' :wink', { delay: 60 })
    const inlineSuggestions = mcodePage.locator('[data-workspace-emoji-suggestions="true"]')
    await expect(inlineSuggestions.getByRole('option', { name: ':wink:' })).toBeVisible()
    await captureProof(mcodePage, testInfo, 'sidebar-rename-picker.png')
    await inlineInput.press('Enter')
    await expect(inlineInput).toHaveValue('Sidebar proof 😉 ')
    await inlineInput.press('Enter')
    await expect(mcodePage.getByText('Sidebar proof 😉', { exact: true }).first()).toBeVisible()
    ...
    const detailsDialog = mcodePage.getByRole('dialog', { name: 'Edit Worktree Details' })
    ...
    await captureProof(mcodePage, testInfo, 'worktree-details-before.png')
    ...
    await captureProof(mcodePage, testInfo, 'worktree-details-picker.png')
    await displayNameInput.press('Enter')
    await expect(displayNameInput).toHaveValue('Details proof 😉 ')
    await detailsDialog.getByRole('button', { name: 'Cancel' }).click()

    await mcodePage.evaluate(() => window.__store!.getState().openModal('worktree-palette'))
    ...
    await captureProof(mcodePage, testInfo, 'cmd-j-before.png')
    ...
    await captureProof(mcodePage, testInfo, 'cmd-j-picker.png')
