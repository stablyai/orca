...
test.describe('Reveal active workspace button', () => {
  test.beforeEach(async ({ mcodePage }) => {
    ...
    await mcodePage.emulateMedia({ reducedMotion: 'reduce' })
  ...
  test('clears sidebar filters before revealing a hidden current workspace', async ({
    ...
    await prepareSidebarForScrollTest(mcodePage)

    const renderedOptions = mcodePage.locator('[data-worktree-sidebar] [role="option"]')
    ...
    const targetRows = mcodePage.locator(
      `[data-worktree-sidebar] [data-worktree-id=${JSON.stringify(targetId)}]`
    )
    const targetRow = targetRows.first()
    const revealButton = mcodePage.getByRole('button', { name: 'Reveal active workspace' })

    await mcodePage.evaluate((targetId) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
  ...
  test('reveals the current workspace when it starts outside the virtualized window', async ({
    ...
    const targetId = await mcodePage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
  ...
  test('uses the active workspace key when the legacy active worktree id is not set', async ({
    ...
    const folderWorktreeId = await mcodePage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
