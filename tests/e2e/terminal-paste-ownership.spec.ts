...
async function installClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ BrowserWindow, ipcMain }) => {
    ...
    const global = globalThis as unknown as {
      __mcodeOriginalClipboardReadTextHandler?: ClipboardReadHandler
    }
    ...
    if (!invokeHandlers || !handler || global.__mcodeOriginalClipboardReadTextHandler) {
      return
    }
    global.__mcodeOriginalClipboardReadTextHandler = handler
    invokeHandlers.set('clipboard:readText', async (event, ...args) => {
      ...
      return global.__mcodeOriginalClipboardReadTextHandler!(event, ...args)
    })
  })
}
...
async function restoreClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ ipcMain }) => {
    ...
    if (invokeHandlers && global.__mcodeOriginalClipboardReadTextHandler) {
      invokeHandlers.set('clipboard:readText', global.__mcodeOriginalClipboardReadTextHandler)
      delete global.__mcodeOriginalClipboardReadTextHandler
    }
  })
}
...
test.describe('terminal paste ownership', () => {
  test('keyboard paste shortcuts send clipboard text to the focused terminal exactly once', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.mcode-paste-ownership-${runId}.mjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(mcodePage, `PASTE_READY_${runId}`, 10_000)

      for (const [index, chord] of keyboardPasteChords().entries()) {
        const payload = `MCODE_E2E_PASTE_${runId}_${index}`
        ...
        await mcodePage.keyboard.press(chord)
  ...
  test('keyboard paste survives transient terminal blur during clipboard read', async ({
    ...
    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.mcode-paste-blur-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `MCODE_E2E_TRANSIENT_BLUR_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await mcodePage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(mcodePage)
      ...
      await mcodePage.keyboard.press(keyboardPasteChords()[0])
      await waitForTerminalOutput(mcodePage, encodedPayload, 10_000, 12_000)
  ...
  test('terminal context-menu Paste sends clipboard text exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.mcode-paste-context-menu-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `MCODE_E2E_CONTEXT_MENU_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await mcodePage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(mcodePage)
      ...
      await mcodePage.getByRole('menuitem', { name: /Paste/ }).click()
      await waitForTerminalOutput(mcodePage, encodedPayload, 10_000, 12_000)
  ...
  test('Windows multiline keyboard paste normalizes terminal newlines with one PTY owner', async ({
    ...
    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const sentinel = `MCODE_E2E_MULTILINE_DONE_${runId}`
    const payload = [
      `MCODE_E2E_MULTILINE_${runId}`,
    ...
    // MCode's direct Windows bracketed-paste path must produce the same bytes.
    const terminalText = payload.replace(/\r?\n/g, '\r')
    const scriptPath = path.join(testRepoPath, `.mcode-paste-multiline-${runId}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(mcodePage, `PASTE_COMPLETE_${runId}:MATCH`, 10_000, 12_000)
  ...
  test('right-click paste sends clipboard text to the focused terminal exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.mcode-paste-right-click-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `MCODE_E2E_RIGHT_CLICK_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await mcodePage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(mcodePage)

      await rightClickActiveTerminalSurface(mcodePage)
      await waitForTerminalOutput(mcodePage, encodedPayload, 10_000, 12_000)
