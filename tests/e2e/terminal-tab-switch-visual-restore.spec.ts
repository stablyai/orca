...
async function startHiddenPtyOutputBurst(page: Page, ptyId: string, runId: string): Promise<void> {
  ...
  await runNodeScriptInTerminal(page, ptyId, script, { prefix: 'mcode-tab-switch-burst' })
}
...
test.describe('Terminal tab switch visual restore', () => {
  ...
  test('keeps full-width geometry after switching away and back', async ({
    ...
    const { firstTabId, secondTabId } = await ensureTwoTerminalTabs(mcodePage)
    await forceWebglOnActiveTab(mcodePage)

    const runId = `${Date.now()}`
    const marker = `${TAB_SWITCH_MARKER_PREFIX}_${runId}`
    const firstPtyId = await waitForPanePtyIdOnTab(mcodePage, firstTabId)
    await writeStaticTabContent(mcodePage, firstTabId, marker, TAB_A_GLYPH_ROW)

    const baseline = await readTabTerminalGeometry(mcodePage, firstTabId, runId)
    ...
    await resetTerminalOutputSchedulerDebug(mcodePage)
    await startHiddenPtyOutputBurst(mcodePage, firstPtyId, runId)

    for (let cycle = 0; cycle < 12; cycle += 1) {
      await activateTerminalTab(mcodePage, secondTabId)
      await injectHiddenStreamingBurst(mcodePage, firstTabId, runId)
      // Why: rapid back-to-back switches mirror the user's leave/return pattern
      // and race the overlay's rAF/50ms refit retries.
      await activateTerminalTab(mcodePage, firstTabId)
      if (cycle % 3 === 0) {
        await activateTerminalTab(mcodePage, secondTabId)
        await activateTerminalTab(mcodePage, firstTabId)
      }
      ...
      const immediate = await readTabTerminalGeometry(mcodePage, firstTabId, runId)
      ...
      const settled = await readTabTerminalGeometry(mcodePage, firstTabId, runId)
    ...
    const schedulerActivity = await waitForHiddenOutputSchedulerActivity(mcodePage)
  ...
  test('keeps geometry after hidden alt-screen TUI redraws during tab switches', async ({
    ...
    const { firstTabId, secondTabId } = await ensureTwoTerminalTabs(mcodePage)
    await forceWebglOnActiveTab(mcodePage)
    await waitForPanePtyIdOnTab(mcodePage, firstTabId)
    ...
    for (let cycle = 0; cycle < 6; cycle += 1) {
      ...
      const { ptyId, cols, rows } = await readPaneIdentityOnTab(mcodePage, firstTabId)
      await setHiddenSnapshotOverride(mcodePage, ptyId, {
        data: buildAltScreenFrame(finalMarker, restoreFrame),
        cols,
        rows
      })
      await activateTerminalTab(mcodePage, secondTabId)
      await writeToPaneTerminal(mcodePage, firstTabId, redraw)
      await activateTerminalTab(mcodePage, firstTabId)

      const geometry = await readTabTerminalGeometry(mcodePage, firstTabId, `${runId}_ALT`)
      const renderedFrame = await readRenderedAltScreenFrame(mcodePage, firstTabId, finalMarker)
      renderPaths.push(
  ...
  test('restores skipped hidden agent output on light tab resume', async ({ mcodePage }) => {
    await waitForSessionReady(mcodePage)
    await waitForActiveWorktree(mcodePage)
    await ensureTerminalVisible(mcodePage)
    await waitForActiveTerminalManager(mcodePage, 30_000)

    const shellTabId = (await getActiveTabId(mcodePage))!
    const agentTabId = await createCodexMarkedTerminalTab(mcodePage)
    await waitForActiveTerminalManager(mcodePage, 30_000)
    await waitForPanePtyIdOnTab(mcodePage, agentTabId)
    const paneIdentity = await readPaneIdentityOnTab(mcodePage, agentTabId)
    const paneKey = `${agentTabId}:${paneIdentity.leafId}`

    await activateTerminalTab(mcodePage, shellTabId)
    const runId = `${Date.now()}`
    ...
    await resetHiddenOutputDebug(mcodePage)
    await injectPaneData(mcodePage, paneKey, hiddenFrame, {
      seq: hiddenFrame.length,
      rawLength: hiddenFrame.length
    })

    await expect
      .poll(async () => (await readHiddenOutputDebug(mcodePage))?.hiddenRendererSkipCount ?? 0, {
        timeout: 5_000,
        message: 'Codex-marked hidden output did not take the skipped renderer path'
      })
      .toBeGreaterThan(0)
    await setHiddenSnapshotOverride(mcodePage, paneIdentity.ptyId, {
    ...
    await activateTerminalTab(mcodePage, agentTabId)

    await expect
      .poll(() => getTerminalContent(mcodePage, 8_000), {
        timeout: 10_000,
  ...
  test('restores skipped hidden Grok output on light tab resume', async ({ mcodePage }) => {
    await waitForSessionReady(mcodePage)
    await waitForActiveWorktree(mcodePage)
    await ensureTerminalVisible(mcodePage)
    await waitForActiveTerminalManager(mcodePage, 30_000)
    ...
    const shellTabId = (await getActiveTabId(mcodePage))!
    const grokTabId = await createGrokMarkedTerminalTab(mcodePage)
    await waitForActiveTerminalManager(mcodePage, 30_000)
    await waitForPanePtyIdOnTab(mcodePage, grokTabId)
    const paneIdentity = await readPaneIdentityOnTab(mcodePage, grokTabId)
    ...
    await activateTerminalTab(mcodePage, grokTabId)
  ...
  test('keeps returned tab glyphs intact across tab switches', async ({ mcodePage }, testInfo) => {
    ...
    await forceWebglOnActiveTab(mcodePage)
    await activateTerminalTab(mcodePage, firstTabId)
    const firstWebgl = await waitForWebglOnTab(mcodePage, firstTabId)
    await activateTerminalTab(mcodePage, secondTabId)
    await mcodePage.evaluate((id) => {
      window.__paneManagers?.get(id)?.setTerminalGpuAcceleration?.('on')
    }, secondTabId)
    const secondWebgl = await waitForWebglOnTab(mcodePage, secondTabId)
    ...
    const firstPtyId = await waitForPanePtyIdOnTab(mcodePage, firstTabId)
    const secondPtyId = await waitForPanePtyIdOnTab(mcodePage, secondTabId)
    await sendToTerminal(mcodePage, firstPtyId, SILENT_FOREGROUND_COMMAND)
    await sendToTerminal(mcodePage, secondPtyId, SILENT_FOREGROUND_COMMAND)
    await mcodePage.waitForTimeout(1_000)

    const runId = `${Date.now()}`
    const markerA = `${TAB_SWITCH_MARKER_PREFIX}_A_${runId}`
    const markerB = `${TAB_SWITCH_MARKER_PREFIX}_B_${runId}`
    await writeStaticTabContent(mcodePage, firstTabId, markerA, TAB_A_GLYPH_ROW)
    await activateTerminalTab(mcodePage, secondTabId)
    await writeStaticTabContent(mcodePage, secondTabId, markerB, TAB_B_GLYPH_ROW)

    await activateTerminalTab(mcodePage, firstTabId)
    await resetAtlasOnTab(mcodePage, firstTabId)
    await mcodePage.waitForTimeout(800)
    const baseline = await captureStableTabScreenshot(mcodePage, firstTabId)
    ...
    for (let cycle = 0; cycle < 8; cycle += 1) {
      ...
      const afterReturn = await captureStableTabScreenshot(mcodePage, firstTabId)
