...
test.describe('terminal reveal paused-render recovery', () => {
  test("reveal repaint forces a render through xterm's paused gate", async ({ mcodePage }) => {
    // Why: __store / __paneManagers live on the main MCode renderer window
    // (mcodePage), not Playwright's default first page.
    const page = mcodePage
