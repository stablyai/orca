...
const SAME_WORKSPACE_PANES = readPositiveInt(
  'MCODE_E2E_OPENCODE_SAME_WORKSPACE_PANES',
  DEFAULT_SAME_WORKSPACE_PANES
)
const CROSS_WORKSPACE_PANES_PER_WORKTREE = readPositiveInt(
  'MCODE_E2E_OPENCODE_CROSS_WORKSPACE_PANES',
  DEFAULT_CROSS_WORKSPACE_PANES_PER_WORKTREE
)
const PRESSURE_BACKGROUND_PANES = readPositiveInt(
  'MCODE_E2E_OPENCODE_PRESSURE_BACKGROUND_PANES',
  DEFAULT_PRESSURE_BACKGROUND_PANES
)
const PRESSURE_OUTPUT_CHARS = readPositiveInt(
  'MCODE_E2E_OPENCODE_PRESSURE_OUTPUT_CHARS',
  DEFAULT_PRESSURE_OUTPUT_CHARS
)
const HIDDEN_PRESSURE_PANES = readPositiveInt(
  'MCODE_E2E_OPENCODE_HIDDEN_PRESSURE_PANES',
  DEFAULT_HIDDEN_PRESSURE_PANES
)
const FRAME_COUNT = readPositiveInt('MCODE_E2E_OPENCODE_FRAME_COUNT', DEFAULT_FRAME_COUNT)
const FRAME_INTERVAL_MS = readPositiveInt(
  'MCODE_E2E_OPENCODE_FRAME_INTERVAL_MS',
  DEFAULT_FRAME_INTERVAL_MS
)
const SCALE_SAME_WORKSPACE_PANES = readPositiveIntList('MCODE_E2E_OPENCODE_SCALE_PANES')
const SCALE_CROSS_WORKSPACE_PANES = readPositiveIntList(
  'MCODE_E2E_OPENCODE_SCALE_CROSS_WORKSPACE_PANES'
)
const SCALE_PRESSURE_PANES = readPositiveIntList('MCODE_E2E_OPENCODE_SCALE_PRESSURE_PANES')
const SCALE_HIDDEN_PRESSURE_PANES = readPositiveIntList(
  'MCODE_E2E_OPENCODE_SCALE_HIDDEN_PRESSURE_PANES'
)
...
async function measureCrossWorkspaceTypingDuringHiddenLoad({
  ...
  const hiddenPanes = await ensureActiveWorktreePaneLoad(mcodePage, hiddenPaneCount)

  await switchToWorktree(mcodePage, firstWorktreeId)
  await expect.poll(() => getActiveWorktreeId(mcodePage), { timeout: 10_000 }).toBe(firstWorktreeId)
  await ensureTerminalVisible(mcodePage)
  await waitForActiveTerminalManager(mcodePage, 30_000)
  const typingPtyId = await waitForActivePanePtyId(mcodePage)

  const runId = randomUUID()
  const scriptPath = path.join(testRepoPath, `.mcode-opencode-cross-${hiddenPaneCount}-${runId}.mjs`)
  writeInteractivePromptScript(scriptPath, runId)
  await resetTerminalPtyOutputDebug(mcodePage)
  ...
  try {
    const measurement = await measureTypingDuringLoad(mcodePage, scriptPath, typingPtyId, runId)
    const debug = await readTerminalPtyOutputDebug(mcodePage)
    const scheduler = await readTerminalOutputSchedulerDebug(mcodePage)
    const mainPressure = await readMainPtyPressureDebug(mcodePage)
    ...
    await load.stop()
    await sendToTerminal(mcodePage, typingPtyId, '\x03').catch(() => undefined)
...
test.describe('Artificial OpenCode terminal load', () => {
  ...
  test('measures baseline typing responsiveness with one active terminal', async ({
    ...
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.mcode-opencode-baseline-typing-${runId}.mjs`)
    writeInteractivePromptScript(scriptPath, runId)
    await resetTerminalPtyOutputDebug(mcodePage)
    try {
      const measurement = await measureTypingDuringLoad(mcodePage, scriptPath, typingPtyId, runId)
  ...
  test('keeps typing responsive while same-workspace panes redraw simultaneously', async ({
    ...
    await waitForSessionReady(mcodePage)
    await waitForActiveWorktree(mcodePage)
    const panes = await ensureActiveWorktreePaneLoad(mcodePage, SAME_WORKSPACE_PANES)
    const [typingPane, ...loadPanes] = panes
    await focusPane(mcodePage, typingPane.paneKey)

    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.mcode-opencode-typing-${runId}.mjs`)
    writeInteractivePromptScript(scriptPath, runId)
    await resetTerminalPtyOutputDebug(mcodePage)
    const load = await startSyntheticOpenCodeInjection({
      frameCount: FRAME_COUNT,
    ...
    try {
      ...
      annotateTypingMeasurement(
        ...
        panes.length,
        measurement,
        await readTerminalPtyOutputDebug(mcodePage),
        await readTerminalOutputSchedulerDebug(mcodePage),
        await readMainPtyPressureDebug(mcodePage)
      )
      expect(measurement.medianLatencyMs).toBeLessThan(MAX_MEDIAN_KEY_LATENCY_MS)
      expect(measurement.worstLatencyMs).toBeLessThan(MAX_WORST_KEY_LATENCY_UNDER_LOAD_MS)
      expect(measurement.maxTimerDriftMs).toBeLessThan(MAX_TIMER_DRIFT_UNDER_LOAD_MS)
  ...
  for (const paneCount of SCALE_SAME_WORKSPACE_PANES) {
    test(`keeps typing responsive at ${paneCount} same-workspace OpenCode panes`, async ({
      ...
      await waitForSessionReady(mcodePage)
      await waitForActiveWorktree(mcodePage)
      const panes = await ensureActiveWorktreePaneLoad(mcodePage, paneCount)
      const [typingPane, ...loadPanes] = panes
      await focusPane(mcodePage, typingPane.paneKey)

      const runId = randomUUID()
      const scriptPath = path.join(testRepoPath, `.mcode-opencode-scale-${paneCount}-${runId}.mjs`)
      writeInteractivePromptScript(scriptPath, runId)
      await resetTerminalPtyOutputDebug(mcodePage)
      const load = await startSyntheticOpenCodeInjection({
        frameCount: FRAME_COUNT,
  ...
  for (const hiddenPressureCase of hiddenPressureCases) {
    test(hiddenPressureCase.title, async ({ mcodePage, testRepoPath }, testInfo) => {
