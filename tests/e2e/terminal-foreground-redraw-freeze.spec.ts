...
test.describe('Terminal foreground redraw freeze repro', () => {
  test('Codex-style line rewrites request a visible row refresh', async ({
    ...
    const webglAttached = await forceActivePaneWebglRenderer(mcodePage)
    ...
    await installActivePaneRefreshProbe(mcodePage)
    try {
      const refreshBaseline = await readRefreshProbe(mcodePage)
      await resetSchedulerDebug(mcodePage)
      const measurement = await measureRendererDuringRewriteBurst(mcodePage, paneKey)
      const scheduler = await readSchedulerDebug(mcodePage)
      ...
      await expect
        .poll(
          async () => {
            const refresh = await readRefreshProbe(mcodePage)
      ...
      await disposeActivePaneRefreshProbe(mcodePage)
    }
  })
  ...
  test('active OpenTUI-style redraw bursts do not monopolize the renderer', async ({
    ...
    await waitForTerminalPtyDataInjector(mcodePage, paneKey)
    await resetSchedulerDebug(mcodePage)
    const measurement = await measureRendererDuringBurst(mcodePage, paneKey)
    const scheduler = await readSchedulerDebug(mcodePage)
  ...
  test('captured OpenCode/OpenTUI redraw bytes do not monopolize foreground writes', async ({
    ...
    await waitForTerminalPtyDataInjector(mcodePage, paneKey)
    await resetSchedulerDebug(mcodePage)
    const measurement = await measureRendererDuringFrames(mcodePage, paneKey, frames)
    const scheduler = await readSchedulerDebug(mcodePage)
    annotateMeasurement(testInfo, measurement, scheduler)
