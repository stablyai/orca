...
test('keeps long repository names inside the cross-project confirmation dialog', async ({
  ...
  const tempRoot = mkdtempSync(path.join(os.tmpdir(), 'mcode-e2e-cross-project-dialog-'))
  const secondRepositoryPath = path.join(tempRoot, LONG_REPOSITORY_NAME)
  createGitRepository(secondRepositoryPath)

  try {
    const secondRepositoryId = await mcodePage.evaluate(async (repositoryPath) => {
    ...
    const currentRepositoryId = await mcodePage.evaluate(() => {
      const store = window.__store
    ...
    await mcodePage.setViewportSize({ width: 640, height: 720 })
    ...
    const confirmation = mcodePage.getByRole('dialog', { name: 'Switch project?' })
    await expect(confirmation).toBeVisible()
    await expect(confirmation).toContainText(LONG_REPOSITORY_NAME)

    if (process.env.MCODE_VISUAL_PROOF === '1') {
