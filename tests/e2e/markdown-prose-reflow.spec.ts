...
test.describe('Markdown prose reflow', () => {
  ...
  test('hard-wrapped prose reflows as one document paragraph', async ({ mcodePage }, testInfo) => {
    let filePath: string | null = null

    try {
      filePath = await openHardWrappedFixture(mcodePage, testInfo)
      const metrics = await getGoalParagraphMetrics(mcodePage)
  ...
  test('deleting an inserted empty paragraph keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      filePath = await openHardWrappedFixture(mcodePage, testInfo)
      await placeCaretAtGoalParagraphEnd(mcodePage)
  ...
  test('deleting the block boundary before a heading keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      filePath = await openHardWrappedFixture(mcodePage, testInfo)
      await placeCaretAtHeadingStart(mcodePage)
      await mcodePage.keyboard.press('Backspace')
