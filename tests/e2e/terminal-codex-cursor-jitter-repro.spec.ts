...
test.describe('Codex terminal cursor jitter repro', () => {
  test('keeps queued-message cursor out of the Working status row in native Windows Codex @headful', async ({
    ...
    await installPtyWriteDiagnostics(mcodePage)
    rmSync(ARTIFACT_DIR, { recursive: true, force: true })
    mkdirSync(ARTIFACT_DIR, { recursive: true })

    const shellCase = SHELL_CASES[0]!
    const { tabId, ptyId } = await prepareCodexTerminal(mcodePage, shellCase)
    await installPtyOutputDiagnostics(mcodePage)
    await mcodePage.keyboard.type(CODEX_REPO_PROMPT)
    await mcodePage.waitForTimeout(250)
    await mcodePage.keyboard.press('Enter')
    await mcodePage.waitForTimeout(1_000)
    if (!CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(mcodePage, tabId, 8_000))) {
      await mcodePage.keyboard.press('Enter')
    }
    await mcodePage.waitForTimeout(3_000)
    const submittedContent = await getTerminalContentForTab(mcodePage, tabId, 8_000)
    writeFileSync(path.join(ARTIFACT_DIR, 'queued-message-after-submit.txt'), submittedContent)
    await expect
      .poll(
        async () =>
          CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(mcodePage, tabId, 8_000)),
        {
          timeout: 30_000,
      ...
      .toBe(true)
    await applyCursorProbeTheme(mcodePage, tabId)
    ...
    await mcodePage.keyboard.insertText('s')
    await expect
      .poll(
        async () =>
          (await readScreenLines(mcodePage, tabId)).some((line) => isQueuedInputLine(line.text)),
        {
          timeout: 5_000,
    ...
    const snapshot = await readScreenSnapshot(mcodePage, shellCase.label, tabId, ptyId)
    const rawChunks = await readPtyOutputDiagnostics(mcodePage)
