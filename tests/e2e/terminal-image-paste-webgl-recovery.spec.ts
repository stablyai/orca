...
test.describe('terminal image paste WebGL recovery @headful', () => {
  test('clears the WebGL atlas after a real image clipboard paste', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.mcode-image-paste-redraw-${marker}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(mcodePage, `READY_${marker}`, 10_000)

      await forceWebgl(mcodePage)
      ...
      await mcodePage.locator('.xterm-helper-textarea').first().focus()
      await mcodePage.evaluate(
        (dataUrl) => window.api.ui.writeClipboardImage(dataUrl),
        CLIPBOARD_IMAGE_DATA_URL
      )
      await mcodePage.keyboard.press(process.platform === 'darwin' ? 'Meta+V' : 'Control+V')
      await waitForTerminalOutput(mcodePage, `DONE_${marker}`, 10_000)

      await expect.poll(() => readAtlasResetCount(mcodePage), { timeout: 2_000 }).toBeGreaterThan(0)
    } finally {
