...
test.describe('terminal TUI wheel reports', () => {
  test('notched mouse wheel ticks produce immediate mouse-reporting TUI scroll reports', async ({
    ...
    const samples = await probeSmallMouseWheelReports(mcodePage, 4)
  ...
  test('fullscreen mouse-reporting TUI scroll distance follows wheel magnitude @headful', async ({
    ...
    await expect
      ...
      .toBe(true)
    await mcodePage.waitForTimeout(1200)
    ...
    await execInTerminal(mcodePage, ptyId, `node ${JSON.stringify(VISIBLE_TUI_FIXTURE_PATH)}`)

    await expect
      .poll(() => readVisibleTuiOffset(mcodePage), {
        timeout: 10_000,
        message: 'visible fullscreen TUI did not render numbered rows'
      })
      .toBe(0)

    await dispatchTuiWheel(mcodePage, {
      deltaY: 10,
    ...
    const cellHeight = await mcodePage.evaluate(() => {
      const state = window.__store?.getState()
      const worktreeId = state?.activeWorktreeId
      const tabId =
        state?.activeTabType === 'terminal'
  ...
  test('TUI scroll setting scales notched mouse wheel reports', async ({ mcodePage }) => {
    await waitForSessionReady(mcodePage)
    await waitForActiveWorktree(mcodePage)
    await ensureTerminalVisible(mcodePage)
    await waitForActiveTerminalManager(mcodePage, 30_000)
    ...
    const slow = await probeTimedSmallMouseWheelReports(mcodePage, {
      drainWaitMs: 120,
      intervalMs: 220,
      ticks: 5
    })
    await mcodePage.waitForTimeout(220)
    const paced = await probeTimedSmallMouseWheelReports(mcodePage, {
      drainWaitMs: 220,
      intervalMs: 80,
      ticks: 5
    })
