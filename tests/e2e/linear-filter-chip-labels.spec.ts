...
test('Linear filter chips keep readable names after the dropdown closes', async ({
  ...
  await expect
    .poll(() => getStoreState<string>(mcodePage, 'activeView'), { timeout: 5_000 })
    .toBe('tasks')
  const filtersButton = mcodePage.getByRole('button', { name: 'Filters', exact: true })
  await expect(filtersButton).toBeVisible()
  await expect(mcodePage.getByText(FIXTURE.issue.title, { exact: true })).toBeVisible()

  await filtersButton.click()
  const popover = mcodePage.locator('[data-slot="popover-content"]')
