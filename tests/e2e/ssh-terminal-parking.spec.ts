...
test.describe('SSH terminal hidden view parking', () => {
  ...
  test('parks a hidden SSH tab and restores its scrollback on reveal', async ({
    ...
    try {
      ...
      const sshPtyId = await waitForActivePanePtyId(mcodePage, 60_000)
      const sshTabId = await getActiveTabId(mcodePage)
      if (!sshTabId) {
      ...
      await parkHiddenTabBehindDecoy(mcodePage, remote.worktreeId, sshTabId, {
      ...
      await expect
        .poll(() => getTerminalContent(mcodePage, 2_000_000), {
          timeout: 15_000,
