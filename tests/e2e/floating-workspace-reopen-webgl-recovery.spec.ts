...
const TOGGLE_EVENT = 'mcode-toggle-floating-terminal'
...
test.describe('floating workspace reopen WebGL recovery @headful', () => {
  test('reopening the floating workspace recovers a corrupted glyph atlas', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(mcodePage, 'REOPEN')
    ...
    expect(await instrumentRecoveryCounters(mcodePage)).toBe(true)

    await toggleFloatingPanel(mcodePage, false)
    ...
    const webglAttachedWhileClosed = await mcodePage.evaluate((worktreeId) => {
      const state = window.__store?.getState()
      const tab = (state?.tabsByWorktree?.[worktreeId] ?? [])[0]
      const manager = tab ? window.__paneManagers?.get(tab.id) : null
      const diagnostics = manager?.getRenderingDiagnostics?.() ?? []
    ...
    await toggleFloatingPanel(mcodePage, true)
    await settleRecoveryWindows(mcodePage)

    const counters = await readRecoveryCounters(mcodePage)
    const afterReopen = await screenshotFloatingTerminal(mcodePage)
  ...
  test('window focus regain recovers the corrupted atlas (harness control)', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(mcodePage, 'CONTROL')
    test.skip(!shots, 'WebGL was not active or atlas corruption could not be injected')
    const { baseline, corrupted } = shots!
    expect(corrupted.equals(baseline)).toBe(false)
    ...
    const afterFocus = await screenshotFloatingTerminal(mcodePage)
