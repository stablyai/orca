...
test('persists the opt-in work item Start behavior', async ({ orcaPage }) => {
  ...
  await waitForSessionReady(orcaPage)
  await openTasksSettings(orcaPage)
  const reloadedBehavior = orcaPage.getByRole('radiogroup', {
    name: 'Work item Start behavior'
  })
  await expect(reloadedBehavior.getByRole('radio', { name: 'Submit after ready' })).toHaveAttribute(
    'aria-checked',
    'true'
  )
})
