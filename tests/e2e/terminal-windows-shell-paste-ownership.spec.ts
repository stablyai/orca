...
test.describe('Windows terminal shell paste ownership', () => {
  ...
  test('PowerShell default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await createWindowsDefaultShellTerminalTab(mcodePage, 'powershell.exe')
    await waitForActiveTerminalManager(mcodePage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const sentinel = `MCODE_E2E_POWERSHELL_DONE_${runId}`
    const powershellEscape = '`'
    const payload = [
      `MCODE_E2E_POWERSHELL_PASTE_${runId}`,
      `PowerShell metacharacters: ${powershellEscape} $ " ' ; | & < > @ { } ( )`,
      'quoted Windows path: C:\\Program Files\\MCode Test\\file name.txt',
      'cmd metacharacters preserved as text: %PATH% !PROMPT! ^ & | < >',
      'Unicode: café 你好 مرحبا 😀',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.mcode-paste-powershell-shell-${runId}.mjs`)
  ...
  test('cmd.exe default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(mcodePage)
    await ensureTerminalVisible(mcodePage)
    await createWindowsDefaultShellTerminalTab(mcodePage, 'cmd.exe')
    await waitForActiveTerminalManager(mcodePage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const sentinel = `MCODE_E2E_CMD_DONE_${runId}`
    const payload = [
      `MCODE_E2E_CMD_PASTE_${runId}`,
      'cmd metacharacters: %PATH% !PROMPT! ^ & | < >',
      'quoted Windows path: C:\\Program Files\\MCode Test\\file name.txt',
      'PowerShell metacharacters: ` $ " \' ; @ { } ( )',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.mcode-paste-cmd-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('Git Bash default terminal keyboard paste preserves POSIX-shaped content with one PTY owner', async ({
    ...
    await skipWhenGitBashUnavailable(mcodePage)

    await waitForSessionReady(mcodePage)
    await waitForActiveWorktree(mcodePage)
    await ensureTerminalVisible(mcodePage)
    await createWindowsDefaultShellTerminalTab(mcodePage, WINDOWS_GIT_BASH_SHELL)
    await waitForActiveTerminalManager(mcodePage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const sentinel = `MCODE_E2E_GIT_BASH_DONE_${runId}`
    const payload = [
      `MCODE_E2E_GIT_BASH_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.mcode-paste-git-bash-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('WSL terminal keyboard paste preserves Linux shell content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(mcodePage)
    await ensureTerminalVisible(mcodePage)
    const wslDistro = await configureActiveProjectWslRuntime(mcodePage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    await createWindowsProjectRuntimeTerminalTab(mcodePage, 'wsl.exe')
    await waitForActiveTerminalManager(mcodePage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const sentinel = `MCODE_E2E_WSL_DONE_${runId}`
    const payload = [
      `MCODE_E2E_WSL_PASTE_${runId}`,
      'POSIX shell metacharacters: $ ` " \' ; | & < > * ? [ ] ( )',
    ...
    const scriptPath = path.join(testRepoPath, `.mcode-paste-wsl-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('existing WSL terminal keeps paste runtime after default shell changes', async ({
    ...
    const wslDistro = await configureActiveProjectWslRuntime(mcodePage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    const tabId = await createWindowsProjectRuntimeTerminalTab(mcodePage, 'wsl.exe')
    await updateWindowsDefaultShellSetting(mcodePage, 'cmd.exe')
    await expect(
      mcodePage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"] [data-shell-icon]`)
    ...
    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const sentinel = `MCODE_E2E_WSL_RETENTION_DONE_${runId}`
    const payload = [
      `MCODE_E2E_WSL_RETENTION_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.mcode-paste-wsl-retention-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
