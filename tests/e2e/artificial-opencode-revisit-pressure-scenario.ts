...
>({
  ...
  const secondWorktreeId = (await getAllWorktreeIds(mcodePage)).find((id) => id !== firstWorktreeId)
  ...
  await sendToTerminal(mcodePage, typingPtyId, `printf '\\n${typingPtyReadyMarker}\\n'\r`)
  await waitForMarkerLatency(mcodePage, typingPtyReadyMarker, 10_000)
  ...
  await deps.focusPane(mcodePage, revisitPane.paneKey)

  const typingScriptPath = path.join(testRepoPath, `.mcode-revisit-typing-${runId}.mjs`)
  const pressureScriptPath = path.join(testRepoPath, `.mcode-revisit-pressure-${runId}.mjs`)
  ...
  try {
    ...
    const pressureBeforeSwitch = await deps.waitForMainPtyPressureBacklog(mcodePage)

    await switchToWorktree(mcodePage, secondWorktreeId)
    await ensureTerminalVisible(mcodePage)
    await waitForActiveTerminalManager(mcodePage, 30_000)
    await waitForTerminalPtyVisible(mcodePage, typingPtyId)
    ...
    const duringPressure = await deps.readMainPtyPressureDebug(mcodePage)
    ...
    const hiddenDebug = await deps.readTerminalPtyOutputDebug(mcodePage)
    ...
    await waitForTerminalPtyVisible(mcodePage, revisitPane.ptyId)
    await deps.focusPane(mcodePage, revisitPane.paneKey)
    await sendToTerminal(mcodePage, revisitPane.ptyId, `printf '\\n${revisitMarker}\\n'\r`)
    const revisitLatencyMs = await waitForMarkerLatency(mcodePage, revisitMarker, 10_000)
    ...
    await deps.focusPane(mcodePage, loadPanes[0]?.paneKey ?? revisitPane.paneKey)
    const pressureDrainLatencyMs = await waitForMarkerLatency(mcodePage, pressureDoneMarker, 20_000)
    const finalScheduler = await deps.readTerminalOutputSchedulerDebug(mcodePage)
    testInfo.annotations.push({
    ...
    await sendToTerminal(mcodePage, revisitPane.ptyId, '\x03').catch(() => undefined)
    await Promise.all(
