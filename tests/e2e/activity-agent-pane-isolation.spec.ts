...
test.describe('Activity Agent Pane Isolation', () => {
  test.beforeEach(async ({ mcodePage }) => {
    ...
    await enableActivityAgentsView(mcodePage)
  ...
  test('selecting agent rows isolates the matching split pane by stable leaf id', async ({
    ...
    const [first, second] = await seedActivityThreadsForSplitPanes(mcodePage, snapshot)

    await agentsSidebarButton(mcodePage).click()
    await expect(mcodePage.getByText(first.prompt)).toBeVisible()
    await expect(mcodePage.getByText(second.prompt)).toBeVisible()

    await mcodePage.getByRole('button').filter({ hasText: first.prompt }).first().click()
    await expect
      .poll(async () => readActivityPaneVisibility(mcodePage), {
        timeout: 10_000,
    ...
    await mcodePage.getByRole('button').filter({ hasText: second.prompt }).first().click()
    await expect
      .poll(async () => readActivityPaneVisibility(mcodePage), {
        timeout: 10_000,
  ...
  test('acknowledged stable pane keys clear the Agents unread badge', async ({ mcodePage }) => {
    await splitActiveTerminalPane(mcodePage, 'vertical')
    await waitForPaneCount(mcodePage, 2)
    const snapshot = await waitForPaneIdentitySnapshot(mcodePage, 2)
    ...
    // call under test.
    const activeLeafId = await mcodePage.evaluate(
    ...
    await expect(agentsSidebarButton(mcodePage)).toHaveAccessibleName(/^Agents\s+1$/)

    await mcodePage.evaluate((paneKey) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
    ...
    await expect(agentsSidebarButton(mcodePage)).toHaveAccessibleName(/^Agents$/)
    await expect(mcodePage.getByRole('button', { name: /^Agents\s+1$/ })).toHaveCount(0)
  })

  test('workspace card agent rows focus the matching terminal split pane', async ({ mcodePage }) => {
    await splitActiveTerminalPane(mcodePage, 'vertical')
    await waitForPaneCount(mcodePage, 2)
    const snapshot = await waitForPaneIdentitySnapshot(mcodePage, 2)
    const [first, second] = await seedActivityThreadsForSplitPanes(mcodePage, snapshot)

    await enableInlineAgentCards(mcodePage)

    await clickWorkspaceCardAgentRow(mcodePage, first.prompt)
    await expect
      .poll(async () => readActivePaneSelection(mcodePage), {
        timeout: 10_000,
    ...
    await clickWorkspaceCardAgentRow(mcodePage, second.prompt)
    await expect
      .poll(async () => readActivePaneSelection(mcodePage), {
        timeout: 10_000,
  ...
  test('workspace card agent rows reveal terminal logs from a non-terminal surface', async ({
    ...
    await waitForPaneCount(mcodePage, 2)
    const snapshot = await waitForPaneIdentitySnapshot(mcodePage, 2)
    const [first] = await seedActivityThreadsForSplitPanes(mcodePage, snapshot)

    await enableInlineAgentCards(mcodePage)
    await expect(terminalPaneForLeaf(mcodePage, first.leafId)).toBeVisible()
    ...
    await expect(await clickFileInExplorer(mcodePage, ['README.md'])).toBe('README.md')
    await expect
      .poll(() => readActivePaneSelection(mcodePage))
      .toMatchObject({
        activeTabType: 'editor',
        activeTabId: snapshot.tabId
      })
    await expect(terminalPaneForLeaf(mcodePage, first.leafId)).toBeHidden()
  ...
  test('workspace card agent rows focus the matching split-group terminal pane', async ({
    ...
    await splitActiveTerminalPane(mcodePage, 'vertical')
    await waitForPaneCount(mcodePage, 2)
    const firstGroupSnapshot = await waitForPaneIdentitySnapshot(mcodePage, 2)
    const [first, second] = await seedActivityThreadsForSplitPanes(mcodePage, firstGroupSnapshot)

    const splitGroup = await createTerminalInNewSplitGroup(mcodePage)
    await waitForActiveTerminalManager(mcodePage, 30_000)
    await waitForPaneCount(mcodePage, 1, 30_000)
    const secondGroupSnapshot = await waitForPaneIdentitySnapshot(mcodePage, 1)
    ...
    await clickWorkspaceCardAgentRow(mcodePage, splitGroupThread.prompt)
    await expect
      .poll(async () => readActivePaneSelection(mcodePage), {
        timeout: 10_000,
