...
test.describe('terminal streaming refocus viewport', () => {
  test('keeps follow-output at the bottom through a queued-write refocus wobble', async ({
    ...
    await execInTerminal(mcodePage, ptyId, nodeTerminalCommand([STREAMING_FIXTURE_PATH]))
    await waitForPhaseOneAtBottom(mcodePage, tabId)

    const framesPromise = sampleRevealFrames(mcodePage, tabId)
    await injectQueuedWriteAndRefocus(mcodePage, tabId, paneKey)
    ...
    await expect
      .poll(() => getTerminalContent(mcodePage), { timeout: 15_000 })
      .toContain('REFOCUS_STREAM_DONE')
    const visibleScrollbar = mcodePage.locator('.xterm-scrollbar.xterm-vertical:visible').first()
