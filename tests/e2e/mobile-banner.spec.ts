...
test('mobile subscribe mounts overlay; collapse → chip; Take back dismisses', async ({
  ...
  const overlay = mcodePage.locator('.mobile-driver-banner')
  await expect(overlay).toHaveCount(0)

  // Fire the IPC events main emits when a mobile client subscribes in 'auto'
  // mode (handleMobileSubscribe in src/main/runtime/mcode-runtime.ts). The
  ...
  await expectExpandedOverlayLeavesPaneReadable(mcodePage, ptyId)
  ...
  await captureAttachment(mcodePage, testInfo, 'overlay-loud.png')
  ...
  await expectChipIsCompactInPane(mcodePage, ptyId)

  await captureAttachment(mcodePage, testInfo, 'overlay-collapsed.png')
...
test('held phone-fit state mounts restore overlay without collapse', async ({
  ...
  await captureAttachment(mcodePage, testInfo, 'overlay-held-fit.png')
...
test('restore this terminal refits the active restored pane', async ({ mcodePage, electronApp }) => {
  await waitForSessionReady(mcodePage)
  await waitForActiveWorktree(mcodePage)
  await ensureTerminalVisible(mcodePage)
  await waitForActiveTerminalManager(mcodePage)
  ...
  await expect(mcodePage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })
  await expect
    .poll(() => getPaneTerminalCols(mcodePage, ptyId), {
...
test('restore all refits non-focused restored terminal panes', async ({
  ...
  const ptyIds = await waitForVisiblePanePtyIds(mcodePage, 2)
  const focusPtyId = await waitForActivePanePtyId(mcodePage)
  ...
  await expect(mcodePage.locator('.mobile-driver-banner')).toHaveCount(2, { timeout: 15_000 })

  await forcePaneToOneColumn(mcodePage, inactivePtyId)
  await expect
    .poll(() => getPaneTerminalCols(mcodePage, inactivePtyId), {
...
test('restore all recovers a hidden workspace held at narrow terminal geometry', async ({
  ...
  await ensureTerminalVisible(mcodePage)
  await waitForActiveTerminalManager(mcodePage)
  const hiddenWorkspacePtyId = await waitForActivePanePtyId(mcodePage)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: hiddenWorkspacePtyId, cols: 45, rows: 20 })
  await expect(mcodePage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })

  await forcePaneToOneColumnAndSwitchWorktree(mcodePage, hiddenWorkspacePtyId, secondWorktreeId)
  await expect
  ...
  await expect
    .poll(() => getPaneTerminalCols(mcodePage, hiddenWorkspacePtyId), {
      message: 'test harness should hold workspace 1 in the bad narrow state'
    })
    .toBeLessThanOrEqual(2)
  await ensureTerminalVisible(mcodePage)
  await waitForActiveTerminalManager(mcodePage)
  const activeWorkspacePtyId = await waitForActivePanePtyId(mcodePage)
  await installRestoreTerminalFitAutoRestoreRecorder(electronApp)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: activeWorkspacePtyId, cols: 45, rows: 20 })
  await expect(
    mcodePage.locator(`[data-pty-id="${activeWorkspacePtyId}"] .mobile-driver-banner`)
