...
test.describe('Worktree switch responsiveness', () => {
  ...
  test('updates the selected workspace in the same click task when changing back', async ({
    ...
    const [firstWorktreeId, secondWorktreeId] = await prepareSidebarForSwitchTest(mcodePage)
    const firstRow = worktreeRow(mcodePage, firstWorktreeId)
    const secondRow = worktreeRow(mcodePage, secondWorktreeId)

    await expect(firstRow).toBeVisible()
    await expect(secondRow).toBeVisible()
    await expect(firstRow).toHaveAttribute('aria-current', 'page')
    await expect(mcodePage.locator('[data-rendered-active-worktree-id]')).toHaveAttribute(
