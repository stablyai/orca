 ...
 *
 * Scenarios are gated behind MCODE_TYPING_BENCH=1 (they are benchmarks that
...
const BENCH_ENABLED = process.env.MCODE_TYPING_BENCH === '1'
...
const LOAD_PANES = readPositiveInt('MCODE_TYPING_BENCH_LOAD_PANES', 4)
const LOAD_RATE_KBPS = readPositiveInt('MCODE_TYPING_BENCH_RATE_KBPS', 256)
const KEY_COUNT = readPositiveInt('MCODE_TYPING_BENCH_KEYS', 32)
const KEY_CADENCE_MS = readPositiveInt('MCODE_TYPING_BENCH_KEY_CADENCE_MS', 250)
const CPU_WORKERS = readPositiveInt('MCODE_TYPING_BENCH_CPU_WORKERS', 0)
const BENCH_LABEL = process.env.MCODE_TYPING_BENCH_LABEL ?? 'dev'
...
test.describe('Multi-workspace sustained typing latency bench', () => {
  ...
  test('baseline: paced typing with no agent load', async ({
    ...
    const probePath = path.join(testRepoPath, `.mcode-mwt-probe-${runId}.mjs`)
    const sidecarPath = path.join(testRepoPath, `.mcode-mwt-arrivals-${runId}.jsonl`)
    writeTypingEchoProbeScript(probePath, runId, sidecarPath)
    try {
      await resetDeliveryDebug(mcodePage)
      await startTypingProbe(mcodePage, typingPtyId, probePath, runId)
      const measurement = await measurePacedTyping(mcodePage, runId, sidecarPath)
      writeBenchReport(
        testInfo,
        'baseline',
        measurement,
        await readSchedulerDebug(mcodePage),
        await readMainDeliveryDebug(mcodePage)
      )
      expect(measurement.missingEchoCount).toBe(0)
      expect(measurement.totalMs?.p50 ?? Number.POSITIVE_INFINITY).toBeLessThan(250)
    } finally {
      await stopPtysQuietly(mcodePage, [typingPtyId])
  ...
  test('typing under sustained hidden multi-workspace agent load', async ({
    ...
    test.skip(!BENCH_ENABLED, 'Bench-only: run via pnpm bench:multi-workspace-typing')
    await waitForSessionReady(mcodePage)
    const typingWorktreeId = await waitForActiveWorktree(mcodePage)
    const loadWorktreeId = (await getAllWorktreeIds(mcodePage)).find((id) => id !== typingWorktreeId)
    ...
    const runId = randomUUID()
    const loadPath = path.join(testRepoPath, `.mcode-mwt-load-${runId}.mjs`)
    const probePath = path.join(testRepoPath, `.mcode-mwt-probe-${runId}.mjs`)
    const sidecarPath = path.join(testRepoPath, `.mcode-mwt-arrivals-${runId}.jsonl`)
    ...
    try {
      await switchToWorktree(mcodePage, loadWorktreeId)
      loadPanes = await ensureActiveWorktreePaneLoad(mcodePage, LOAD_PANES)
      await startSustainedLoadInPanes(mcodePage, loadPanes, loadPath, runId, testRepoPath)

      await switchToWorktree(mcodePage, typingWorktreeId)
      await expect
        .poll(() => getActiveWorktreeId(mcodePage), { timeout: 10_000 })
      ...
      await expect
        .poll(
          async () => (await readMainDeliveryDebug(mcodePage))?.hiddenDeliveryDroppedChars ?? 0,
      ...
      await switchToWorktree(mcodePage, loadWorktreeId).catch(() => undefined)
      await stopPtysQuietly(
        mcodePage,
        loadPanes.map((pane) => pane.ptyId)
      )
      await switchToWorktree(mcodePage, typingWorktreeId).catch(() => undefined)
  ...
  test('typing under sustained visible split agent load', async ({
    ...
    try {
      ...
      panes = await ensureActiveWorktreePaneLoad(mcodePage, 2)
      const [typingPane, ...loadPanes] = panes
      await startSustainedLoadInPanes(mcodePage, loadPanes, loadPath, runId, testRepoPath)
      await focusPane(mcodePage, typingPane.paneKey)

      await resetDeliveryDebug(mcodePage)
      await startTypingProbe(mcodePage, typingPane.ptyId, probePath, runId)
      const measurement = await measurePacedTyping(mcodePage, runId, sidecarPath)
      writeBenchReport(
        testInfo,
