...
test.describe('New workspace composer linked item across project switches', () => {
  ...
  test.beforeEach(async ({ mcodePage }) => {
    ...
    tempRoot = mkdtempSync(path.join(os.tmpdir(), 'mcode-e2e-linked-item-'))
    secondRepoPath = path.join(tempRoot, SECOND_PROJECT_NAME)
    createGitRepo(secondRepoPath)
    await addSecondProject(mcodePage, secondRepoPath)
  })
  ...
  test('keeps a Jira issue linked when the project changes', async ({ mcodePage }) => {
    const jiraSourceContext = await getJiraSourceContext(mcodePage)
    ...
    const composer = mcodePage.getByRole('dialog')
    ...
    await switchComposerProject(mcodePage, SECOND_PROJECT_NAME)

    await expect(sourcePill).toContainText('RDG-344 Migrate homepage from NuxtJS to NextJS')
  })

  test('clears a repo-scoped GitHub issue when the project changes', async ({ mcodePage }) => {
    await openComposerWithLinkedWorkItem(
      mcodePage,
      {
        type: 'issue',
