...
// Why the explicit budget: this is the only suite here that pins a commit no sibling shares, so
// it extracts a second checkout of its own and cannot finish inside the 30s default.
const TIMEOUT_MS = 180_000

test(
  'pre-v41 code opens, acknowledges and writes a v41 database, then current code reopens it',
  async () => {
