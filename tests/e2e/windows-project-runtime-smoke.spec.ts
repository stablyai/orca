...
test.describe('Windows project runtime smoke', () => {
  test('keeps a Windows-host project and WSL project available side by side', async ({
    ...
    const wsl = await mcodePage.evaluate(async () => ({
    ...
    const wslRepoPath = await createGitRepo('mcode-e2e-project-runtime-', 'wsl-runtime-project')

    const smoke = await mcodePage.evaluate(
    ...
    await openRepoSettings(mcodePage, smoke.hostRepoId)
    const hostSection = mcodePage.locator(`[data-settings-section="repo-${smoke.hostRepoId}"]`)
    ...
    await chooseProjectRuntime(mcodePage, smoke.hostRepoId, 'WSL')
    await expect(
      hostSection.getByText(`This project runs in ${smoke.wslDistro} via WSL.`)
    ).toBeVisible()
    const hostAfterWslUiSwitch = await mcodePage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
    ...
    await chooseProjectRuntime(mcodePage, smoke.hostRepoId, 'Windows')
    await expect(hostSection.getByText('This project runs on Windows.')).toBeVisible()
    const hostAfterWindowsUiSwitch = await mcodePage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
        project.sourceRepoIds.includes(hostRepoId)
      )
    ...
    await openRepoSettings(mcodePage, smoke.wslRepoId)
    const wslSection = mcodePage.locator(`[data-settings-section="repo-${smoke.wslRepoId}"]`)
    ...
    await mcodePage.evaluate(async (repoId) => {
