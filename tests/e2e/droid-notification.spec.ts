...
test.describe('Droid notifications', () => {
  test('Codex hook completion dispatches while its worktree is inactive', async ({
    ...
    await sendToTerminal(mcodePage, ptyId, `printf '${readyMarker}\\n'\r`)
    await waitForTerminalOutput(mcodePage, readyMarker)

    const { paneKey, worktreeId } = await waitForActivePaneHookDescriptor(mcodePage)
    ...
    await expect
      .poll(
        async () =>
          (await getRendererOrCachedAgentStatuses(mcodePage)).some(
            (status) =>
    ...
    await switchToOtherExistingWorktree(mcodePage)
    ...
    await expect
      .poll(
        async () =>
          (await getAgentStatuses(mcodePage)).some(
            (status) =>
    ...
    await expect
      .poll(async () => isWorktreeUnread(mcodePage, worktreeId), {
        timeout: 10_000,
  ...
  test('recognized agent title completion dispatches one task-complete notification', async ({
    ...
    await emitOscTitle(mcodePage, ptyId, 'Codex working')
    await expect
      .poll(async () => (await getRendererTitleLog(mcodePage)).includes('Codex working'), {
        timeout: 10_000,
        message: 'Codex working title did not reach the renderer before completion'
      })
      .toBe(true)

    await emitOscTitle(mcodePage, ptyId, 'Codex done')
    await expect
      .poll(async () => (await getRendererTitleLog(mcodePage)).includes('Codex done'), {
        timeout: 10_000,
  ...
  test('Factory Droid needs-input native title does not dispatch a task-complete notification', async ({
    ...
    await emitOscTitle(mcodePage, ptyId, '⠋ Droid')
    await emitOscTitle(mcodePage, ptyId, 'Factory Droid needs input')

    await expect
      .poll(
        async () => (await getRendererTitleLog(mcodePage)).includes('Factory Droid needs input'),
        {
          timeout: 10_000,
