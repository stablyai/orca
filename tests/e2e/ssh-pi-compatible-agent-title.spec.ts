...
test.describe('Docker SSH Pi-compatible agent titles', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set MCODE_E2E_SSH_DOCKER=1 to run Docker-backed SSH relay tests.')
  ...
  test('classifies OMP and Pi title transitions from a remote terminal', async ({
    ...
    try {
      ...
      const terminalHandle = await findTerminalByPtyId(mcodePage, ptyId)

      const marker = `PI_COMPATIBLE_TITLE_READY_${Date.now()}`
      await sendToTerminal(mcodePage, ptyId, `printf '${marker}\\n'\r`)
      await waitForTerminalOutput(mcodePage, marker, 20_000, 60_000)

      await emitOscTitle(mcodePage, ptyId, '\u280b OMP')
      await expect
        .poll(async () => readTerminalAgentStatus(mcodePage, terminalHandle), {
          timeout: 10_000,
          message: 'Remote OMP working title did not classify as an agent status'
        })
        .toMatchObject({ isRunningAgent: true, status: 'working' })

      await emitOscTitle(mcodePage, ptyId, 'OMP ready')
      await expect
        .poll(async () => readTerminalAgentStatus(mcodePage, terminalHandle), {
          timeout: 10_000,
      ...
      await emitOscTitle(mcodePage, ptyId, '\u280b Pi')
      await expect
        .poll(async () => readTerminalAgentStatus(mcodePage, terminalHandle), {
          timeout: 10_000,
