/**
 * Shared Electron fixture for MCode E2E tests.
 *
...
type MCodeTestFixtures = {
  ...
  mcodeAppExtraEnv: Record<string, string>
  ...
  // developer's shell or already-running MCode instance.
  launchEnv: NodeJS.ProcessEnv
}

type MCodeWorkerFixtures = {
  /** Absolute path to the test git repo created by globalSetup. */
  testRepoPath: string
}

// Why: parse + warn at module scope so a bad MCODE_E2E_SLOWMO_MS value logs once
// per worker instead of once per test (otherwise hundreds of lines per CI run).
const MCODE_E2E_SLOWMO_MS_RAW = process.env.MCODE_E2E_SLOWMO_MS
const MCODE_E2E_SLOWMO_MS = ((): number => {
  if (MCODE_E2E_SLOWMO_MS_RAW === undefined) {
    return 0
  }
  const parsed = Number(MCODE_E2E_SLOWMO_MS_RAW)
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[mcode-e2e] MCODE_E2E_SLOWMO_MS="${MCODE_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
function shouldLaunchHeadful(testInfo: TestInfo): boolean {
  // Why: MCODE_E2E_FORCE_HEADFUL lets a developer watch any spec in a real
  // window without retagging it `@headful` or switching projects.
  if (process.env.MCODE_E2E_FORCE_HEADFUL === '1') {
    return true
  }
  return testInfo.project.metadata.mcodeHeadful === true
}
...
// same MCODE_E2E_FORWARD_APP_LOGS-gated stdout/stderr capture.
export function forwardElectronProcessLogs(app: ElectronApplication, testInfo: TestInfo): void {
  if (process.env.MCODE_E2E_FORWARD_APP_LOGS !== '1') {
    return
  }
...
/**
 * Extended Playwright test with MCode-specific fixtures.
 *
 * `mcodePage` — the main MCode renderer window.
 *
 ...
 */
export const test = base.extend<MCodeTestFixtures, MCodeWorkerFixtures>({
  ...
  electronApp: async (
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'mcode-e2e-userdata-'))
    ...
    // MCode's own agent runtime) set it so Electron behaves as a plain Node
    ...
    const homeIsolation = createElectronHomeIsolation({
      ...
      launchEnv,
      extraEnv: mcodeAppExtraEnv,
      userDataDir
    })
    // Why: MCODE_E2E_SLOWMO_MS adds a pause between every Playwright action so a
    // developer running with MCODE_E2E_FORCE_HEADFUL=1 can actually watch what
    // the test does. Defaults to 0 (no slowdown) for normal runs.
    const slowMo = MCODE_E2E_SLOWMO_MS
    // Why: MCODE_E2E_RECORD_VIDEO=1 captures a webm of the renderer so a
    ...
    const recordVideoDir = process.env.MCODE_E2E_RECORD_VIDEO === '1' ? testInfo.outputDir : null
    if (recordVideoDir) {
      mkdirSync(recordVideoDir, { recursive: true })
    }
    const app = await electron.launch({
      args: [...mcodeAppExtraArgs, ...getMCodeElectronLaunchArgs(mainPath, headful)],
      ...(slowMo > 0 ? { slowMo } : {}),
      ...(recordVideoDir ? { recordVideo: { dir: recordVideoDir } } : {}),
      // Why: keep NODE_ENV=development so window.__store is exposed and
      // dev-only helpers activate. MCODE_E2E_USER_DATA_DIR overrides the usual
      // shared dev profile so every spec gets a clean persistence root.
      // Why: MCODE_E2E_HEADLESS suppresses mainWindow.show() so the app
      // window stays hidden during test runs, avoiding focus stealing and
      // screen clutter. Playwright interacts via CDP regardless.
      // Why: MCODE_E2E_HEADLESS suppresses mainWindow.show() for CI/headless
      // runs. MCODE_E2E_HEADFUL overrides this for tests that need a visible
      ...
      env: {
        ...
        !cleanEnv.MCODE_RELAY_PATH
          ? { MCODE_RELAY_PATH: path.join(process.cwd(), 'out', 'relay') }
          : {}),
        ...(headful ? { MCODE_E2E_HEADFUL: '1' } : { MCODE_E2E_HEADLESS: '1' })
      }
    })
  ...
  mcodeAppExtraEnv: [{}, { option: true }],
  mcodeAppExtraArgs: [[], { option: true }],
  ...
  mcodePage: async ({ sharedPage }, provideFixture) => {
