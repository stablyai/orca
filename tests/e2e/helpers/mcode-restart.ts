 ...
 *
 * Why: the default `mcodePage` fixture creates a fresh `userDataDir` per test
...
type RestartSession = {
  ...
  launch: (options?: LaunchOptions) => Promise<LaunchedMCode>
...
export function createRestartSession(
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'mcode-e2e-restart-'))
  ...
  const launch = async (options?: LaunchOptions): Promise<LaunchedMCode> => {
    runtimeWsPort ??= await reserveRestartRuntimeWsPort()
    const app = await electron.launch({
      args: getMCodeElectronLaunchArgs(mainPath, headful),
      env: {
        ...homeIsolation.env,
        ...options?.extraEnv,
        MCODE_E2E_RUNTIME_WS_PORT: String(runtimeWsPort)
      }
    })
  ...
  const dispose = async (): Promise<void> => {
    ...
    if (process.env.MCODE_E2E_PRESERVE_RESTART_PROFILE === '1') {
      ...
      () =>
        readRestartRendererState(() =>
          page.evaluate(async (repoId) => {
            ...
            // worktree is external to MCode's workspace root after the visibility rollout.
