...
export { ensureMCodeRuntimeLaunched, parseJsonOutput, runMCodeCli, stopMCodeRuntime, type CliResult }

export async function ensureTextEditLaunched(): Promise<void> {
  await killTextEdit()
  textEditTempDir = await mkdtemp(join(tmpdir(), 'mcode-computer-e2e-'))
...
export async function ensureSafariDraftFixtureLaunched(): Promise<SafariDraftFixture> {
  ...
  safariDraftTempDir = await mkdtemp(join(tmpdir(), 'mcode-computer-safari-e2e-'))
  safariDraftTitle = `MCode Computer Use Draft Fixture ${Date.now()}`
  ...
  if (title) {
    try {
      const envelope = parseJsonOutput<{
        ...
        (await runMCodeCli(['computer', 'list-windows', '--app', 'com.apple.Safari', '--json']))
          .stdout
      )
...
async function waitForComputerWindowTitle(
  ...
  while (Date.now() < deadline) {
    try {
      ...
      }>((await runMCodeCli(['computer', 'list-windows', '--app', app, '--json'])).stdout)
...
export async function ensureGeditLaunched(): Promise<void> {
  await killGedit()
  linuxTempDir = await mkdtemp(join(tmpdir(), 'mcode-computer-linux-e2e-'))
...
export async function ensureNotepadLaunched(): Promise<void> {
  await killNotepad()
  windowsTempDir = await mkdtemp(join(tmpdir(), 'mcode-computer-windows-e2e-'))
  const filePath = join(windowsTempDir, `mcode-notepad-${Date.now()}.txt`)
  await writeFile(filePath, 'seed', 'utf8')
