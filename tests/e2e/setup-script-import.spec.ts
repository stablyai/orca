...
test.describe('Setup script import prompt', () => {
  ...
  test('imports Superset local overlays through the prompt UI', async ({ mcodePage }, testInfo) => {
    const repoPath = createSupersetSetupRepo(testInfo.outputPath('superset-setup-repo'))
    const repoId = await addAndActivateRepo(mcodePage, repoPath)
    ...
    await mcodePage.getByRole('button', { name: 'Save local setup' }).click()

    await expect(
      mcodePage.getByText('2 unsupported fields skipped. Saved the setup command.')
    ).toBeVisible()

    const localCommands = await openImportedSetupSettingsFromToast(mcodePage, repoId)
  ...
  test('imports cmux setup commands through the prompt UI', async ({ mcodePage }, testInfo) => {
    ...
    await expect(
      mcodePage.getByRole('button', { name: "project's settings", exact: true })
    ).toBeVisible()

    const repoSettings = await openRepoSettings(mcodePage, repoId)
