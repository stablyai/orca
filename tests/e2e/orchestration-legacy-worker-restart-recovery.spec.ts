...
import { FAKE_CODEX_RESTART_WORKER_SOURCE } from './helpers/fake-codex-restart-worker-source'
import { expectRestoredAgentInWorktreeInventory } from './helpers/restored-agent-worktree-inventory'
...
const fakeCodexSource = FAKE_CODEX_RESTART_WORKER_SOURCE
...
for (const contractVersion of [LEGACY_CONTRACT_VERSION, CURRENT_CONTRACT_VERSION]) {
  ...
  {}, testInfo) => {
    ...
    try {
      ...
      expect(readLedger(interruptionLedgerPath)).toEqual([])
      expect(isProcessAlive(initialSpawn.pid)).toBe(true)
      await expectRestoredAgentInWorktreeInventory({
        page: second.page,
        client: secondClient,
        ptyId: initialWorker.ptyId,
        paneKey: workerPaneKey,
        worktreeId,
        agentType: 'codex'
      })
