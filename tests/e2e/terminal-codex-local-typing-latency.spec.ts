...
test.describe('local Codex terminal typing latency', () => {
  test('keeps Codex prompt typing responsive @local-real-codex', async ({ mcodePage }, testInfo) => {
    test.skip(
    ...
    try {
      await sendToTerminal(mcodePage, ptyId, launchCommand)
      await dismissCodexPromptsIfPresent(mcodePage)
      const composerMarker = await waitForCodexComposer(mcodePage)
      ...
      await installCodexEchoLatencyProbe(mcodePage, typed)
      for (const char of typed) {
        ...
        await mcodePage.waitForTimeout(KEYSTROKE_INTERVAL_MS)
      }
      // Why: the last keystroke's echo can still be in flight when typing ends.
      await mcodePage.waitForTimeout(1_000)
      const report = await collectCodexEchoLatencyReport(mcodePage)
