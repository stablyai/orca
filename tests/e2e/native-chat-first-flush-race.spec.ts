...
test.describe('Native chat first-flush transcript race (#8401)', () => {
  test('stays in loading (never errors) until a not-yet-flushed transcript appears, then hydrates live', async ({
    ...
    const scratchDir = mkdtempSync(path.join(os.tmpdir(), 'mcode-e2e-native-chat-'))
    ...
    try {
      ...
      await seedClaudeProviderSession(mcodePage, {
      ...
      await expect(mcodePage.getByText(LOADING_TITLE)).toBeVisible({ timeout: 10_000 })
      await expect(mcodePage.getByText(ERROR_TITLE)).toHaveCount(0)
      ...
      await expect(mcodePage.getByText(assistantText)).toBeVisible({ timeout: 30_000 })
      await expect(mcodePage.getByText(ERROR_TITLE)).toHaveCount(0)
      await mcodePage.screenshot({
