...
function emitState(reason) {
  ...
  process.stdout.write('\\x1b]1337;MCodeImeState=' + Buffer.from(JSON.stringify({
    reason,
    model,
    cursor,
    submitted
...
async function installImeEventProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    const targetWindow = window as unknown as { __mcodeImeEventLog?: ImeEventLogEntry[] }
    targetWindow.__mcodeImeEventLog = []
    ...
    const record = (event: Event): void => {
      ...
      targetWindow.__mcodeImeEventLog!.push({
        type: event.type,
        at: performance.now(),
...
async function readImeEventLog(page: Page): Promise<ImeEventLogEntry[]> {
  return page.evaluate(() => {
    const targetWindow = window as unknown as { __mcodeImeEventLog?: ImeEventLogEntry[] }
    return targetWindow.__mcodeImeEventLog ?? []
  })
}
...
async function dispatchSogouEmptyCompositionUpdate(page: Page): Promise<void> {
  ...
  // popup is still open (#6765); MCode's tracker must not flip inactive on it.
  await page.evaluate(() => {
    const active = document.activeElement
    if (!(active instanceof HTMLTextAreaElement)) {
      throw new Error('xterm helper textarea is not focused')
...
async function dispatchSogouPostCompositionEnd(page: Page, data: string): Promise<void> {
  ...
  // compositionend; target the terminal element so MCode's tracker sees the end
...
test.describe('Chinese IME terminal chat input repro', () => {
  test('keeps composed Chinese text, cursor movement, and Backspace stable in the agent input surface', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.mcode-chinese-ime-harness-${runId}.cjs`)
    ...
    try {
      ...
      await installImeEventProbe(mcodePage)

      await dispatchImeProcessKey(session, 'KeyN')
      await composeAndCommitChineseText(session, mcodePage, ['n', 'ni', '你', '你好'], '你好')
      await waitForLivePrompt(mcodePage, '你好')
      await attachImeEvidence(mcodePage, testInfo, 'after-compose-hello')
      await mcodePage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(mcodePage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await waitForLivePrompt(mcodePage, '一二三四五六七八九十')
      for (let index = 0; index < 5; index += 1) {
        await mcodePage.keyboard.press('ArrowLeft')
      }
      await dispatchImeProcessKey(session, 'KeyZ')
      await composeAndCommitChineseText(session, mcodePage, ['z', 'zh', '中'], '中')
      await waitForLivePrompt(mcodePage, '一二三四五中六七八九十')
      await attachImeEvidence(mcodePage, testInfo, 'after-middle-insert')
      ...
      await mcodePage.keyboard.press('Backspace')
      await waitForLivePrompt(mcodePage, '一二三四五六七八九十')
      await attachImeEvidence(mcodePage, testInfo, 'after-single-backspace')
      ...
      const log = await readImeEventLog(mcodePage)
      expect(
      ...
      await attachImeEvidence(mcodePage, testInfo, 'final-ime-evidence').catch(() => undefined)
  ...
  test('commits the active Pinyin preedit when Shift toggles the Windows IME', async ({
    ...
    await reloadWithWindowsImePolicy(mcodePage)
    await waitForSessionReady(mcodePage)
    await waitForActiveWorktree(mcodePage)
    await ensureTerminalVisible(mcodePage)
    await waitForActiveTerminalManager(mcodePage, 30_000)
    ...
    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.mcode-windows-shift-ime-${runId}.cjs`)
    const inputLogPath = path.join(testRepoPath, `.mcode-windows-shift-ime-${runId}.jsonl`)
    ...
    try {
      ...
      await waitForLivePrompt(mcodePage, 's')
      await expect.poll(() => readActiveCompositionText(mcodePage)).toBe('')
      await expect.poll(async () => (await readPromptState(mcodePage))?.submitted).toEqual([])
      await expect
        .poll(() => readPtyInputs(inputLogPath).slice(inputCountBeforeShift))
        .toEqual(['s'])
    } finally {
      await attachImeEvidence(mcodePage, testInfo, 'windows-shift-ime-evidence').catch(
        () => undefined
      )
      await session.detach().catch(() => undefined)
      await sendToTerminal(mcodePage, ptyId, '\x03').catch(() => undefined)
  ...
  test('keeps Sogou-style candidate selection keys out of the PTY while committing Chinese text', async ({
    ...
    await reloadWithLinuxImePolicy(mcodePage)
    await waitForSessionReady(mcodePage)
    await waitForActiveWorktree(mcodePage)
    await ensureTerminalVisible(mcodePage)
    await waitForActiveTerminalManager(mcodePage, 30_000)
    ...
    const ptyId = await waitForActivePanePtyId(mcodePage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.mcode-sogou-ime-harness-${runId}.cjs`)
    ...
    try {
      ...
      writeFileSync(scriptPath, terminalImeHarnessScript(runId))
      session = await mcodePage.context().newCDPSession(mcodePage)
      await sendToTerminal(mcodePage, ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      harnessStarted = true
      await waitForTerminalOutput(mcodePage, `IME_HARNESS_READY_${runId}`, 10_000, 20_000)
      await focusActiveTerminalInput(mcodePage)
      ...
      await mcodePage.waitForTimeout(80)
      await dispatchSogouEmptyCompositionUpdate(mcodePage)
      await dispatchCandidateSelectionKey(session, { key: ' ', code: 'Space', keyCode: 32 }, () =>
        commitImeText(session, '你')
      )
      await waitForLivePrompt(mcodePage, '你')
      await attachImeEvidence(mcodePage, testInfo, 'sogou-after-space-commit')
      await mcodePage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(mcodePage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await waitForLivePrompt(mcodePage, '你好')
      await attachImeEvidence(mcodePage, testInfo, 'sogou-after-digit-commit')
      await mcodePage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(mcodePage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      const postCompositionLogStart = (await readImeEventLog(mcodePage)).length
      await setImeComposition(session, 'zaijian')
      await mcodePage.waitForTimeout(80)
      await dispatchSogouEmptyCompositionUpdate(mcodePage)
      await dispatchSogouPostCompositionEnd(mcodePage, '再见')
      await dispatchCandidateSelectionKey(session, { key: '3', code: 'Digit3', keyCode: 51 }, () =>
        commitImeText(session, '再见')
      )
      await waitForLivePrompt(mcodePage, '再见')
      const postCompositionLog = await readImeEventLog(mcodePage)
      ...
      await attachImeEvidence(mcodePage, testInfo, 'sogou-after-post-composition-digit-commit')
      await mcodePage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(mcodePage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await expect
        ...
        .toBe('')
      await attachImeEvidence(mcodePage, testInfo, 'sogou-after-orphan-keyup-digit-suppression')

      const promptState = await readPromptState(mcodePage)
      expect(
        promptState?.submitted,
        'candidate Space/digit selectors and pinyin preedit must not leak into the PTY'
      ).toEqual(['你', '你好', '再见', ''])
    } finally {
      await attachImeEvidence(mcodePage, testInfo, 'sogou-final-ime-evidence').catch(() => undefined)
      await session?.detach().catch(() => undefined)
      if (harnessStarted) {
        await sendToTerminal(mcodePage, ptyId, '\x03').catch(() => undefined)
      }
  ...
  test('keeps composed Chinese text stable in the real Codex TUI input @real-codex-ime', async ({
    ...
    test.skip(
      process.env.MCODE_E2E_REAL_CODEX_IME !== '1',
      'Set MCODE_E2E_REAL_CODEX_IME=1 to exercise the locally installed Codex TUI'
    )
    ...
    try {
      await launchCodexTui(mcodePage, ptyId)
      await installImeEventProbe(mcodePage)

      await dispatchImeProcessKey(session, 'KeyN')
      await composeAndCommitChineseText(session, mcodePage, ['n', 'ni', '你', '你好'], '你好')
      await waitForCleanTerminalText(mcodePage, /你好/, 'Codex input did not show composed Chinese')
      await attachImeEvidence(mcodePage, testInfo, 'codex-after-compose-hello', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(mcodePage, 20_000))
      })
      ...
      await attachImeEvidence(mcodePage, testInfo, 'codex-after-composition-backspace', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(mcodePage, 20_000))
      })

      const cleanTerminal = stripTerminalControls(await getTerminalContent(mcodePage, 20_000))
      expect(
        cleanTerminal,
      ...
      await attachImeEvidence(mcodePage, testInfo, 'codex-final-ime-evidence', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(mcodePage, 20_000))
      }).catch(() => undefined)
      await session.detach().catch(() => undefined)
      await sendToTerminal(mcodePage, ptyId, '\x03/quit\r').catch(() => undefined)
    }
  })
})
