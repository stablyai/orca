...
const PARKING_DELAY_MS = Number(process.env.MCODE_E2E_TERMINAL_PARKING_DELAY_MS) || 2_000
...
test.describe('terminal pane close vs hidden/park lifecycle keeps layout consistent', () => {
  test('control: close while visible', async ({ mcodePage }) => {
    const { tabId } = await setUpSplitTab(mcodePage)
    await closeLastPaneOnTab(mcodePage, tabId)
    await expectLayoutConsistent(mcodePage, tabId, 1, 'close-visible')
  })

  test('close and hide the tab in the same tick', async ({ mcodePage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(mcodePage)
    ...
    await waitForTabRemounted(mcodePage, tabId)
    await expectLayoutConsistent(mcodePage, tabId, 1, 'close-then-hide-same-tick')
  })

  test('close while hidden but still mounted (hot-retain window)', async ({ mcodePage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(mcodePage)
    ...
    await activateTerminalTab(mcodePage, tabId)
    await waitForTabRemounted(mcodePage, tabId)
    await expectLayoutConsistent(mcodePage, tabId, 1, 'close-while-hidden-mounted')
  })

  test('close immediately after reveal remount, before panes settle', async ({ mcodePage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(mcodePage)
    await createActiveTerminalTab(mcodePage, worktreeId)
    await parkHiddenTabBehindDecoy(mcodePage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    ...
    await expectLayoutConsistent(mcodePage, tabId, 1, 'close-mid-reveal')
  })

  test('clean visible close survives a later park/reveal cycle', async ({ mcodePage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(mcodePage)
    await closeLastPaneOnTab(mcodePage, tabId)
    await expectLayoutConsistent(mcodePage, tabId, 1, 'pre-park close')
    await createActiveTerminalTab(mcodePage, worktreeId)
    await parkHiddenTabBehindDecoy(mcodePage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    })
    await activateTerminalTab(mcodePage, tabId)
    await waitForTabRemounted(mcodePage, tabId)
    await expectLayoutConsistent(mcodePage, tabId, 1, 'post-park-reveal')
  })

  test('split pane shell exits while hidden but still mounted', async ({ mcodePage }) => {
    const { worktreeId, tabId, splitPtyId } = await setUpSplitTab(mcodePage)
    await createActiveTerminalTab(mcodePage, worktreeId)
    // The setup-script analog: the split's shell ends on its own while the
    // tab is hidden-but-mounted.
    await sendToTerminal(mcodePage, splitPtyId, 'exit\r')
    await mcodePage.waitForTimeout(PARKING_DELAY_MS / 2)
    await activateTerminalTab(mcodePage, tabId)
    await waitForTabRemounted(mcodePage, tabId)
    await expectLayoutConsistent(mcodePage, tabId, 1, 'shell-exit-while-hidden-mounted', splitPtyId)
  })

  test('split pane shell exits while the tab is parked', async ({ mcodePage }) => {
    const { worktreeId, tabId, splitPtyId } = await setUpSplitTab(mcodePage)
    await createActiveTerminalTab(mcodePage, worktreeId)
    await parkHiddenTabBehindDecoy(mcodePage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    ...
    await sendToTerminal(mcodePage, splitPtyId, 'exit\r')
    await mcodePage.waitForTimeout(PARKING_DELAY_MS)
    await activateTerminalTab(mcodePage, tabId)
    await waitForTabRemounted(mcodePage, tabId)
    ...
    await expectLayoutConsistent(mcodePage, tabId, 1, 'shell-exit-while-parked', splitPtyId)
  })
})
