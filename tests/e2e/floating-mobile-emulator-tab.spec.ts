...
test('floating Mobile Emulator tab renders content and closes from the tab strip', async ({
  ...
  const tab = await seedFloatingSimulatorTab(mcodePage)
  await openFloatingPanelIfNeeded(mcodePage)

  const openPanel = mcodePage.locator(OPEN_PANEL_SELECTOR).first()
  ...
  await attachPanelScreenshot(mcodePage, testInfo)
