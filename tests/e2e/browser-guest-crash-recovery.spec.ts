...
async function createBrowserFixture(
  ...
  const fixtureDir = mkdtempSync(path.join(os.tmpdir(), 'mcode-browser-recovery-'))
...
test('browser chrome recovers a live registered file guest after renderer loss', async ({
  ...
  await expect
    .poll(() => readBrowserGuestState(mcodePage, browserTab.id), { timeout: 10_000 })
    .toMatchObject({
      chromePresent: true,
      marker: 'painted-file-guest',
      url: fixtureUrl
    })
  const before = await readBrowserGuestState(mcodePage, browserTab.id)
  expect(before.webContentsId).not.toBeNull()
  const beforeProcessId = await readGuestProcessId(electronApp, before.webContentsId!)
  await expect
    .poll(
      async () =>
        (await listRegisteredBrowserPages(mcodePage, worktreeId)).result?.tabs?.find(
  ...
  const recovered = await readBrowserGuestState(mcodePage, browserTab.id)
  ...
  await expect
    .poll(() => listRegisteredBrowserPages(mcodePage, worktreeId), { timeout: 10_000 })
    .toMatchObject({
      ok: true,
  ...
  await mcodePage.evaluate((browserPageId) => {
    return window.api.browser.unregisterGuest({ browserPageId })
  }, browserTab.activePageId)
  await expect
    .poll(
      async () =>
        (await listRegisteredBrowserPages(mcodePage, worktreeId)).result?.tabs?.some(
          (tab) => tab.browserPageId === browserTab.activePageId
        ) ?? false
    )
    .toBe(false)
  ...
  const resumeRecovered = await readBrowserGuestState(mcodePage, browserTab.id)
  ...
  const backgroundTab = await mcodePage.evaluate(
    ({ targetWorktreeId }) =>
  ...
  await expect
    .poll(() => readBrowserGuestState(mcodePage, backgroundTab!.id), { timeout: 10_000 })
  ...
  await expect
    .poll(() => readBrowserGuestState(mcodePage, browserTab.id), { timeout: 10_000 })
    .toMatchObject({ chromePresent: true, marker: 'painted-file-guest', url: fixtureUrl })
  const rendererReloaded = await readBrowserGuestState(mcodePage, browserTab.id)
  expect(rendererReloaded.webContentsId).not.toBe(beforeRendererReloadId)

  await mcodePage.evaluate((targetBrowserTabId) => {
    window.__store?.getState().setActiveBrowserTab(targetBrowserTabId)
  }, backgroundTab!.id)
  const hiddenBefore = await readBrowserGuestState(mcodePage, browserTab.id)
...
test('dom-ready ID loss waits for validation without reloading the guest', async ({
  ...
  await expect
    .poll(() => readBrowserGuestState(mcodePage, browserTab.id))
  ...
  await expect
    .poll(() => listRegisteredBrowserPages(mcodePage, worktreeId))
    .toMatchObject({
      ok: true,
      result: { tabs: [{ browserPageId: browserTab.activePageId, url: fixtureUrl }] }
    })
...
test('explicit navigation repairs a recovery error without dom-ready churn', async ({
  ...
  await expect
    .poll(() => readBrowserPageRecoveryState(mcodePage, browserTab.id, browserTab.activePageId))
  ...
  const addressBar = mcodePage.locator(
    `[data-browser-overlay-tab-id="${browserTab.id}"] [data-mcode-browser-address-bar="true"]`
  )
  ...
  await expect(mcodePage.getByText('Recovery fixture error', { exact: true })).toBeVisible()
  expect(
    await electronApp.evaluate(() => {
      const testState = globalThis as typeof globalThis & {
        browserRecoveryValidationCalls?: number
...
test('attachment keeps recovery error until document readiness', async ({
  ...
  await mcodePage
    ...
    .nth(2)
    .click()
  const recoveryErrorCode = await mcodePage.evaluate(
    ({ workspaceId, browserPageId }) =>
...
test('minimized browser guest stays painted and registered after restore @headful', async ({
  ...
  await occludeHeadedWindow(electronApp, mcodePage)
  await revealHeadedWindow(electronApp, mcodePage)

  await expect
    .poll(() => readBrowserGuestState(mcodePage, browserTab.id))
    .toMatchObject({ chromePresent: true, marker: 'painted-file-guest', url: fixtureUrl })
  const restored = await readBrowserGuestState(mcodePage, browserTab.id)
