...
async function createProjectHeaderSortFixture(): Promise<string[]> {
  ...
  const root = realpathSync(await mkdtemp(path.join(os.tmpdir(), 'mcode-e2e-project-sort-')))
  tempRoots.push(root)
...
test.describe('Project Group manual sorting', () => {
  test('dragging a project header body reorders the visible project headers', async ({
    ...
    const projects = await seedProjectHeaderSortScenario(mcodePage, repoPaths)

    await expect
      .poll(() => getProjectHeaderOrder(mcodePage, projects), {
        timeout: 12_000,
  ...
  test('dragging a duplicate-ranked Project Group header reorders the visible headers', async ({
    ...
    await waitForSessionReady(mcodePage)
    const groups = await seedDuplicateTabOrderProjectGroups(mcodePage)

    await expect
      .poll(() => getProjectGroupHeaderOrder(mcodePage, groups), {
        timeout: 12_000,
