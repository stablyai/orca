#!/usr/bin/env bash
# Capture one-shot macOS launch diagnostics for a published MCode release.
#
# Usage:
#   MCODE_DIAGNOSTIC_TAG=v1.4.42-rc.1 bash config/scripts/macos-launch-diagnostics.sh
#   bash config/scripts/macos-launch-diagnostics.sh --tag v1.4.42-rc.1
set -euo pipefail

REPO="${MCODE_DIAGNOSTIC_REPO:-mcode-ide/mcode}"
TAG="${MCODE_DIAGNOSTIC_TAG:-}"
KEEP=0
...
if [[ -z "$TAG" ]]; then
  echo "Set MCODE_DIAGNOSTIC_TAG or pass --tag vX.Y.Z-rc.N." >&2
  exit 2
fi

ARCH="$(uname -m)"
case "$ARCH" in
  arm64) ASSET="mcode-macos-arm64.dmg" ;;
  x86_64) ASSET="mcode-macos-x64.dmg" ;;
  *)
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mcode-launch-diagnostics.XXXXXXXX")"
OUT_DIR="$WORK_DIR/output"
MOUNT_DIR="$WORK_DIR/mount"
APP_DIR="$WORK_DIR/MCode.app"
DMG_PATH="$WORK_DIR/$ASSET"
mkdir -p "$OUT_DIR" "$MOUNT_DIR"

if [[ -d "$HOME/Desktop" ]]; then
  ZIP_PATH="$HOME/Desktop/mcode-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
else
  ZIP_PATH="$PWD/mcode-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
fi

diag_log() {
  printf '[mcode-diagnostics] %s\n' "$*"
}
...
write_environment_report() {
  {
    ...
    echo "$SHELL"
    echo
    echo "## current MCode processes before diagnostics"
    pgrep -fl 'MCode|mcode' || true
  } >"$OUT_DIR/environment.txt"
}

ensure_no_existing_mcode() {
  local existing
  existing="$(pgrep -x MCode || true)"
  if [[ -z "$existing" ]]; then
    return 0
  fi

  {
    echo "An MCode process is already running. Close MCode and run this script again."
    echo
    ps -p "$(printf '%s' "$existing" | paste -sd, -)" -o pid=,ppid=,command= || true
  } | tee "$OUT_DIR/existing-mcode-process.txt" >&2
  exit 2
}
...
start_log_stream() {
  local file="$1"
  local predicate='process == "MCode" OR eventMessage CONTAINS[c] "MCode" OR eventMessage CONTAINS[c] "com.mcode.desktop"'
...
latest_mcode_pid_for_app() {
  pgrep -nf "$APP_DIR/Contents/MacOS/MCode" || true
}
...
wait_for_probe() {
  ...
  for i in $(seq 1 200); do
    local app_pid
    app_pid="$(latest_mcode_pid_for_app)"
...
run_launchservices_probe() {
  ...
  if [[ -n "$extra_env_name" ]]; then
    open -n -W \
      ...
      --env MCODE_STARTUP_DIAGNOSTICS=trace \
      --env MCODE_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
      --env MCODE_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
...
run_direct_exec_probe() {
  ...
  ELECTRON_ENABLE_LOGGING=1 \
    MCODE_STARTUP_DIAGNOSTICS=trace \
    MCODE_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
    MCODE_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
    "$APP_DIR/Contents/MacOS/MCode" >"$stdout_file" 2>"$stderr_file" &
  local runner_pid="$!"
  wait_for_probe "$runner_pid" "$label"
  echo "ended_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')" >>"$OUT_DIR/$label.meta"
  stop_log_stream "$stream_pid"
...
run_launchservices_probe "launchservices-bypass-lock" "MCODE_BYPASS_SINGLE_INSTANCE_LOCK" "1"
run_direct_exec_probe
write_system_log_snapshot
package_results
