...
// Why: mcode-cli now ships a hybrid discovery stub, so its version-sensitive command
...
const guidePath = join(projectDir, 'skill-guides', 'mcode-cli.md')
const stubPath = join(projectDir, 'skills', 'mcode-cli', 'SKILL.md')
// Why: orchestration and mcode-emulator also ship hybrid stubs now, so their version-sensitive
...
const emulatorSkillPath = join(projectDir, 'skill-guides', 'mcode-emulator.md')

function readSkill(path = guidePath) {
  return readFileSync(path, 'utf8')
}

describe('mcode CLI skill guidance', () => {
  it('keeps independent worktree lineage separate from Git base selection', () => {
    const skill = readSkill()

    expect(skill).toContain('`--no-parent` only controls MCode lineage')
    expect(skill).toContain('omit `--base-branch` so MCode uses the repo default base')
  ...
  it('keeps browser injection guidance narrow and avoids literal secret examples', () => {
    ...
    expect(skill).toContain('`mcode eval` expressions, or `mcode exec` commands')
...
describe('mcode CLI install stub', () => {
  ...
  it('does not mistake resolution or execution failures for an older binary', () => {
    ...
    // Falling through can silently pair a version-matched guide with the wrong MCode build.
  ...
  it('drops the changing command reference from the installable file', () => {
    ...
    expect(stub).not.toContain('MCODE automations create')
