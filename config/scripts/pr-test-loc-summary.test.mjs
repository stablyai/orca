...
describe('PR test LoC summary', () => {
  ...
  it('reads the next page from a GitHub Link header', () => {
    expect(
      nextLink(
        '<https://api.github.com/repos/mcode-ide/mcode/pulls/1/files?page=2>; rel="next", <https://api.github.com/repos/mcode-ide/mcode/pulls/1/files?page=3>; rel="last"'
      )
    ).toBe('https://api.github.com/repos/mcode-ide/mcode/pulls/1/files?page=2')
    expect(
      nextLink('<https://api.github.com/repos/mcode-ide/mcode/pulls/1/files?page=1>; rel="prev"')
    ).toBe(undefined)
  })

  it('paginates pull files until Link rel=next is gone', async () => {
    const pages = {
      'https://api.github.com/repos/mcode-ide/mcode/pulls/9/files?per_page=100': {
        body: [{ filename: 'src/app.ts', additions: 2, deletions: 0 }],
        link: '<https://api.github.com/repos/mcode-ide/mcode/pulls/9/files?page=2>; rel="next"'
      },
      'https://api.github.com/repos/mcode-ide/mcode/pulls/9/files?page=2': {
  ...
  it('counts and merges from a files JSON fixture', () => {
    const root = mkdtempSync(join(tmpdir(), 'mcode-pr-test-loc-'))
    tempDirs.push(root)
    ...
    expect(result.stdout.startsWith('<!-- mcode-pr-loc -->')).toBe(true)
    ...
    expect(result.stdout.match(/<!-- mcode-pr-loc -->/g)).toHaveLength(1)
  })
