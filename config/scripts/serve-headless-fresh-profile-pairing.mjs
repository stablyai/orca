...
const mcodeDevScript = path.join(scriptDir, 'mcode-dev.mjs')
const ensureNativeRuntimeScript = path.join(scriptDir, 'ensure-native-runtime.mjs')
const fixedProfileDir = process.env.MCODE_HEADLESS_PAIRING_PROFILE_DIR
...
const profileDir =
  fixedProfileDir ?? mkdtempSync(path.join(tmpdir(), 'mcode-headless-pairing-profile-'))
...
delete childEnv.MCODE_CODEX_HOME
Object.assign(childEnv, {
  // Why: a fresh temporary MCode profile must not make the default Codex lane
  // read or mutate the developer profile during a pairing smoke test.
  MCODE_DEV_USER_DATA_PATH: profileDir,
  HOME: isolatedHome,
...
console.error(`[headless-pairing] starting: mcode-dev serve --json${formatForwardedArgs(serveArgs)}`)

child = spawn(process.execPath, [mcodeDevScript, 'serve', '--json', ...serveArgs], {
  cwd: repoRoot,
...
/**
 * Parses wrapper flags and forwards everything else to `mcode serve`.
 */
 ...
 */
function printHelp() {
  console.log(`Usage: node config/scripts/serve-headless-fresh-profile-pairing.mjs [--keep] [mcode serve flags]

Starts mcode-dev serve --json with a fresh isolated userData profile, ensures Electron's dev runtime is usable, and prints the pairing URL.
...
Environment:
  MCODE_HEADLESS_PAIRING_ADDRESS=<host|host:port|ws://...>  Override the auto pairing address.
  MCODE_HEADLESS_PAIRING_PROFILE_DIR=/path/to/profile       Use a fixed profile directory.
`)
}
...
function resolveDefaultPairingAddress() {
  const configured = process.env.MCODE_HEADLESS_PAIRING_ADDRESS?.trim()
  if (configured) {
    return configured
  }
...
function printReadyLine(line) {
  ...
  if (!payload || payload.type !== 'mcode_server_ready') {
    return false
  }
  console.log(`MCode server ready: ${payload.boundEndpoint ?? 'websocket unavailable'}`)
...
function stopChild(signal) {
  ...
  if (process.platform === 'win32' && child.pid) {
    // Why: child.kill() only targets mcode-dev on Windows; taskkill walks the
  ...
  if (process.platform !== 'win32' && child.pid) {
    // Why: mcode-dev synchronously owns the CLI child, which owns Electron; kill
...
function cleanupProfile() {
  ...
  if (!existsSync(profileDir) || !profileDir.includes('mcode-headless-pairing-profile-')) {
