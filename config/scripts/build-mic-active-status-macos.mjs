...
// sign) derives the correct code identifier automatically, matching the
// pattern used by build-notification-status-macos.mjs.
...
const sourcePath = path.join(repoRoot, 'native', 'mic-active-status-macos', 'main.swift')
const defaultOutputPath = path.join(
  repoRoot,
  'native',
  'mic-active-status-macos',
  '.build',
  'release',
...
const workDir = path.join(tmpdir(), `orca-mic-active-status-${process.pid}`)
...
try {
  ...
  const builtBinaries = triples.map((triple) => {
    const output = path.join(workDir, `orca-mic-active-status-${triple}`)
    execFileSync(
      'swiftc',
      [
        '-O',
...
<dict>
  ...
  <string>orca-mic-active-status</string>
</dict>
</plist>
`
}
