...
function parseFrontmatter(markdown, sourcePath) {
  ...
  const packageRequirement = values.metadata?.graphify_package
  if (packageRequirement !== undefined && typeof packageRequirement !== 'string') {
    throw new Error(`Guide package requirement must be a string: ${sourcePath}`)
  }
  return {
    name: values.name,
  ...
  const guideEntries = guides
    .map((guide) => {
      ...
      return [
        ...
        ...(guide.packageRequirement
          ? [`    packageRequirement: ${JSON.stringify(guide.packageRequirement)},`]
          : []),
...
async function buildArtifacts(repoRoot = REPO_ROOT) {
  ...
  for (const name of expectedNames) {
    ...
    guides.push({
      ...
      ...(frontmatter.packageRequirement
        ? { packageRequirement: frontmatter.packageRequirement }
        : {}),
      markdown,
      aliases
    })
