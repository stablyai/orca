...
if (process.platform !== 'win32') {
  throw new Error(
    'Windows Programs known-folder bridge compilation requires a Windows host; refusing to package without it.'
  )
}

const repoRoot = resolve(import.meta.dirname, '../..')
const sourcePath = join(repoRoot, 'native', 'windows-programs-path', 'WindowsProgramsPath.cs')
const outputPath = readArg('--output') ?? defaultOutputPath(repoRoot)
const compilerPath = findFrameworkCompiler(process.env)

if (!compilerPath) {
  throw new Error(
    'Unable to find the .NET Framework C# compiler required for orca-programs-path.exe.'
  )
}
...
function defaultOutputPath(projectRoot) {
  return join(projectRoot, 'native', 'windows-programs-path', '.build', 'orca-programs-path.exe')
}
