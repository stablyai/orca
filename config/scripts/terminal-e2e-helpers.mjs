#!/usr/bin/env node
/**
 * Terminal E2E helpers for agent-browser + CDP testing against a running MCode
 ...
 *
 * Usage:
 *   import { MCodeTerminal } from './terminal-e2e-helpers.mjs'
 *
 *   const term = new MCodeTerminal(9444)       // CDP port
 *   await term.connect()
...
export class MCodeTerminal {
  /** @param {number} cdpPort — the --remote-debugging-port used when launching MCode */
  constructor(cdpPort) {
    this.port = cdpPort
  }
   ...
   */
  probePtyIdWithScreenshot(maxId = 10, screenshotPath = tempScreenshotPath('mcode-pty-probe.png')) {
    for (let i = 1; i <= maxId; i++) {
  ...
  /**
   * Take a screenshot of the MCode window.
   *
   * @param {string} path — output file path
   * @returns {string} the screenshot path
   */
  screenshot(path = tempScreenshotPath('mcode-terminal.png')) {
    ab(this.port, ['screenshot', path])
    return path
  }

  /**
   * Open a new terminal tab in MCode.
   * @returns {void}
   */
  newTerminal() {
  ...
  readLang(ptyId) {
    ...
    return this.screenshot(tempScreenshotPath('mcode-lang-check.png'))
  }
}
...
if (process.argv[1]?.endsWith('terminal-e2e-helpers.mjs')) {
  ...
  const term = new MCodeTerminal(port)
  console.log('Connecting to MCode on CDP port', port, '...')
  term.connect()
