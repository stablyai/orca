...
const binaryPath = path.join(packagePath, '.build', 'release', 'mcode-computer-use-macos')
const appPath = path.join(packagePath, '.build', 'release', 'MCode Computer Use.app')
const appExecutablePath = path.join(appPath, 'Contents', 'MacOS', 'mcode-computer-use-macos')
...
const bundleId = process.env.MCODE_COMPUTER_MACOS_BUNDLE_ID ?? 'com.mcode.desktop.computer-use'
const displayName = 'MCode Computer Use'
...
function buildUniversalBinary() {
  const builtBinaries = universalTriples.map((triple) => {
    ...
    return path.join(packagePath, '.build', triple, 'release', 'mcode-computer-use-macos')
  })
...
function codesignArgs(identity, targetPath) {
  ...
  if (process.env.MCODE_MAC_RELEASE === '1' && identity !== '-') {
...
function resolveSigningIdentity() {
  const explicitIdentity = process.env.MCODE_COMPUTER_MACOS_SIGN_IDENTITY ?? process.env.CSC_NAME
  if (explicitIdentity) {
  ...
  if (process.env.MCODE_MAC_RELEASE !== '1' && developmentMatch) {
...
<dict>
  ...
  <string>mcode-computer-use-macos</string>
  ...
  <string>MCode Computer Use needs Accessibility permission to read and interact with app interfaces when you ask MCode to use apps.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>MCode Computer Use needs Screen Recording permission to capture app windows when you ask MCode to inspect your screen.</string>
</dict>
</plist>
`
}
