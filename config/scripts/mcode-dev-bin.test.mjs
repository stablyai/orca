...
const wrapperPath = path.join(projectDir, 'config', 'scripts', 'mcode-dev.mjs')

describe('mcode-dev package bin', () => {
  it('uses a Node entrypoint for cross-platform package installs', () => {
    expect(packageJson.bin['mcode-dev']).toBe('./config/scripts/mcode-dev.mjs')
    expect(readFileSync(wrapperPath, 'utf8')).toMatch(/^#!\/usr\/bin\/env node\n/)
  })

  it('runs the dev CLI through Node without requiring Bash', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'mcode-dev-bin-'))
    ...
    writeFileSync(
      ...
      [
        ...
        '  devCliInvocation: process.env.MCODE_DEV_CLI_INVOCATION,',
        '  appExecutable: process.env.MCODE_APP_EXECUTABLE',
        '}));'
      ].join('\n'),
      'utf8'
    )
    ...
    execFileSync(process.execPath, [wrapperPath, '--help'], {
      env: {
        ...process.env,
        MCODE_DEV_CLI_ENTRY_PATH: cliEntry,
        MCODE_DEV_USER_DATA_PATH: path.join(root, 'user-data'),
        MCODE_APP_EXECUTABLE: path.join(root, 'Electron')
      },
      stdio: 'ignore'
    })
