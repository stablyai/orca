...
[[ $(jq -r '.attemptId' <<<"$downgrade_verdict") == "e2e-attempt-2" ]] || fail "rejected verdict missing attempt binding: $downgrade_verdict"
[[ $(jq -r '.targetVersion' <<<"$downgrade_verdict") == "0.0.0-test-old" ]] || fail "rejected verdict missing targetVersion: $downgrade_verdict"
