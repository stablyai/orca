...
// Why: macOS 26 Clear/Tinted appearances need the compiled Icon Composer catalog plus CFBundleIconName;
// icon.icns stays for older macOS. The catalog is generated by `pnpm build:icons` (Xcode 26), so a checkout
// without it must still package with the legacy icon rather than fail.
const macAppearanceIconCatalog = existsSync(
  resolve(__dirname, '..', 'resources', 'build', 'Assets.car')
)
  ? { from: 'resources/build/Assets.car', to: 'Assets.car' }
  : null
...
module.exports = {
  ...
  afterPack: async (context) => {
    ...
    if (context.electronPlatformName === 'darwin') {
      ...
      // Why: release builds fail closed on a missing packed catalog, while local builds remain usable with the legacy icon.
      const packagedMacAppearanceIconCatalog = join(resourcesDir, 'Assets.car')
      if (!existsSync(packagedMacAppearanceIconCatalog)) {
        if (isMacRelease) {
          throw new Error(
            `[mac-icon-appearance] Packaged Assets.car is missing at ${packagedMacAppearanceIconCatalog}; run pnpm build:icons on Xcode 26 and commit resources/build/Assets.car.`
          )
        }
        console.warn(
          '[mac-icon-appearance] Assets.car is missing; run pnpm build:icons with Xcode 26. This build ships the legacy icon only.'
        )
      }
    }
  ...
  mac: {
    ...
    extendInfo: {
      ...(macAppearanceIconCatalog ? { CFBundleIconName: 'icon' } : {}),
    ...
    extraResources: [
      ...
      ...(macAppearanceIconCatalog ? [macAppearanceIconCatalog] : []),
      {
