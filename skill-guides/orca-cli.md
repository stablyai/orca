...
ORCA automations create --name "Daily review" --trigger daily --time 09:00 --prompt "Review open changes" --provider codex --model gpt-5.6-sol --effort high --repo id:<repoId> --json
...
`--model <id>` and `--effort <level>` are saved on the automation and applied to every fresh run. Use `ORCA automations edit <automationId> --default-model --json` to restore the agent defaults. `--effort` requires `--model`.
