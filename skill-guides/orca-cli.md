...
- `terminal bridge --json` is a local-only NDJSON stream for external interactive tools. Stdout emits terminal events; stdin accepts `{"type":"input","data":"..."}`, `{"type":"resize","cols":80,"rows":24}` (cols 20–240, rows 8–120), and `{"type":"close"}`. Input and viewport updates are applied in frame order; resize frames claim coordinated viewport ownership until the bridge closes. Keep stdin open while consuming events because stdin EOF closes the bridge.
