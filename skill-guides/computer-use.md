...
name: computer-use
description: >-
  Use MCode's computer-use CLI to inspect and operate local desktop app windows
  ...
  accessibility actions. Also use for browser windows, webviews, MCode app UI,
  or other desktop UI. Triggers include "computer use", "mcode computer", "read
...
Use this skill for desktop UI through `mcode computer`. When the requested target is a website or web app, operate the desktop browser app/window that contains the page.
...
- In every command example, `MCODE` is a documentation placeholder — including examples that
  name a specific shell. Replace it with that chosen executable before running the command;
  do not create a shell variable or run `MCODE` literally. Blocks that name no shell are
...
MCODE computer get-app-state --app com.spotify.client --json
MCODE computer click --app com.spotify.client --element-index 42 --json
```
...
```text
MCODE computer get-app-state --app com.microsoft.edgemac --json
MCODE computer get-app-state --app Spotify --json
MCODE computer get-app-state --app pid:12345 --json
```
...
```text
MCODE computer permissions --json
MCODE computer capabilities --json
MCODE computer list-apps --json
MCODE computer list-windows --app <app> --json
MCODE computer get-app-state --app <app> --json
MCODE computer get-app-state --app <app> --restore-window --json
MCODE computer click --app <app> --element-index <index> --json
MCODE computer click --app <app> --x 100 --y 100 --json
MCODE computer click --app <app> --x 100 --y 100 --modifiers CmdOrCtrl+Shift --json
MCODE computer click --app <app> --element-index <index> --mouse-button right --json
MCODE computer click --app <app> --element-index <index> --mouse-button middle --json
MCODE computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
MCODE computer set-value --app <app> --element-index <index> --value "text" --json
MCODE computer type-text --app <app> --text "text" --json
MCODE computer press-key --app <app> --key Return --json
MCODE computer hotkey --app <app> --key CmdOrCtrl+A --json
MCODE computer paste-text --app <app> --text "text" --json
MCODE computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
MCODE computer drag --app <app> --from-element-index <index> --to-element-index <index> --json
MCODE computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | MCODE computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```text
MCODE computer get-app-state --app com.microsoft.edgemac --restore-window --json
MCODE computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
MCODE computer press-key --app com.microsoft.edgemac --key Return --json
```
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `MCODE computer ... --app Gmail` unchanged because `mcode computer` app selectors refer to desktop apps, not website names.
...
- `screenshot_failed`: use `--no-screenshot` if tree state is enough; if the message names Screen Recording or screenshots permission, run `MCODE computer permissions --id screenshots --json`.
- `accessibility_error`: run `MCODE computer capabilities --json`; if the message names Accessibility permission, run `MCODE computer permissions --id accessibility --json`.
- Empty tree or no screenshot: app may have no visible window, be minimized, or need permissions.
- Permission errors: run `MCODE computer permissions --json`, or `MCODE computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one permission, use the setup UI, then retry.

## Next Action

Confirm MCode status unless already checked, then run `MCODE computer capabilities --json`. For website or web-app targets such as Gmail, identify the desktop browser app/window that contains the page, then get that target app state with `MCODE computer get-app-state --app <app> --json`.
