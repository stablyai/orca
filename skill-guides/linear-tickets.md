...
`project edit` changes only the fields you pass, and needs at least one. Never build a replacement collection or an edited `--content` out of `project show` output when that output is marked `truncated: true` — reads cap content and collections lower than writes accept, so re-sending what you read back drops every member and every character past the cap. Re-read the full value first. Repeated `--member`, `--team`, and `--label` values **replace** the whole collection rather than appending, so pass them only when the user authorized that exact complete set; use `--clear-members` or `--clear-labels` to empty one. There is no `--clear-status`, `--clear-color`, or `--clear-teams`, and an edit cannot remove every team. `project edit` has no `--write-id`: Linear accepts no id on that mutation, so Orca skips a no-op edit and verifies the write by reading every requested field back. If it returns `linear_write_unconfirmed`, run `orca linear project show <project> --json` and compare before retrying — never blindly retry a collection replacement, because another actor may have edited it.
