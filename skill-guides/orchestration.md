...
- Orca may synthesize its own `escalation` when it classifies a supervised worker as `wedged`: the dispatch is `ready` and every progress source it can observe stayed silent past the configured threshold. Only that classification escalates. A worker Orca cannot observe is `unknown`, and a worker waiting on an `ask` or parked in `check --wait` is `blocked`; neither escalates. The message carries `payload.kind = "wedged_worker_signal"` and `payload.wedgedWorker.escalationCount`, and it is a detection signal only: Orca took no action on the worker and never will. Read the worker with `worker-show` and `worker-read` before you decide anything, and keep waiting when the worker is merely slow. A rising `escalationCount` is the same wedge reported again, not a new one.
