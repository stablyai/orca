...
- Orca may synthesize its own `escalation` when a supervised worker shows no progress evidence for the configured threshold. It carries `payload.kind = "wedged_worker_signal"` and `payload.wedgedWorker.escalationCount`, and it is a detection signal only: Orca took no action on the worker and never will. Read the worker with `worker-show` and `worker-read` before you decide anything, and keep waiting when the worker is merely slow. A rising `escalationCount` is the same wedge reported again, not a new one.
