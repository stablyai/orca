...
- Every Delivery message carries `senderLiveness`: the sender's state (`working`, `blocked`, `waiting`, `idle`, or `unknown`), when its last turn boundary was, which source proved it, and its dispatch state. Read it instead of calling `worker-show` or `terminal read` to decide whether the sender is still working. `unknown` means the runtime cannot prove liveness — a federated worker, a pane that is gone, stale evidence, or a harness that reports no status — so fall back to inspection only for those. An older runtime omits the field entirely.
