...
Do not release a worker because of a timeout, TUI idle state, heartbeat, status, question, escalation, or rejected/stale `worker_done`. If release returns `release_pending` or `release_unknown`, do not substitute `terminal close`; follow the exact recovery action in the receipt. A replayed Delivery may repeat `worker-release` safely. `terminal close` has no exception here at all: a receipt only proves what was true when it was written, and a terminal an earlier Dispatch left behind can be adopted by a later `worker-start --terminal` or by a human before you act on it. `worker-release` is the reclaim path precisely because the host decides ownership at the moment of the action, not at the moment of the receipt.
