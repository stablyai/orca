...
- `worker-show --dispatch <id>` says `ready`: keep waiting or read bounded output. JSON `agentStatus.status` of `permission` means the exact agent terminal is blocked on a harness prompt (tool/permission approval) even while the Dispatch still looks ready — open that terminal before assuming the worker is idle. `agentStatus` comes from supported agent hooks, not terminal-output inference, so a missing or non-`permission` value does not rule out an unseen harness prompt for an agent without hook coverage.
