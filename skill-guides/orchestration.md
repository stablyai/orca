...
- When an outstanding Delivery blocks other mail, `queuedMatchingMessages: true` means at least one still-unseen row matches the requested `--types`. Queued mail is not necessarily newer: routing moves a message between mailboxes without renumbering it, so mail released from a settled Dispatch can queue up behind the batch carrying an older sequence. An omitted value from an older runtime is unknown and human output labels it unavailable. The replay remains the same exact batch: acknowledge it before checking or waiting for the queued mail, and never infer that the queued rows were consumed or included; the printed acknowledgment command retains the exact `--run` scope.
