...
MCode is a **thin wrapper**: you guide, detect, and scaffold; you never own the user's cloud account,
...
**The one branch that shapes everything — connection mode:** **MCode-server** (`create` runs `mcode serve`
in the env and emits a `pairingCode`; §7c/§7f) vs **SSH** (`create` runs no server and emits a
`connection.type:"ssh"` block MCode dials into; §7g/§7h). Settle this first — it changes the `create`
output shape and half the templates.

Keep MCode's checkout behavior unchanged by default: omit `checkoutMode`, emit schema version 1, and
let MCode create a linked worktree. Only use `checkoutMode: provisioned-root` when the user explicitly
...
**Quick-start (happy path):** interview the user (connection mode MCode-server vs SSH, provider, agent CLI,
git auth — §1.2) + read the provider's CLI docs → scaffold `scripts/mcode-vm/` from §7 → run the
base-snapshot script, then the auth script (you invoke these by hand; not via `mcode.yaml`) → wire
`environmentRecipes` in `mcode.yaml` → `mcode vm recipe doctor <id> --json` (free) → then the `--provision`
...
a long time, or need the user at the keyboard. Never create an MCode workspace or commit unless asked.

1. **Inspect the repo** for an existing `environmentRecipes` entry, `scripts/mcode-vm/`, a state file, or setup
...
2. **Interview the user up front** — gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** how MCode attaches to the environment — an **MCode server** (the VM runs
     `mcode serve` and MCode pairs over its pairing URL; worked example §7f) or **SSH** (MCode connects to
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, … For non-obvious providers, also
     ...
     (host/port/user/key or proxy command) or only a provider-mediated interactive shell; MCode SSH mode
     needs the former.
...
7. **Wire the recipe** so `mcode.yaml` points create/suspend/resume/destroy at the scripts (§8). The
   workspace composer reads `environmentRecipes` from the project's primary checkout of `mcode.yaml`, **not** from
   a feature branch or worktree. So a recipe added only on a branch won't appear as a "Run on" option
   until that `mcode.yaml` change is committed and merged to the project's primary branch. Tell the user
...
8. **Dry-run doctor** — `mcode vm recipe doctor <recipe-id> --repo-path <repo> --json` (free, static; §9).
   Fix every failure before going live.
9. **[CHECKPOINT] Live self-test** — get the user's OK once, then run
   `mcode vm recipe doctor <recipe-id> --provision --json` as a loop: it runs create → validates →
...
- **Connection mode** (MCode server vs SSH) confirmed with the user — see §1 step 2; it shapes the recipe.
...
A repo-local JSON file (e.g. `scripts/mcode-vm/<provider>-state.json`) threads non-secret values between
...
Scaffold under `scripts/mcode-vm/`. These are **shapes** — fill in the provider's real commands. All
...
- **Local-side** (`create`/`suspend`/`resume`/`destroy` + the base-snapshot/auth scripts the user
  ...
  or require WSL/Git-Bash and point `mcode.yaml` at the right launcher.
...
Worked Vercel commands for this phase are in §7f. You run this script by hand (not via `mcode.yaml`),
...
# name = mcode-${MCODE_RECIPE_ID}-${MCODE_VM_INSTANCE_ID} (sanitized, length-capped)
...
# 3. remote exec: start mcode serve in the background and read the recipe JSON it writes (see below)
...
**The exact `mcode serve` invocation and its output (verified — do not improvise the flags).** Inside the
VM, run:
...
**Binary name:** in a VM built from source (the Phase-2 flow), run it as `pnpm exec mcode-dev serve …`
from the repo root — `mcode-dev` is the in-repo entrypoint and is what the §7f example uses. Plain
`mcode serve …` is the same command when the built CLI is installed on the VM's PATH. The flags/output
...
```json
{ "schemaVersion": 1, "pairingCode": "<mcode pairing URL>", "projectRoot": "<the --project-root you passed>" }
```
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # MCode passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in mcode.yaml)
```
...
recipe_id="${MCODE_RECIPE_ID:-vercel-sandbox}"
recipe_id="${recipe_id//./-}"  # Vercel names forbid dots.
instance_id="${MCODE_VM_INSTANCE_ID:-$(date +%s)}"
max_recipe_id_length=$((128 - ${#instance_id} - 6))  # Preserve the unique instance suffix.
[ "$max_recipe_id_length" -gt 0 ] || { echo "MCODE_VM_INSTANCE_ID is too long for a Vercel sandbox name" >&2; exit 1; }
name="mcode-${recipe_id:0:max_recipe_id_length}-${instance_id}"
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  --env "GH_TOKEN=$gh_token" --env "MCODE_PROJECT_ROOT=$project_root" \
  --env "MCODE_REPO_URL=$repo_url" --env "MCODE_REPO_REF=$repo_ref" \
  -- bash -lc 'set -euo pipefail; cd "$MCODE_PROJECT_ROOT"; \
    ...
    git fetch origin "$MCODE_REPO_REF"; \
    git checkout -B "$MCODE_REPO_REF" FETCH_HEAD; \
    rm -f /tmp/askpass.sh; \
    c="$(git rev-parse HEAD)"; [ -f .mcode-built ] && [ "$(cat .mcode-built)" = "$c" ] || { \
      ...
      printf "%s" "$c" > .mcode-built; }' >&2

# 3. (remote) start mcode serve in the background, writing recipe JSON to a file; poll until it parses
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  --env "MCODE_PORT=$port" --env "MCODE_PROJECT_ROOT=$project_root" --env "MCODE_PAIRING_ADDRESS=$pairing_ws" \
  -- bash -lc 'set -euo pipefail; cd "$MCODE_PROJECT_ROOT"; rm -f /tmp/mcode-recipe.json /tmp/mcode-serve.log; \
    nohup pnpm exec mcode-dev serve --port "$MCODE_PORT" --project-root "$MCODE_PROJECT_ROOT" \
      --pairing-address "$MCODE_PAIRING_ADDRESS" --recipe-json >/tmp/mcode-recipe.json 2>/tmp/mcode-serve.log </dev/null & \
    pid=$!; for _ in $(seq 1 80); do \
      node -e "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/mcode-recipe.json\",\"utf8\"))" >/dev/null 2>&1 && { cat /tmp/mcode-recipe.json; exit 0; }; \
      kill -0 "$pid" 2>/dev/null || { cat /tmp/mcode-serve.log >&2; exit 1; }; sleep 0.25; \
    done; cat /tmp/mcode-serve.log >&2; echo "serve recipe JSON timed out" >&2; exit 1')"
...
`userData.resourceId` from stdin (§7d). This is the **MCode-server** connection mode (the recipe emits a
...
- **`create` does NOT run `mcode serve` and does NOT emit a `pairingCode`.** MCode itself connects to the
  host over its SSH relay, brings up the git + filesystem providers, and imports the repo. The script's
  only job is to make the host ready and **print SSH connection details** MCode will dial.
- The result uses a `connection` block with `type: "ssh"` and a `target`, **not** the flat
  `pairingCode`/`projectRoot` shape. Exact shape (MCode rejects anything else):
...
`MCODE_RECIPE_RESULT_SCHEMA_VERSION`, `MCODE_REPO_URL`, `MCODE_REPO_REF`, `MCODE_REPO_REF_HEAD`, and
`MCODE_REPO_BRANCH`. Use `MCODE_REPO_REF` to fetch the selected source, but create
`MCODE_REPO_BRANCH` at the exact `MCODE_REPO_REF_HEAD` commit; resolving the symbolic ref again can race
with an upstream update. `MCODE_REPO_URL` and `MCODE_REPO_REF` are a matched fetch pair, including when
the desktop source uses multiple remotes. Return that primary checkout at `projectRoot` and emit the
same SSH result with:

```bash
[ -n "${MCODE_REPO_REF_HEAD:-}" ] || { echo "missing pinned source commit" >&2; exit 1; }
git fetch origin "$MCODE_REPO_REF"
git cat-file -e "${MCODE_REPO_REF_HEAD}^{commit}"
git checkout -B "$MCODE_REPO_BRANCH" "$MCODE_REPO_REF_HEAD"
```
...
- `relayGracePeriodSeconds` (optional): how long MCode keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO mcode serve here)
...
# 2. print the SSH connection block (NO pairingCode, NO mcode serve). host/port/username tell MCode's
...
`destroy: none` and omit suspend/resume. (MCode still disconnects/reconnects its own SSH relay on
...
`connection.type:"ssh"` block above instead of starting `mcode serve`.
...
- If committing from an interactive shell, force the runtime entrypoint back to `sshd`:
  `docker commit --change='ENTRYPOINT ["/usr/local/bin/mcode-docker-ssh-entrypoint"]' …`.
...
docker run -d --name "$name" -p 127.0.0.1::22 -e "MCODE_SSH_PUBLIC_KEY=$pubkey" "$auth_image"
...
require WSL/Git-Bash (and point `mcode.yaml` at e.g. `bash ./scripts/mcode-vm/<name>.sh` via a `.cmd`
...
# MCode-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
...
environmentRecipes:
  - id: cloud-sandbox
    name: Cloud Sandbox
    create: ./scripts/mcode-vm/cloud-sandbox-create.sh
    suspend: ./scripts/mcode-vm/cloud-sandbox-suspend.sh
    resume: ./scripts/mcode-vm/cloud-sandbox-resume.sh
    destroy: ./scripts/mcode-vm/cloud-sandbox-destroy.sh
```
...
**MCode-server mode** — boot the env, start `mcode serve` in it, and print serve's result:

```json
{
  "schemaVersion": 1,
  "pairingCode": "mcode-pairing-code-or-url",
  "projectRoot": "/absolute/path/to/repo/on/remote",
  "userData": { "provider": "example", "resourceId": "provider-resource-id" }
}
```

Here `pairingCode` (from `mcode serve --recipe-json`) and `projectRoot` are required; `schemaVersion` (`1`)
and `userData` are optional.

**SSH mode** — do **not** run `mcode serve`; print the `connection.type:"ssh"` block instead (full shape +
...
`checkoutMode: provisioned-root` to the recipe, require `MCODE_RECIPE_RESULT_SCHEMA_VERSION=2`, create
the requested `MCODE_REPO_BRANCH` at the pinned `MCODE_REPO_REF_HEAD` commit (use `MCODE_REPO_REF` only
...
Start MCode remotely with `mcode serve --port "$PORT" --project-root "$ABS_ROOT" --pairing-address
...
`mcode vm recipe doctor <recipe-id> --repo-path <repo> --json` validates **static wiring only** — it does
...
`mcode vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` actually runs the recipe end
...
- Don't make MCode own provider lifecycle beyond invoking the configured scripts.
- Don't commit or create an MCode workspace unless asked.
