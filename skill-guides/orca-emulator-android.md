...
control is local to the host that owns the SDK. Remote and SSH device control is out of
scope beyond a plain `adb connect` to an address that is already reachable (see
"Connecting a network / cloud-phone device" below); Orca does not set up VPNs, SSH
tunnels, port-forwards, adb pairing, or remote USB.
...
- **Disconnect is explicit and only in Settings.** Closing the emulator pane/tab or quitting
  Orca never disconnects a network device — it only stops the local scrcpy transport tunnel,
  leaving the shared adb server's connection intact for other tools. Click **Disconnect** in
  Settings to actually drop it.
...
- `--device <serial>` such as `emulator-5554` or a connected `host:port` serial, from
