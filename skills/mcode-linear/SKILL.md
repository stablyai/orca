...
This file is a discovery stub, not the usage guide. The full, version-matched MCode Linear
reference is served by the `mcode` binary itself — kept out of this file on purpose so it can
never drift from the binary that will actually run your commands.

Engage MCode's Linear CLI (`mcode linear ...`) whenever you work a Linear-linked task: read
...
`MCODE skills get mcode-linear`. Beyond these commands, ask the user rather than guessing a
