---
name: mcode-per-workspace-env
description: >-
  Set up, review, debug, or validate MCode per-workspace environment recipes —
  ...
  per-workspace environments, fix an `environmentRecipes` entry in `mcode.yaml`, scaffold
  provider lifecycle scripts, or resolve an `mcode vm recipe doctor` failure.
---
